...
function createTempStorageRoot(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "kwizera-motion-gen-test-"));
}

const ANALYSIS_SAMPLE = {
  productId: "mg-test-product",
  productName: "Motion Generation Test Product",
  category: ProductAnalysisCategory.Software,
  subcategory: "saas",
  brand: "TestBrand",
  description: "SaaS product for motion generation validation",
  features: ["automation"],
...
async function preparePipeline(
  ...
  await piFoundation.getTargetAudienceIntelligenceEngine().analyzeAudience({ productId: "mg-test-product" });
  ...
  await piFoundation.getStoryboardIntelligenceEngine().createStoryboard({ productId: "mg-test-product" });
...
describe("AiMotionGenerationEngine", () => {
  ...
  it("initializes and registers with video generation foundation", async () => {
    ...
    await core.start("motion-gen-test");

    const engine = core.getManager().videoGenerationFoundation!.getMotionGenerationEngine();
    ...
    const module = core
      ...
      .getModule("motion-planning-generation-engine");
    expect(module?.implemented).toBe(true);
  });

  it("generates motion plans from scenes and camera plans", async () => {
    const core = createAiCore({ storageRootOverride: storageRoot });
    await core.start("motion-gen-test");
    ...
    const engine = genFoundation.getMotionGenerationEngine();
    const result = await engine.generateMotionPlans({ storyboardId });

    expect(result.success).toBe(true);
    expect(result.plans!.length).toBeGreaterThanOrEqual(4);
    expect(result.plans!.every((p) => p.validated)).toBe(true);
    expect(result.plans![0].cameraSynchronization.syncPoints.length).toBeGreaterThanOrEqual(1);
    expect(result.plans![0].platformOptimizations.length).toBeGreaterThanOrEqual(7);
    expect(result.plans![0].scores.motionQualityScore).toBeGreaterThanOrEqual(55);
  });

  it("rejects planning without upstream camera plans", async () => {
    const core = createAiCore({ storageRootOverride: storageRoot });
    await core.start("motion-gen-test");

    const engine = core.getManager().videoGenerationFoundation!.getMotionGenerationEngine();
    const result = await engine.generateMotionPlans({ storyboardId: "nonexistent" });

    expect(result.success).toBe(false);
  });

  it("supports search by motion type and storyboard", async () => {
    const core = createAiCore({ storageRootOverride: storageRoot });
    await core.start("motion-gen-test");
    ...
    const engine = genFoundation.getMotionGenerationEngine();
    await engine.generateMotionPlans({ storyboardId });

    const byStoryboard = engine.searchMotionPlans({ storyboardId });
    expect(byStoryboard.length).toBeGreaterThanOrEqual(4);

    const byType = engine.searchMotionPlans({ motionType: MotionType.Combined });
