...
function createTempStorageRoot(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "kwizera-vfx-gen-test-"));
}

const ANALYSIS_SAMPLE = {
  productId: "vfx-test-product",
  productName: "Visual Effects Test Product",
  category: ProductAnalysisCategory.Software,
  subcategory: "saas",
  brand: "TestBrand",
  description: "SaaS product for visual effects validation",
  features: ["automation"],
...
async function preparePipeline(
  ...
  await piFoundation.getTargetAudienceIntelligenceEngine().analyzeAudience({ productId: "vfx-test-product" });
  ...
  await piFoundation.getStoryboardIntelligenceEngine().createStoryboard({ productId: "vfx-test-product" });
...
describe("AiVisualEffectsGenerationEngine", () => {
  ...
  it("initializes and registers with video generation foundation", async () => {
    ...
    await core.start("vfx-gen-test");

    const engine = core.getManager().videoGenerationFoundation!.getVisualEffectsGenerationEngine();
    ...
    const module = core
      ...
      .getModule("visual-effects-planning-generation-engine");
    expect(module?.implemented).toBe(true);
  });

  it("generates visual effect plans from full upstream pipeline", async () => {
    const core = createAiCore({ storageRootOverride: storageRoot });
    await core.start("vfx-gen-test");
    ...
    const engine = genFoundation.getVisualEffectsGenerationEngine();
    const result = await engine.generateVisualEffectPlans({ storyboardId });
    ...
    expect(result.plans![0].scores.visualEffectsScore).toBeGreaterThanOrEqual(55);
  });

  it("rejects planning without upstream animation plans", async () => {
    const core = createAiCore({ storageRootOverride: storageRoot });
    await core.start("vfx-gen-test");

    const engine = core.getManager().videoGenerationFoundation!.getVisualEffectsGenerationEngine();
    const result = await engine.generateVisualEffectPlans({ storyboardId: "nonexistent" });

    expect(result.success).toBe(false);
  });

  it("supports search by effect type and storyboard", async () => {
    const core = createAiCore({ storageRootOverride: storageRoot });
    await core.start("vfx-gen-test");
    ...
    const engine = genFoundation.getVisualEffectsGenerationEngine();
    await engine.generateVisualEffectPlans({ storyboardId });

    const byStoryboard = engine.searchVisualEffectPlans({ storyboardId });
    expect(byStoryboard.length).toBeGreaterThanOrEqual(4);

    const byType = engine.searchVisualEffectPlans({ planType: VisualEffectPlanType.Combined });
