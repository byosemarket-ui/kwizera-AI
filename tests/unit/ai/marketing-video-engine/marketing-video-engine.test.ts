...
function createTempStorageRoot(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "kwizera-marketing-video-test-"));
}

const ANALYSIS_SAMPLE = {
  productId: "mv-test-product",
  productName: "Marketing Video Test Product",
  category: ProductAnalysisCategory.Software,
  subcategory: "saas",
  brand: "TestBrand",
  description: "SaaS product for marketing video validation",
  features: ["automation"],
...
async function preparePipeline(
  ...
  await piFoundation.getTargetAudienceIntelligenceEngine().analyzeAudience({ productId: "mv-test-product" });
  ...
  await piFoundation.getStoryboardIntelligenceEngine().createStoryboard({ productId: "mv-test-product" });
...
describe("AiMarketingVideoEngine", () => {
  ...
  it("initializes and registers with video generation foundation", async () => {
    ...
    await core.start("marketing-video-test");

    const engine = core.getManager().videoGenerationFoundation!.getMarketingVideoEngine();
    ...
    const module = core
      ...
      .getModule("marketing-video-generation-engine");
    expect(module?.implemented).toBe(true);
  });

  it("generates marketing video plans from full upstream pipeline", async () => {
    const core = createAiCore({ storageRootOverride: storageRoot });
    await core.start("marketing-video-test");
    ...
    const engine = genFoundation.getMarketingVideoEngine();
    const result = await engine.generateMarketingVideoPlans({ storyboardId });

    expect(result.success).toBe(true);
    expect(result.plans!.length).toBeGreaterThanOrEqual(1);
    expect(result.plans!.every((p) => p.validated)).toBe(true);
    expect(result.plans![0].hookOptimization.first3SecondsStrategy.length).toBeGreaterThan(5);
    expect(result.plans![0].platformOptimizations.length).toBeGreaterThanOrEqual(8);
    expect(result.plans![0].scores.marketingQualityScore).toBeGreaterThanOrEqual(55);
  });

  it("rejects planning without upstream audio sync plans", async () => {
    const core = createAiCore({ storageRootOverride: storageRoot });
    await core.start("marketing-video-test");

    const engine = core.getManager().videoGenerationFoundation!.getMarketingVideoEngine();
    const result = await engine.generateMarketingVideoPlans({ storyboardId: "nonexistent" });

    expect(result.success).toBe(false);
  });

  it("supports search by campaign and product", async () => {
    const core = createAiCore({ storageRootOverride: storageRoot });
    await core.start("marketing-video-test");
    ...
    const byProduct = engine.searchMarketingVideoPlans({ productId: "mv-test-product" });
    expect(byProduct.length).toBeGreaterThanOrEqual(1);

    const byType = engine.searchMarketingVideoPlans({ planType: MarketingVideoPlanType.Combined });
    expect(byType.length).toBeGreaterThanOrEqual(1);

    const byCampaign = engine.searchMarketingVideoPlans({ campaignId: plan.profile.campaignId });
    expect(byCampaign.length).toBeGreaterThanOrEqual(1);
  });
});
