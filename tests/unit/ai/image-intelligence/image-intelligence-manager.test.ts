...
describe("ImageIntelligenceManager", () => {
  it("builds persisted per-image profiles with quality, background, view, object, scene, and enhancement intelligence", async () => {
    ...
    await workspace.uploadImage(project.id, { fileName: "black-steel-bottle-front-studio.png", mimeType: "image/png", dataBase64: "iVBORw0KGgo=" }); await workspace.uploadImage(project.id, { fileName: "black-steel-bottle-side-studio.png", mimeType: "image/png", dataBase64: "iVBORw0KGgo=" });
    ...
    expect(profiles).toHaveLength(2); expect(profiles[0].background.type).toBe("controlled studio background"); expect(profiles[0].cameraAngle).toBe("front product view"); expect(profiles[0].objects.some((object) => object.label === "bottle")).toBe(true); expect(profiles[0].enhancements.length).toBeGreaterThan(0); expect(profiles[0].boundaries.detected).toBe(true); expect(profiles[0].resolution.tier).toBeTruthy(); expect(profiles[0].viewRole).toBe("front"); expect((await manager.analyzeProject(project.id))[0].cached).toBe(true);
  ...
  it("detects duplicate uploads without modifying original image bytes", async () => {
    ...
    await workspace.uploadImage(project.id, { fileName: "bottle-front-copy.png", mimeType: "image/png", dataBase64: "iVBORw0KGgo=" });
