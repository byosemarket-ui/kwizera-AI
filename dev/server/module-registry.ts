...
function extractStepFromScript(scriptPath: string): { step: string; phase: number; name: string } | null {
  ...
  const validationMatch = content.match(
    /Step\s+(\d+)([A-Z])\s+(?:[—–-]\s+)?(.+?)\s+Engine\s+Validation/i
  ) ?? content.match(
    /Step\s+(\d+)([A-Z])\s+(?:[—–-]\s+)?(.+?)\s+Validation/i
  );
  if (validationMatch) {
  ...
  const certMatch = content.match(
    /Phase\s+(\d+)\s+Step\s+(\d+)([A-Z])\s+(.+?)\s+Certification/i
  ) ?? content.match(
    /Phase\s+(\d+)\s+Step\s+(\d+)([A-Z])/i
  );
  if (certMatch) {
...
function resolveAiPath(validateKey: string): string | null {
  const candidates = [
    validateKey,
    `${validateKey}-engine`,
  ...
  const fuzzy = aiDirs.find(
    (d) => d === validateKey || d === `${validateKey}-engine` || d.replace(/-engine$/, "") === normalized
  );
  return fuzzy ? `ai/${fuzzy}` : null;
}

function parseValidationReport(step: string): {
  status: ModuleStatus;
  readinessScore: number | null;
  lastValidated: string | null;
  reportFile: string | null;
...
function parseReportContent(content: string, reportFile: string): {
  status: ModuleStatus;
  readinessScore: number | null;
  lastValidated: string | null;
  reportFile: string;
...
function stepWithinLimit(step: string, maxStep: string): boolean {
...
function buildBlueprintModules(): ModuleEntry[] {
  return BLUEPRINT_STEPS.map((bp) => {
    ...
    return {
      ...
      aiPath: null,
      validateKey: null,
      status: exists ? ("blueprint" as ModuleStatus) : "fail",
...
function buildImplementationModules(): ModuleEntry[] {
  ...
  for (const [key, command] of Object.entries(scripts)) {
    ...
    if (!stepWithinLimit(meta.step, phaseDef.maxStep) && !isCert) continue;
...
export function buildRegistry(forceRefresh = false): PhaseSummary[] {
  if (registryCache && !forceRefresh) return registryCache;
  ...
  const result = PHASE_DEFINITIONS.map((phaseDef) => {
    ...
    let status: ModuleStatus = "unknown";
    if (modules.length === 0) status = "not-run";
    else if (passed === modules.length) status = "pass";
    else if (modules.some((m) => m.status === "fail")) status = "fail";
    else if (modules.some((m) => m.status === "pass" || m.status === "blueprint")) status = "unknown";
...
export function findModule(validateKey: string): ModuleEntry | undefined {
  const all = buildRegistry().flatMap((p) => p.modules);
  return all.find((m) => m.validateKey === validateKey || m.id === validateKey);
}

export function listAiModules(): string[] {
  return fs
