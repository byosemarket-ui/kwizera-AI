...
import {
  ...
  VideoType,
  createAiCore,
  type KnowledgeHealthCheckResult,
  type KnowledgeHealthMonitorStatusReport,
  type KnowledgeAuditResult,
  type KnowledgeBatchValidationResult,
  type KnowledgeValidationStatusReport,
  type KnowledgeGraphStatusReport,
} from "../ai/index.js";
import { DEFAULT_STORAGE_ROOT } from "../storage/paths/storage-paths.js";
import type { AiKnowledgeStorageEngine } from "../ai/knowledge-storage-engine/knowledge-storage-engine.js";
import type { BrandAnalysisInput } from "../ai/brand-knowledge-engine/types.js";
import type { CreativeAnalysisInput } from "../ai/creative-knowledge-engine/types.js";
import type { ImageAnalysisInput } from "../ai/image-knowledge-engine/types.js";
import type { LanguageAnalysisInput } from "../ai/language-knowledge-engine/types.js";
import type { MarketingAnalysisInput } from "../ai/marketing-knowledge-engine/types.js";
import type { ProductAnalysisInput } from "../ai/product-knowledge-engine/types.js";
...
async function seedBulkRecords(storage: AiKnowledgeStorageEngine, count: number): Promise<void> {
...
function scoreDetail(
  result: { success: boolean; record?: { scores: object } },
  scoreKey: string,
  label: string
): string {
  if (!result.success || !result.record) return "failed";
  const scores = result.record.scores as Record<string, number>;
...
async function main(): Promise<void> {
  ...
  let healthCheck: KnowledgeHealthCheckResult | null = null;
  let audit: KnowledgeAuditResult | null = null;
  let healthStatus: KnowledgeHealthMonitorStatusReport | null = null;
  let graphIntegrity: { valid: boolean } | null = null;
  let validationBatch: KnowledgeBatchValidationResult | null = null;
  let validationStatus: KnowledgeValidationStatusReport | null = null;
  let graphStatus: KnowledgeGraphStatusReport | null = null;
  ...
  try {
    ...
    const productInput: ProductAnalysisInput = {
    ...
    const brandInput: BrandAnalysisInput = {
    ...
    const marketingInput: MarketingAnalysisInput = {
    ...
    const languageInput: LanguageAnalysisInput = {
    ...
    const creativeInput: CreativeAnalysisInput = {
    ...
    const msPerRecord =
      performance.totalRecords > 0 ? performance.stressSeedMs! / performance.totalRecords : 0;
    ...
    const baseScores = {
      ...
      performanceScore: Math.round(
        ((passRate(stressResults) + (performance.startupMs! < 120000 ? 1 : 0.7)) / 2) * 100
      ),
    ...
    const reports = {
      ...
      graph: buildGraphReport(graphIntegrity!, performance as PerformanceMetrics, graphStatus!),
    ...
    fs.writeFileSync(
      path.join(certRecordDir, "phase-4-certification.json"),
      JSON.stringify(
        {
          phase: 4,
          step: "4O",
...
function buildCertificationReport(
  ...
  stress: StressConfig,
  approved: boolean,
  healthStatus: KnowledgeHealthMonitorStatusReport
): string {
...
function buildHealthReport(
  healthResults: Record<string, CertResult>,
  healthStatus: KnowledgeHealthMonitorStatusReport,
  healthCheck: KnowledgeHealthCheckResult,
  audit: KnowledgeAuditResult,
  scores: EngineeringScores
): string {
...
function buildValidationReport(
  batch: KnowledgeBatchValidationResult,
