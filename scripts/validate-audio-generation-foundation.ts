...
import {
  ...
  SUPPORTED_AUDIO_GENERATION_SOURCES,
  ...
  type AudioGenerationFoundationStatusReport,
  type AudioGenerationHealthReport,
} from "../ai/index.js";
import {
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const registryPath = path.join(
      ...
      "audio-generation",
      "registry",
      "audio-generation-registry.json"
    );
    ...
    results.sampleVoice = {
      passed:
        foundation.getAssetRegistry().getAsset(voice.assetId)?.assetType === AudioGenerationAssetType.Voice &&
    ...
    results.sampleAmbient = {
      passed:
        foundation.getAssetRegistry().getAsset(ambient.assetId)?.assetType === AudioGenerationAssetType.AmbientSound,
    ...
    results.assetRegistry = {
      passed:
        foundation.getAssetRegistry().getCount() >= 5 &&
...
function buildFoundationReport(
  status: AudioGenerationFoundationStatusReport,
  health: AudioGenerationHealthReport,
...
function buildBlueprintReport(
  foundation: NonNullable<ReturnType<ReturnType<typeof createAiCore>["getManager"]>["audioGenerationFoundation"]>,
  allPassed: boolean
): string {
...
function buildArchitectureReport(status: AudioGenerationFoundationStatusReport): string {
