...
import {
  ...
  type VisualPlanningEngineStatusReport,
  type VisualPlanningRecord,
} from "../ai/index.js";
...
async function main(): Promise<void> {
  ...
  try {
    ...
    results.backgroundPlanning = {
      passed:
        Boolean(tech.record?.backgroundPlanning.studioBackground) &&
    ...
    results.cameraPlanning = {
      passed:
        Boolean(tech.record?.cameraPlanning.heroShot) &&
    ...
    results.compositionPlanning = {
      passed:
        tech.record?.scenePlans.every((s) => Boolean(s.depth) && Boolean(s.colorPalette)) === true,
      detail: "Depth and color palette planned per scene",
    };

    results.brandConsistency = {
      passed:
        tech.record?.brandConsistency.logoPlacement === true &&
    ...
    results.graphicElements = {
      passed:
        Boolean(tech.record?.graphicElements.titles) &&
    ...
    results.visualStyle = {
      passed:
        Boolean(tech.record?.visualStyle.cinematic) &&
    ...
    results.visualScores = {
      passed:
        (tech.record?.scores.visualPlanningScore ?? 0) >= 55 &&
    ...
    fs.writeFileSync(
      path.join(projectStateDir, "Visual-Planning-Report.md"),
      buildVisualPlanningReport(status, results, storageRoot, allPassed, tech.record, fashion.record, beauty.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Camera-Planning-Report.md"),
      buildCameraPlanningReport(tech.record, fashion.record, beauty.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Background-Planning-Report.md"),
      buildBackgroundPlanningReport(tech.record, fashion.record, beauty.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Visual-Readiness-Report.md"),
      buildReadinessReport(status, tech.record, fashion.record, beauty.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(process.cwd(), "STEP-5I-VALIDATION-REPORT.md"),
      buildVisualPlanningReport(status, results, storageRoot, allPassed, tech.record, fashion.record, beauty.record),
      "utf8"
    );
...
function buildVisualPlanningReport(
  status: VisualPlanningEngineStatusReport,
  results: Record<string, { passed: boolean; detail: string }>,
  storageRoot: string,
  allPassed: boolean,
  tech?: VisualPlanningRecord,
  fashion?: VisualPlanningRecord,
  beauty?: VisualPlanningRecord
): string {
  return [
...
function buildCameraPlanningReport(
  tech?: VisualPlanningRecord,
  fashion?: VisualPlanningRecord,
  beauty?: VisualPlanningRecord
): string {
  const rows = [tech, fashion, beauty].filter(Boolean) as VisualPlanningRecord[];
  const lines = [
  ...
  for (const record of rows) {
    ...
    lines.push(
      ...
      "",
      `- **Close-up:** ${c.closeUp}`,
      `- **Medium Shot:** ${c.mediumShot}`,
      `- **Wide Shot:** ${c.wideShot}`,
      `- **Hero Shot:** ${c.heroShot}`,
      `- **Orbit:** ${c.orbit}`,
      `- **Zoom:** ${c.zoom}`,
      `- **Pan:** ${c.pan}`,
      `- **Tilt:** ${c.tilt}`,
    ...
    for (const scene of record.scenePlans) {
      lines.push(
        `| ${scene.sceneNumber} | ${scene.cameraAngle.slice(0, 30)} | ${scene.cameraDistance.slice(0, 25)} | ${scene.cameraMovement.slice(0, 30)} |`
      );
    }
    lines.push("");
  }
...
function buildBackgroundPlanningReport(
  tech?: VisualPlanningRecord,
  fashion?: VisualPlanningRecord,
  beauty?: VisualPlanningRecord
): string {
  ...
  for (const record of rows) {
    ...
    lines.push(
      `## ${record.productId}`,
      "",
      `- **Studio:** ${b.studioBackground}`,
      `- **Lifestyle:** ${b.lifestyleBackground}`,
      `- **Transparent:** ${b.transparentBackground}`,
      `- **Gradient:** ${b.gradientBackground}`,
      `- **Environment:** ${b.environmentBackground}`,
      `- **Branded:** ${b.brandedBackground}`,
      `- **Custom:** ${b.customBackground}`,
      "",
...
function buildReadinessReport(
  ...
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r.productId} | ${r.scores.visualPlanningScore}/100 | ${r.scores.compositionScore}/100 | ${r.scores.brandConsistencyScore}/100 | ${r.scores.creativeScore}/100 | ${r.scores.marketingScore}/100 | ${r.productionReady ? "✅" : "❌"} | ${r.validated ? "✅" : "❌"} |`
    ),
    "",
    "## Performance",
    "",
