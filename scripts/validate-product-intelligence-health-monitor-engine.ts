...
import {
  ...
  type MonitoredProductIntelligenceModuleHealthScore,
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const logFile = path.join(
      storageRoot,
      "logs",
      `product-intelligence-health-monitor-engine-${logDate}.jsonl`
    );
    results.logging = {
    ...
    const relationshipModule = modules.find(
      (m) => m.module === MonitoredProductIntelligenceModule.ProductRelationships
    );
    ...
    const analysisRecordsPath = path.join(
      foundation.getProductAnalysisEngine().getEngineDir(),
      "product-analysis-records.json"
    );
    ...
    const scriptRecordsPath = path.join(
      foundation.getIntelligenceRoot(),
      "script",
      "engine",
      "script-planning-records.json"
    );
    ...
    fs.writeFileSync(
      path.join(process.cwd(), "STEP-5N-VALIDATION-REPORT.md"),
...
function buildReport(
  ...
  storageRoot: string,
  allPassed: boolean,
  check: ProductIntelligenceHealthCheckResult,
