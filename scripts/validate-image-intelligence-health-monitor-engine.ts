...
import {
  ...
  type MonitoredImageIntelligenceModuleHealthScore,
} from "../ai/index.js";
...
const SAMPLE_HERO: ImageAnalysisEngineInput = {
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const logFile = path.join(
      storageRoot,
      "logs",
      `image-intelligence-health-monitor-engine-${logDate}.jsonl`
    );
    results.logging = {
    ...
    const understandingRecordsPath = path.join(
      ...
      "understanding",
      "engine",
      "image-understanding-records.json"
    );
    ...
    const registryPath = path.join(
      foundation.getIntelligenceRoot(),
      "registry",
      "image-intelligence-registry.json"
    );
    ...
    fs.writeFileSync(
      path.join(process.cwd(), "STEP-6N-VALIDATION-REPORT.md"),
...
function buildReport(
  ...
  storageRoot: string,
  allPassed: boolean,
  check: ImageIntelligenceHealthCheckResult,
