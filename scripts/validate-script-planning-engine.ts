...
import {
  ...
  type ScriptPlanningEngineStatusReport,
  type ScriptPlanningRecord,
} from "../ai/index.js";
...
async function main(): Promise<void> {
  ...
  try {
    ...
    results.sceneScriptPlanning = {
      passed:
        (tech.record?.scenePlans.length ?? 0) >= 5 &&
    ...
    results.scriptStructure = {
      passed:
        Boolean(tech.record?.scriptStructure.hook) &&
    ...
    results.narrationPlanning = {
      passed:
        Boolean(tech.record?.voicePreparation.voiceStyle) &&
    ...
    results.subtitlePlanning = {
      passed:
        Object.keys(tech.record?.subtitlePreparation.subtitleTiming ?? {}).length >= 5 &&
    ...
    results.timingPlanning = {
      passed:
        tech.record?.scenePlans.every((s) => Boolean(s.estimatedReadingTime) && Boolean(s.estimatedDisplayTime)) ===
        true,
    ...
    results.scriptScores = {
      passed:
        (tech.record?.scores.scriptPlanningScore ?? 0) >= 55 &&
    ...
    fs.writeFileSync(
      path.join(projectStateDir, "Script-Planning-Report.md"),
      buildScriptPlanningReport(status, results, storageRoot, allPassed, tech.record, fashion.record, beauty.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Narration-Planning-Report.md"),
      buildNarrationPlanningReport(tech.record, fashion.record, beauty.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Subtitle-Planning-Report.md"),
      buildSubtitlePlanningReport(tech.record, fashion.record, beauty.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Script-Readiness-Report.md"),
      buildReadinessReport(status, tech.record, fashion.record, beauty.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(process.cwd(), "STEP-5H-VALIDATION-REPORT.md"),
      buildScriptPlanningReport(status, results, storageRoot, allPassed, tech.record, fashion.record, beauty.record),
      "utf8"
    );
...
function buildScriptPlanningReport(
  status: ScriptPlanningEngineStatusReport,
  results: Record<string, { passed: boolean; detail: string }>,
  storageRoot: string,
  allPassed: boolean,
  tech?: ScriptPlanningRecord,
  fashion?: ScriptPlanningRecord,
  beauty?: ScriptPlanningRecord
): string {
  return [
...
function buildNarrationPlanningReport(
  tech?: ScriptPlanningRecord,
  fashion?: ScriptPlanningRecord,
  beauty?: ScriptPlanningRecord
): string {
  const rows = [tech, fashion, beauty].filter(Boolean) as ScriptPlanningRecord[];
  const lines = [
  ...
  for (const record of rows) {
    ...
    lines.push(
      ...
      `- **Pause Locations:** ${v.pauseLocations.join("; ")}`,
      `- **Emphasis Points:** ${v.emphasisPoints.join("; ")}`,
      "",
      "| # | Purpose | Planned Narration | Reading Time |",
      "|---|---------|-------------------|--------------|"
    );
    for (const scene of record.scenePlans) {
      lines.push(
        `| ${scene.sceneNumber} | ${scene.scenePurpose} | ${scene.plannedNarration.slice(0, 50)}... | ${scene.estimatedReadingTime} |`
      );
    }
    lines.push("");
  }
...
function buildSubtitlePlanningReport(
  tech?: ScriptPlanningRecord,
  fashion?: ScriptPlanningRecord,
  beauty?: ScriptPlanningRecord
): string {
  ...
  for (const record of rows) {
    ...
    lines.push(
      ...
      `- **Line Validation:** ${s.lineLengthValidation}`,
      `- **Sync Rules:** ${s.synchronizationRules.join("; ")}`,
      "",
      "| # | Planned Subtitle | Display Time | Reading Duration |",
      "|---|------------------|--------------|------------------|"
    );
    for (const scene of record.scenePlans) {
      lines.push(
        `| ${scene.sceneNumber} | ${scene.plannedSubtitle.slice(0, 40)}... | ${scene.estimatedDisplayTime} | ${s.readingDuration[scene.sceneNumber] ?? scene.estimatedReadingTime} |`
      );
    }
    lines.push("");
  }
...
function buildReadinessReport(
  ...
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r.productId} | ${r.scores.scriptPlanningScore}/100 | ${r.scores.storytellingScore}/100 | ${r.scores.marketingScore}/100 | ${r.scores.readabilityScore}/100 | ${r.scores.brandConsistencyScore}/100 | ${r.productionReady ? "✅" : "❌"} | ${r.validated ? "✅" : "❌"} |`
    ),
    "",
    "## Performance",
    "",
