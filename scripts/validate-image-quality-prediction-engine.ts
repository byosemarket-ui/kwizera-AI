...
import {
  ...
  type ImageQualityPredictionEngineStatusReport,
  type ImageQualityPredictionRecord,
} from "../ai/index.js";
...
async function main(): Promise<void> {
  ...
  try {
    ...
    results.qualityScores = {
      passed:
        (product.record?.scores.overallImageQualityScore ?? 0) >= 55 &&
    ...
    results.predictionAccuracy = {
      passed:
        (product.record?.predictions.productionSuccessProbability ?? 0) >= 55 &&
    ...
    results.relationshipDetection = {
      passed:
        (product.record?.relationships.relatedImagePlans.length ?? 0) >= 1 &&
    ...
    fs.writeFileSync(
      path.join(projectStateDir, "Image-Quality-Prediction-Report.md"),
      buildPredictionReport(status, results, storageRoot, allPassed, product.record, campaign.record, banner.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Image-Risk-Analysis-Report.md"),
      buildRiskReport(product.record, campaign.record, banner.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Image-Production-Quality-Report.md"),
      buildProductionQualityReport(product.record, campaign.record, banner.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Image-Quality-Recommendations.md"),
      buildRecommendationsReport(product.record, campaign.record, banner.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(process.cwd(), "STEP-6L-VALIDATION-REPORT.md"),
      buildPredictionReport(status, results, storageRoot, allPassed, product.record, campaign.record, banner.record),
      "utf8"
    );
...
function buildPredictionReport(
  status: ImageQualityPredictionEngineStatusReport,
  results: Record<string, { passed: boolean; detail: string }>,
  storageRoot: string,
  allPassed: boolean,
  product?: ImageQualityPredictionRecord,
  campaign?: ImageQualityPredictionRecord,
  banner?: ImageQualityPredictionRecord
): string {
  return [
...
function buildRiskReport(
  ...
  const rows = [product, campaign, banner].filter(Boolean) as ImageQualityPredictionRecord[];
  return [
    ...
    ...rows.map((r) => {
      ...
      for (const risk of r.risks) counts[risk.severity]++;
    ...
    "",
    ...rows.flatMap((r) =>
      r.risks.map((risk) => `- ${r.imageId}: [${risk.severity}] ${risk.category} — ${risk.description}`)
    ),
    "",
  ].join("\n");
}
...
function buildProductionQualityReport(
  ...
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r.imageId} | ${r.scores.overallImageQualityScore} | ${r.scores.technicalQualityScore} | ${r.scores.compositionScore} | ${r.scores.lightingScore} | ${r.scores.colorScore} | ${r.scores.brandConsistencyScore} | ${r.scores.marketingEffectivenessScore} | ${r.scores.productionReadinessScore} |`
    ),
    "",
    "## Platform Readiness",
    "",
    ...rows.map(
      (r) =>
        `- ${r.imageId}: primary ${r.profile.platform} — ${r.platformQuality.find((p) => p.platform === r.profile.platform)?.readinessScore ?? 0}/100`
    ),
    "",
  ].join("\n");
}
  ...
  return [
    ...
    ...rows.map((r) => [
      ...
      ...r.recommendations.map(
        (rec) => `- **[${rec.priority}]** ${rec.category}: ${rec.suggestion} — _${rec.reason}_`
      ),
      "",
