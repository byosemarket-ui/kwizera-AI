...
import {
  ...
  type AudioRenderEngineStatusReport,
  type AudioRenderRecord,
} from "../ai/index.js";
...
async function main(): Promise<void> {
  ...
  try {
    ...
    results.renderValidation = {
      passed:
        (tech.record?.renderValidation.length ?? 0) === ALL_AUDIO_RENDER_VALIDATION_STAGES.length &&
    ...
    results.trackValidation = {
      passed:
        (tech.record?.trackValidation.length ?? 0) === ALL_AUDIO_RENDER_TRACK_CHECKS.length &&
    ...
    results.timelineValidation = {
      passed:
        (tech.record?.timelineValidation.length ?? 0) === ALL_AUDIO_RENDER_TIMELINE_CHECKS.length &&
    ...
    results.assetValidation = {
      passed:
        (tech.record?.assetValidation.length ?? 0) === ALL_AUDIO_RENDER_ASSET_TYPES.length &&
    ...
    results.renderSettings = {
      passed:
        (tech.record?.renderSettings.sampleRate ?? 0) >= 44100 &&
    ...
    fs.writeFileSync(
      path.join(projectStateDir, "AI-Audio-Rendering-Preparation-Report.md"),
      buildMainReport(status, results, allPassed, tech.record, health.record, finance.record, film.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Render-Profile-Report.md"),
      buildProfileReport(tech.record, health.record, finance.record, film.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Track-Integrity-Report.md"),
      buildTrackReport(tech.record, health.record, finance.record, film.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Timeline-Integrity-Report.md"),
      buildTimelineReport(tech.record, health.record, finance.record, film.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Rendering-Readiness-Report.md"),
      buildReadinessReport(status, tech.record, health.record, finance.record, film.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(process.cwd(), "STEP-10K-VALIDATION-REPORT.md"),
      buildMainReport(status, results, allPassed, tech.record, health.record, finance.record, film.record),
      "utf8"
    );
...
function buildMainReport(
  status: AudioRenderEngineStatusReport,
  results: Record<string, { passed: boolean; detail: string }>,
  allPassed: boolean,
  tech?: AudioRenderRecord,
  health?: AudioRenderRecord,
  finance?: AudioRenderRecord,
  film?: AudioRenderRecord
): string {
  return [
...
function buildProfileReport(...records: (AudioRenderRecord | undefined)[]): string {
  const rows = records.filter(Boolean) as AudioRenderRecord[];
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r.profile.platform} | ${r.profile.platform} | ${r.renderSettings.sampleRate}Hz | ${r.renderSettings.bitDepth}-bit | ${r.renderSettings.codec} | ${r.renderSettings.loudnessTarget} LUFS |`
    ),
  ].join("\n");
}

function buildTrackReport(...records: (AudioRenderRecord | undefined)[]): string {
  const rows = records.filter(Boolean) as AudioRenderRecord[];
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r.profile.platform} | ${r.trackStructure.length} | ${r.trackValidation.filter((t) => t.validated).length}/${r.trackValidation.length} | ${r.scores.trackIntegrityScore}/100 |`
    ),
  ].join("\n");
}

function buildTimelineReport(...records: (AudioRenderRecord | undefined)[]): string {
  const rows = records.filter(Boolean) as AudioRenderRecord[];
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r.profile.platform} | ${r.timelineStructure.length} | ${r.timelineValidation.filter((t) => t.validated).length}/${r.timelineValidation.length} | ${r.scores.timelineIntegrityScore}/100 |`
    ),
  ].join("\n");
}
...
function buildReadinessReport(
  status: AudioRenderEngineStatusReport,
  ...records: (AudioRenderRecord | undefined)[]
): string {
  const rows = records.filter(Boolean) as AudioRenderRecord[];
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r.profile.platform} | ${r.scores.renderReadinessScore} | ${r.scores.assetQualityScore} | ${r.scores.trackIntegrityScore} | ${r.scores.timelineIntegrityScore} | ${r.scores.platformCompatibilityScore} | ${r.scores.aiConfidenceScore} | ${r.renderReady ? "✅" : "❌"} |`
    ),
  ].join("\n");
}
