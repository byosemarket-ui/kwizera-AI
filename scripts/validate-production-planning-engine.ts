...
import {
  ...
  type ProductionPlanningEngineStatusReport,
  type ProductionPlanningRecord,
} from "../ai/index.js";
...
async function main(): Promise<void> {
  ...
  try {
    ...
    results.workflowPlanning = {
      passed:
        Boolean(tech.record?.workflow.preProduction) &&
    ...
    results.dependencyValidation = {
      passed:
        tech.record?.dependencies.storyboard === true &&
    ...
    results.exportPreparation = {
      passed:
        Boolean(tech.record?.exportPreparation.mp4) &&
    ...
    results.recoveryPlan = {
      passed:
        Boolean(tech.record?.recoveryPlan.checkpointStrategy) &&
    ...
    results.sceneProduction = {
      passed:
        (tech.record?.sceneProductionPlans.length ?? 0) >= 5 &&
    ...
    fs.writeFileSync(
      path.join(projectStateDir, "Production-Planning-Report.md"),
      buildProductionPlanningReport(status, results, storageRoot, allPassed, tech.record, fashion.record, beauty.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Workflow-Report.md"),
      buildWorkflowReport(tech.record, fashion.record, beauty.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Asset-Validation-Report.md"),
      buildAssetValidationReport(tech.record, fashion.record, beauty.record),
      "utf8"
    );
    fs.writeFileSync(
    ...
    fs.writeFileSync(
      path.join(process.cwd(), "STEP-5K-VALIDATION-REPORT.md"),
      buildProductionPlanningReport(status, results, storageRoot, allPassed, tech.record, fashion.record, beauty.record),
      "utf8"
    );
...
function buildProductionPlanningReport(
  status: ProductionPlanningEngineStatusReport,
  results: Record<string, { passed: boolean; detail: string }>,
  storageRoot: string,
  allPassed: boolean,
  tech?: ProductionPlanningRecord,
  fashion?: ProductionPlanningRecord,
  beauty?: ProductionPlanningRecord
): string {
  return [
...
function buildWorkflowReport(
  ...
  const rows = [tech, fashion, beauty].filter(Boolean) as ProductionPlanningRecord[];
  const lines = [
  ...
  for (const record of rows) {
    ...
    lines.push(
      ...
      "",
      `- **Pre-Production:** ${w.preProduction}`,
      `- **Asset Validation:** ${w.assetValidation}`,
      `- **Scene Preparation:** ${w.scenePreparation}`,
      `- **Visual Preparation:** ${w.visualPreparation}`,
      `- **Audio Preparation:** ${w.audioPreparation}`,
      `- **Rendering Preparation:** ${w.renderingPreparation}`,
      `- **Export Preparation:** ${w.exportPreparation}`,
      `- **Delivery Preparation:** ${w.deliveryPreparation}`,
      ""
    );
  }
...
function buildAssetValidationReport(
  tech?: ProductionPlanningRecord,
  fashion?: ProductionPlanningRecord,
  beauty?: ProductionPlanningRecord
): string {
  ...
  for (const record of rows) {
    ...
    lines.push(
      `## ${record.productId}`,
      "",
      `- **Asset Readiness Score:** ${record.scores.assetReadinessScore}/100`,
      `- **Required Assets:** ${required.length}`,
      `- **Videos:** ${record.assets.videos.length}`,
      `- **Voice-over:** ${record.assets.voiceOver.length}`,
      `- **Subtitles:** ${record.assets.subtitles.length}`,
      "",
...
function buildReadinessReport(
  ...
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r.productId} | ${r.scores.productionReadinessScore}/100 | ${r.scores.assetReadinessScore}/100 | ${r.scores.workflowReadinessScore}/100 | ${r.scores.dependencyScore}/100 | ${r.scores.performanceScore}/100 | ${r.productionReady ? "✅" : "❌"} | ${r.validated ? "✅" : "❌"} |`
    ),
    "",
    "## Performance",
    "",
