...
import {
  ...
  type ImageGenerationFoundationStatusReport,
  type ImageGenerationHealthReport,
} from "../ai/index.js";
import {
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const registryPath = path.join(
      ...
      "image-generation",
      "registry",
      "image-generation-registry.json"
    );
    ...
    results.samplePrompt = {
      passed:
        foundation.getAssetRegistry().getAsset(prompt.assetId)?.assetType === ImageGenerationAssetType.Prompt &&
...
function buildFoundationReport(
  status: ImageGenerationFoundationStatusReport,
  health: ImageGenerationHealthReport,
...
function buildArchitectureReport(status: ImageGenerationFoundationStatusReport): string {
