...
import {
  ...
  type AudioProductionEngineStatusReport,
  type AudioProductionRecord,
} from "../ai/index.js";
...
async function main(): Promise<void> {
  ...
  try {
    ...
    results.workflowValidation = {
      passed:
        (tech.record?.workflowValidation.length ?? 0) === ALL_AUDIO_PRODUCTION_WORKFLOW_STAGES.length &&
    ...
    results.assetValidation = {
      passed:
        (tech.record?.assetValidation.length ?? 0) === ALL_AUDIO_PRODUCTION_ASSET_TYPES.length &&
    ...
    results.dependencyValidation = {
      passed:
        (tech.record?.dependencyValidation.length ?? 0) === ALL_AUDIO_PRODUCTION_DEPENDENCIES.length &&
    ...
    results.trackValidation = {
      passed:
        (tech.record?.productionStructure.trackStructure.length ?? 0) >= 5 &&
    ...
    results.renderPreparation = {
      passed:
        Boolean(tech.record?.renderPreparation.sampleRate) &&
    ...
    results.exportPreparation = {
      passed:
        (tech.record?.exportPreparation.exports.length ?? 0) >= ALL_AUDIO_PRODUCTION_EXPORT_FORMATS.length &&
    ...
    fs.writeFileSync(
      path.join(projectStateDir, "AI-Audio-Production-Report.md"),
    ...
    fs.writeFileSync(
      ...
      buildWorkflowReport(tech.record, health.record, finance.record, film.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Asset-Validation-Report.md"),
      buildAssetReport(tech.record, health.record, finance.record, film.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Track-Validation-Report.md"),
    ...
    fs.writeFileSync(
      path.join(process.cwd(), "STEP-10J-VALIDATION-REPORT.md"),
      buildMainReport(status, results, allPassed, tech.record, health.record, finance.record, film.record),
      "utf8"
    );
...
function buildMainReport(
  status: AudioProductionEngineStatusReport,
  results: Record<string, { passed: boolean; detail: string }>,
  allPassed: boolean,
  tech?: AudioProductionRecord,
  health?: AudioProductionRecord,
  finance?: AudioProductionRecord,
  film?: AudioProductionRecord
): string {
  return [
...
function buildWorkflowReport(...records: (AudioProductionRecord | undefined)[]): string {
  const rows = records.filter(Boolean) as AudioProductionRecord[];
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r.profile.audioPlanId.slice(0, 25)}... | ${r.profile.platform} | ${r.workflowValidation.filter((w) => w.validated).length}/${r.workflowValidation.length} | ${r.scores.workflowScore}/100 |`
    ),
  ].join("\n");
}

function buildAssetReport(...records: (AudioProductionRecord | undefined)[]): string {
  const rows = records.filter(Boolean) as AudioProductionRecord[];
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r.profile.platform} | ${r.profile.platform} | ${r.assetValidation.filter((a) => a.validated).length}/${r.assetValidation.length} | ${r.scores.assetReadinessScore}/100 |`
    ),
  ].join("\n");
}

function buildTrackReport(...records: (AudioProductionRecord | undefined)[]): string {
  const rows = records.filter(Boolean) as AudioProductionRecord[];
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r.profile.platform} | ${r.productionStructure.trackStructure.length} | ${r.productionStructure.busStructure.length} | ${r.productionStructure.timelineStructure.length} | ${r.scores.trackIntegrityScore}/100 |`
    ),
  ].join("\n");
}
...
function buildReadinessReport(
  status: AudioProductionEngineStatusReport,
  ...records: (AudioProductionRecord | undefined)[]
): string {
  const rows = records.filter(Boolean) as AudioProductionRecord[];
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r.profile.platform} | ${r.scores.productionReadinessScore} | ${r.scores.assetReadinessScore} | ${r.scores.workflowScore} | ${r.scores.trackIntegrityScore} | ${r.scores.dependencyScore} | ${r.scores.aiConfidenceScore} | ${r.productionReady ? "✅" : "❌"} |`
    ),
  ].join("\n");
}
