...
import {
  ...
  type AudioPlanningEngineStatusReport,
  type AudioPlanningRecord,
} from "../ai/index.js";
...
async function main(): Promise<void> {
  ...
  try {
    ...
    results.sceneAudioPlanning = {
      passed:
        (tech.record?.sceneAudioPlans.length ?? 0) >= 5 &&
    ...
    results.voicePlanning = {
      passed:
        Boolean(tech.record?.voicePlanning.voiceStyle) &&
    ...
    results.musicPlanning = {
      passed:
        Boolean(tech.record?.musicPlanning.introMusic) &&
    ...
    results.soundEffectPlanning = {
      passed:
        (tech.record?.soundEffectPlanning.transitionSounds.length ?? 0) >= 2 &&
    ...
    results.synchronization = {
      passed:
        Object.keys(tech.record?.synchronization.voiceTiming ?? {}).length >= 5 &&
    ...
    results.emotionalFlow = {
      passed:
        Boolean(tech.record?.emotionalFlow.excitement) &&
    ...
    results.upstreamAlignment = {
      passed:
        tech.record?.storyboardId === storyboard?.storyboardId &&
    ...
    results.audioScores = {
      passed:
        (tech.record?.scores.audioPlanningScore ?? 0) >= 55 &&
    ...
    fs.writeFileSync(
      path.join(projectStateDir, "Audio-Planning-Report.md"),
      buildAudioPlanningReport(status, results, storageRoot, allPassed, tech.record, fashion.record, beauty.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Voice-Planning-Report.md"),
      buildVoicePlanningReport(tech.record, fashion.record, beauty.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Music-Planning-Report.md"),
      buildMusicPlanningReport(tech.record, fashion.record, beauty.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Synchronization-Report.md"),
      buildSynchronizationReport(tech.record, fashion.record, beauty.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Audio-Readiness-Report.md"),
      buildReadinessReport(status, tech.record, fashion.record, beauty.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(process.cwd(), "STEP-5J-VALIDATION-REPORT.md"),
      buildAudioPlanningReport(status, results, storageRoot, allPassed, tech.record, fashion.record, beauty.record),
      "utf8"
    );
...
function buildAudioPlanningReport(
  status: AudioPlanningEngineStatusReport,
  results: Record<string, { passed: boolean; detail: string }>,
  storageRoot: string,
  allPassed: boolean,
  tech?: AudioPlanningRecord,
  fashion?: AudioPlanningRecord,
  beauty?: AudioPlanningRecord
): string {
  return [
...
function buildVoicePlanningReport(
  tech?: AudioPlanningRecord,
  fashion?: AudioPlanningRecord,
  beauty?: AudioPlanningRecord
): string {
  const rows = [tech, fashion, beauty].filter(Boolean) as AudioPlanningRecord[];
  const lines = [
  ...
  for (const record of rows) {
    ...
    lines.push(
      ...
      `- **Gender Preference:** ${v.voiceGenderPreference}`,
      `- **Age Style:** ${v.voiceAgeStyle}`,
      `- **Speaking Speed:** ${v.speakingSpeed}`,
      `- **Speaking Tone:** ${v.speakingTone}`,
      `- **Emotional Tone:** ${v.emotionalTone}`,
      `- **Emphasis Points:** ${v.emphasisPoints.join("; ")}`,
      "",
      "| # | Purpose | Planned Voice-Over | Reading Duration |",
      "|---|---------|-------------------|------------------|"
    );
    for (const scene of record.sceneAudioPlans) {
      lines.push(
        `| ${scene.sceneNumber} | ${scene.scenePurpose} | ${scene.plannedVoiceOver.slice(0, 50)}... | ${v.readingDuration[scene.sceneNumber] ?? scene.plannedNarrationTiming} |`
      );
    }
    lines.push("");
  }
...
function buildMusicPlanningReport(
  tech?: AudioPlanningRecord,
  fashion?: AudioPlanningRecord,
  beauty?: AudioPlanningRecord
): string {
  ...
  for (const record of rows) {
    ...
    lines.push(
      ...
      `- **Style:** ${m.musicStyle}`,
      `- **Mood:** ${m.musicMood}`,
      `- **Energy:** ${m.musicEnergy}`,
      `- **Intro:** ${m.introMusic}`,
      `- **Background:** ${m.backgroundMusic}`,
      `- **Ending:** ${m.endingMusic}`,
      `- **Fade In:** ${m.fadeIn}`,
      `- **Fade Out:** ${m.fadeOut}`,
      `- **Volume Strategy:** ${m.volumeStrategy}`,
      ""
    );
  }
...
function buildSynchronizationReport(
  tech?: AudioPlanningRecord,
  fashion?: AudioPlanningRecord,
  beauty?: AudioPlanningRecord
): string {
  ...
  for (const record of rows) {
    ...
    lines.push(
      `## ${record.productId}`,
      "",
      `- **CTA Timing:** ${s.ctaTiming}`,
      "",
      "| # | Voice Timing | Music Timing | Subtitle Timing | Transition |",
      "|---|--------------|--------------|-----------------|------------|"
    );
    for (const scene of record.sceneAudioPlans) {
      lines.push(
        `| ${scene.sceneNumber} | ${(s.voiceTiming[scene.sceneNumber] ?? "").slice(0, 30)}... | ${(s.musicTiming[scene.sceneNumber] ?? "").slice(0, 25)}... | ${(s.subtitleTiming[scene.sceneNumber] ?? "").slice(0, 25)}... | ${(s.transitionTiming[scene.sceneNumber] ?? "").slice(0, 25)}... |`
      );
    }
    lines.push("");
  }
...
function buildReadinessReport(
  ...
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r.productId} | ${r.scores.audioPlanningScore}/100 | ${r.scores.voicePlanningScore}/100 | ${r.scores.musicPlanningScore}/100 | ${r.scores.synchronizationScore}/100 | ${r.scores.brandConsistencyScore}/100 | ${r.productionReady ? "✅" : "❌"} | ${r.validated ? "✅" : "❌"} |`
    ),
    "",
    "## Performance",
    "",
