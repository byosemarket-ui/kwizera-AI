...
import {
  ...
  type RenderingPreparationEngineStatusReport,
  type RenderingPreparationRecord,
} from "../ai/index.js";
...
async function main(): Promise<void> {
  ...
  try {
    ...
    results.renderSettings = {
      passed: Boolean(
        firstPlan?.renderSettings.resolution &&
    ...
    results.resourcePlanning = {
      passed: Boolean(
        firstPlan?.resourcePlanning.cpuAllocation &&
    ...
    results.recoveryPlanning = {
      passed: Boolean(
        firstPlan?.recoveryPlan.checkpointStrategy &&
    ...
    results.renderScores = {
      passed:
        (firstPlan?.scores.renderReadinessScore ?? 0) >= 55 &&
    ...
    fs.writeFileSync(
      path.join(projectStateDir, "AI-Rendering-Preparation-Report.md"),
      buildMainReport(status, results, storageRoot, allPassed, techRender.plans, fashionRender.plans, beautyRender.plans),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Render-Profile-Report.md"),
      buildRenderProfileReport(techRender.plans),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Resource-Planning-Report.md"),
      buildResourceReport(techRender.plans),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Timeline-Integrity-Report.md"),
      buildTimelineReport(techRender.plans),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Rendering-Readiness-Report.md"),
      buildReadinessReport(status, techRender.plans, fashionRender.plans, beautyRender.plans),
      "utf8"
    );
    fs.writeFileSync(
      path.join(process.cwd(), "STEP-8K-VALIDATION-REPORT.md"),
      buildMainReport(status, results, storageRoot, allPassed, techRender.plans, fashionRender.plans, beautyRender.plans),
      "utf8"
    );
...
function buildMainReport(
  status: RenderingPreparationEngineStatusReport,
  results: Record<string, { passed: boolean; detail: string }>,
  storageRoot: string,
  allPassed: boolean,
  tech?: RenderingPreparationRecord[],
  fashion?: RenderingPreparationRecord[],
  beauty?: RenderingPreparationRecord[]
): string {
  return [
...
function buildRenderProfileReport(plans?: RenderingPreparationRecord[]): string {
  ...
  for (const plan of plans ?? []) {
    ...
    lines.push(
      `## ${p.renderPlanId}`,
      "",
      `- Render Plan ID: ${p.renderPlanId}`,
      `- Project ID: ${p.projectId}`,
      `- Production ID: ${p.productionId}`,
      `- Video ID: ${p.videoId}`,
      `- Platform: ${p.platform}`,
      `- Render Version: ${p.renderVersion}`,
      `- Resolution: ${s.resolution}`,
      `- Codec: ${s.codec}`,
      `- Output profiles: ${plan.outputProfiles.length}`,
      ""
    );
  }
  return lines.join("\n");
...
function buildResourceReport(plans?: RenderingPreparationRecord[]): string {
  const lines = ["# Resource Planning Report — Step 8K", "", `**Date:** ${new Date().toISOString()}`, ""];
  for (const plan of plans ?? []) {
    const r = plan.resourcePlanning;
    lines.push(
      `## ${plan.profile.renderPlanId}`,
      "",
      `- CPU: ${r.cpuAllocation}`,
      `- GPU: ${r.gpuAllocation}`,
      `- RAM: ${r.ramAllocation}`,
      `- Storage: ${r.storageAllocation}`,
      `- Cache: ${r.cacheAllocation}`,
      `- Render queue: ${r.renderQueue}`,
      `- Parallel prep: ${r.parallelRenderingPreparation}`,
      ""
    );
  }
  return lines.join("\n");
...
function buildTimelineReport(plans?: RenderingPreparationRecord[]): string {
  ...
  for (const plan of plans ?? []) {
    ...
    lines.push(
      `## ${plan.profile.renderPlanId}`,
      "",
      `- All timelines valid: ${t.allTimelinesValid}`,
      `- Scene timeline: ${t.sceneTimeline.length} entries`,
      `- Camera timeline: ${t.cameraTimeline.length} entries`,
      `- Render timeline: ${t.renderTimeline.join(" → ")}`,
      `- Integrity score: ${plan.scores.timelineIntegrityScore}`,
      ""
    );
  }
  return lines.join("\n");
