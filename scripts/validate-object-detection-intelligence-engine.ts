...
import {
  ...
  type ObjectDetectionEngineStatusReport,
  type ObjectDetectionRecord,
} from "../ai/index.js";
...
async function preparePipeline(
  foundation: ReturnType<ReturnType<typeof createAiCore>["getManager"]>["imageIntelligenceFoundation"],
  sample: ImageAnalysisEngineInput,
  understandOpts?: { marketingGoal?: ImageUnderstandingMarketingGoal; industry?: string; platform?: ImageUnderstandingPlatform }
): Promise<void> {
...
async function main(): Promise<void> {
  ...
  try {
    ...
    results.productDetection = {
      passed:
        Boolean(product.record?.productDetection.mainProduct) &&
    ...
    results.logoDetection = {
      passed:
        product.record?.logoDetection.logoPresent === true &&
    ...
    results.textDetection = {
      passed:
        product.record?.textDetection.textPresent === false ||
    ...
    results.objectProperties = {
      passed: product.record?.objects.every(
        (o) =>
          o.objectId &&
          o.objectType &&
          o.objectName &&
    ...
    results.qualityScores = {
      passed:
        (product.record?.scores.objectDetectionScore ?? 0) >= 55 &&
    ...
    fs.writeFileSync(
      path.join(projectStateDir, "Object-Detection-Report.md"),
      buildObjectDetectionReport(status, results, storageRoot, allPassed, product.record, lifestyle.record, banner.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Product-Detection-Report.md"),
      buildProductReport(product.record, lifestyle.record, banner.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Logo-Detection-Report.md"),
      buildLogoReport(product.record, lifestyle.record, banner.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Object-Readiness-Report.md"),
      buildReadinessReport(status, allPassed),
      "utf8"
    );
    fs.writeFileSync(
      path.join(process.cwd(), "STEP-6D-VALIDATION-REPORT.md"),
      buildObjectDetectionReport(status, results, storageRoot, allPassed, product.record, lifestyle.record, banner.record),
      "utf8"
    );
...
function buildObjectDetectionReport(
  status: ObjectDetectionEngineStatusReport,
  results: Record<string, { passed: boolean; detail: string }>,
  storageRoot: string,
  allPassed: boolean,
  product?: ObjectDetectionRecord,
  lifestyle?: ObjectDetectionRecord,
  banner?: ObjectDetectionRecord
): string {
  return [
...
function buildProductReport(
  ...
  const rows = [product, lifestyle, banner].filter(Boolean) as ObjectDetectionRecord[];
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r.imageId} | ${r.productDetection.mainProduct ?? "n/a"} | ${r.productDetection.secondaryProducts.join(", ") || "none"} | ${r.productDetection.productVisibility} | ${r.productDetection.productPosition} | ${r.productDetection.productGrouping} | ${r.productDetection.productPresentation} |`
    ),
    "",
...
function buildLogoReport(
  ...
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r.imageId} | ${r.logoDetection.logoPresent} | ${r.logoDetection.brandAssociation || "n/a"} | ${r.logoDetection.logoPosition} | ${r.logoDetection.logoVisibility} | ${r.logoDetection.logoSize} | ${r.logoDetection.logoRegions.length} |`
    ),
    "",
...
function buildReadinessReport(status: ObjectDetectionEngineStatusReport, allPassed: boolean): string {
  return [
