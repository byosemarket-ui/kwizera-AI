...
import {
  ...
  type MemoryFoundationStatusReport,
...
async function main(): Promise<void> {
  ...
  try {
    ...
    results.logging = {
      passed: Boolean(
        foundation.logger.getLogDirectory() && fs.existsSync(foundation.logger.getLogDirectory()!)
      ),
    ...
    fs.writeFileSync(
      reportPath,
      buildReport(report, health, results, storageRoot, allPassed),
      "utf8"
    );
...
function buildReport(
  status: MemoryFoundationStatusReport,
