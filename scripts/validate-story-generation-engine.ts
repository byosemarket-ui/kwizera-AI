...
import {
  ...
  type StoryboardGenerationEngineStatusReport,
...
async function main(): Promise<void> {
  ...
  try {
    ...
    results.storyStructure = {
      passed: Boolean(
        tech.record?.storyStructure.openingHook &&
    ...
    results.marketingPlanning = {
      passed: Boolean(
        tech.record?.marketingPlanning.conversionStrategy &&
    ...
    results.storyboardScores = {
      passed:
        (tech.record?.scores.storyQualityScore ?? 0) >= 55 &&
    ...
    fs.writeFileSync(
      path.join(projectStateDir, "AI-Storyboard-Generation-Report.md"),
      buildMainReport(status, results, storageRoot, allPassed, tech.record, fashion.record, beauty.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Scene-Storyboard-Report.md"),
      buildSceneReport(tech.record, fashion.record, beauty.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Shot-Planning-Report.md"),
      buildShotReport(tech.record, fashion.record, beauty.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Marketing-Storyboard-Report.md"),
      buildMarketingReport(tech.record, fashion.record, beauty.record),
      "utf8"
    );
    fs.writeFileSync(
    ...
    fs.writeFileSync(
      path.join(process.cwd(), "STEP-8B-VALIDATION-REPORT.md"),
      buildMainReport(status, results, storageRoot, allPassed, tech.record, fashion.record, beauty.record),
      "utf8"
    );
...
function buildMainReport(
  status: StoryboardGenerationEngineStatusReport,
  results: Record<string, { passed: boolean; detail: string }>,
  storageRoot: string,
  allPassed: boolean,
  tech?: StoryboardGenerationRecord,
  fashion?: StoryboardGenerationRecord,
  beauty?: StoryboardGenerationRecord
): string {
  return [
...
function buildSceneReport(
  ...
  const rows = [tech, fashion, beauty].filter(Boolean) as StoryboardGenerationRecord[];
  const lines = [
  ...
  for (const record of rows) {
    ...
    for (const scene of record.scenes) {
      lines.push(
        `| ${scene.sceneOrder} | ${scene.scenePurpose} | ${scene.sceneDuration} | ${scene.sceneMood} | ${scene.sceneObjective.slice(0, 40)}... |`
      );
    }
    lines.push("");
  }
  ...
  for (const record of rows) {
    ...
    for (const scene of record.scenes) {
      ...
      for (const shot of scene.shots) {
        lines.push(
          `| ${shot.shotOrder} | ${shot.shotType} | ${shot.cameraAngle} | ${shot.cameraMovement} | ${shot.duration} |`
        );
      }
      lines.push("");
    }
...
function buildMarketingReport(
  ...
  for (const record of rows) {
    lines.push(
      `| ${record.profile.productId} | ${record.scores.marketingScore}/100 | ${record.marketingPlanning.productReveal.slice(0, 30)}... | ${record.marketingPlanning.ctaPlacement.slice(0, 25)}... | ${record.marketingPlanning.conversionStrategy.slice(0, 30)}... | ${record.marketingReady ? "✅" : "❌"} |`
    );
  }
  ...
  if (tech) {
    lines.push(
      `- Attention: ${tech.viewerJourney.attentionPhase.slice(0, 60)}...`,
      `- Interest: ${tech.viewerJourney.interestPhase.slice(0, 60)}...`,
      `- Desire: ${tech.viewerJourney.desirePhase.slice(0, 60)}...`,
      `- Action: ${tech.viewerJourney.actionPhase.slice(0, 60)}...`
    );
  }
...
function buildReadinessReport(
  ...
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r.profile.productId} | ${r.scores.storyQualityScore} | ${r.scores.marketingScore} | ${r.scores.creativeScore} | ${r.scores.cinematicScore} | ${r.scores.productionReadinessScore} | ${r.scores.aiConfidenceScore} | ${r.productionReady ? "✅" : "❌"} |`
    ),
    "",
    "## Performance",
    "",
