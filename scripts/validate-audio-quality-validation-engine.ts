...
import {
  ...
  type AudioQualityValidationEngineStatusReport,
  type AudioQualityValidationRecord,
} from "../ai/index.js";
...
async function main(): Promise<void> {
  ...
  try {
    ...
    results.audioQualityChecks = {
      passed:
        (tech.record?.audioQuality.length ?? 0) === ALL_AUDIO_QUALITY_CHECKS.length &&
    ...
    results.trackValidation = {
      passed:
        (tech.record?.trackValidation.length ?? 0) === ALL_AUDIO_QUALITY_TRACK_CHECKS.length &&
    ...
    results.timelineValidation = {
      passed:
        (tech.record?.timelineValidation.length ?? 0) === ALL_AUDIO_QUALITY_TIMELINE_CHECKS.length &&
    ...
    results.syncValidation = {
      passed:
        (tech.record?.syncValidation.length ?? 0) === ALL_AUDIO_SYNC_CHECKS.length &&
    ...
    results.brandValidation = {
      passed:
        (tech.record?.brandValidation.length ?? 0) === ALL_AUDIO_BRAND_VALIDATION_CHECKS.length &&
    ...
    results.technicalValidation = {
      passed:
        (tech.record?.technicalValidation.length ?? 0) === ALL_AUDIO_TECHNICAL_VALIDATION_CHECKS.length &&
    ...
    results.qualityScores = {
      passed:
        (tech.record?.scores.overallAudioQualityScore ?? 0) >= 55 &&
    ...
    fs.writeFileSync(
      path.join(projectStateDir, "AI-Audio-Quality-Validation-Report.md"),
      buildMainReport(status, results, allPassed, tech.record, health.record, finance.record, film.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Audio-Quality-Report.md"),
      buildAudioQualityReport(tech.record, health.record, finance.record, film.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Synchronization-Validation-Report.md"),
      buildSyncReport(tech.record, health.record, finance.record, film.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Brand-Validation-Report.md"),
      buildBrandReport(tech.record, health.record, finance.record, film.record),
      "utf8"
    );
    fs.writeFileSync(
    ...
    fs.writeFileSync(
      path.join(process.cwd(), "STEP-10L-VALIDATION-REPORT.md"),
      buildMainReport(status, results, allPassed, tech.record, health.record, finance.record, film.record),
      "utf8"
    );
...
function buildMainReport(
  status: AudioQualityValidationEngineStatusReport,
  results: Record<string, { passed: boolean; detail: string }>,
  allPassed: boolean,
  tech?: AudioQualityValidationRecord,
  health?: AudioQualityValidationRecord,
  finance?: AudioQualityValidationRecord,
  film?: AudioQualityValidationRecord
): string {
  return [
...
function buildAudioQualityReport(...records: (AudioQualityValidationRecord | undefined)[]): string {
  const rows = records.filter(Boolean) as AudioQualityValidationRecord[];
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r.profile.platform} | ${r.profile.platform} | ${r.audioQuality.filter((e) => e.validated).length}/${r.audioQuality.length} | ${r.scores.loudnessScore} | ${r.scores.frequencyBalanceScore} | ${r.scores.overallAudioQualityScore}/100 |`
    ),
  ].join("\n");
}

function buildSyncReport(...records: (AudioQualityValidationRecord | undefined)[]): string {
  const rows = records.filter(Boolean) as AudioQualityValidationRecord[];
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r.profile.platform} | ${r.syncValidation.filter((s) => s.validated).length}/${r.syncValidation.length} | ${r.scores.synchronizationScore}/100 | ${r.issues.filter((i) => i.category === "sync-problem").length} |`
    ),
  ].join("\n");
}

function buildBrandReport(...records: (AudioQualityValidationRecord | undefined)[]): string {
  const rows = records.filter(Boolean) as AudioQualityValidationRecord[];
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r.profile.platform} | ${r.brandValidation.filter((b) => b.validated).length}/${r.brandValidation.length} | ${r.scores.brandConsistencyScore}/100 | ${r.approved ? "✅" : "❌"} |`
    ),
  ].join("\n");
}
...
function buildReadinessReport(
  status: AudioQualityValidationEngineStatusReport,
  ...records: (AudioQualityValidationRecord | undefined)[]
): string {
  const rows = records.filter(Boolean) as AudioQualityValidationRecord[];
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r.profile.platform} | ${r.scores.overallAudioQualityScore} | ${r.scores.productionReadinessScore} | ${r.scores.platformCompatibilityScore} | ${r.scores.aiConfidenceScore} | ${r.approved ? "✅" : "❌"} | ${r.renderReady ? "✅" : "❌"} |`
    ),
  ].join("\n");
}
