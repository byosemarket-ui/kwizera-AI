...
import {
  ...
  type ProductUnderstandingEngineStatusReport,
  type ProductUnderstandingRecord,
} from "../ai/index.js";
...
async function main(): Promise<void> {
  ...
  try {
    ...
    results.valueAnalysis = {
      passed:
        (tech.record?.valueAnalysis.functionalValue ?? 0) >= 50 &&
    ...
    results.customerUnderstanding = {
      passed:
        (tech.record?.customer.customerNeeds.length ?? 0) >= 2 &&
    ...
    results.understandingScores = {
      passed:
        (tech.record?.scores.understandingScore ?? 0) >= 55 &&
    ...
    fs.writeFileSync(
      path.join(projectStateDir, "Product-Understanding-Report.md"),
      buildUnderstandingReport(status, results, storageRoot, allPassed, tech.record, fashion.record, beauty.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Customer-Value-Report.md"),
      buildCustomerValueReport(tech.record, fashion.record, beauty.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Business-Value-Report.md"),
      buildBusinessValueReport(tech.record, fashion.record, beauty.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(process.cwd(), "STEP-5C-VALIDATION-REPORT.md"),
      buildUnderstandingReport(status, results, storageRoot, allPassed, tech.record, fashion.record, beauty.record),
      "utf8"
    );
...
function buildUnderstandingReport(
  status: ProductUnderstandingEngineStatusReport,
  results: Record<string, { passed: boolean; detail: string }>,
  storageRoot: string,
  allPassed: boolean,
  tech?: ProductUnderstandingRecord,
  fashion?: ProductUnderstandingRecord,
  beauty?: ProductUnderstandingRecord
): string {
  return [
...
function buildCustomerValueReport(
  tech?: ProductUnderstandingRecord,
  fashion?: ProductUnderstandingRecord,
  beauty?: ProductUnderstandingRecord
): string {
  const rows = [tech, fashion, beauty].filter(Boolean) as ProductUnderstandingRecord[];
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r.identity.productName} | ${r.customer.targetCustomer} | ${r.scores.customerValueScore}/100 | ${r.customer.customerNeeds.length} | ${r.customer.customerPainPoints.length} |`
    ),
    "",
  ].join("\n");
}

function buildBusinessValueReport(
  tech?: ProductUnderstandingRecord,
  fashion?: ProductUnderstandingRecord,
  beauty?: ProductUnderstandingRecord
): string {
  ...
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r.identity.productName} | ${r.scores.businessValueScore}/100 | ${r.valueAnalysis.commercialValue} | ${r.valueAnalysis.marketValue} | ${r.valueAnalysis.brandValue} | ${r.uniqueValue.uniqueSellingPoints.length} |`
    ),
    "",
  ].join("\n");
}
