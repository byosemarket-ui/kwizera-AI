...
import {
  ...
  SourceImageCategory,
  TextToImagePlatform,
  type ImageToImageGenerationEngineStatusReport,
  type ImageToImageGenerationRecord,
} from "../ai/index.js";
...
async function main(): Promise<void> {
  ...
  try {
    ...
    results.sourceImageAnalysis = {
      passed: Boolean(
        tech.record?.sourceAnalysis.subject &&
    ...
    results.maskPlanning = {
      passed:
        (tech.record?.maskPlan.masks.length ?? 0) >= 5 &&
    ...
    results.backgroundPlanning = {
      passed: Boolean(
        tech.record?.backgroundPlan.description &&
    ...
    fs.writeFileSync(
      path.join(projectStateDir, "AI-Image-to-Image-Report.md"),
      buildMainReport(status, results, storageRoot, allPassed, tech.record, fashion.record, beauty.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Source-Image-Analysis-Report.md"),
      buildSourceAnalysisReport(tech.record, fashion.record, beauty.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Transformation-Planning-Report.md"),
      buildTransformationReport(tech.record, fashion.record, beauty.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Mask-Planning-Report.md"),
      buildMaskReport(tech.record, fashion.record, beauty.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Image-to-Image-Readiness-Report.md"),
      buildReadinessReport(status, tech.record, fashion.record, beauty.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(process.cwd(), "STEP-9C-VALIDATION-REPORT.md"),
      buildMainReport(status, results, storageRoot, allPassed, tech.record, fashion.record, beauty.record),
      "utf8"
    );
...
function buildMainReport(
  status: ImageToImageGenerationEngineStatusReport,
  results: Record<string, { passed: boolean; detail: string }>,
  storageRoot: string,
  allPassed: boolean,
  tech?: ImageToImageGenerationRecord,
  fashion?: ImageToImageGenerationRecord,
  beauty?: ImageToImageGenerationRecord
): string {
  return [
...
function buildSourceAnalysisReport(
  tech?: ImageToImageGenerationRecord,
  fashion?: ImageToImageGenerationRecord,
  beauty?: ImageToImageGenerationRecord
): string {
  const rows = [tech, fashion, beauty].filter(Boolean) as ImageToImageGenerationRecord[];
  const lines = [
  ...
  for (const record of rows) {
    lines.push(
      `| ${record.profile.productId} | ${record.sourceAnalysis.subject.slice(0, 25)}... | ${record.sourceAnalysis.background.slice(0, 20)}... | ${record.sourceAnalysis.imageQuality} | ${record.sourceAnalysis.resolution} | ${record.scores.identityPreservationScore}/100 |`
    );
  }
...
function buildMaskReport(
  ...
  for (const record of rows) {
    ...
    for (const mask of record.maskPlan.masks) {
      lines.push(
        `| ${mask.maskId.slice(-12)} | ${mask.maskType} | ${mask.editable ? "✅" : "❌"} | ${mask.protected ? "✅" : "❌"} | ${mask.region.slice(0, 30)}... |`
      );
    }
    lines.push("");
  }
...
function buildReadinessReport(
  ...
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r.profile.productId} | ${r.scores.transformationQualityScore} | ${r.scores.identityPreservationScore} | ${r.scores.styleConsistencyScore} | ${r.scores.brandConsistencyScore} | ${r.scores.productionReadinessScore} | ${r.scores.aiConfidenceScore} | ${r.productionReady ? "✅" : "❌"} |`
    ),
    "",
    "## Performance",
    "",
