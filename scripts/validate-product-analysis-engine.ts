...
import {
  ...
  ProductBusinessType,
  type ProductAnalysisEngineStatusReport,
  type ProductAnalysisIntelligenceRecord,
} from "../ai/index.js";
...
const SAMPLE_ELECTRONICS: ProductAnalysisEngineInput = {
...
async function main(): Promise<void> {
  ...
  try {
    ...
    results.classification = {
      passed:
        electronics.record?.classification.industry === ProductAnalysisIndustry.Technology &&
    ...
    results.qualityScores = {
      passed:
        (electronics.record?.scores.marketingReadinessScore ?? 0) >= 50 &&
...
function buildAnalysisReport(
  status: ProductAnalysisEngineStatusReport,
...
function buildClassificationReport(
  electronics: ProductAnalysisIntelligenceRecord | undefined,
  fashion: ProductAnalysisIntelligenceRecord | undefined,
  beauty: ProductAnalysisIntelligenceRecord | undefined
): string {
  ...
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r!.profile.productName} | ${r!.classification.industry} | ${r!.classification.category} | ${r!.classification.subcategory} | ${r!.classification.useCase} | ${r!.classification.targetCustomer} | ${r!.classification.businessType} |`
    ),
    "",
  ].join("\n");
}

function buildReadinessReport(status: ProductAnalysisEngineStatusReport, allPassed: boolean): string {
  return [
