...
import {
  ...
  type ProductionImagePlanningEngineStatusReport,
  type ProductionImagePlanningRecord,
} from "../ai/index.js";
...
async function main(): Promise<void> {
  ...
  try {
    ...
    results.assetValidation = {
      passed:
        (product.record?.assets.originalImages.length ?? 0) >= 1 &&
    ...
    results.relationshipDetection = {
      passed:
        (product.record?.relationships.relatedCreativeImagePlans.length ?? 0) >= 1 &&
    ...
    fs.writeFileSync(
      path.join(projectStateDir, "Production-Image-Planning-Report.md"),
      buildPlanningReport(status, results, storageRoot, allPassed, product.record, campaign.record, banner.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Image-Workflow-Report.md"),
      buildWorkflowReport(product.record, campaign.record, banner.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Image-Asset-Validation-Report.md"),
      buildAssetReport(product.record, campaign.record, banner.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Production-Image-Readiness-Report.md"),
      buildReadinessReport(status, allPassed),
      "utf8"
    );
    fs.writeFileSync(
      path.join(process.cwd(), "STEP-6K-VALIDATION-REPORT.md"),
      buildPlanningReport(status, results, storageRoot, allPassed, product.record, campaign.record, banner.record),
      "utf8"
    );
...
function buildPlanningReport(
  status: ProductionImagePlanningEngineStatusReport,
  results: Record<string, { passed: boolean; detail: string }>,
  storageRoot: string,
  allPassed: boolean,
  product?: ProductionImagePlanningRecord,
  campaign?: ProductionImagePlanningRecord,
  banner?: ProductionImagePlanningRecord
): string {
  return [
...
function buildWorkflowReport(
  ...
  const rows = [product, campaign, banner].filter(Boolean) as ProductionImagePlanningRecord[];
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r.imageId} | ✅ | ✅ | ${r.workflow.compositionValidation.slice(0, 20)}... | ${r.workflow.brandValidation.slice(0, 15)}... | ${r.workflow.creativeValidation.slice(0, 15)}... | ${r.workflow.renderingPreparation.slice(0, 20)}... | ${r.workflow.exportPreparation.slice(0, 15)}... |`
    ),
    "",
    "## Workflow Steps",
    "",
...
function buildAssetReport(
  ...
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r.imageId} | ${r.assets.originalImages[0]?.status ?? "n/a"} | ${r.assets.logos.length} | ${r.assets.fonts.length} | ${r.assets.templates.length} | ${r.assets.brandAssets.length} | ${r.scores.assetReadinessScore} |`
    ),
    "",
    ...
    ...rows.map(
      (r) =>
        `- ${r.imageId}: ${r.dependencies.passedCount}/${r.dependencies.totalRequired} passed — ${r.dependencies.allRequiredPassed ? "APPROVED" : "BLOCKED"}`
    ),
    "",
  ].join("\n");
}

function buildReadinessReport(status: ProductionImagePlanningEngineStatusReport, allPassed: boolean): string {
  return [
