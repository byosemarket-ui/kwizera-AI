...
import {
  ...
  type AudioSynchronizationEngineStatusReport,
  type AudioSynchronizationRecord,
} from "../ai/index.js";
...
async function main(): Promise<void> {
  ...
  try {
    ...
    results.voiceSynchronization = {
      passed: Boolean(
        firstPlan?.voiceSynchronization.voiceTiming &&
    ...
    results.musicSynchronization = {
      passed: Boolean(
        firstPlan?.musicSynchronization.musicPlacement &&
    ...
    results.soundEffectSynchronization = {
      passed: Boolean(
        firstPlan?.soundEffectSynchronization.effectTiming &&
    ...
    results.subtitleSynchronization = {
      passed: Boolean(
        firstPlan?.subtitleSynchronization.subtitleTiming &&
    ...
    results.audioMixingPlan = {
      passed: Boolean(
        firstPlan?.audioMixing.voiceLevel &&
    ...
    results.audioScores = {
      passed:
        (firstPlan?.scores.audioSynchronizationScore ?? 0) >= 55 &&
    ...
    fs.writeFileSync(
      path.join(projectStateDir, "AI-Audio-Synchronization-Report.md"),
      buildMainReport(status, results, storageRoot, allPassed, techAudio.plans, fashionAudio.plans, beautyAudio.plans),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Voice-Synchronization-Report.md"),
      buildVoiceReport(techAudio.plans),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Music-Synchronization-Report.md"),
      buildMusicReport(techAudio.plans),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Subtitle-Synchronization-Report.md"),
      buildSubtitleReport(techAudio.plans),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Audio-Readiness-Report.md"),
      buildReadinessReport(status, techAudio.plans, fashionAudio.plans, beautyAudio.plans),
      "utf8"
    );
    fs.writeFileSync(
      path.join(process.cwd(), "STEP-8H-VALIDATION-REPORT.md"),
      buildMainReport(status, results, storageRoot, allPassed, techAudio.plans, fashionAudio.plans, beautyAudio.plans),
      "utf8"
    );
...
function buildMainReport(
  status: AudioSynchronizationEngineStatusReport,
  results: Record<string, { passed: boolean; detail: string }>,
  storageRoot: string,
  allPassed: boolean,
  tech?: AudioSynchronizationRecord[],
  fashion?: AudioSynchronizationRecord[],
  beauty?: AudioSynchronizationRecord[]
): string {
  return [
...
function buildVoiceReport(plans?: AudioSynchronizationRecord[]): string {
  ...
  for (const plan of plans?.slice(0, 6) ?? []) {
    ...
    lines.push(
      ...
      "",
      `- Voice timing: ${v.voiceTiming}`,
      `- Speech alignment: ${v.speechAlignment}`,
      `- Emotion timing: ${v.emotionTiming}`,
      `- Lip sync: ${v.lipSyncBlueprint}`,
      ""
    );
  }
  return lines.join("\n");
...
function buildMusicReport(plans?: AudioSynchronizationRecord[]): string {
  ...
  for (const plan of plans?.slice(0, 6) ?? []) {
    ...
    lines.push(
      `## ${plan.profile.sceneId}`,
      "",
      `- Placement: ${m.musicPlacement}`,
      `- Timing: ${m.musicTiming}`,
      `- Beat detection: ${m.beatDetection}`,
      `- Fade in/out: ${m.musicFadeIn} / ${m.musicFadeOut}`,
      ""
    );
  }
  return lines.join("\n");
...
function buildSubtitleReport(plans?: AudioSynchronizationRecord[]): string {
  ...
  for (const plan of plans?.slice(0, 6) ?? []) {
    ...
    lines.push(
      `## ${plan.profile.sceneId}`,
      "",
      `- Subtitle timing: ${s.subtitleTiming}`,
      `- Caption timing: ${s.captionTiming}`,
      `- Reading speed: ${s.readingSpeedValidation}`,
