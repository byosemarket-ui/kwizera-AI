...
import {
  ALL_PRODUCT_CONSISTENCY_RULES,
  ALL_PRODUCT_IMAGE_GEN_PLATFORMS,
  ALL_PRODUCT_MARKETING_VARIATIONS,
  ...
  type ProductImageGenerationEngineStatusReport,
  type ProductImageGenerationRecord,
} from "../ai/index.js";
...
async function main(): Promise<void> {
  ...
  try {
    ...
    results.backgroundPlanning = {
      passed: Boolean(
        tech.record?.backgroundPlan.backgroundDescription &&
    ...
    results.productConsistency = {
      passed:
        (tech.record?.consistencyPlan.rules.length ?? 0) === ALL_PRODUCT_CONSISTENCY_RULES.length &&
    ...
    results.productImageScores = {
      passed:
        (tech.record?.scores.productPresentationScore ?? 0) >= 55 &&
    ...
    fs.writeFileSync(
      path.join(projectStateDir, "AI-Product-Image-Generation-Report.md"),
      buildMainReport(status, results, storageRoot, allPassed, tech.record, fashion.record, beauty.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Product-Photography-Report.md"),
      buildPhotographyReport(tech.record, fashion.record, beauty.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Lighting-Planning-Report.md"),
      buildLightingReport(tech.record, fashion.record, beauty.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Background-Planning-Report.md"),
      buildBackgroundReport(tech.record, fashion.record, beauty.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Product-Readiness-Report.md"),
      buildReadinessReport(status, tech.record, fashion.record, beauty.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(process.cwd(), "STEP-9D-VALIDATION-REPORT.md"),
      buildMainReport(status, results, storageRoot, allPassed, tech.record, fashion.record, beauty.record),
      "utf8"
    );
...
function buildMainReport(
  status: ProductImageGenerationEngineStatusReport,
  results: Record<string, { passed: boolean; detail: string }>,
  storageRoot: string,
  allPassed: boolean,
  tech?: ProductImageGenerationRecord,
  fashion?: ProductImageGenerationRecord,
  beauty?: ProductImageGenerationRecord
): string {
  return [
...
function buildPhotographyReport(
  tech?: ProductImageGenerationRecord,
  fashion?: ProductImageGenerationRecord,
  beauty?: ProductImageGenerationRecord
): string {
  const rows = [tech, fashion, beauty].filter(Boolean) as ProductImageGenerationRecord[];
  const lines = [
  ...
  for (const record of rows) {
    lines.push(
      `| ${record.profile.productId} | ${record.photographyPlan.primaryMode} | ${record.photographyPlan.modes.length} | ${record.scores.photographyScore}/100 |`
    );
  }
...
function buildBackgroundReport(
  ...
  for (const record of rows) {
    lines.push(
      `| ${record.profile.productId} | ${record.backgroundPlan.primaryBackground} | ${record.backgroundPlan.backgroundDescription.slice(0, 35)}... | ${record.backgroundPlan.replacementStrategy.slice(0, 30)}... |`
    );
  }
...
function buildReadinessReport(
  ...
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r.profile.productId} | ${r.scores.productPresentationScore} | ${r.scores.photographyScore} | ${r.scores.brandConsistencyScore} | ${r.scores.marketplaceReadinessScore} | ${r.scores.productionReadinessScore} | ${r.scores.aiConfidenceScore} | ${r.productionReady && r.marketplaceReady ? "✅" : "❌"} |`
    ),
    "",
    "## Performance",
    "",
