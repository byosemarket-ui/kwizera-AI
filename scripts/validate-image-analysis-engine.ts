...
import {
  ...
  ImageFileFormat,
  type ImageAnalysisEngineStatusReport,
  type ImageAnalysisIntelligenceRecord,
} from "../ai/index.js";
...
async function main(): Promise<void> {
  ...
  try {
    ...
    results.technicalAnalysis = {
      passed:
        product.record?.technical.width === 2400 &&
    ...
    results.qualityScores = {
      passed:
        (product.record?.scores.visualQualityScore ?? 0) >= 50 &&
...
function buildAnalysisReport(
  status: ImageAnalysisEngineStatusReport,
...
function buildClassificationReport(
  product: ImageAnalysisIntelligenceRecord | undefined,
  lifestyle: ImageAnalysisIntelligenceRecord | undefined,
  banner: ImageAnalysisIntelligenceRecord | undefined
): string {
  ...
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r!.technical.imageName} | ${r!.classification.imageType} | ${r!.classification.category} | ${r!.classification.subcategory} | ${r!.classification.creativeStyle} | ${r!.classification.useCase} |`
    ),
    "",
  ].join("\n");
}
...
function buildQualityReport(
  ...
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r!.technical.imageName} | ${r!.scores.imageCompletenessScore} | ${r!.scores.technicalQualityScore} | ${r!.scores.visualQualityScore} | ${r!.scores.analysisConfidenceScore} |`
    ),
    "",
  ].join("\n");
}
