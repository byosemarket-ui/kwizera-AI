...
import {
  ...
  SUPPORTED_GENERATION_ASSET_TYPES,
  GENERATION_BLUEPRINT_STAGES,
  ...
  type VideoGenerationFoundationStatusReport,
  type VideoGenerationHealthReport,
} from "../ai/index.js";
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const registryPath = path.join(
      ...
      "video-generation",
      "registry",
      "video-generation-registry.json"
    );
    ...
    results.sampleProject = {
      passed:
        foundation.getProjectManager().getProject(project.projectId)?.blueprintId === blueprint.blueprintId &&
    ...
    results.sampleScene = {
      passed:
        foundation.getProjectManager().getProject(project.projectId)?.sceneIds.includes(scene.assetId) === true,
      detail: `Scene ${scene.assetId} linked to project`,
    };

    results.sampleTimeline = {
      passed:
        foundation.getProjectManager().getProject(project.projectId)?.timelineIds.includes(timeline.assetId) === true,
      detail: `Timeline ${timeline.assetId} linked to project`,
    };

    results.assetRegistry = {
      passed:
        foundation.getAssetRegistry().getCount() >= 6 &&
    ...
    const edit = foundation.getWorkflow().recordEdit(
      project.projectId,
      GenerationWorkflowActionType.Edit,
      "Adjust scene timing",
      "gen-state-v1",
      "gen-state-v2",
      videoId
    ...
    results.multiProjectSupport = {
      passed:
        foundation.getProjectManager().getProjectCount() >= 2 &&
...
function buildFoundationReport(
  status: VideoGenerationFoundationStatusReport,
  health: VideoGenerationHealthReport,
...
function buildBlueprintReport(
  foundation: NonNullable<ReturnType<ReturnType<typeof createAiCore>["getManager"]>["videoGenerationFoundation"]>,
  allPassed: boolean
): string {
...
${
  blueprint
    ? blueprint.stages
        .map(
          (s) =>
            `- **${s.stage}** — order ${s.order}, quality ${s.qualityScore}, readiness ${s.readinessScore}`
        )
        .join("\n")
    : "- No stages"
}
...
function buildArchitectureReport(status: VideoGenerationFoundationStatusReport): string {
