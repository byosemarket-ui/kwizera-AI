...
import {
  ...
  type ImageIntelligenceOptimizationRecord,
} from "../ai/index.js";
...
const SAMPLE_CAMPAIGN: ImageAnalysisEngineInput = {
...
async function main(): Promise<void> {
  ...
  try {
    ...
    results.optimizationProfile = {
      passed:
        Boolean(hero.record?.profile.optimizationId) &&
    ...
    results.moduleOptimization = {
      passed:
        (hero.record?.moduleResults.length ?? 0) === 11 &&
    ...
    results.optimizationStrategies = {
      passed:
        Boolean(hero.record?.strategies.cacheOptimization) &&
    ...
    results.cacheOptimization = {
      passed:
        (hero.record?.cache.images.length ?? 0) >= 1 &&
    ...
    results.performanceImprovement = {
      passed:
        (hero.record?.performance.planningSpeedMs ?? 999) <= (hero.record?.performance.planningSpeedBeforeMs ?? 0) &&
    ...
    results.workflowOptimization = {
      passed:
        Boolean(hero.record?.strategies.workflowOptimization) &&
    ...
    results.optimizationScores = {
      passed:
        (hero.record?.scores.overallImprovementScore ?? 0) >= 5 &&
    ...
    results.relationshipDetection = {
      passed:
        (hero.record?.relationships.qualityPredictions.length ?? 0) >= 1 &&
    ...
    fs.writeFileSync(
      path.join(projectStateDir, "Image-Optimization-Report.md"),
      buildOptimizationReport(status, results, storageRoot, allPassed, hero.record, campaign.record, banner.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Image-Performance-Optimization-Report.md"),
      buildPerformanceReport(hero.record, campaign.record, banner.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Image-Recommendation-Optimization-Report.md"),
      buildRecommendationReport(hero.record, campaign.record, banner.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Image-Workflow-Optimization-Report.md"),
      buildWorkflowReport(hero.record, campaign.record, banner.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(process.cwd(), "STEP-6M-VALIDATION-REPORT.md"),
      buildOptimizationReport(status, results, storageRoot, allPassed, hero.record, campaign.record, banner.record),
      "utf8"
    );
...
function buildOptimizationReport(
  status: ImageIntelligenceOptimizationEngineStatusReport,
  results: Record<string, { passed: boolean; detail: string }>,
  storageRoot: string,
  allPassed: boolean,
  hero?: ImageIntelligenceOptimizationRecord,
  campaign?: ImageIntelligenceOptimizationRecord,
  banner?: ImageIntelligenceOptimizationRecord
): string {
  return [
...
function buildPerformanceReport(
  ...
  const rows = [hero, campaign, banner].filter(Boolean) as ImageIntelligenceOptimizationRecord[];
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r.imageId} | ${r.performance.planningSpeedBeforeMs}ms | ${r.performance.planningSpeedMs}ms | ${r.performance.searchSpeedBeforeMs}ms | ${r.performance.searchSpeedMs}ms | ${r.scores.planningImprovementScore}% | ${r.scores.searchImprovementScore}% |`
    ),
    "",
    "## Resource Estimates",
    "",
    ...rows.map(
      (r) => `- **${r.imageId}:** ~${r.performance.memoryEstimateMb}MB memory, ~${r.performance.diskUsageEstimateKb}KB disk`
    ),
    "",
  ].join("\n");
}
...
function buildWorkflowReport(
  ...
  for (const record of rows) {
    lines.push(
      `## ${record.imageId} — ${record.profile.platform}`,
      "",
      ...
      `- **Performance Optimization:** ${record.strategies.performanceOptimization}`,
      "",
