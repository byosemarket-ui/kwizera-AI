...
import {
  ...
  type KnowledgeRetrievalStatusReport,
} from "../ai/index.js";
...
async function main(): Promise<void> {
  ...
  try {
    ...
    results.relatedGroups = {
      passed:
        retrieved.relatedGroups.relatedKnowledge.length >= 1 ||
    ...
    fs.writeFileSync(
      reportPath,
      buildReport(report, results, storageRoot, allPassed, {
        keywordMs,
        categoryMs,
        semanticMs,
        relationshipMs,
...
function buildReport(
  status: KnowledgeRetrievalStatusReport,
