...
import {
  ...
  FrameRateMode,
  VideoColorSpace,
  type VideoAnalysisEngineStatusReport,
  type VideoAnalysisIntelligenceRecord,
} from "../ai/index.js";
...
async function main(): Promise<void> {
  ...
  try {
    ...
    results.technicalAnalysis = {
      passed:
        commercial.record?.technical.width === 1920 &&
    ...
    results.qualityScores = {
      passed:
        (commercial.record?.scores.visualQualityScore ?? 0) >= 50 &&
    ...
    results.timelineAnalysis = {
      passed:
        (commercial.record?.timeline.sceneCount ?? 0) >= 3 &&
    ...
    results.frameAnalysis = {
      passed:
        (commercial.record?.frame.totalFrames ?? 0) > 0 &&
    ...
    results.audioAnalysis = {
      passed:
        (commercial.record?.audio.tracks.length ?? 0) >= 1 &&
    ...
    results.indexCreation = {
      passed:
        (commercial.record?.indexes.keyframeIndexIds.length ?? 0) > 0 &&
    ...
    fs.writeFileSync(
      path.join(projectStateDir, "Video-Analysis-Report.md"),
      buildVideoAnalysisReport(status, results, storageRoot, allPassed),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Timeline-Analysis-Report.md"),
      buildTimelineReport(commercial.record, social.record, tutorial.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Frame-Analysis-Report.md"),
      buildFrameReport(commercial.record, social.record, tutorial.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Audio-Analysis-Report.md"),
      buildAudioReport(commercial.record, social.record, tutorial.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Production-Readiness-Report.md"),
      buildProductionReadinessReport(status, commercial.record, social.record, tutorial.record, allPassed),
      "utf8"
    );
    fs.writeFileSync(
      path.join(process.cwd(), "STEP-7B-VALIDATION-REPORT.md"),
      buildVideoAnalysisReport(status, results, storageRoot, allPassed),
      "utf8"
    );
...
function buildVideoAnalysisReport(
  status: VideoAnalysisEngineStatusReport,
...
function buildTimelineReport(
  commercial: VideoAnalysisIntelligenceRecord | undefined,
  social: VideoAnalysisIntelligenceRecord | undefined,
  tutorial: VideoAnalysisIntelligenceRecord | undefined
): string {
  ...
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r!.technical.videoName} | ${(r!.timeline.timelineLengthMs / 1000).toFixed(1)}s | ${r!.timeline.sceneCount} | ${r!.timeline.shotCount} | ${r!.timeline.segments.length} | ${r!.indexes.timelineIndexIds.length} |`
    ),
    "",
...
function buildFrameReport(
  ...
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r!.technical.videoName} | ${r!.frame.totalFrames} | ${r!.frame.keyFrames} | ${r!.frame.frameConsistencyScore} | ${r!.frame.missingFrames} | ${r!.frame.motionDensity} | ${r!.frame.visualComplexity} |`
    ),
    "",
    "## Index Summary",
    "",
    ...rows.map(
      (r) =>
        `- **${r!.technical.videoName}:** ${r!.indexes.keyframeIndexIds.length} keyframe indexes, ${r!.indexes.frameIndexIds.length} frame indexes`
    ),
    "",
  ].join("\n");
}
...
function buildAudioReport(
  ...
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r!.technical.videoName} | ${r!.audio.tracks.length} | ${r!.audio.primaryLanguage} | ${r!.audio.synchronizationScore} | ${r!.audio.overallAudioQualityScore} | ${r!.indexes.audioIndexIds.length} |`
    ),
    "",
  ].join("\n");
}
...
function buildProductionReadinessReport(
  ...
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r!.technical.videoName} | ${r!.productionReadiness.editingReadiness} | ${r!.productionReadiness.aiGenerationReadiness} | ${r!.productionReadiness.marketingReadiness} | ${r!.productionReadiness.productionReadiness} | ${r!.productionReadiness.renderingReadiness} | ${r!.productionReadiness.exportReadiness} | ${r!.scores.productionReadinessScore} |`
    ),
    "",
    "## Quality Scores",
    "",
    ...
    ...rows.map(
      (r) =>
        `| ${r!.technical.videoName} | ${r!.scores.videoCompletenessScore} | ${r!.scores.technicalQualityScore} | ${r!.scores.frameQualityScore} | ${r!.scores.audioQualityScore} | ${r!.scores.visualQualityScore} | ${r!.scores.aiConfidenceScore} |`
    ),
    "",
  ].join("\n");
}
