...
import {
  ...
  type VideoUnderstandingEngineStatusReport,
  type VideoUnderstandingRecord,
} from "../ai/index.js";
...
async function main(): Promise<void> {
  ...
  try {
    ...
    results.sceneUnderstanding = {
      passed:
        (commercial.record?.scenes.length ?? 0) >= 3 &&
    ...
    results.storyUnderstanding = {
      passed:
        Boolean(commercial.record?.story.narrativeStructure) &&
    ...
    results.productUnderstanding = {
      passed:
        commercial.record?.product.mainProduct === "KWIZERA Pro Studio" &&
    ...
    results.brandUnderstanding = {
      passed:
        commercial.record?.brand.brandIdentity === "KWIZERA" &&
    ...
    results.marketingUnderstanding = {
      passed:
        Boolean(commercial.record?.marketing.campaignGoal) &&
    ...
    results.audienceUnderstanding = {
      passed:
        Boolean(commercial.record?.audience.targetAudience) &&
    ...
    results.understandingScores = {
      passed:
        (commercial.record?.scores.videoUnderstandingScore ?? 0) >= 55 &&
    ...
    results.knowledgeGraph = {
      passed:
        (commercial.record?.knowledgeGraph.nodes.length ?? 0) >= 5 &&
    ...
    fs.writeFileSync(
      path.join(projectStateDir, "Video-Understanding-Report.md"),
      buildUnderstandingReport(status, results, storageRoot, allPassed),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Story-Understanding-Report.md"),
      buildStoryReport(commercial.record, social.record, tutorial.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Audience-Understanding-Report.md"),
      buildAudienceReport(commercial.record, social.record, tutorial.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Marketing-Understanding-Report.md"),
      buildMarketingReport(commercial.record, social.record, tutorial.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Video-Readiness-Report.md"),
      buildReadinessReport(status, commercial.record, social.record, tutorial.record, allPassed),
      "utf8"
    );
    fs.writeFileSync(
      path.join(process.cwd(), "STEP-7C-VALIDATION-REPORT.md"),
      buildUnderstandingReport(status, results, storageRoot, allPassed),
      "utf8"
    );
...
function buildUnderstandingReport(
  status: VideoUnderstandingEngineStatusReport,
...
function buildStoryReport(
  commercial: VideoUnderstandingRecord | undefined,
  social: VideoUnderstandingRecord | undefined,
  tutorial: VideoUnderstandingRecord | undefined
): string {
  ...
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r!.identity.videoName} | ${r!.story.storyType} | ${r!.story.narrativeStructure} | ${r!.story.emotionalJourney} | ${r!.scores.storytellingScore} |`
    ),
    "",
...
function buildAudienceReport(
  ...
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r!.identity.videoName} | ${r!.audience.targetAudience} | ${r!.audience.engagementOpportunity.slice(0, 40)}... | ${r!.audience.viewerRetentionOpportunity.slice(0, 40)}... | ${r!.audience.conversionOpportunity.slice(0, 40)}... | ${r!.scores.audienceAlignmentScore} |`
    ),
    "",
  ].join("\n");
}
...
function buildMarketingReport(
  ...
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r!.identity.videoName} | ${r!.marketing.campaignGoal} | ${r!.marketing.marketingStrength} | ${r!.marketing.ctaOpportunity.slice(0, 50)}... | ${r!.scores.marketingScore} |`
    ),
    "",
  ].join("\n");
}
...
function buildReadinessReport(
  ...
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r!.identity.videoName} | ${r!.scores.videoUnderstandingScore} | ${r!.scores.storytellingScore} | ${r!.scores.marketingScore} | ${r!.scores.brandConsistencyScore} | ${r!.scores.productionReadinessScore} | ${r!.scores.aiConfidenceScore} |`
    ),
    "",
    ...
    ...rows.map(
      (r) =>
        `- **${r!.identity.videoName}:** ${r!.knowledgeGraph.nodes.length} nodes, ${r!.knowledgeGraph.edges.length} edges`
    ),
    "",
  ].join("\n");
}
