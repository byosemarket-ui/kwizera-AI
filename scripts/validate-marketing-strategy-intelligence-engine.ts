...
import {
  ...
  StrategyType,
  BusinessGoalType,
  type MarketingStrategyEngineStatusReport,
...
async function main(): Promise<void> {
  ...
  try {
    ...
    results.businessGoalAnalysis = {
      passed:
        (tech.record?.businessGoals.salesObjectives.length ?? 0) >= 1 &&
    ...
    results.audienceAlignment = {
      passed:
        (tech.record?.audienceAlignment.customerNeeds.length ?? 0) >= 2 &&
    ...
    results.strategySelection = {
      passed:
        tech.record?.selectedStrategies.some((s) => s.priority === "primary") === true &&
    ...
    results.creativePreparation = {
      passed:
        Boolean(tech.record?.creativePreparation.storyboardDirection) &&
    ...
    results.campaignDirection = {
      passed:
        Boolean(tech.record?.campaignDirection.campaignFocus) &&
    ...
    results.strategyScores = {
      passed:
        (tech.record?.scores.strategyQualityScore ?? 0) >= 55 &&
    ...
    const logFile = path.join(
      storageRoot,
      "logs",
      `marketing-strategy-intelligence-engine-${logDate}.jsonl`
    );
    results.logging = {
    ...
    fs.writeFileSync(
      path.join(projectStateDir, "Business-Goal-Report.md"),
      buildBusinessGoalReport(tech.record, fashion.record, beauty.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Strategy-Readiness-Report.md"),
      buildStrategyReadinessReport(status, tech.record, fashion.record, beauty.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(process.cwd(), "STEP-5E-VALIDATION-REPORT.md"),
      buildMarketingStrategyReport(
        status,
        results,
        storageRoot,
...
function buildMarketingStrategyReport(
  status: MarketingStrategyEngineStatusReport,
  results: Record<string, { passed: boolean; detail: string }>,
  storageRoot: string,
  allPassed: boolean,
  tech?: MarketingStrategyRecord,
  fashion?: MarketingStrategyRecord,
  beauty?: MarketingStrategyRecord
): string {
  return [
...
function buildBusinessGoalReport(
  tech?: MarketingStrategyRecord,
  fashion?: MarketingStrategyRecord,
  beauty?: MarketingStrategyRecord
): string {
  const rows = [tech, fashion, beauty].filter(Boolean) as MarketingStrategyRecord[];
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r.productId} | ${r.marketingObjective} | ${r.businessGoals.salesObjectives.length} | ${r.businessGoals.marketingObjectives.length} | ${r.businessGoals.brandObjectives.length} | ${r.scores.businessAlignmentScore}/100 |`
    ),
    "",
...
function buildStrategyReadinessReport(
  status: MarketingStrategyEngineStatusReport,
  tech?: MarketingStrategyRecord,
  fashion?: MarketingStrategyRecord,
  beauty?: MarketingStrategyRecord
  ...
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r.productId} | ${r.scores.marketingReadinessScore}/100 | ${r.scores.confidenceScore}/100 | ${r.creativePreparation.storyboardReady ? "✅" : "❌"} | ${r.creativePreparation.scriptPlanningReady ? "✅" : "❌"} | ${r.creativePreparation.visualPlanningReady ? "✅" : "❌"} | ${r.creativePreparation.productionPlanningReady ? "✅" : "❌"} | ${r.validated ? "✅" : "❌"} |`
    ),
    "",
    "## Performance",
    "",
