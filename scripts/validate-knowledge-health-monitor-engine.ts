...
import {
  ...
  type MonitoredKnowledgeModuleHealthScore,
} from "../ai/index.js";
...
const SAMPLE_CREATIVE: CreativeAnalysisInput = {
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const relationshipModule = modules.find(
      (m) => m.module === MonitoredKnowledgeModule.KnowledgeRelationships
    );
    ...
    if (corruptEntry) {
      fs.writeFileSync(
        path.join(corruptEntry.storageLocation, "current.json"),
...
function buildReport(
  status: KnowledgeHealthMonitorStatusReport,
  results: Record<string, { passed: boolean; detail: string }>,
  storageRoot: string,
  allPassed: boolean,
  check: KnowledgeHealthCheckResult,
  modules: MonitoredKnowledgeModuleHealthScore[],
  checkMs: number
): string {
  void checkMs;
  return [
