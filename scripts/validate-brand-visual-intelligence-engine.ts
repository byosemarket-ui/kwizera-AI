...
import {
  ...
  type BrandVisualIntelligenceEngineStatusReport,
  type BrandVisualIntelligenceRecord,
} from "../ai/index.js";
...
const SAMPLE_KWIZERA_PRODUCT: ImageAnalysisEngineInput = {
...
const SAMPLE_KWIZERA_LIFESTYLE: ImageAnalysisEngineInput = {
...
const SAMPLE_GLOWLAB: ImageAnalysisEngineInput = {
...
async function main(): Promise<void> {
  ...
  try {
    ...
    results.logoValidation = {
      passed:
        (kwizeraProduct.record?.logoAnalysis.logoVisibility ?? 0) >= 50 &&
    ...
    results.colorValidation = {
      passed:
        (kwizeraProduct.record?.colorAnalysis.primaryBrandColors.length ?? 0) >= 1 &&
    ...
    results.qualityScores = {
      passed:
        (kwizeraProduct.record?.scores.brandConsistencyScore ?? 0) >= 55 &&
    ...
    fs.writeFileSync(
      path.join(projectStateDir, "Brand-Visual-Report.md"),
      buildBrandVisualReport(status, results, storageRoot, allPassed, kwizeraProduct.record, kwizeraLifestyle.record, glowlab.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Logo-Validation-Report.md"),
      buildLogoReport(kwizeraProduct.record, kwizeraLifestyle.record, glowlab.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Brand-Color-Report.md"),
      buildColorReport(kwizeraProduct.record, kwizeraLifestyle.record, glowlab.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Typography-Report.md"),
      buildTypographyReport(kwizeraProduct.record, kwizeraLifestyle.record, glowlab.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Brand-Readiness-Report.md"),
      buildReadinessReport(status, allPassed),
      "utf8"
    );
    fs.writeFileSync(
      path.join(process.cwd(), "STEP-6H-VALIDATION-REPORT.md"),
      buildBrandVisualReport(status, results, storageRoot, allPassed, kwizeraProduct.record, kwizeraLifestyle.record, glowlab.record),
      "utf8"
    );
...
function buildBrandVisualReport(
  status: BrandVisualIntelligenceEngineStatusReport,
  results: Record<string, { passed: boolean; detail: string }>,
  storageRoot: string,
  allPassed: boolean,
  product?: BrandVisualIntelligenceRecord,
  lifestyle?: BrandVisualIntelligenceRecord,
  glowlab?: BrandVisualIntelligenceRecord
): string {
  return [
...
function buildLogoReport(
  product?: BrandVisualIntelligenceRecord,
  lifestyle?: BrandVisualIntelligenceRecord,
  glowlab?: BrandVisualIntelligenceRecord
): string {
  const rows = [product, lifestyle, glowlab].filter(Boolean) as BrandVisualIntelligenceRecord[];
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r.imageId} | ${r.profile.brandName} | ${r.logoAnalysis.logoVisibility} | ${r.logoAnalysis.logoPosition} | ${r.logoAnalysis.logoSize} | ${r.logoAnalysis.logoContrast} | ${r.logoAnalysis.logoSafeArea} | ${r.logoAnalysis.logoConsistency} |`
    ),
    "",
    "## Logo Quality Scores",
    "",
...
function buildColorReport(
  ...
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r.imageId} | ${r.profile.brandName} | ${r.colorAnalysis.primaryBrandColors.join(", ")} | ${r.colorAnalysis.secondaryColors.join(", ") || "none"} | ${r.colorAnalysis.accentColors.join(", ") || "none"} | ${r.colorAnalysis.colorHarmony} | ${r.scores.colorConsistencyScore} |`
    ),
    "",
  ].join("\n");
}

function buildTypographyReport(
  product?: BrandVisualIntelligenceRecord,
  lifestyle?: BrandVisualIntelligenceRecord,
  glowlab?: BrandVisualIntelligenceRecord
): string {
  ...
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r.imageId} | ${r.profile.brandName} | ${r.typography.primaryFont} | ${r.typography.secondaryFont} | ${r.typography.headingStyle} | ${r.typography.bodyStyle} | ${r.typography.ctaStyle} | ${r.scores.typographyScore} |`
    ),
    "",
  ].join("\n");
}

function buildReadinessReport(status: BrandVisualIntelligenceEngineStatusReport, allPassed: boolean): string {
  return [
