...
import {
  ALL_BRAND_DESIGN_CONSISTENCY_ELEMENTS,
  ALL_BRAND_DESIGN_GEN_PLATFORMS,
  ALL_BRAND_DESIGN_LOGO_VARIANTS,
  ALL_BRAND_DESIGN_MATERIALS,
  ALL_BRAND_DESIGN_PRINT_FORMATS,
  ALL_BRAND_DESIGN_SOCIAL_FORMATS,
  ALL_BRAND_DESIGN_TYPES,
  ...
  type BrandingDesignEngineStatusReport,
  type BrandingDesignRecord,
} from "../ai/index.js";
...
async function main(): Promise<void> {
  ...
  try {
    ...
    results.layoutPlanning = {
      passed: Boolean(
        tech.record?.designPlanning.layoutStructure &&
    ...
    results.logoPlanning = {
      passed:
        (tech.record?.logoPlanning.variants.length ?? 0) === ALL_BRAND_DESIGN_LOGO_VARIANTS.length &&
    ...
    results.graphicDesign = {
      passed:
        (tech.record?.marketingMaterials.materials.length ?? 0) >= 4 &&
    ...
    results.printDesign = {
      passed:
        (food.record?.printDesign.formats.length ?? 0) === ALL_BRAND_DESIGN_PRINT_FORMATS.length &&
    ...
    results.brandConsistency = {
      passed:
        (tech.record?.brandConsistency.elements.length ?? 0) === ALL_BRAND_DESIGN_CONSISTENCY_ELEMENTS.length &&
    ...
    results.colorManagement = {
      passed:
        (tech.record?.colorManagement.rgbPalette.length ?? 0) >= 2 &&
    ...
    results.brandingScores = {
      passed:
        (tech.record?.scores.brandingScore ?? 0) >= 55 &&
    ...
    fs.writeFileSync(
      path.join(projectStateDir, "AI-Branding-Graphic-Design-Report.md"),
      buildMainReport(status, results, storageRoot, allPassed, tech.record, fashion.record, food.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Logo-Planning-Report.md"),
      buildLogoReport(tech.record, fashion.record, food.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Graphic-Design-Report.md"),
      buildGraphicReport(tech.record, fashion.record, food.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Print-Design-Report.md"),
      buildPrintReport(tech.record, fashion.record, food.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Branding-Readiness-Report.md"),
      buildReadinessReport(status, tech.record, fashion.record, food.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(process.cwd(), "STEP-9H-VALIDATION-REPORT.md"),
      buildMainReport(status, results, storageRoot, allPassed, tech.record, fashion.record, food.record),
      "utf8"
    );
...
function buildMainReport(
  status: BrandingDesignEngineStatusReport,
  results: Record<string, { passed: boolean; detail: string }>,
  storageRoot: string,
  allPassed: boolean,
  tech?: BrandingDesignRecord,
  fashion?: BrandingDesignRecord,
  food?: BrandingDesignRecord
): string {
  return [
...
function buildLogoReport(
  ...
  const rows = [tech, fashion, food].filter(Boolean) as BrandingDesignRecord[];
  const lines = [
  ...
  for (const record of rows) {
    lines.push(
      `| ${record.profile.productId} | ${record.logoPlanning.variants.length} | ${record.logoPlanning.primaryLogoNotes.slice(0, 25)}... | ${record.logoPlanning.usageGuidelines.length} rules | ${record.scores.brandingScore}/100 |`
    );
  }
...
function buildPrintReport(
  ...
  for (const record of rows) {
    lines.push(
      `| ${record.profile.productId} | ${record.printDesign.formats.length} | ${record.colorManagement.cmykPalette.length} colors | ${record.printDesign.printNotes[0]?.slice(0, 20) ?? "n/a"}... | ${record.printReady ? "✅" : "❌"} | ${record.scores.printReadinessScore}/100 |`
    );
  }
...
function buildReadinessReport(
  ...
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r.profile.productId} | ${r.scores.brandingScore} | ${r.scores.graphicDesignScore} | ${r.scores.layoutScore} | ${r.scores.typographyScore} | ${r.scores.brandConsistencyScore} | ${r.scores.printReadinessScore} | ${r.scores.aiConfidenceScore} | ${r.productionReady ? "✅" : "❌"} |`
    ),
    "",
    "## Performance",
    "",
