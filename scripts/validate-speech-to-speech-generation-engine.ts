...
import {
  ...
  S2sOutputUseCase,
  S2sPlatform,
  SUPPORTED_S2S_LANGUAGES,
  VoiceType,
  type SpeechToSpeechGenerationEngineStatusReport,
  type SpeechToSpeechGenerationRecord,
} from "../ai/index.js";
...
async function main(): Promise<void> {
  ...
  try {
    ...
    results.speechAnalysis = {
      passed: Boolean(
        tech.record?.speechAnalysis.language &&
    ...
    results.voiceTransformation = {
      passed: Boolean(
        tech.record?.voiceTransformation.voiceMapping &&
    ...
    results.emotionPreservation = {
      passed: Boolean(
        tech.record?.emotionPreservation.sourceEmotion &&
    ...
    results.timingPreservation = {
      passed: Boolean(
        tech.record?.timingPreservation.segmentTiming.length >= 1 &&
    ...
    results.pronunciationAdaptation = {
      passed: Boolean(
        tech.record?.pronunciationAdaptation.numberReadingRules.length >= 2 &&
    ...
    results.multiLanguageSupport = {
      passed:
        tech.record?.profile.language === S2sLanguage.English &&
    ...
    results.relationships = {
      passed:
        (tech.record?.relationships.sourceAudio.length ?? 0) >= 1 &&
    ...
    fs.writeFileSync(
      path.join(projectStateDir, "AI-Speech-to-Speech-Report.md"),
      buildMainReport(status, results, storageRoot, allPassed, tech.record, health.record, finance.record, kinyarwanda.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Speech-Analysis-Report.md"),
      buildSpeechAnalysisReport(tech.record, health.record, finance.record, kinyarwanda.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Voice-Transformation-Report.md"),
      buildVoiceTransformationReport(tech.record, health.record, finance.record, kinyarwanda.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Emotion-Preservation-Report.md"),
      buildEmotionReport(tech.record, health.record, finance.record, kinyarwanda.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Speech-to-Speech-Readiness-Report.md"),
      buildReadinessReport(status, tech.record, health.record, finance.record, kinyarwanda.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(process.cwd(), "STEP-10C-VALIDATION-REPORT.md"),
      buildMainReport(status, results, storageRoot, allPassed, tech.record, health.record, finance.record, kinyarwanda.record),
      "utf8"
    );
...
function buildMainReport(
  status: SpeechToSpeechGenerationEngineStatusReport,
  results: Record<string, { passed: boolean; detail: string }>,
  storageRoot: string,
  allPassed: boolean,
  tech?: SpeechToSpeechGenerationRecord,
  health?: SpeechToSpeechGenerationRecord,
  finance?: SpeechToSpeechGenerationRecord,
  rw?: SpeechToSpeechGenerationRecord
): string {
  return [
...
function buildSpeechAnalysisReport(...records: (SpeechToSpeechGenerationRecord | undefined)[]): string {
  const rows = records.filter(Boolean) as SpeechToSpeechGenerationRecord[];
  const lines = [
  ...
  for (const r of rows) {
    lines.push(
      `| ${r.transformationId.slice(0, 30)}... | ${r.speechAnalysis.language} | ${r.speechAnalysis.accent} | ${r.speechAnalysis.speakerSegments.length} | ${r.speechAnalysis.detectedEmotion} | ${r.speechAnalysis.audioQualityScore}/100 |`
    );
  }
  return lines.join("\n");
}

function buildVoiceTransformationReport(...records: (SpeechToSpeechGenerationRecord | undefined)[]): string {
  const rows = records.filter(Boolean) as SpeechToSpeechGenerationRecord[];
  const lines = [
  ...
  for (const r of rows) {
    lines.push(
      `| ${r.profile.productId ?? r.transformationId.slice(0, 20)} | ${r.voiceTransformation.sourceVoiceType} | ${r.voiceTransformation.targetVoiceType} | ${r.voiceTransformation.accentAdaptation.slice(0, 35)}... | ${r.scores.transformationQualityScore}/100 |`
    );
  }
  return lines.join("\n");
}

function buildEmotionReport(...records: (SpeechToSpeechGenerationRecord | undefined)[]): string {
  const rows = records.filter(Boolean) as SpeechToSpeechGenerationRecord[];
  const lines = [
  ...
  for (const r of rows) {
    lines.push(
      `| ${r.profile.productId ?? r.transformationId.slice(0, 20)} | ${r.emotionPreservation.sourceEmotion} | ${r.emotionPreservation.targetEmotion} | ${r.emotionPreservation.preservationScore} | ${r.scores.emotionPreservationScore}/100 |`
    );
  }
  return lines.join("\n");
}
...
function buildReadinessReport(
  status: SpeechToSpeechGenerationEngineStatusReport,
  ...records: (SpeechToSpeechGenerationRecord | undefined)[]
): string {
  const rows = records.filter(Boolean) as SpeechToSpeechGenerationRecord[];
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r.profile.language} | ${r.scores.transformationQualityScore} | ${r.scores.pronunciationScore} | ${r.scores.emotionPreservationScore} | ${r.scores.timingPreservationScore} | ${r.scores.brandConsistencyScore} | ${r.scores.productionReadinessScore} | ${r.scores.aiConfidenceScore} | ${r.productionReady ? "✅" : "❌"} |`
    ),
    "",
    "## Performance",
    "",
