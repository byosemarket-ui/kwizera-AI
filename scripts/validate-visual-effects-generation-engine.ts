...
import {
  ...
  type VisualEffectsGenerationEngineStatusReport,
  type VisualEffectsGenerationRecord,
} from "../ai/index.js";
...
async function main(): Promise<void> {
  ...
  try {
    ...
    results.lightingEffects = {
      passed: Boolean(
        firstPlan?.lightingEffects.glow &&
    ...
    results.atmosphericEffects = {
      passed: Boolean(
        firstPlan?.atmosphericEffects.particles &&
    ...
    results.productEffects = {
      passed: Boolean(
        firstPlan?.productEffects.productGlow &&
    ...
    results.environmentEffects = {
      passed: Boolean(
        firstPlan?.environmentEffects.ambientMotion &&
    ...
    results.transitionEffects = {
      passed: Boolean(
        firstPlan?.transitionEffects.fade &&
    ...
    results.textGraphicEffects = {
      passed: Boolean(
        firstPlan?.textGraphicEffects.textGlow &&
    ...
    results.colorEffects = {
      passed: Boolean(
        firstPlan?.colorEffects.colorGrading &&
    ...
    results.cinematicEffects = {
      passed: Boolean(
        firstPlan?.cinematicEffects.depthOfField &&
    ...
    results.visualEffectsScores = {
      passed:
        (firstPlan?.scores.visualEffectsScore ?? 0) >= 55 &&
    ...
    fs.writeFileSync(
      path.join(projectStateDir, "AI-Visual-Effects-Report.md"),
      buildMainReport(status, results, storageRoot, allPassed, techVfx.plans, fashionVfx.plans, beautyVfx.plans),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Lighting-Effects-Report.md"),
      buildLightingReport(techVfx.plans),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Atmospheric-Effects-Report.md"),
      buildAtmosphericReport(techVfx.plans),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Product-Effects-Report.md"),
      buildProductReport(techVfx.plans),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Visual-Effects-Readiness-Report.md"),
      buildReadinessReport(status, techVfx.plans, fashionVfx.plans, beautyVfx.plans),
      "utf8"
    );
    fs.writeFileSync(
      path.join(process.cwd(), "STEP-8G-VALIDATION-REPORT.md"),
      buildMainReport(status, results, storageRoot, allPassed, techVfx.plans, fashionVfx.plans, beautyVfx.plans),
      "utf8"
    );
...
function buildMainReport(
  status: VisualEffectsGenerationEngineStatusReport,
  results: Record<string, { passed: boolean; detail: string }>,
  storageRoot: string,
  allPassed: boolean,
  tech?: VisualEffectsGenerationRecord[],
  fashion?: VisualEffectsGenerationRecord[],
  beauty?: VisualEffectsGenerationRecord[]
): string {
  return [
...
function buildLightingReport(plans?: VisualEffectsGenerationRecord[]): string {
  ...
  for (const plan of plans?.slice(0, 6) ?? []) {
    ...
    lines.push(
      ...
      "",
      `- Glow: ${l.glow}`,
      `- Light rays: ${l.lightRays}`,
      `- Bloom: ${l.bloom}`,
      `- Rim light: ${l.rimLight}`,
      `- Volumetric: ${l.volumetricLighting}`,
      ""
    );
  }
  return lines.join("\n");
...
function buildAtmosphericReport(plans?: VisualEffectsGenerationRecord[]): string {
...
function buildProductReport(plans?: VisualEffectsGenerationRecord[]): string {
  const lines = [
  ...
  for (const plan of plans?.slice(0, 8) ?? []) {
    ...
    lines.push(
      `| ${plan.profile.sceneId.slice(-24)} | ${p.productGlow.slice(0, 20)}... | ${p.productHighlight.slice(0, 20)}... | ${p.shine.slice(0, 20)}... | ${p.premiumReveal.slice(0, 20)}... |`
    );
  }
  lines.push("");
  return lines.join("\n");
