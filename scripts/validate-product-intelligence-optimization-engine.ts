...
import {
  ...
  type ProductIntelligenceOptimizationEngineStatusReport,
  type ProductIntelligenceOptimizationRecord,
} from "../ai/index.js";
...
async function main(): Promise<void> {
  ...
  try {
    ...
    results.optimizationProfile = {
      passed:
        Boolean(tech.record?.profile.optimizationId) &&
    ...
    results.moduleOptimization = {
      passed:
        (tech.record?.moduleResults.length ?? 0) === 11 &&
    ...
    results.optimizationStrategies = {
      passed:
        Boolean(tech.record?.strategies.cacheOptimization) &&
    ...
    results.cacheOptimization = {
      passed:
        (tech.record?.cache.products.length ?? 0) >= 1 &&
    ...
    results.performanceImprovement = {
      passed:
        (tech.record?.performance.planningSpeedMs ?? 999) <= (tech.record?.performance.planningSpeedBeforeMs ?? 0) &&
    ...
    results.workflowOptimization = {
      passed:
        Boolean(tech.record?.strategies.workflowOptimization) &&
    ...
    results.optimizationScores = {
      passed:
        (tech.record?.scores.overallImprovementScore ?? 0) >= 5 &&
    ...
    results.relationshipDetection = {
      passed:
        (tech.record?.relationships.qualityPredictions.length ?? 0) >= 1 &&
    ...
    fs.writeFileSync(
      path.join(projectStateDir, "Product-Optimization-Report.md"),
      buildProductOptimizationReport(status, results, storageRoot, allPassed, tech.record, fashion.record, beauty.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Performance-Optimization-Report.md"),
      buildPerformanceReport(tech.record, fashion.record, beauty.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Recommendation-Optimization-Report.md"),
      buildRecommendationReport(tech.record, fashion.record, beauty.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Workflow-Optimization-Report.md"),
      buildWorkflowReport(tech.record, fashion.record, beauty.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(process.cwd(), "STEP-5M-VALIDATION-REPORT.md"),
      buildProductOptimizationReport(status, results, storageRoot, allPassed, tech.record, fashion.record, beauty.record),
      "utf8"
    );
...
function buildProductOptimizationReport(
  status: ProductIntelligenceOptimizationEngineStatusReport,
  results: Record<string, { passed: boolean; detail: string }>,
  storageRoot: string,
  allPassed: boolean,
  tech?: ProductIntelligenceOptimizationRecord,
  fashion?: ProductIntelligenceOptimizationRecord,
  beauty?: ProductIntelligenceOptimizationRecord
): string {
  return [
...
function buildPerformanceReport(
  ...
  const rows = [tech, fashion, beauty].filter(Boolean) as ProductIntelligenceOptimizationRecord[];
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r.productId} | ${r.performance.planningSpeedBeforeMs}ms | ${r.performance.planningSpeedMs}ms | ${r.performance.searchSpeedBeforeMs}ms | ${r.performance.searchSpeedMs}ms | ${r.scores.planningImprovementScore}% | ${r.scores.searchImprovementScore}% |`
    ),
    "",
    "## Resource Estimates",
    "",
    ...rows.map(
      (r) => `- **${r.productId}:** ~${r.performance.memoryEstimateMb}MB memory, ~${r.performance.diskUsageEstimateKb}KB disk`
    ),
    "",
  ].join("\n");
}
...
function buildRecommendationReport(
  ...
  for (const record of rows) {
    lines.push(
      ...
      `- ${record.strategies.knowledgeRetrievalOptimization}`,
      "",
      "### Module Improvements",
...
function buildWorkflowReport(
  ...
  for (const record of rows) {
    lines.push(
      ...
      "",
      `- **Workflow Efficiency:** ${record.scores.workflowEfficiencyScore}/100`,
      `- **Planning Optimization:** ${record.strategies.planningOptimization}`,
      `- **Workflow Optimization:** ${record.strategies.workflowOptimization}`,
      "",
      "| Module | Improved | Detail |",
      "|--------|----------|--------|"
    );
