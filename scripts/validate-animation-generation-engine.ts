...
import {
  ...
  MarketingObjective,
  AnimationPlanType,
  ANIMATION_PLATFORM_TARGETS,
  ProductAnalysisCategory,
  ProductAvailabilityStatus,
  ProductBusinessType,
  ProductUnderstandingMarketingGoal,
  StoryboardGenerationPlatform,
  type AnimationGenerationEngineStatusReport,
  type AnimationGenerationRecord,
} from "../ai/index.js";
...
async function main(): Promise<void> {
  ...
  try {
    ...
    results.characterAnimation = {
      passed: Boolean(
        firstPlan?.characterAnimation.gesture &&
    ...
    results.productAnimation = {
      passed: Boolean(
        firstPlan?.productAnimation.rotation &&
    ...
    results.objectAnimation = {
      passed: Boolean(
        firstPlan?.objectAnimation.movement &&
    ...
    results.logoAnimation = {
      passed: Boolean(
        firstPlan?.logoAnimation.logoReveal &&
    ...
    results.environmentAnimation = {
      passed: Boolean(
        firstPlan?.environmentAnimation.particles &&
    ...
    results.transitionAnimation = {
      passed: Boolean(
        firstPlan?.transitionAnimation.fade &&
    ...
    results.animationTimeline = {
      passed: Boolean(
        firstPlan?.timeline.animationStart &&
    ...
    results.animationScores = {
      passed:
        (firstPlan?.scores.animationQualityScore ?? 0) >= 55 &&
    ...
    fs.writeFileSync(
      path.join(projectStateDir, "AI-Animation-Report.md"),
      buildMainReport(status, results, storageRoot, allPassed, techAnimation.plans, fashionAnimation.plans, beautyAnimation.plans),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Character-Animation-Report.md"),
      buildCharacterReport(techAnimation.plans),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Product-Animation-Report.md"),
      buildProductReport(techAnimation.plans),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Animation-Synchronization-Report.md"),
      buildSyncReport(techAnimation.plans),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Animation-Readiness-Report.md"),
      buildReadinessReport(status, techAnimation.plans, fashionAnimation.plans, beautyAnimation.plans),
      "utf8"
    );
    fs.writeFileSync(
      path.join(process.cwd(), "STEP-8F-VALIDATION-REPORT.md"),
      buildMainReport(status, results, storageRoot, allPassed, techAnimation.plans, fashionAnimation.plans, beautyAnimation.plans),
      "utf8"
    );
...
function buildMainReport(
  status: AnimationGenerationEngineStatusReport,
  results: Record<string, { passed: boolean; detail: string }>,
  storageRoot: string,
  allPassed: boolean,
  tech?: AnimationGenerationRecord[],
  fashion?: AnimationGenerationRecord[],
  beauty?: AnimationGenerationRecord[]
): string {
  return [
...
function buildCharacterReport(plans?: AnimationGenerationRecord[]): string {
  ...
  for (const plan of plans?.slice(0, 6) ?? []) {
    ...
    lines.push(
      ...
      "",
      `- Idle: ${c.idle}`,
      `- Gesture: ${c.gesture.slice(0, 60)}...`,
      `- Facial: ${c.facialAnimation}`,
      `- Lip sync: ${c.lipMovementPlan}`,
      `- Eye movement: ${c.eyeMovement}`,
      `- Hand movement: ${c.handMovement.slice(0, 60)}...`,
      ""
    );
  }
  return lines.join("\n");
...
function buildProductReport(plans?: AnimationGenerationRecord[]): string {
  const lines = [
  ...
  for (const plan of plans?.slice(0, 8) ?? []) {
    ...
    lines.push(
      `| ${plan.profile.sceneId.slice(-24)} | ${p.rotation.slice(0, 20)}... | ${p.reveal.slice(0, 20)}... | ${p.showcase.slice(0, 20)}... | ${p.highlight.slice(0, 20)}... |`
    );
  }
  lines.push("");
  return lines.join("\n");
...
function buildSyncReport(plans?: AnimationGenerationRecord[]): string {
  ...
  for (const plan of plans?.slice(0, 6) ?? []) {
    ...
    lines.push(
      `## ${plan.profile.sceneId}`,
      "",
      `- Motion sync: ${s.motionSync.join("; ")}`,
      `- Camera sync: ${s.cameraSync.slice(0, 2).join("; ")}...`,
      `- Audio sync: ${s.audioSync.join("; ")}`,
      `- Transition sync: ${s.transitionSync.join("; ")}`,
      `- Timeline: ${plan.timeline.animationStart} → ${plan.timeline.animationEnd} (${plan.timeline.animationDuration})`,
      ""
    );
  }
  return lines.join("\n");
