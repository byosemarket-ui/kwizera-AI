...
import {
  ...
  CreativePlatform,
  MarketingObjective,
  MarketingVideoPlanType,
  MARKETING_VIDEO_PLATFORM_TARGETS,
  ProductAnalysisCategory,
  ProductAvailabilityStatus,
  ProductBusinessType,
  ProductUnderstandingMarketingGoal,
  StoryboardGenerationPlatform,
  type MarketingVideoEngineStatusReport,
...
async function main(): Promise<void> {
  ...
  try {
    ...
    results.marketingStrategy = {
      passed: Boolean(
        firstPlan?.marketingStrategy.campaignObjective &&
    ...
    results.hookOptimization = {
      passed: Boolean(
        firstPlan?.hookOptimization.first3SecondsStrategy &&
    ...
    results.productPresentation = {
      passed: Boolean(
        firstPlan?.productPresentation.productRevealTiming &&
    ...
    results.ctaPlanning = {
      passed: Boolean(
        firstPlan?.callToAction.ctaTiming &&
    ...
    results.engagementOptimization = {
      passed: Boolean(
        firstPlan?.engagementOptimization.viewerRetentionStrategy &&
    ...
    results.conversionOptimization = {
      passed: Boolean(
        firstPlan?.conversionOptimization.purchaseMotivation &&
    ...
    results.marketingScores = {
      passed:
        (firstPlan?.scores.marketingQualityScore ?? 0) >= 55 &&
    ...
    fs.writeFileSync(
      path.join(projectStateDir, "AI-Marketing-Video-Report.md"),
      buildMainReport(status, results, storageRoot, allPassed, techMarketing.plans, fashionMarketing.plans, beautyMarketing.plans),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Marketing-Strategy-Report.md"),
      buildStrategyReport(techMarketing.plans),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "CTA-Planning-Report.md"),
      buildCtaReport(techMarketing.plans),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Engagement-Optimization-Report.md"),
      buildEngagementReport(techMarketing.plans),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Marketing-Readiness-Report.md"),
      buildReadinessReport(status, techMarketing.plans, fashionMarketing.plans, beautyMarketing.plans),
      "utf8"
    );
    fs.writeFileSync(
      path.join(process.cwd(), "STEP-8I-VALIDATION-REPORT.md"),
      buildMainReport(status, results, storageRoot, allPassed, techMarketing.plans, fashionMarketing.plans, beautyMarketing.plans),
      "utf8"
    );
...
function buildMainReport(
  status: MarketingVideoEngineStatusReport,
  results: Record<string, { passed: boolean; detail: string }>,
  storageRoot: string,
  allPassed: boolean,
  tech?: MarketingVideoRecord[],
  fashion?: MarketingVideoRecord[],
  beauty?: MarketingVideoRecord[]
): string {
  return [
...
function buildCtaReport(plans?: MarketingVideoRecord[]): string {
  ...
  for (const plan of plans ?? []) {
    ...
    lines.push(
      ...
      "",
      `- Timing: ${c.ctaTiming}`,
      `- Position: ${c.ctaPosition}`,
      `- Style: ${c.ctaStyle}`,
      `- Animation: ${c.ctaAnimation}`,
      `- Priority: ${c.ctaPriority}`,
      ""
    );
  }
  return lines.join("\n");
...
function buildEngagementReport(plans?: MarketingVideoRecord[]): string {
  ...
  for (const plan of plans ?? []) {
    ...
    lines.push(
      `## ${plan.profile.storyboardId}`,
      "",
      `- Retention: ${e.viewerRetentionStrategy}`,
      `- Emotional journey: ${e.emotionalJourney}`,
      `- Curiosity triggers: ${e.curiosityTriggers.join("; ")}`,
      `- Shareability: ${e.shareability}`,
      ""
    );
  }
  return lines.join("\n");
