...
import {
  ...
  type ImageQualityValidationEngineStatusReport,
  type ImageQualityValidationRecord,
} from "../ai/index.js";
...
async function prepareProductionPipeline(
...
async function prepareRenderPipeline(
  imgFoundation: NonNullable<ReturnType<ReturnType<typeof createAiCore>["getManager"]>["imageGenerationFoundation"]>,
  productId: string,
  productionId: string,
...
async function main(): Promise<void> {
  ...
  try {
    ...
    results.imageQualityValidation = {
      passed:
        (tech.record?.imageQuality.length ?? 0) === ALL_IMAGE_QUALITY_CHECKS.length &&
    ...
    results.layerValidation = {
      passed:
        (tech.record?.layerValidation.length ?? 0) === ALL_QUALITY_LAYER_CHECKS.length &&
    ...
    results.maskValidation = {
      passed:
        (tech.record?.maskValidation.length ?? 0) === ALL_QUALITY_MASK_TYPES.length &&
    ...
    results.typographyValidation = {
      passed:
        (tech.record?.typographyValidation.length ?? 0) === ALL_TYPOGRAPHY_CHECKS.length &&
    ...
    results.brandValidation = {
      passed:
        (tech.record?.brandValidation.length ?? 0) === ALL_BRAND_VALIDATION_CHECKS.length &&
...
function buildMainReport(
  status: ImageQualityValidationEngineStatusReport,
  results: Record<string, { passed: boolean; detail: string }>,
  storageRoot: string,
  allPassed: boolean,
  tech?: ImageQualityValidationRecord,
  fashion?: ImageQualityValidationRecord,
  food?: ImageQualityValidationRecord
): string {
  return [
...
function buildVisualReport(tech?: ImageQualityValidationRecord, fashion?: ImageQualityValidationRecord, food?: ImageQualityValidationRecord): string {
  const rows = [tech, fashion, food].filter(Boolean) as ImageQualityValidationRecord[];
...
function buildColorReport(tech?: ImageQualityValidationRecord, fashion?: ImageQualityValidationRecord, food?: ImageQualityValidationRecord): string {
  const rows = [tech, fashion, food].filter(Boolean) as ImageQualityValidationRecord[];
...
function buildBrandReport(tech?: ImageQualityValidationRecord, fashion?: ImageQualityValidationRecord, food?: ImageQualityValidationRecord): string {
  const rows = [tech, fashion, food].filter(Boolean) as ImageQualityValidationRecord[];
...
function buildPrintReport(tech?: ImageQualityValidationRecord, fashion?: ImageQualityValidationRecord, food?: ImageQualityValidationRecord): string {
  const rows = [tech, fashion, food].filter(Boolean) as ImageQualityValidationRecord[];
