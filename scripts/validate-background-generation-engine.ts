...
import {
  ...
  ALL_BACKGROUND_GEN_TYPES,
  ALL_SUBJECT_PRESERVATION_TARGETS,
  ...
  type BackgroundGenerationEngineStatusReport,
  type BackgroundGenerationRecord,
} from "../ai/index.js";
...
async function main(): Promise<void> {
  ...
  try {
    ...
    results.backgroundAnalysis = {
      passed: Boolean(
        tech.record?.backgroundAnalysis.backgroundType &&
    ...
    results.subjectPreservation = {
      passed:
        (tech.record?.subjectPreservation.targets.length ?? 0) === ALL_SUBJECT_PRESERVATION_TARGETS.length &&
    ...
    results.lightingMatching = {
      passed: Boolean(
        tech.record?.lightingMatching.lightDirection &&
    ...
    results.depthPlanning = {
      passed: Boolean(
        tech.record?.depthPlanning.foreground &&
    ...
    results.backgroundScores = {
      passed:
        (tech.record?.scores.backgroundQualityScore ?? 0) >= 55 &&
    ...
    fs.writeFileSync(
      path.join(projectStateDir, "AI-Background-Generation-Report.md"),
    ...
    fs.writeFileSync(
      ...
      buildAnalysisReport(tech.record, fashion.record, food.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Lighting-Matching-Report.md"),
      buildLightingReport(tech.record, fashion.record, food.record),
      "utf8"
    );
    fs.writeFileSync(
      path.join(projectStateDir, "Depth-Planning-Report.md"),
      buildDepthReport(tech.record, fashion.record, food.record),
      "utf8"
    );
    fs.writeFileSync(
    ...
    fs.writeFileSync(
      path.join(process.cwd(), "STEP-9E-VALIDATION-REPORT.md"),
      buildMainReport(status, results, storageRoot, allPassed, tech.record, fashion.record, food.record),
      "utf8"
    );
...
function buildMainReport(
  status: BackgroundGenerationEngineStatusReport,
  results: Record<string, { passed: boolean; detail: string }>,
  storageRoot: string,
  allPassed: boolean,
  tech?: BackgroundGenerationRecord,
  fashion?: BackgroundGenerationRecord,
  food?: BackgroundGenerationRecord
): string {
  return [
...
function buildAnalysisReport(
  ...
  const rows = [tech, fashion, food].filter(Boolean) as BackgroundGenerationRecord[];
  const lines = [
  ...
  for (const record of rows) {
    lines.push(
      `| ${record.profile.productId} | ${record.backgroundAnalysis.backgroundType} | ${record.backgroundAnalysis.sceneEnvironment.slice(0, 25)}... | ${record.backgroundAnalysis.lightingDirection.slice(0, 20)}... | ${record.backgroundAnalysis.shadowDirection.slice(0, 20)}... | ${record.scores.subjectPreservationScore}/100 |`
    );
  }
...
function buildReadinessReport(
  ...
  return [
    ...
    ...rows.map(
      (r) =>
        `| ${r.profile.productId} | ${r.scores.backgroundQualityScore} | ${r.scores.subjectPreservationScore} | ${r.scores.lightingConsistencyScore} | ${r.scores.brandConsistencyScore} | ${r.scores.productionReadinessScore} | ${r.scores.aiConfidenceScore} | ${r.productionReady ? "✅" : "❌"} |`
    ),
    "",
    "## Performance",
    "",
