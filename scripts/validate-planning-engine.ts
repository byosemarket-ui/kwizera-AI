...
async function main(): Promise<void> {
  ...
  try {
    ...
    results.resourceEstimation = {
      passed:
        (plan.executionPlan?.requiredResources.storageBytes ?? 0) > 0 &&
    ...
    results.riskAnalysis = {
      passed:
        plan.riskAnalysis.recoveryOptions.length >= 2 &&
    ...
    results.recoveryPlanning = {
      passed:
        (plan.executionPlan?.recoveryStrategy.checkpoints.length ?? 0) > 0 &&
    ...
    results.decisionIntegration = {
      passed:
        ...
        decisionResult.planningResult!.stepsCompleted.includes(
          PlanningStep.SendToWorkflowEngine
        ),
...
function buildMarkdownReport(
  status: ReturnType<
    import("../ai/planning/planning-engine.js").AiPlanningEngine["buildStatusReport"]
  >,
