...
async function main(): Promise<void> {
  ...
  try {
    ...
    results.logging = {
      passed: Boolean(
        manager.logger.getLogDirectory() && fs.existsSync(manager.logger.getLogDirectory()!)
      ),
    ...
    fs.writeFileSync(
      reportPath,
      buildReport(healthReport, results, storageRoot, allPassed),
      "utf8"
    );
...
function buildReport(
  status: ReturnType<
    import("../ai/module-manager/module-manager.js").AiModuleManager["buildStatusReport"]
  >,
