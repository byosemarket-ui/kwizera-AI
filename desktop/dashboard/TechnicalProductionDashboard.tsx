/**
 * Internal technical production dashboard.
 * Preserved for advanced/internal surfaces — not rendered as Customer Home.
 * Engines: live-engine, widget-store, module-slots remain the source of truth.
 */
import { useCallback, useEffect, useRef, useState } from "react";
import {
  Activity, FolderKanban, LayoutDashboard, MonitorCog, RefreshCw, X,
} from "lucide-react";
import type {
...
interface TechnicalProductionDashboardProps {
  onNavigate: (workspace: string) => void;
  workspaceLabel?: string;
}

/** Internal production dashboard — do not mount as Customer Home. */
export function TechnicalProductionDashboard({
  onNavigate,
  workspaceLabel = "Production",
}: TechnicalProductionDashboardProps) {
      ...
      case "current-workspace":
        return frame(
          <InfoCard
            ...
            detail="Internal production surface"
  ...
  return (
    <div className="professional-dashboard" data-technical-dashboard="true" data-dragging={dragId ?? undefined}>
      <header className="dash-header">
        <div>
          <span>INTERNAL PRODUCTION</span>
          <h2>Technical production dashboard</h2>
          <p>
...
function ActivityFeed({ projects, fallback }: { projects: DashboardProject[]; fallback: string }) {
  const events = projects.slice(0, 4).map((p) => ({
