export class ImageAnalysisScorer {
    computeScores(technical, visual, content, missingFields) {
        const totalFields = 18;
        const filledFields = totalFields - missingFields.length;
        const imageCompletenessScore = Math.round(Math.max(0, Math.min(100, (filledFields / totalFields) * 100)));
        ...
        if (technical.bitDepth >= 8)
            technicalQualityScore += 5;
        if (technical.fileSizeBytes > 0 && technical.fileSizeBytes < 20_000_000)
        ...
        if (content.products.length > 0 || content.logos.length > 0)
            analysisConfidenceScore += 10;
        if (visual.dominantColors.length >= 2)
            analysisConfidenceScore += 10;
...
//# sourceMappingURL=image-analysis-scorer.js.map