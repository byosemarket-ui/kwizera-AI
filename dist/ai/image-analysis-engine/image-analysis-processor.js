...
import { ImageAnalysisType, } from "./types.js";
...
export class ImageAnalysisProcessor {
    ...
    async analyze(input) {
        ...
        const scores = this.scorer.computeScores(analysis.technical, analysis.visual, analysis.content, missingFields);
        ...
        const intelligenceValidation = this.foundation.validateImageIntelligence({
            ...
            verificationStatus: scores.analysisConfidenceScore >= 75
    ...
    search(query) {
        ...
        if (query.product) {
            ...
            results = results.filter((r) => r.content.products.some((p) => p.toLowerCase().includes(q)) ||
        ...
        if (query.dominantColor) {
            ...
            results = results.filter((r) => r.visual.dominantColors.some((c) => c.toLowerCase().includes(q)));
        }
...
//# sourceMappingURL=image-analysis-processor.js.map