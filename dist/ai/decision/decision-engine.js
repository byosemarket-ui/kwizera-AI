...
import { DecisionEngineError, DecisionStatus, DecisionStep, } from "./types.js";
...
export class AiDecisionEngine {
    ...
    history = new DecisionHistoryStore();
    priorityManager = new DecisionPriorityManager();
    qualityEvaluator = new QualityEvaluator();
    solutionGenerator = new SolutionGenerator();
    solutionScorer = new SolutionScorer();
    ...
    setReasoningEngine(engine) {
        this.reasoningEngine = engine;
    }
    setPlanningEngine(engine) {
    ...
    async decide(request) {
        ...
        try {
            ...
            const objective = request.statedObjective ??
                request.availableData.objective ??
                request.userRequest;
                ...
                if (!reasoningResult.readyForDecision) {
                    return this.buildIncompleteResult({
                        ...
                        recommendations: [
                            ...
                            ...(reasoningResult.explanation?.internalNotes.filter((n) => n.startsWith("Missing") || n.includes("collect")) ?? []),
            ...
            const knowledgeResult = await this.knowledgeProvider.search(objective, request.availableData);
            // Step 6 — Detect Missing Information
            stepsCompleted.push(DecisionStep.DetectMissing);
            const missingInformation = this.qualityEvaluator.detectMissingInformation(request, request.availableData);
            ...
            const qualityAssessment = this.qualityEvaluator.evaluate(request, request.availableData);
            ...
            const comparisonNote = this.solutionScorer.compare(candidates.map((c) => ({
                ...c,
            ...
            const scored = this.solutionScorer.score(candidates, request, qualityAssessment.score);
            ...
            this.logger.log(validation.passed ? "info" : "warn", "validation", validation.passed ? "Decision validated" : "Decision validation failed", { decisionId, checks: validation.checks });
            ...
            const workflowId = reasoningResult?.recommendation?.suggestedWorkflow ?? selected.workflowId;
            const preliminaryHandoff = {
                workflowId,
    ...
    buildStatusReport() {
        ...
        const averageDecisionMs = total > 0
        ...
        const accuracy = total > 0 ? Math.round((approved.length / this.history.getCount()) * 100) : 100;
        const knownIssues = [];
        if (!this.initialized) {
