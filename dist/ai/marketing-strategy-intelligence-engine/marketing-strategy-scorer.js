export class MarketingStrategyScorer {
    computeScores(businessGoals, audienceAlignment, strategies, creativePreparation, campaignDirection, objective) {
        ...
        const confidenceScore = Math.round((strategyQualityScore + audienceAlignmentScore + businessAlignmentScore + marketingReadinessScore) / 4);
        return {
    ...
    isStrategyValid(scores, strategies, audienceAlignment) {
        const diagnostics = [];
    ...
    computeStrategyQuality(strategies, objective) {
        let score = 50;
        if (strategies.some((s) => s.priority === "primary"))
            score += 15;
        if (strategies.length >= 3)
            score += 10;
        if (strategies.every((s) => s.rationale.length >= 30))
            score += 10;
        if (strategies.every((s) => s.expectedOutcome.length >= 20))
            score += 10;
        if (strategies.some((s) => s.rationale.includes(objective.replace(/-/g, " "))))
            score += 5;
        return Math.min(100, score);
    }
    computeBusinessAlignment(goals, objective) {
        const sections = [
        ...
        if (allGoals.includes(objectiveKeyword.split(" ")[0]))
            score += 10;
        return Math.min(100, score);
    }
    computeMarketingReadiness(prep, campaign) {
        const flags = [
...
//# sourceMappingURL=marketing-strategy-scorer.js.map