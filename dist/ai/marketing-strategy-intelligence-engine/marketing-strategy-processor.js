...
import { BusinessGoalType, } from "./types.js";
export class MarketingStrategyProcessor {
    ...
    async strategize(input) {
        ...
        const audienceAlignment = this.analyzer.buildAudienceAlignment(understanding, analysis, audienceIntelligence, input.preferredPlatforms);
        const selectedStrategies = this.analyzer.selectStrategies(input.marketingObjective, understanding, analysis);
        const creativePreparation = this.analyzer.prepareCreativeDirection(selectedStrategies, understanding, audienceAlignment);
        const campaignDirection = this.analyzer.prepareCampaignDirection(input.marketingObjective, selectedStrategies, audienceAlignment, understanding, input.campaignId);
        const scores = this.scorer.computeScores(businessGoals, audienceAlignment, selectedStrategies, creativePreparation, campaignDirection, input.marketingObjective);
        ...
        const strategyId = input.strategyId ?? existing?.strategyId ?? `marketing-strategy-${input.productId}-${input.marketingObjective}`;
        const draft = {
            strategyId,
            productId: input.productId,
        ...
        draft.relationships = this.linker.detectRelationships(draft, this.records.getAll(), understanding, analysis, audienceIntelligence, input.campaignId);
        const intelligenceValidation = this.foundation.validateProductIntelligence({
            qualityScore: scores.strategyQualityScore,
            confidenceScore: scores.confidenceScore,
            verificationStatus: scores.confidenceScore >= 75
    ...
    search(query) {
        ...
        if (query.brand) {
            ...
            results = results.filter((r) => r.relationships.brands.some((b) => b.toLowerCase().includes(brandLower)));
        }
        if (query.audience) {
            const audienceLower = query.audience.toLowerCase();
            results = results.filter((r) => r.audienceAlignment.targetAudience.toLowerCase().includes(audienceLower) ||
                r.relationships.audiences.some((a) => a.toLowerCase().includes(audienceLower)));
        }
        if (query.industry) {
            const industryLower = query.industry.toLowerCase();
            results = results.filter((r) => r.audienceAlignment.customerNeeds.some((n) => n.toLowerCase().includes(industryLower)) ||
                r.businessGoals.growthObjectives.some((g) => g.toLowerCase().includes(industryLower)));
        }
        if (query.strategyType) {
            results = results.filter((r) => r.selectedStrategies.some((s) => s.strategyType === query.strategyType));
        }
        if (query.platform) {
            results = results.filter((r) => r.audienceAlignment.preferredPlatforms.includes(query.platform));
        }
        if (query.businessGoal) {
            const prefix = this.businessGoalPrefix(query.businessGoal);
            results = results.filter((r) => r.relationships.businessGoals.some((g) => g.startsWith(prefix)));
        }
        if (query.text) {
            const textLower = query.text.toLowerCase();
            results = results.filter((r) => r.strategyId.toLowerCase().includes(textLower) ||
                r.selectedStrategies.some((s) => s.strategyType.includes(textLower) ||
...
//# sourceMappingURL=marketing-strategy-processor.js.map