...
export class MotionGenerationProcessor {
    ...
    async generateMotionPlans(input) {
        ...
        try {
            ...
            const invalid = pairs.find((p) => !p.scene.validated ||
            ...
            for (const { scene, cameraPlan } of pairs) {
                ...
                const draft = {
                    ...draftBase,
                    scores,
                    relationships: this.linker.detectRelationships({ ...draftBase, scores }, scene, cameraPlan, input),
    ...
    search(query) {
        ...
        if (query.motionPlanId)
            results = results.filter((r) => r.motionPlanId === query.motionPlanId);
        if (query.sceneId)
        ...
        if (query.motionType)
            results = results.filter((r) => r.motionType === query.motionType);
        if (query.productId)
        ...
        if (query.keywords) {
            ...
            results = results.filter((r) => r.storytellingOptimization.marketingMoment.toLowerCase().includes(kw) ||
                r.productMotion.showcaseMotion.toLowerCase().includes(kw));
        }
        if (query.text) {
            const textLower = query.text.toLowerCase();
            results = results.filter((r) => r.motionPlanId.toLowerCase().includes(textLower) ||
                r.profile.sceneId.toLowerCase().includes(textLower));
        }
        return results.slice(0, query.limit ?? 50);
    }
    resolveSceneCameraPairs(input) {
...
//# sourceMappingURL=motion-generation-processor.js.map