...
export class TaskModuleExecutor {
    execute(planTask, registry, simulateFailure = false) {
        ...
        if (simulateFailure) {
            ...
            return {
                ...
                record: this.buildRecord(planTask, startedAt, durationMs, TaskExecutionStatus.Failed, true, "SIMULATED_FAILURE"),
            };
        }
        const durationMs = Math.round(performance.now() - start);
        return {
            success: true,
            record: this.buildRecord(planTask, startedAt, durationMs, TaskExecutionStatus.Completed, true, undefined, `Task coordinated to ${planTask.moduleId} (${entry.status})`),
        };
    }
    buildRecord(planTask, startedAt, durationMs, status, coordinated, error, message) {
        return {
            taskId: planTask.id,
...
//# sourceMappingURL=task-module-executor.js.map