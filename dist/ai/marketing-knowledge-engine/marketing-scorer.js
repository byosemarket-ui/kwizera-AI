...
export class MarketingScorer {
    computeScores(brand, structure, campaign, customer, content, storytelling) {
        ...
        const marketingQualityScore = Math.round(brand.brandConsistency * 0.2 +
            campaign.brandingConsistency * 0.15 +
            (hasBenefits ? 20 : 8) +
            (hasSocialProof ? 15 : 5) +
            (hasCta ? 20 : 5) +
            (content.headlines.length >= 2 ? 10 : 3));
        const brandConsistencyScore = Math.round(brand.brandConsistency * 0.5 +
            campaign.brandingConsistency * 0.3 +
            (brand.brandColors.length >= 2 ? 10 : 0) +
            (brand.brandMessaging ? 10 : 0));
        const customerRelevanceScore = Math.round(customer.customerNeeds.length * 8 +
            customer.buyingTriggers.length * 7 +
            customer.trustFactors.length * 6 +
            (customer.customerIntent ? 15 : 0));
        const campaignStructureScore = Math.round((structure.hook ? 15 : 0) +
        ...
        const storytellingScore = Math.round((storytelling.hookTiming <= 5 ? 25 : 12) +
        ...
        const conversionReadinessScore = Math.round((hasCta ? 30 : 5) +
        ...
        const aiConfidenceScore = Math.round((marketingQualityScore +
