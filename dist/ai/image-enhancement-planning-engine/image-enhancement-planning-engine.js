...
import { ImageEnhancementPlanningEngineError, } from "./types.js";
/**
...
export class AiImageEnhancementPlanningEngine {
    ...
    logger = new EnhancementPlanningLogger();
    records = new ImageEnhancementPlanningRecordStore();
    analyzer = new EnhancementPlanningAnalyzer();
    scorer = new EnhancementPlanningScorer();
    linker = new EnhancementPlanningLinker();
    processor = null;
    planningTimes = [];
    searchTimes = [];
    relationshipTimes = [];
    initialize(foundation, storageRoot) {
        ...
        this.processor = new EnhancementPlanningProcessor(foundation, this.analyzer, this.scorer, this.linker, this.records, this.logger);
    ...
    detectRelationships(imageId) {
        ...
        const updated = this.linker.detectRelationships(record, this.records.getAll(), analysis, understanding, this.foundation.getBackgroundIntelligenceEngine().getBackground(imageId), this.foundation.getCompositionIntelligenceEngine().getComposition(imageId), this.foundation.getLightingColorIntelligenceEngine().getLightingColor(imageId), record.relationships.relatedProjects, record.relationships.relatedKnowledge);
        this.relationshipTimes.push(Date.now() - start);
        return updated;
    }
    async repairEnhancementPlan(imageId) {
        this.ensureReady();
    ...
    buildStatusReport() {
        ...
        return {
            ...
            enhancementPlanningStatus: "resolution, noise, sharpening, exposure, contrast, color, background and object enhancement planned",
    ...
    ensureReady() {
        if (!this.initialized || !this.foundation || !this.processor) {
            throw new ImageEnhancementPlanningEngineError("Image Enhancement Planning Engine not initialized", "NOT_INITIALIZED");
        }
    }
}
//# sourceMappingURL=image-enhancement-planning-engine.js.map