...
export class EnhancementPlanningProcessor {
    ...
    async plan(input) {
        ...
        const built = this.analyzer.buildFromIntelligence(analysis, understanding, detection, background, composition, lightingColor, brandVisual, input.projectId, input.platform);
        const platformReadiness = this.scorer.computePlatformReadiness(built.qualityAnalysis, built.profile.platform);
        const hasRestorationNeed = built.restorationPlan.qualityRecovery.includes("multi-pass") ||
        ...
        draft.relationships = this.linker.detectRelationships(draft, this.records.getAll(), analysis, understanding, background, composition, lightingColor, input.relatedProjects, input.relatedKnowledge);
...
//# sourceMappingURL=enhancement-planning-processor.js.map