export class CreativeVideoScorer {
    computeScores(storyboard, recommendationCount, templateMatchScore, storytellingBase, marketingBase, brandConsistency, productionBase) {
        let creativeScore = 55;
        if (storyboard.sceneOrder.length >= 3)
            creativeScore += 15;
        if (storyboard.sceneTiming.length >= 3)
            creativeScore += 10;
        ...
        const productionReadinessScore = Math.round((productionBase + creativeScore + marketingScore) / 3);
        const aiConfidenceScore = Math.round((creativeScore +
            storytellingScore +
            marketingScore +
            visualImpactScore +
    ...
    isPlanValid(scores, sceneCount, recommendationCount, templateCount) {
        const diagnostics = [];
        if (sceneCount < 2)
            diagnostics.push("At least 2 storyboard scenes required");
        if (recommendationCount < 3)
            diagnostics.push("At least 3 creative recommendations required");
        if (templateCount < 1)
            diagnostics.push("At least 1 creative template match required");
...
//# sourceMappingURL=creative-video-scorer.js.map