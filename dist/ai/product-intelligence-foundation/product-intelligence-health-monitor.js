...
import { ProductIntelligenceHealthLevel, } from "./types.js";
...
export class ProductIntelligenceHealthMonitor {
    ...
    async runHealthCheck(storage, registry, access, integrationReady) {
        ...
        const failedModules = modules.filter((m) => m.healthStatus === ProductIntelligenceHealthLevel.Critical ||
    ...
    verifyRegistryHealth(modules) {
        return (modules.length >= PREPARED_PRODUCT_INTELLIGENCE_MODULES.length &&
            modules.every((m) => m.storageLocation.length > 0));
    }
    scoreToLevel(score) {
        if (score >= 95)
            return ProductIntelligenceHealthLevel.Excellent;
        if (score >= 80)
            return ProductIntelligenceHealthLevel.Good;
        if (score >= 60)
            return ProductIntelligenceHealthLevel.Warning;
        if (score >= 40)
            return ProductIntelligenceHealthLevel.Critical;
...
//# sourceMappingURL=product-intelligence-health-monitor.js.map