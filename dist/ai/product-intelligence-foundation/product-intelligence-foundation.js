...
import { ProductIntelligenceFoundationError, ProductIntelligenceHealthLevel, ProductIntelligenceLifecycleState, } from "./types.js";
/**
...
export class AiProductIntelligenceFoundation {
    ...
    lifecycleState = ProductIntelligenceLifecycleState.Initializing;
    startupMs = 0;
    lastIntegrity = null;
    lastHealth = null;
    logger = new ProductIntelligenceFoundationLogger();
    history = new ProductIntelligenceHistoryStore();
    integration = new ProductIntelligenceIntegrationBridge(this.logger);
    storage = new ProductIntelligenceStorageManager(this.logger);
    registry = new ProductIntelligenceRegistry(this.logger);
    integrityVerifier = new ProductIntelligenceIntegrityVerifier(this.logger);
    healthMonitor = new ProductIntelligenceHealthMonitor(this.logger);
    accessCoordinator = null;
    qualityValidator = null;
    productAnalysisEngine = new AiProductAnalysisEngine();
    productUnderstandingEngine = new AiProductUnderstandingEngine();
    targetAudienceIntelligenceEngine = new AiTargetAudienceIntelligenceEngine();
    marketingStrategyIntelligenceEngine = new AiMarketingStrategyIntelligenceEngine();
    creativeDirectionEngine = new AiCreativeDirectionEngine();
    storyboardIntelligenceEngine = new AiStoryboardIntelligenceEngine();
    scriptPlanningEngine = new AiScriptPlanningEngine();
    visualPlanningEngine = new AiVisualPlanningEngine();
    audioPlanningEngine = new AiAudioPlanningEngine();
    productionPlanningEngine = new AiProductionPlanningEngine();
    qualityPredictionEngine = new AiQualityPredictionEngine();
    productIntelligenceOptimizationEngine = new AiProductIntelligenceOptimizationEngine();
    productIntelligenceHealthMonitorEngine = new AiProductIntelligenceHealthMonitorEngine();
    initialize(core, storageRoot, memoryFoundation, knowledgeFoundation, moduleManager, stateManager, recoveryEngine, systemHealthMonitor) {
        this.core = core;
        ...
        this.accessCoordinator = new ProductIntelligenceAccessCoordinator(this.logger, this.history, this.registry, this.storage);
        ...
        this.productIntelligenceHealthMonitorEngine.initialize(this, storageRoot, path.join(DEFAULT_STORAGE_ROOT, "project-state"));
        this.integration.connect(core, memoryFoundation, knowledgeFoundation, moduleManager, stateManager, recoveryEngine, systemHealthMonitor);
    ...
    registerProductIntelligenceModule(registration) {
        this.ensureReady();
        const full = {
            ...registration,
    ...
    validateProductIntelligence(metadata) {
        this.ensureReady();
    ...
    refreshIntegration(memoryFoundation, knowledgeFoundation, moduleManager, stateManager, recoveryEngine, systemHealthMonitor) {
        if (!this.core)
            return;
        this.integration.connect(this.core, memoryFoundation, knowledgeFoundation, moduleManager, stateManager, recoveryEngine, systemHealthMonitor);
    }
    ...
    getProductAnalysisEngine() {
        return this.productAnalysisEngine;
    }
    getProductUnderstandingEngine() {
    ...
    getProductIntelligenceOptimizationEngine() {
        return this.productIntelligenceOptimizationEngine;
    }
    getProductIntelligenceHealthMonitorEngine() {
    ...
    ensureReady() {
        if (!this.initialized || !this.accessCoordinator || !this.qualityValidator) {
            throw new ProductIntelligenceFoundationError("Product Intelligence Foundation not initialized", "NOT_INITIALIZED");
        }
    }
}
//# sourceMappingURL=product-intelligence-foundation.js.map