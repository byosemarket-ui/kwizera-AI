...
import { KnowledgeHealthLevel, } from "./types.js";
export class KnowledgeHealthMonitor {
    ...
    async runHealthCheck(storage, registry, access, integrationReady) {
        ...
        const failedModules = modules.filter((m) => m.healthStatus === KnowledgeHealthLevel.Critical || m.healthStatus === KnowledgeHealthLevel.Failed);
    ...
    scoreToLevel(score) {
        if (score >= 95)
            return KnowledgeHealthLevel.Excellent;
        if (score >= 80)
            return KnowledgeHealthLevel.Good;
        if (score >= 60)
            return KnowledgeHealthLevel.Warning;
        if (score >= 40)
            return KnowledgeHealthLevel.Critical;
        return KnowledgeHealthLevel.Failed;
    }
    verifyLogDirectory(logDir) {
        return fs.existsSync(logDir);
    }
    verifyStorageWritable(knowledgeRoot) {
        try {
...
//# sourceMappingURL=knowledge-health-monitor.js.map