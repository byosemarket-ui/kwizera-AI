...
import { ProjectMemoryEngineError, ProjectStatus, } from "./types.js";
/**
...
export class AiProjectMemoryEngine {
    ...
    logger = new ProjectMemoryLogger();
    history = new ProjectHistoryStore();
    checkpoints = new ProjectCheckpointStore();
    projects = new Map();
    ...
    initialize(foundation, storageRoot) {
        ...
        this.processor = new ProjectProcessor(foundation, this.history, this.checkpoints, this.scorer, this.linker, this.logger, this.projects);
        this.restorer = new ProjectRestorer(this.processor, this.checkpoints, this.history, this.logger);
    ...
    async restoreProject(projectId, checkpointId) {
        this.ensureReady();
        const result = await this.restorer.restore(projectId, checkpointId);
        if (result.success)
            this.restoreTimes.push(result.durationMs);
        return result;
    }
    ...
    searchProjects(query) {
        ...
        if (query.brand) {
            ...
            results = results.filter((p) => p.tags.some((t) => t.toLowerCase().includes(lower)) ||
    ...
    buildStatusReport() {
        ...
        const active = projects.filter((p) => p.status !== ProjectStatus.Archived && p.status !== ProjectStatus.Completed).length;
    ...
    ensureReady() {
        if (!this.initialized || !this.foundation) {
            throw new ProjectMemoryEngineError("Project Memory Engine not initialized", "NOT_INITIALIZED");
        }
    }
}
//# sourceMappingURL=project-memory-engine.js.map