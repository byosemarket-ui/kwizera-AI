...
import { inferPriority, inferQueueCategory, inferTaskType, } from "../task-manager/task-manager.js";
import { WorkflowEngineError, WorkflowState, WorkflowStep, } from "./types.js";
/**
...
export class AiWorkflowEngine {
    ...
    history = new WorkflowHistoryStore();
    planValidator = new WorkflowPlanValidator();
    dependencyManager = new WorkflowDependencyManager();
    ...
    async execute(input) {
        ...
        try {
            ...
            const tracking = this.progressTracker.createTracking(scheduled, plan.estimatedTime.totalMs);
            ...
            while (taskIndex < scheduled.length) {
                ...
                if (this.taskManager) {
                    ...
                    result = this.taskCoordinator.coordinate(task, this.core.registry, simulateFail);
                    if (!result.success) {
                        state = WorkflowState.Paused;
                        ...
                        const recovery = await this.recoveryManager.attemptRecovery(task, this.taskCoordinator, this.core.registry, tracking);
                        recoveryEvents.push(recovery.event);
                        this.logger.log(recovery.recovered ? "info" : "warn", "recovery", recovery.event.message, { workflowRunId, taskId: task.id });
                ...
                this.progressTracker.updateAfterTask(tracking, task.id, result.record.durationMs, plan.estimatedTime.perTaskMs[task.id] ?? task.estimatedMs);
    ...
    buildStatusReport() {
        ...
        const averageWorkflowMs = total > 0
    ...
    buildFailureResult(params) {
        const tracking = params.tracking ??
            this.progressTracker.createTracking(params.input.executionPlan.taskList, params.input.executionPlan.estimatedTime.totalMs);
