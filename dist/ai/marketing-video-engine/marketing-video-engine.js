...
import { MarketingVideoEngineError, } from "./types.js";
/**
...
export class AiMarketingVideoEngine {
    ...
    logger = new MarketingVideoLogger();
    records = new MarketingVideoRecordStore();
    analyzer = new MarketingVideoAnalyzer();
    scorer = new MarketingVideoScorer();
    linker = new MarketingVideoLinker();
    processor = null;
    planningTimes = [];
    searchTimes = [];
    recommendationTimes = [];
    initialize(foundation, storageRoot) {
        ...
        this.processor = new MarketingVideoProcessor(foundation, this.analyzer, this.scorer, this.linker, this.records, this.logger);
    ...
    getMarketingVideoPlan(marketingVideoId) {
        this.ensureReady();
        return this.records.get(marketingVideoId) ?? null;
    }
    getMarketingVideoPlansByStoryboard(storyboardId) {
        this.ensureReady();
        return this.records.getByStoryboard(storyboardId);
    }
    searchMarketingVideoPlans(query) {
        this.ensureReady();
    ...
    async repairMarketingVideoPlans(storyboardId, platform) {
        this.ensureReady();
    ...
    buildStatusReport() {
        ...
        if (!this.startupComplete)
            readinessScore -= 25;
        if (!this.foundation?.getAudioSynchronizationEngine().isStartupComplete())
            readinessScore -= 10;
...
//# sourceMappingURL=marketing-video-engine.js.map