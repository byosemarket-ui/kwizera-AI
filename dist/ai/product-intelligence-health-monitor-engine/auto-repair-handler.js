import { ProductIntelligenceHealthScoreLevel, ProductIntelligenceWarningType, } from "./types.js";
export class ProductIntelligenceAutoRepairHandler {
    ...
    async attemptRepairs(warnings) {
        const repairs = [];
        let success = true;
        const critical = warnings.some((w) => w.severity === ProductIntelligenceHealthScoreLevel.Critical ||
            w.severity === ProductIntelligenceHealthScoreLevel.Failed);
        const hasCriticalWarning = warnings.some((w) => w.type === ProductIntelligenceWarningType.DatabaseProblems ||
        ...
        if (critical || hasCriticalWarning || warnings.length > 3) {
            ...
            this.foundation.integration.reportCriticalIssue(`Critical product intelligence health: ${warnings.map((w) => w.message).join("; ")}`);
