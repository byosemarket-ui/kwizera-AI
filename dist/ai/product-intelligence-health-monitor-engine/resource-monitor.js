...
export class ProductIntelligenceResourceMonitor {
    ...
    measure() {
        ...
        return {
            ...
            cpuUsagePercent: 0,
            searchPerformanceMs: Math.max(script.performance.averageSearchMs, analysis.performance.averageSearchMs),
...
export function deriveProductIntelligencePerformanceIssues(metrics) {
    const issues = [];
    if (metrics.planningPerformanceMs > 120000)
        issues.push("Slow product planning detected");
    if (metrics.searchPerformanceMs > 200)
        issues.push("Slow product search detected");
    if (metrics.diskUsageMb > 5000)
        issues.push("High product intelligence disk usage");
