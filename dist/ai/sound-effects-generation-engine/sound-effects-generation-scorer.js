export class SoundEffectsGenerationScorer {
    computeScores(analysis, soundEffectPlan, foleyPlan, environmentalPlan, cinematicPlan, timelinePlan, syncPlan, productionInstructions, context) {
        ...
        const aiConfidenceScore = Math.round((realismScore +
    ...
    isSoundPlanValid(scores, record) {
        const diagnostics = [];
        if (scores.realismScore < 55)
            diagnostics.push(`Realism score ${scores.realismScore} below threshold (55)`);
        if (scores.synchronizationScore < 55)
            diagnostics.push(`Synchronization score ${scores.synchronizationScore} below threshold (55)`);
        if (scores.layerQualityScore < 55)
            diagnostics.push(`Layer quality score ${scores.layerQualityScore} below threshold (55)`);
    ...
    isBrandConsistent(context, instructions) {
        if (!context.brandName)
            return instructions.renderNotes.length >= 1;
    ...
    totalSoundLayers(plan) {
        return (plan.foleySounds.length +
    ...
    computeRealismScore(analysis, plan, foley) {
        let score = 45;
        if (analysis.objects.length >= 1)
            score += 15;
        if (analysis.environment.length >= 5)
            score += 10;
        if (foley.foleyTypes.length >= 1)
            score += 15;
        if (this.totalSoundLayers(plan) >= 3)
            score += 15;
    ...
    computeLayerQualityScore(plan, env, cinematic) {
        let score = 45;
        if (env.ambientLayers.length >= 1)
            score += 15;
        if (env.spatialNotes.length >= 1)
            score += 10;
        if (cinematic.cinematicTypes.length >= 1)
            score += 15;
        if (this.totalSoundLayers(plan) >= 2)
            score += 15;
        return Math.min(100, score);
    }
    ...
    computeProductionReadiness(analysis, timeline, sync, instructions) {
        ...
        if (analysis.durationSec > 0)
            score += 10;
        if (timeline.layerPositions.length >= 2)
            score += 15;
        if (sync.hitPoints.length >= 1)
            score += 15;
...
//# sourceMappingURL=sound-effects-generation-scorer.js.map