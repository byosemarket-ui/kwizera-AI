...
import { LightingColorIntelligenceEngineError, } from "./types.js";
/**
...
export class AiLightingColorIntelligenceEngine {
    ...
    logger = new LightingColorLogger();
    records = new LightingColorIntelligenceRecordStore();
    analyzer = new LightingColorAnalyzer();
    scorer = new LightingColorScorer();
    linker = new LightingColorLinker();
    processor = null;
    analysisTimes = [];
    searchTimes = [];
    relationshipTimes = [];
    initialize(foundation, storageRoot) {
        ...
        this.processor = new LightingColorProcessor(foundation, this.analyzer, this.scorer, this.linker, this.records, this.logger);
    ...
    async analyzeLightingColor(input) {
        this.ensureReady();
    ...
    getLightingColor(imageId) {
        this.ensureReady();
        return this.records.get(imageId) ?? null;
    }
    searchLightingColor(query) {
        this.ensureReady();
    ...
    detectRelationships(imageId) {
        ...
        const updated = this.linker.detectRelationships(record, this.records.getAll(), analysis, understanding, composition, background, record.relationships.relatedProjects, record.relationships.relatedKnowledge);
        this.relationshipTimes.push(Date.now() - start);
        return updated;
    }
    async repairLightingColor(imageId) {
        this.ensureReady();
    ...
    buildStatusReport() {
        ...
        return {
            ...
            lightingAnalysisStatus: "type, direction, intensity, uniformity, exposure, shadows, highlights and reflections analyzed",
            colorAnalysisStatus: "palette, harmony, contrast, saturation, vibrance, hue distribution, white balance and temperature analyzed",
    ...
    ensureReady() {
        if (!this.initialized || !this.foundation || !this.processor) {
            throw new LightingColorIntelligenceEngineError("Lighting & Color Intelligence Engine not initialized", "NOT_INITIALIZED");
        }
    }
}
//# sourceMappingURL=lighting-color-intelligence-engine.js.map