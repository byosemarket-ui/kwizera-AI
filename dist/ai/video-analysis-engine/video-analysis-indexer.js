    ...
    createIndexes(record, projectId) {
        ...
        for (const segment of record.timeline.segments) {
            if (segment.type === "scene") {
                const sceneEntry = frameIndex.indexScene(project, record.videoId, segment.segmentId, segment.startMs, segment.endMs, timelineEntry.timelineId);
                indexes.sceneIndexIds.push(sceneEntry.indexId);
            }
            else if (segment.type === "shot") {
                const shotEntry = frameIndex.indexShot(project, record.videoId, segment.segmentId, segment.segmentId.replace("shot", "scene-1"), segment.startMs);
        ...
        if (record.frame.totalFrames > 0) {
            ...
            const midEntry = frameIndex.indexFrame(project, record.videoId, midFrame, Math.round((midFrame / record.technical.fps) * 1000), { timelineId: timelineEntry.timelineId });
...
//# sourceMappingURL=video-analysis-indexer.js.map