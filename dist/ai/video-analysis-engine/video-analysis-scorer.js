export class VideoAnalysisScorer {
    computeScores(technical, frame, timeline, audio, visual, missingFields) {
        const totalFields = 22;
        const filledFields = totalFields - missingFields.length;
        const videoCompletenessScore = Math.round(Math.max(0, Math.min(100, (filledFields / totalFields) * 100)));
        let technicalQualityScore = 60;
        if (technical.width >= 1920 && technical.height >= 1080)
            technicalQualityScore += 15;
        else if (technical.width >= 1280)
            technicalQualityScore += 8;
        if (technical.fps >= 24)
            technicalQualityScore += 8;
        if (technical.bitrateKbps >= 5000)
            technicalQualityScore += 7;
        if (technical.durationMs > 0 && technical.durationMs < 600_000)
            technicalQualityScore += 5;
        if (Object.keys(technical.metadata).length > 0)
            technicalQualityScore += 5;
        technicalQualityScore = Math.min(100, technicalQualityScore);
        let frameQualityScore = 65;
        if (frame.frameConsistencyScore >= 80)
            frameQualityScore += 12;
        if (frame.missingFrames === 0)
            frameQualityScore += 8;
        if (frame.corruptedFrames === 0)
            frameQualityScore += 7;
        if (frame.keyFrames > 0)
            frameQualityScore += 5;
        if (frame.motionDensity >= 30 && frame.motionDensity <= 80)
            frameQualityScore += 5;
        ...
        if (visual.sharpness >= 70)
            visualQualityScore += 12;
        if (visual.brightness >= 40 && visual.brightness <= 80)
            visualQualityScore += 8;
        if (visual.contrast >= 50)
            visualQualityScore += 8;
        if (visual.visualStability >= 70)
        ...
        const productionReadiness = {
            editingReadiness: Math.min(100, Math.round(frameQualityScore * 0.4 + technicalQualityScore * 0.35 + (timeline.sceneCount > 0 ? 25 : 10))),
            aiGenerationReadiness: Math.min(100, Math.round(visualQualityScore * 0.45 + frameQualityScore * 0.35 + 20)),
            marketingReadiness: Math.min(100, Math.round(visualQualityScore * 0.4 + audioQualityScore * 0.3 + 30)),
            productionReadiness: Math.min(100, Math.round((technicalQualityScore + frameQualityScore + audioQualityScore + visualQualityScore) / 4)),
            renderingReadiness: Math.min(100, Math.round(technicalQualityScore * 0.55 + frameQualityScore * 0.25 + 20)),
            exportReadiness: Math.min(100, Math.round(technicalQualityScore * 0.65 + 20)),
        };
        const productionReadinessScore = Math.round((productionReadiness.editingReadiness +
    ...
    isAnalysisValid(scores, missingFields, criticallyIncomplete) {
        ...
        if (scores.videoCompletenessScore < 45) {
            diagnostics.push(`Video completeness score ${scores.videoCompletenessScore} below minimum threshold (45)`);
        }
    ...
    buildRecommendations(scores, frame, audio) {
        const recs = [];
...
//# sourceMappingURL=video-analysis-scorer.js.map