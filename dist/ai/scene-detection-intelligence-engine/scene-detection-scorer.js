...
export class SceneDetectionScorer {
    computeScores(scenes, shots, transitions, timelineLengthMs, sceneCountExpected, shotCountExpected, frameConsistencyScore) {
        let sceneDetectionScore = 55;
        if (scenes.length >= 3)
            sceneDetectionScore += 15;
        if (scenes.length >= sceneCountExpected)
            sceneDetectionScore += 10;
        if (scenes.every((s) => s.durationMs > 0))
            sceneDetectionScore += 10;
        if (scenes.every((s) => s.purpose.length >= 10))
            sceneDetectionScore += 5;
        sceneDetectionScore = Math.min(100, sceneDetectionScore);
        let shotDetectionScore = 50;
        if (shots.length >= scenes.length)
            shotDetectionScore += 15;
        if (shots.length >= shotCountExpected * 0.5)
            shotDetectionScore += 10;
        if (shots.every((s) => s.shotType))
            shotDetectionScore += 10;
        if (shots.filter((s) => s.cameraChange).length > 0)
            shotDetectionScore += 5;
        shotDetectionScore = Math.min(100, shotDetectionScore);
        let transitionScore = 50;
        if (transitions.length >= scenes.length - 1)
            transitionScore += 20;
        if (transitions.some((t) => t.type !== TransitionType.Cut))
            transitionScore += 10;
        if (transitions.every((t) => t.fromSceneId && t.toSceneId))
            transitionScore += 10;
        ...
        if (timelineLengthMs > 0) {
            ...
            if (coverage >= 0.95 && coverage <= 1.05)
                timelineAccuracyScore += 25;
            else if (coverage >= 0.85)
                timelineAccuracyScore += 15;
        }
        timelineAccuracyScore += Math.round(frameConsistencyScore * 0.15);
        timelineAccuracyScore = Math.min(100, timelineAccuracyScore);
        const aiConfidenceScore = Math.round((sceneDetectionScore + shotDetectionScore + transitionScore + timelineAccuracyScore) / 4);
        return {
            sceneDetectionScore,
            shotDetectionScore,
            transitionScore,
    ...
    isDetectionValid(scores, sceneCount, shotCount) {
        const diagnostics = [];
        if (sceneCount < 2) {
...
//# sourceMappingURL=scene-detection-scorer.js.map