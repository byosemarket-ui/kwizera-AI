...
import { SceneClassification, ScenePriority, ShotType, TransitionType, } from "./types.js";
const ROLE_TO_CLASSIFICATION = {
...
export class SceneDetectionAnalyzer {
    ...
    detectScenes(analysis, understanding) {
    ...
    classifyByOrder(index, total, analysis) {
        if (index === 0)
            return SceneClassification.Intro;
        if (index === 1)
            return SceneClassification.Hook;
        if (index === total - 1)
            return SceneClassification.Outro;
        if (index === total - 2)
            return SceneClassification.Cta;
        ...
        if (type === VideoAnalysisType.Interview)
            return SceneClassification.Testimonial;
        if (type === VideoAnalysisType.Commercial || type === VideoAnalysisType.Advertisement) {
            if (index >= total - 3)
                return SceneClassification.BrandScene;
        }
        if (type === VideoAnalysisType.Tutorial)
            return SceneClassification.ProductDemo;
        return SceneClassification.BRoll;
    }
    assignPriority(classification, index, total) {
    ...
    inferPurpose(classification, analysis) {
    ...
    detectShots(analysis, scenes) {
        ...
        for (let i = 0; i < shotCount; i++) {
            ...
            const parentScene = scenes.find((s) => startMs >= s.startMs && endMs <= s.endMs) ??
    ...
    inferShotType(index, total, analysis) {
        if (index === 0)
    ...
    detectTransitions(scenes, shots, analysis) {
        const transitions = [];
    ...
    inferTransitionType(index, total, analysis) {
        if (index === 0)
            return TransitionType.Fade;
        if (index === total - 2)
            return TransitionType.Dissolve;
        if (analysis.visual.motionDensity > 70)
            return TransitionType.Cut;
        if (analysis.classification.videoType === VideoAnalysisType.SocialMedia)
            return TransitionType.ZoomTransition;
        if (index % 4 === 0)
            return TransitionType.Wipe;
        return TransitionType.Cut;
    }
    buildSceneRelationships(scenes, analysis, understanding) {
        return scenes.map((scene, i) => {
            const relatedSceneIds = [];
            if (scenes[i - 1])
                relatedSceneIds.push(scenes[i - 1].sceneId);
            if (scenes[i + 1])
                relatedSceneIds.push(scenes[i + 1].sceneId);
            for (const other of scenes) {
                if (other.sceneId === scene.sceneId)
                    continue;
    ...
    buildRecommendations(scenes, shots, transitions, analysis) {
        const recs = [];
        if (scenes.length < 3) {
            recs.push({
                category: "scene",
...
//# sourceMappingURL=scene-detection-analyzer.js.map