...
import { SceneClassification, ShotType, TransitionType, } from "./types.js";
export class SceneDetectionProcessor {
    ...
    async detect(input) {
        ...
        const scores = this.scorer.computeScores(detected.scenes, detected.shots, detected.transitions, analysis.technical.durationMs, analysis.timeline.sceneCount, analysis.timeline.shotCount, analysis.frame.frameConsistencyScore);
        const validation = this.scorer.isDetectionValid(scores, detected.scenes.length, detected.shots.length);
        if (!validation.valid) {
        ...
        draft.relationships = this.linker.detectRelationships(draft, this.records.getAll(), analysis, input.relatedProjects, input.relatedKnowledge, input.relatedStoryboards, input.relatedScripts);
    ...
    search(query) {
        ...
        if (query.timelineId) {
            results = results.filter((r) => r.sceneRelationships.some((sr) => sr.timelineId === query.timelineId));
        }
        ...
        if (query.text) {
            ...
            results = results.filter((r) => r.scenes.some((s) => s.purpose.toLowerCase().includes(q)) ||
...
//# sourceMappingURL=scene-detection-processor.js.map