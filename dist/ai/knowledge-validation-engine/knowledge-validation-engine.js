...
import { KnowledgeAccessPermission, KnowledgeCategory, KnowledgeModuleStatus, KnowledgeSource, KnowledgeVerificationStatus, } from "../knowledge-foundation/types.js";
...
import { KnowledgeValidationEngineError, KnowledgeValidationLevel, } from "./types.js";
/**
...
export class AiKnowledgeValidationEngine {
    ...
    logger = new KnowledgeValidationLogger();
    ...
    initialize(foundation, storageRoot) {
        ...
        this.runner = new KnowledgeValidationRunner(foundation, this.sourceValidator, this.relationshipValidator, this.consistencyValidator, this.integrityValidator, this.logger, statePath);
        this.reportGenerator = new ValidationReportGenerator(storageRoot, this.runner, this.integrityValidator, this.relationshipValidator);
    ...
    async validateKnowledge(knowledgeId) {
        ...
        if (this.activeValidations.has(knowledgeId)) {
            return (this.runner.getValidationResult(knowledgeId) ?? {
                knowledgeId,
                valid: false,
    ...
    async validateRelationships(repair = false) {
        this.ensureReady();
        return this.relationshipValidator.validateAll(repair);
    }
    async validateConsistency(repair = false) {
        this.ensureReady();
    ...
    isTrusted(knowledgeId) {
        this.ensureReady();
        return this.runner.isTrusted(knowledgeId);
    }
    getValidationLevel(knowledgeId) {
        this.ensureReady();
    ...
    async handleKnowledgeChange(knowledgeId, operation) {
        this.ensureReady();
    ...
    buildStatusReport() {
        ...
        const rejectedCount = results.filter((r) => r.validationLevel === KnowledgeValidationLevel.Rejected).length;
        return {
            engineStatus: this.startupComplete ? "operational" : "initializing",
            knowledgeValidationStatus: this.totalValidations > 0
                ? `${this.totalValidations} validation(s) completed`
                : "awaiting first validation",
            qualityStatus: results.length > 0
    ...
    async revalidateOnChange(knowledgeId, operation) {
    ...
    recordValidationTiming(durationMs) {
    ...
    ensureReady() {
        if (!this.initialized || !this.foundation) {
            throw new KnowledgeValidationEngineError("Knowledge Validation Engine not initialized", "NOT_INITIALIZED");
        }
    }
}
//# sourceMappingURL=knowledge-validation-engine.js.map