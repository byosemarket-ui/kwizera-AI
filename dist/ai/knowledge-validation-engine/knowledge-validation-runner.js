...
import { KnowledgeValidationLevel, TRUSTED_CONFIDENCE_MIN, TRUSTED_CONSISTENCY_MIN, TRUSTED_QUALITY_MIN, TRUSTED_RELIABILITY_MIN, } from "./types.js";
export class KnowledgeValidationRunner {
    ...
    logger;
    statePath;
    structureValidator = new KnowledgeStructureValidator();
    versionValidator = new KnowledgeVersionValidator();
    qualityScorer = new KnowledgeQualityScorer();
    validationState = new Map();
    constructor(foundation, sourceValidator, relationshipValidator, consistencyValidator, integrityValidator, logger, statePath) {
    ...
    async validateRecord(knowledgeId) {
        ...
        const metadataValid = Boolean(record.classification) &&
        ...
        if (relationship.issues.some((i) => i.includes("Self-referential"))) {
            ...
            await storage.updateRecord(knowledgeId, { relatedKnowledge: fixed }, "knowledge-validation-engine");
            repairs.push("Removed self-referential relationship");
        }
        const scores = this.qualityScorer.score(record, structure.warnings, relationship.issues, source.issues);
        ...
        const result = this.buildResult(knowledgeId, valid, validationLevel, scores, issues, warnings, repairs, start, {
    ...
    getValidationResult(knowledgeId) {
    ...
    resolveValidationLevel(record, scores, issues) {
        ...
        if (scores.qualityScore >= TRUSTED_QUALITY_MIN &&
    ...
    buildResult(knowledgeId, valid, validationLevel, scores, issues, warnings, repairs, start, checks) {
        const trusted = validationLevel === KnowledgeValidationLevel.Trusted;
        const verificationStatus = validationLevel === KnowledgeValidationLevel.Rejected
    ...
    persistState() {
        ...
        fs.writeFileSync(this.statePath, JSON.stringify([...this.validationState.values()], null, 2), "utf8");
    }
}
//# sourceMappingURL=knowledge-validation-runner.js.map