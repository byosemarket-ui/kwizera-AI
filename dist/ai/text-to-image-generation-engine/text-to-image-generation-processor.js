...
export class TextToImageGenerationProcessor {
    ...
    async generateImagePlan(input) {
        ...
        try {
            ...
            const productionInstructions = this.analyzer.buildProductionInstructions(profile, compositionPlan, lightingPlan);
            const recommendations = this.analyzer.buildRecommendations(promptAnalysis, compositionPlan, context);
            const scores = this.scorer.computeScores(promptAnalysis, compositionPlan, lightingPlan, stylePlan, colorPlan, platformOptimizations, variations, context);
    ...
    search(query) {
        ...
        if (query.imagePlanId)
            results = results.filter((r) => r.imagePlanId === query.imagePlanId);
        if (query.promptId)
            results = results.filter((r) => r.profile.promptId === query.promptId);
        if (query.productId)
        ...
        if (query.style)
            results = results.filter((r) => r.profile.style === query.style);
        if (query.keywords) {
            const kw = query.keywords.toLowerCase();
            results = results.filter((r) => r.promptAnalysis.subject.toLowerCase().includes(kw) ||
                r.compositionPlan.composition.toLowerCase().includes(kw) ||
                r.promptAnalysis.objects.some((o) => o.toLowerCase().includes(kw)));
        }
        if (query.text) {
            const textLower = query.text.toLowerCase();
            results = results.filter((r) => r.imagePlanId.toLowerCase().includes(textLower) ||
    ...
    applySafeRepairs(promptAnalysis, compositionPlan, lightingPlan, diagnostics) {
        const repairs = [];
...
//# sourceMappingURL=text-to-image-generation-processor.js.map