import { ImageTransformationBackgroundType, ImageToImagePlatform, ImageTransformationStyle, ImageTransformationVariationType, MaskType, PLATFORM_CONFIG, PreservationRule, SourceImageCategory, TransformationType, ALL_IMAGE_TO_IMAGE_PLATFORMS, ALL_TRANSFORMATION_TYPES, ALL_PRESERVATION_RULES, } from "./types.js";
...
export class ImageToImageGenerationAnalyzer {
    analyzeSourceImage(source, context, textToImagePlan) {
    ...
    buildProfile(input, source, platform, version, context) {
    ...
    buildTransformationPlan(input, analysis, profile, context) {
    ...
    buildPreservationPlan(input, analysis) {
    ...
    buildMaskPlan(profile, analysis) {
    ...
    buildBackgroundPlan(profile, analysis) {
        const plans = {
    ...
    buildProductionInstructions(profile, maskPlan, transformationPlan) {
        return {
    ...
    buildRecommendations(analysis, preservationPlan, context) {
        const recs = [];
    ...
    resolveSourceMetadata(input, context) {
        if (input.sourceImageMetadata)
            return input.sourceImageMetadata;
    ...
    extractContextFromProduct(productId, productName, brandName, understanding, creative, strategy, input, textToImagePlan) {
        return {
            productId,
            productName,
            brandName,
    ...
    inferTransformationTypes(input, analysis) {
        const types = [TransformationType.StyleTransfer];
    ...
    describeTransformation(type, analysis, profile, context) {
        const map = {
    ...
    getPreserveElementsForType(type, analysis) {
        if (type === TransformationType.BackgroundReplacement)
            return [analysis.subject, ...analysis.objects.slice(0, 1)];
        if (type === TransformationType.StyleTransfer)
            return [analysis.subject, "brand logo", "product shape"];
        return [analysis.subject];
    }
    preservationNote(rule, analysis) {
        const map = {
    ...
    inferCategory(industry) {
        if (industry === "fashion")
            return SourceImageCategory.Lifestyle;
        if (industry === "beauty")
            return SourceImageCategory.Product;
        if (industry === "technology")
            return SourceImageCategory.Brand;
        return SourceImageCategory.Product;
    }
...
//# sourceMappingURL=image-to-image-generation-analyzer.js.map