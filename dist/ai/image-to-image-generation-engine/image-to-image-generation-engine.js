...
import { ImageToImageGenerationEngineError, } from "./types.js";
/**
...
export class AiImageToImageGenerationEngine {
    ...
    logger = new ImageToImageGenerationLogger();
    records = new ImageToImageGenerationRecordStore();
    analyzer = new ImageToImageGenerationAnalyzer();
    scorer = new ImageToImageGenerationScorer();
    linker = new ImageToImageGenerationLinker();
    processor = null;
    generationTimes = [];
    searchTimes = [];
    analysisTimes = [];
    initialize(foundation, storageRoot) {
        ...
        this.processor = new ImageToImageGenerationProcessor(foundation, this.analyzer, this.scorer, this.linker, this.records, this.logger);
    ...
    getTransformationPlan(transformationPlanId) {
        this.ensureReady();
        return this.records.get(transformationPlanId) ?? null;
    }
    getTransformationPlansBySourceImage(sourceImageId) {
        this.ensureReady();
        return this.records.getBySourceImage(sourceImageId);
    }
    getTransformationPlansByProduct(productId) {
        this.ensureReady();
    ...
    async repairTransformationPlan(sourceImageId, platform) {
        this.ensureReady();
    ...
    buildStatusReport() {
        ...
        const avgTransformationQuality = all.length > 0
    ...
    ensureReady() {
        if (!this.initialized || !this.foundation || !this.processor) {
            throw new ImageToImageGenerationEngineError("Image-to-Image Generation Engine not initialized", "NOT_INITIALIZED");
        }
    }
}
//# sourceMappingURL=image-to-image-generation-engine.js.map