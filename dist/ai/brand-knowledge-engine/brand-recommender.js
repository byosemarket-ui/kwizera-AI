...
export class BrandRelationshipLinker {
    detectSimilar(record, allRecords) {
        ...
        for (const other of allRecords) {
            if (other.brandId === record.brandId)
                continue;
            const sameName = record.profile.brandName.toLowerCase() === other.profile.brandName.toLowerCase();
            const score = this.similarityScore(record, other);
            if (score < 35 && !sameName)
                continue;
    ...
    similarityScore(a, b) {
        let score = 0;
        if (a.profile.brandName.toLowerCase() === b.profile.brandName.toLowerCase())
            score += 40;
        if (a.profile.industry === b.profile.industry)
            score += 25;
        if (a.marketingStyle === b.marketingStyle)
            score += 15;
        if (a.profile.brandTargetAudience === b.profile.brandTargetAudience)
            score += 10;
...
//# sourceMappingURL=brand-recommender.js.map