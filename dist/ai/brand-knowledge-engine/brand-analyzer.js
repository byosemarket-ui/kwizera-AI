import { BrandMarketingStyle, KnowledgeBrandIndustry, } from "./types.js";
...
export class BrandAnalyzer {
    analyze(input) {
        ...
        const industry = input.industry ?? input.profile?.industry ?? KnowledgeBrandIndustry.Technology;
        ...
        const profile = {
            ...
            industry,
            brandMission: input.brandMission ?? input.profile?.brandMission ?? `Empower through ${brandName}`,
            brandVision: input.brandVision ??
            ...
            brandPersonality: input.brandPersonality ?? input.profile?.brandPersonality ?? defaults.personality,
            brandTone: input.brandTone ?? input.profile?.brandTone ?? defaults.tone,
            brandTargetAudience: input.brandTargetAudience ??
    ...
    evaluateConsistency(profile, visual, communication, input) {
        const inconsistencies = [];
        const logoUsage = visual.logo && visual.logoUsageRules.length >= 2 ? 90 : visual.logo ? 70 : 40;
        ...
        const voiceConsistency = communication.brandVoice && profile.brandTone ? 90 : communication.brandVoice ? 72 : 45;
        const messagingConsistency = communication.messagingStyle && profile.brandPositioning ? 88 : 60;
        const visualConsistency = visual.designLanguage && visual.visualIdentity ? 90 : visual.designLanguage ? 75 : 50;
        const marketingConsistency = communication.marketingTone === profile.brandTone ? 92 : communication.marketingTone ? 78 : 55;
        const animationConsistency = visual.motionStyle && visual.introStyle && visual.outroStyle ? 88 : 65;
        if (logoUsage < 70)
            inconsistencies.push("Logo usage rules incomplete");
        if (colorUsage < 70)
            inconsistencies.push("Brand color palette insufficient");
        if (voiceConsistency < 70)
            inconsistencies.push("Brand voice and tone misaligned");
        if (marketingConsistency < 75)
            inconsistencies.push("Marketing tone differs from brand tone");
