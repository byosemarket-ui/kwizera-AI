...
export class ProductScorer {
    computeScores(profile, visual, brand, marketing, customer) {
        ...
        const productQualityScore = Math.round((hasDescription ? 20 : 5) +
            (hasFeatures ? 25 : 8) +
            (hasSpecs ? 15 : 5) +
            (profile.materials.length >= 1 ? 10 : 0) +
            (profile.colors.length >= 1 ? 10 : 0) +
            visual.productQuality * 0.2);
        const presentationScore = Math.round(visual.productVisibility * 0.35 +
            visual.productQuality * 0.35 +
            (visual.productPlacement ? 15 : 0) +
            (visual.productBackground ? 15 : 0));
        const marketingReadinessScore = Math.round((hasCta ? 25 : 5) +
            (hasUsp ? 20 : 5) +
        ...
        const customerRelevanceScore = Math.round(customer.customerNeeds.length * 10 +
            customer.customerInterests.length * 6 +
            (customer.buyingMotivation ? 15 : 0) +
            customer.preferredPlatforms.length * 5);
        const aiConfidenceScore = Math.round((productQualityScore +
            presentationScore +
    ...
    isAnalysisValid(profile, scores) {
        const diagnostics = [];
