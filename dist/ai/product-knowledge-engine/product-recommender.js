...
export class ProductRelationshipLinker {
    detectSimilar(record, allRecords) {
        ...
        for (const other of allRecords) {
            if (other.productId === record.productId)
                continue;
            const sameBrand = record.profile.brand === other.profile.brand && record.profile.brand !== "Unknown Brand";
    ...
    similarityScore(a, b) {
        let score = 0;
        if (a.profile.brand === b.profile.brand && a.profile.brand !== "Unknown Brand")
            score += 35;
        if (a.profile.category === b.profile.category)
            score += 25;
        if (a.profile.subcategory === b.profile.subcategory)
            score += 15;
        if (a.profile.marketingGoal === b.profile.marketingGoal)
            score += 10;
        if (a.profile.targetAudience === b.profile.targetAudience)
            score += 10;
...
//# sourceMappingURL=product-recommender.js.map