import { KnowledgeProductCategory, KnowledgeProductMarketingGoal, } from "./types.js";
...
export class ProductAnalyzer {
    analyze(input) {
        const category = input.category ?? input.profile?.category ?? KnowledgeProductCategory.Electronics;
        ...
        const profile = {
            ...
            targetAudience: input.targetAudience ?? input.profile?.targetAudience ?? "general consumers",
        ...
        const visual = {
            productAppearance: input.visual?.productAppearance ?? defaults.visual.productShape ?? "clean-modern",
            ...
            productPackaging: input.visual?.productPackaging ?? defaults.visual.productPackaging ?? "retail-box",
        ...
        const marketing = {
            productBenefits: input.marketing?.productBenefits ??
                profile.features.map((f) => `Benefit: ${f}`),
            customerProblems: input.marketing?.customerProblems ?? [
                "needs better quality",
                "wants reliable solution",
            ],
            productSolutions: input.marketing?.productSolutions ?? [
                `${productName} delivers premium ${profile.subcategory} experience`,
            ],
            uniqueSellingPoints: input.marketing?.uniqueSellingPoints ?? profile.features.slice(0, 3),
            emotionalAppeal: input.marketing?.emotionalAppeal ?? "confidence and satisfaction",
            ...
            productPositioning: input.marketing?.productPositioning ??
                `Premium ${profile.subcategory} for ${profile.targetAudience}`,
        };
        const customer = {
            customerNeeds: input.customer?.customerNeeds ?? ["quality", "value", "reliability"],
            customerInterests: input.customer?.customerInterests ?? [profile.category, profile.subcategory, brand],
            buyingMotivation: input.customer?.buyingMotivation ?? "solve a specific need with trusted product",
            preferredPresentation: input.customer?.preferredPresentation ?? "hero-product-on-clean-background",
            preferredMarketingStyle: input.customer?.preferredMarketingStyle ?? "benefit-driven-visual",
            preferredPlatforms: input.customer?.preferredPlatforms ?? ["instagram", "website", "youtube"],
        };
        return {
            profile,
            visual,
...
//# sourceMappingURL=product-analyzer.js.map