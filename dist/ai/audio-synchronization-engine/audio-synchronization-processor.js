...
export class AudioSynchronizationProcessor {
    ...
    async generateAudioSyncPlans(input) {
        ...
        try {
            ...
            for (const { scene, cameraPlan, motionPlan, animationPlan, vfxPlan } of bundles) {
                ...
                const draftBase = this.analyzer.buildAudioSyncPlan(scene, cameraPlan, motionPlan, animationPlan, vfxPlan, version, {
                ...
                const draft = {
                    ...draftBase,
                    scores,
                    relationships: this.linker.detectRelationships({ ...draftBase, scores }, scene, motionPlan, cameraPlan, animationPlan, vfxPlan, input),
    ...
    search(query) {
        ...
        if (query.keywords) {
            ...
            results = results.filter((r) => r.voiceSynchronization.voiceTiming.toLowerCase().includes(kw) ||
        ...
        if (query.text) {
            ...
            results = results.filter((r) => r.audioSynchronizationId.toLowerCase().includes(textLower) ||
...
//# sourceMappingURL=audio-synchronization-processor.js.map