...
import { StoryboardGenerationEngineError, } from "./types.js";
/**
...
export class AiStoryboardGenerationEngine {
    ...
    logger = new StoryGenerationLogger();
    records = new StoryGenerationRecordStore();
    analyzer = new StoryGenerationAnalyzer();
    scorer = new StoryGenerationScorer();
    linker = new StoryGenerationLinker();
    processor = null;
    generationTimes = [];
    searchTimes = [];
    scenePlanningTimes = [];
    initialize(foundation, storageRoot) {
        ...
        this.processor = new StoryGenerationProcessor(foundation, this.analyzer, this.scorer, this.linker, this.records, this.logger);
    ...
    getStoryboardsByProject(projectId) {
        this.ensureReady();
    ...
    buildStatusReport() {
        ...
        const avgStoryQuality = all.length > 0
    ...
    ensureReady() {
        if (!this.initialized || !this.foundation || !this.processor) {
            throw new StoryboardGenerationEngineError("Storyboard Generation Engine not initialized", "NOT_INITIALIZED");
        }
    }
}
//# sourceMappingURL=story-generation-engine.js.map