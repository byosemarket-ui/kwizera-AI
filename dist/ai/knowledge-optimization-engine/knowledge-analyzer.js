...
export class KnowledgeAnalyzer {
    ...
    async analyze() {
        ...
        for (const entry of entries) {
            ...
            if (read.success && read.record) {
                ...
                if (!record.summary || record.summary.length < INCOMPLETE_SUMMARY_MIN)
                    incompleteRecords++;
                if (record.qualityScore < LOW_QUALITY_THRESHOLD)
                    lowQualityRecords++;
            }
        }
        ...
        const classificationQualityScore = Math.max(0, 100 - Math.round((incompleteRecords / Math.max(entries.length, 1)) * 100));
        ...
        const fragmentationScore = entries.length > 0 ? Math.min(100, Math.round((duplicateGroups / entries.length) * 100)) : 0;
        const report = {
    ...
    computeAverageQuality() {
        ...
        const entries = storage.getIndexEntries();
        if (entries.length === 0)
            return { qualityScore: 100, completeness: 100 };
        let qualitySum = 0;
...
//# sourceMappingURL=knowledge-analyzer.js.map