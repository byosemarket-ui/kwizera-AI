import { ImageQualityPredictionPlatform, } from "./types.js";
export class ImageQualityPredictionAnalyzer {
    ...
    buildPredictions(ctx, checks, risks) {
        ...
        const productionSuccessProbability = Math.max(0, Math.min(100, Math.round(ctx.productionPlan.scores.productionReadinessScore * 0.4 +
            checkRatio * 100 * 0.3 +
        ...
        const opportunities = [];
        if (!checks.lightingConsistency)
            opportunities.push("Improve lighting consistency before production");
        if (!checks.colorConsistency)
            opportunities.push("Harmonize color palette with brand guidelines");
        if (!checks.compositionConsistency)
            opportunities.push("Refine composition balance and hierarchy");
        if (!checks.typographyReadiness)
            opportunities.push("Strengthen typography readiness for overlays");
        ...
        return {
            ...
            marketingImpact: Math.round((ctx.understanding.scores.marketingReadinessScore +
                ctx.creativePlan.scores.marketingScore +
                ctx.brandVisual.scores.marketingReadinessScore) /
                3),
            viewerAttentionPotential: Math.round((ctx.composition.visualHierarchy.mainSubjectVisibility +
                ctx.composition.visualHierarchy.ctaVisibility +
                ctx.creativePlan.scores.visualImpactScore) /
                3),
            readability: Math.round((ctx.creativePlan.scores.readabilityScore +
            ...
            productionComplexity: Math.min(100, Math.round(30 + criticalCount * 20 + highCount * 10 + risks.length * 3)),
...
//# sourceMappingURL=image-quality-prediction-analyzer.js.map