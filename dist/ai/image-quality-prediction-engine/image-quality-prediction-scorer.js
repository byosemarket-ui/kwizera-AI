export class ImageQualityPredictionScorer {
    computeScores(ctx, checks) {
        const { analysis, composition, lightingColor, brandVisual, enhancementPlan, creativePlan, productionPlan } = ctx;
        const technicalQualityScore = Math.round((analysis.scores.technicalQualityScore +
            analysis.scores.visualQualityScore +
            analysis.visual.sharpness +
            (100 - analysis.visual.noiseLevel)) /
            4);
        const compositionScore = Math.round((composition.compositionAnalysis.balance +
        ...
        const marketingEffectivenessScore = Math.round((ctx.understanding.scores.marketingReadinessScore +
        ...
        const overallImageQualityScore = Math.round((technicalQualityScore +
            compositionScore +
            lightingScore +
            colorScore +
            ...
            6);
        const aiConfidenceScore = Math.round((overallImageQualityScore +
...
//# sourceMappingURL=image-quality-prediction-scorer.js.map