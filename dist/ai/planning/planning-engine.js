...
import { PlanningEngineError, PlanningStatus, PlanningStep, } from "./types.js";
/**
...
export class AiPlanningEngine {
    logger = new PlanningLogger();
    history = new PlanningHistoryStore();
    taskBreakdown = new TaskBreakdown();
    dependencyAnalyzer = new DependencyAnalyzer();
    resourceEstimator = new ResourceEstimator();
    riskAnalyzer = new PlanRiskAnalyzer();
    ...
    async planFromDecision(input) {
        ...
        try {
            ...
            const moduleIds = getRequiredModules(input.planningType, input.workflowHandoff.requiredModules);
            const registryModuleIds = new Set(this.core.registry.getAllEntries().map((e) => e.id));
            ...
            for (const field of criticalFields) {
                if (input.availableData[field] === undefined ||
            ...
            const { resources, time } = this.resourceEstimator.estimate(input.planningType, tasks, moduleIds);
            ...
            const recoveryStrategy = this.recoveryPlanner.create(tasks, input.workflowHandoff.workflowId);
            ...
            this.logger.log(validation.passed ? "info" : "warn", "validation", validation.passed ? "Execution plan validated" : "Plan validation failed", { planId, checks: validation.checks });
    ...
    buildStatusReport() {
        ...
        const averagePlanningMs = total > 0
        ...
        const planningQuality = this.history.getCount() > 0
    ...
    buildExpectedOutput(input) {
        return `${input.planningType} deliverable for: ${input.objective.slice(0, 100)}`;
    }
    buildIncompleteResult(params) {
        const riskAnalysis = params.riskAnalysis ??
