...
export class ObjectDetectionProcessor {
    ...
    async detect(input) {
        ...
        const scores = this.scorer.computeScores(built.objects, built.productDetection, built.textDetection, built.logoDetection);
        ...
        draft.relationships = this.linker.detectRelationships(draft, this.records.getAll(), analysis, understanding, input.relatedProjects, input.relatedKnowledge);
    ...
    search(query) {
        ...
        if (query.product) {
            ...
            results = results.filter((r) => r.productDetection.mainProduct?.toLowerCase().includes(q) ||
        ...
        if (query.category) {
            ...
            results = results.filter((r) => r.objects.some((o) => o.objectName.toLowerCase().includes(q) || o.objectType.includes(q)));
        }
        if (query.project) {
            results = results.filter((r) => r.relationships.relatedProjects.includes(query.project));
        }
        if (query.campaign) {
            results = results.filter((r) => r.relationships.relatedMarketingCampaigns.some((c) => c.includes(query.campaign)));
        }
...
//# sourceMappingURL=object-detection-processor.js.map