...
import { DetectedObjectType, ObjectOrientation, ObjectPosition, } from "./types.js";
export class ObjectDetectionAnalyzer {
    buildFromIntelligence(analysis, understanding) {
        const objects = [];
        let index = 0;
        ...
        if (analysis.content.background) {
            objects.push(this.createObject(analysis.imageId, ++index, DetectedObjectType.BackgroundObject, analysis.content.background, { x: 0, y: 0, width: 100, height: 100 }, 90, ObjectPosition.FullFrame));
        }
        if (objects.length === 0 && analysis.content.foreground) {
            objects.push(this.createObject(analysis.imageId, ++index, DetectedObjectType.Product, analysis.content.foreground, { x: 25, y: 20, width: 50, height: 60 }, 82, ObjectPosition.Center));
        }
    ...
    createObject(imageId, index, type, name, region, confidence, position, orientation = ObjectOrientation.Unknown) {
        return {
    ...
    inferObjectType(name) {
        ...
        if (lower.includes("person") || lower.includes("model"))
            return DetectedObjectType.Person;
        if (lower.includes("car") || lower.includes("vehicle"))
            return DetectedObjectType.Vehicle;
        if (lower.includes("food"))
            return DetectedObjectType.Food;
        if (lower.includes("chair") || lower.includes("furniture"))
            return DetectedObjectType.Furniture;
        if (lower.includes("phone") || lower.includes("device"))
            return DetectedObjectType.Electronics;
        if (lower.includes("icon"))
            return DetectedObjectType.Icon;
        if (lower.includes("animal"))
            return DetectedObjectType.Animal;
        if (lower.includes("building"))
            return DetectedObjectType.Building;
        if (lower.includes("cloth") || lower.includes("jacket"))
            return DetectedObjectType.Clothing;
        return DetectedObjectType.BackgroundObject;
    }
    linkObjectRelationships(objects) {
    ...
    buildRecommendations(objects, product, logo, text) {
        const recs = [];
...
//# sourceMappingURL=object-detection-analyzer.js.map