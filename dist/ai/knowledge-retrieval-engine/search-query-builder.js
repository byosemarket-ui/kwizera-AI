...
export class KnowledgeSearchQueryBuilder {
    filterCandidates(entries, query) {
        ...
        if (query.text) {
            ...
            if (mode === KnowledgeSearchMode.Exact) {
                candidates = candidates.filter((e) => e.title.toLowerCase() === text || e.knowledgeId === text);
            }
        ...
        if (mode === KnowledgeSearchMode.Recommendation) {
            candidates.sort((a, b) => new Date(b.lastUpdated).getTime() - new Date(a.lastUpdated).getTime());
        }
        this.applyTypeFilter(candidates, query);
        return candidates;
    }
    computeSemanticScore(query, searchableText) {
    ...
    applyContextFilter(candidates, context, mode) {
    ...
    applyTypeFilter(candidates, query) {
        ...
        for (const [field, type] of Object.entries(typeMap)) {
            ...
            if (typeof value === "string" && value.length > 0) {
                const filtered = candidates.filter((e) => e.knowledgeType === type && e.searchableText.includes(value.toLowerCase()));
