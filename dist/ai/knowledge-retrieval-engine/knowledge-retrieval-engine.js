...
import { KnowledgeRetrievalEngineError, KnowledgeSearchMode, } from "./types.js";
const EMPTY_GROUPS = {
    relatedKnowledge: [],
    relatedMemory: [],
    relatedProjects: [],
...
export class AiKnowledgeRetrievalEngine {
    ...
    logger = new KnowledgeRetrievalLogger();
    queryBuilder = new KnowledgeSearchQueryBuilder();
    usageTracker = new KnowledgeUsageTracker(this.logger);
    cache = new KnowledgeRetrievalCache(this.logger);
    ranker = null;
    relatedFinder = null;
    validator = null;
    searchTimes = [];
    ...
    async search(query) {
        ...
        for (const rankedResult of ranked) {
            ...
            if (hydrated) {
                ...
                if (hydrated.record)
                    fromCache = fromCache || this.wasFromCache(rankedResult.knowledgeId);
            }
            else {
    ...
    async retrieve(knowledgeId, requesterId = "knowledge-retrieval-engine") {
        this.ensureReady();
        ...
        }, allEntries, [knowledgeId, ...relatedKnowledge.map((r) => r.knowledgeId)], 5);
    ...
    async filterByScores(candidates, query, requesterId) {
        const filtered = [];
        for (const entry of candidates) {
            const read = await this.storageEngine.getRecord(entry.knowledgeId, requesterId);
            if (!read.record)
                continue;
            if (query.minQualityScore !== undefined && read.record.qualityScore < query.minQualityScore)
                continue;
    ...
    async hydrateResult(ranked, requesterId) {
        ...
        if (record) {
            ...
            enrichedRanking.compositeScore = Math.min(100, Math.round(enrichedRanking.compositeScore * 0.6 +
    ...
    wasFromCache(knowledgeId) {
        const indexEntry = this.storageEngine.findIndexEntry(knowledgeId);
        return Boolean(indexEntry && this.cache.get(knowledgeId, indexEntry));
    }
    ensureReady() {
        if (!this.initialized || !this.storageEngine || !this.foundation) {
            throw new KnowledgeRetrievalEngineError("Knowledge Retrieval Engine not initialized", "NOT_INITIALIZED");
        }
    }
}
//# sourceMappingURL=knowledge-retrieval-engine.js.map