...
function plannedAsset(assetId, assetType, description, source, required = true) {
    return { assetId, assetType, description, source, status: "planned", required };
}
export class ProductionPlanningAnalyzer {
    buildProfile(input, storyboard, scriptPlan, visualPlan, audioPlan, version) {
    ...
    buildWorkflow(storyboard, scriptPlan, visualPlan, audioPlan) {
        return {
    ...
    buildAssetManagement(storyboard, scriptPlan, visualPlan, audioPlan, creative) {
        ...
        return {
            ...
            icons: visualPlan.scenePlans.filter((s) => s.iconPlacement !== "none").map((s, i) => plannedAsset(`icon-scene-${s.sceneNumber}`, "icon", s.iconPlacement, visualPlan.visualPlanId)),
            videos: Array.from({ length: sceneCount }, (_, i) => plannedAsset(`video-scene-${i + 1}`, "video", `Scene ${i + 1} video segment`, storyboard.storyboardId)),
            ...
            voiceOver: scriptPlan.scenePlans.map((s) => plannedAsset(`vo-scene-${s.sceneNumber}`, "voice-over", s.plannedNarration, scriptPlan.scriptPlanId)),
            backgrounds: visualPlan.scenePlans.map((s) => plannedAsset(`bg-scene-${s.sceneNumber}`, "background", s.backgroundStyle, visualPlan.visualPlanId)),
            ...
            animations: visualPlan.scenePlans.map((s) => plannedAsset(`anim-scene-${s.sceneNumber}`, "animation", s.motionDirection, visualPlan.visualPlanId)),
            effects: visualPlan.scenePlans.map((s) => plannedAsset(`fx-scene-${s.sceneNumber}`, "effect", s.visualEffectsPlan, visualPlan.visualPlanId)),
            subtitles: scriptPlan.scenePlans.map((s) => plannedAsset(`sub-scene-${s.sceneNumber}`, "subtitle", s.plannedSubtitle, scriptPlan.scriptPlanId)),
            captions: scriptPlan.scenePlans.filter((s) => s.plannedOnScreenText.length > 5).map((s) => plannedAsset(`caption-scene-${s.sceneNumber}`, "caption", s.plannedOnScreenText, scriptPlan.scriptPlanId)),
        };
    }
    buildDependencyValidation(foundation, storyboard, scriptPlan, visualPlan, audioPlan, creative, strategy, understanding) {
        const issues = [];
        ...
        const knowledgeOk = storyboard.relationships.knowledgeRecords.length > 0 ||
            scriptPlan.relationships.knowledgeRecords.length > 0;
        if (!storyboardOk)
            issues.push("Storyboard not production-ready");
        if (!scriptOk)
            issues.push("Script plan not production-ready");
        if (!visualOk)
            issues.push("Visual plan not production-ready");
        if (!audioOk)
            issues.push("Audio plan not production-ready");
        if (!creativeOk)
            issues.push("Creative direction not validated");
        if (!strategyOk)
            issues.push("Marketing strategy not validated");
        if (!productIntelOk)
            issues.push("Product intelligence not validated");
        if (issues.length > 0) {
    ...
    buildRenderPreparation(storyboard) {
    ...
    buildExportPreparation(storyboard) {
    ...
    buildRecoveryPlan(productionPlanId) {
        return {
    ...
    buildSceneProductionPlans(storyboard, scriptPlan, visualPlan, audioPlan) {
    ...
    validateAlignment(sceneProductionPlans, storyboard, scriptPlan, visualPlan, audioPlan) {
        const issues = [];
        ...
        if (scriptPlan.storyboardId !== storyboard.storyboardId)
            issues.push("Script plan storyboard mismatch");
        if (visualPlan.storyboardId !== storyboard.storyboardId)
            issues.push("Visual plan storyboard mismatch");
        if (audioPlan.storyboardId !== storyboard.storyboardId)
            issues.push("Audio plan storyboard mismatch");
        if (visualPlan.scriptPlanId !== scriptPlan.scriptPlanId)
            issues.push("Visual/script plan mismatch");
        if (audioPlan.scriptPlanId !== scriptPlan.scriptPlanId)
            issues.push("Audio/script plan mismatch");
        if (audioPlan.visualPlanId !== visualPlan.visualPlanId)
            issues.push("Audio/visual plan mismatch");
        return { aligned: issues.length === 0, issues };
    }
    getAllRequiredAssets(assets) {
        return [
            ...assets.images,
            ...assets.logos,
            ...assets.fonts,
...
//# sourceMappingURL=production-planning-analyzer.js.map