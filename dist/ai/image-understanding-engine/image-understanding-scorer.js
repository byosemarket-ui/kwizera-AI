export class ImageUnderstandingScorer {
    computeScores(purpose, scene, visual, product, brand, marketing) {
        let imageUnderstandingScore = 58;
        if (purpose.primaryPurpose.length >= 20)
            imageUnderstandingScore += 12;
        if (scene.preparedScenes.length >= 2)
            imageUnderstandingScore += 8;
        if (visual.mainSubject)
            imageUnderstandingScore += 10;
        if (visual.visualHierarchy)
            imageUnderstandingScore += 5;
        imageUnderstandingScore = Math.min(100, imageUnderstandingScore);
        let productUnderstandingScore = 40;
        if (product.productReadiness)
            productUnderstandingScore += 30;
        ...
        if (marketing.promotionalPurpose.length >= 15)
            marketingReadinessScore += 15;
        if (marketing.storytellingOpportunity.length >= 15)
            marketingReadinessScore += 15;
        if (marketing.ctaOpportunity.length >= 10)
            marketingReadinessScore += 10;
        ...
        if (visual.composition.length >= 10)
            creativeReadinessScore += 15;
        if (scene.sceneDescription.length >= 20)
            creativeReadinessScore += 15;
        if (purpose.creativeIntent.length >= 15)
            creativeReadinessScore += 10;
        creativeReadinessScore = Math.min(100, creativeReadinessScore);
        const aiConfidenceScore = Math.round((imageUnderstandingScore +
...
//# sourceMappingURL=image-understanding-scorer.js.map