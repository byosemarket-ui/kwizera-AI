...
import { ImageUnderstandingEngineError, ImageUnderstandingMarketingGoal, } from "./types.js";
/**
...
export class AiImageUnderstandingEngine {
    ...
    logger = new ImageUnderstandingLogger();
    records = new ImageUnderstandingRecordStore();
    analyzer = new ImageUnderstandingAnalyzer();
    scorer = new ImageUnderstandingScorer();
    linker = new ImageUnderstandingLinker();
    processor = null;
    understandingTimes = [];
    searchTimes = [];
    relationshipTimes = [];
    initialize(foundation, storageRoot) {
        ...
        this.processor = new ImageUnderstandingProcessor(foundation, this.analyzer, this.scorer, this.linker, this.records, this.logger);
    ...
    detectRelationships(imageId) {
        ...
        const updated = this.linker.detectRelationships(record, this.records.getAll(), analysis, record.relationships.relatedProjects, record.relationships.relatedKnowledge, record.relationships.relatedStoryboards);
        this.relationshipTimes.push(Date.now() - start);
        return updated;
    }
    async repairUnderstanding(imageId) {
        this.ensureReady();
    ...
    ensureReady() {
        if (!this.initialized || !this.foundation || !this.processor) {
            throw new ImageUnderstandingEngineError("Image Understanding Engine not initialized", "NOT_INITIALIZED");
        }
    }
}
//# sourceMappingURL=image-understanding-engine.js.map