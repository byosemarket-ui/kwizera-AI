...
import { VideoSceneRole, VideoStoryType, VideoUnderstandingMarketingGoal, } from "./types.js";
const STORY_BY_TYPE = {
...
export class VideoUnderstandingAnalyzer {
    buildFromAnalysis(analysis, marketingGoal = VideoUnderstandingMarketingGoal.Conversion, storyType = VideoStoryType.Other, industry = "general") {
    ...
    inferPrimaryPurpose(analysis, product, brand, goal) {
    ...
    assignSceneRole(index, total, analysis) {
        if (index === 0)
            return VideoSceneRole.Opening;
        if (index === 1)
            return VideoSceneRole.Hook;
        if (index === total - 1)
            return VideoSceneRole.Ending;
        if (index === total - 2)
            return VideoSceneRole.Cta;
        const type = analysis.classification.videoType;
        if (type === VideoAnalysisType.ProductShowcase &&
    ...
    buildSceneRelationships(scenes) {
    ...
    buildStoryUnderstanding(analysis, storyType, goal, product, brand) {
        return {
            storyType,
    ...
    inferNarrativeStructure(analysis, storyType) {
        if (storyType === VideoStoryType.ProblemSolution)
            return "problem-agitate-solution";
        if (storyType === VideoStoryType.Tutorial)
            return "intro-demonstration-practice-summary";
        if (storyType === VideoStoryType.ProductDemo)
            return "context-demo-benefits-cta";
    ...
    buildBrandUnderstanding(analysis, brandName) {
        const logoPresence = analysis.relationships.relatedBrands.length > 0 &&
    ...
    buildAudienceUnderstanding(analysis, industry, goal, storyType) {
        ...
        return {
            ...
            viewerRetentionOpportunity: analysis.technical.durationMs <= 60_000
    ...
    inferAudience(industry, videoType) {
        if (industry === "technology")
            return "creative professionals and marketing teams";
        if (industry === "education")
            return "learners and training audiences";
        if (videoType === VideoAnalysisType.SocialMedia)
            return "social media consumers";
        if (videoType === VideoAnalysisType.Corporate)
            return "business stakeholders";
        return "general audience";
    }
    buildMarketingUnderstanding(analysis, goal, product, campaign) {
        let strength = 55;
        if (analysis.productionReadiness.marketingReadiness >= 70)
            strength += 15;
        if (product)
            strength += 10;
        if (campaign)
            strength += 10;
        if (analysis.audio.overallAudioQualityScore >= 75)
            strength += 5;
        ...
        return {
            ...
            ctaOpportunity: analysis.classification.videoType === VideoAnalysisType.Commercial ||
    ...
    buildStructure(analysis, scenes) {
        const durationMs = analysis.technical.durationMs;
    ...
    buildRecommendations(analysis, story, product, brand, marketing, audience) {
        const recs = [];
...
//# sourceMappingURL=video-understanding-analyzer.js.map