...
export class CompositionProcessor {
    ...
    async analyze(input) {
        ...
        const built = this.analyzer.buildFromIntelligence(analysis, understanding, detection, background, input.industry);
        const scores = this.scorer.computeScores(built.compositionAnalysis, built.visualHierarchy, built.productPlacement, built.suitability);
        ...
        draft.relationships = this.linker.detectRelationships(draft, this.records.getAll(), analysis, understanding, detection, background, input.relatedProjects, input.relatedKnowledge);
...
//# sourceMappingURL=composition-processor.js.map