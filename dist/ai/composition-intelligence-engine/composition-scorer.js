export class CompositionScorer {
    computeScores(composition, hierarchy, placement, suitability) {
        const compositionQualityScore = Math.round((composition.balance + composition.symmetry + placement.productVisibility) / 3);
        const visualBalanceScore = Math.min(100, composition.balance);
        const visualHierarchyScore = Math.round((hierarchy.mainSubjectVisibility +
        ...
        const marketingReadinessScore = Math.round(suitabilityValues.reduce((a, b) => a + b, 0) / suitabilityValues.length);
        let creativeReadinessScore = 55;
        if (composition.ruleOfThirds || composition.centerComposition)
            creativeReadinessScore += 12;
        if (hierarchy.mainSubjectVisibility >= 70)
            creativeReadinessScore += 10;
        if (composition.negativeSpace >= 40)
            creativeReadinessScore += 8;
        if (placement.productEmphasis)
            creativeReadinessScore += 10;
        creativeReadinessScore = Math.min(100, creativeReadinessScore);
        const aiConfidenceScore = Math.round((compositionQualityScore +
            visualBalanceScore +
    ...
    isAnalysisValid(scores, composition) {
        const diagnostics = [];
...
//# sourceMappingURL=composition-scorer.js.map