...
import { CreativePlatform, CreativeDirectionStyle, } from "./types.js";
...
export class CreativeDirectionAnalyzer {
    buildProfile(input, strategy, audience, understanding, analysis) {
    ...
    buildVisualDirection(profile, understanding, analysis, strategy) {
        const industry = analysis.classification.industry;
    ...
    buildCinematicDirection(profile, strategy, understanding) {
    ...
    buildBrandDirection(profile, understanding, analysis) {
    ...
    buildMarketingDirection(profile, strategy, understanding, audience) {
    ...
    buildPlatformDirections(profile, strategy, understanding) {
    ...
    platformsFromStrategy(strategy) {
        const result = [];
        for (const p of strategy.audienceAlignment.preferredPlatforms) {
            if (p === "tiktok")
                result.push(CreativePlatform.TikTok);
            else if (p === "instagram")
                result.push(CreativePlatform.InstagramReels);
            else if (p === "facebook")
                result.push(CreativePlatform.Facebook);
            else if (p === "youtube")
                result.push(CreativePlatform.YouTubeShorts, CreativePlatform.YouTube);
            else if (p === "whatsapp")
                result.push(CreativePlatform.WhatsAppStatus);
            else if (p === "website")
                result.push(CreativePlatform.Website);
        }
        return [...new Set(result)];
    }
    buildEmotionalDirection(style, audience, understanding) {
...
//# sourceMappingURL=creative-direction-analyzer.js.map