...
export class CreativeDirectionProcessor {
    ...
    async plan(input) {
        ...
        if (!audience?.validated) {
            ...
            if (!audienceResult.success || !audienceResult.record) {
                return this.reject(start, audienceResult.message ?? "Complete audience intelligence required before creative direction", audienceResult.diagnostics.length > 0
        ...
        let strategy = input.strategyId
            ...
            : strategyEngine.getStrategiesByProduct(input.productId).find((s) => s.marketingObjective === (input.campaignGoal ?? s.marketingObjective)) ?? strategyEngine.getStrategiesByProduct(input.productId)[0];
        if (!strategy?.validated) {
            ...
            if (!strategyResult.success || !strategyResult.record) {
                return this.reject(start, strategyResult.message ?? "Complete marketing strategy required before creative direction", strategyResult.diagnostics.length > 0
                    ? strategyResult.diagnostics
                    : ["Marketing strategy must be prepared and validated"]);
            }
            strategy = strategyResult.record;
        }
        const profile = this.analyzer.buildProfile(input, strategy, audience, understanding, analysis);
        const visualDirection = this.analyzer.buildVisualDirection(profile, understanding, analysis, strategy);
        const cinematicDirection = this.analyzer.buildCinematicDirection(profile, strategy, understanding);
        const brandDirection = this.analyzer.buildBrandDirection(profile, understanding, analysis);
        const marketingDirection = this.analyzer.buildMarketingDirection(profile, strategy, understanding, audience);
        const platformDirections = this.analyzer.buildPlatformDirections(profile, strategy, understanding);
        const scores = this.scorer.computeScores(profile, visualDirection, brandDirection, marketingDirection, platformDirections, strategy, audience);
        const validation = this.scorer.isCreativeDirectionValid(scores, profile, brandDirection, marketingDirection, platformDirections);
        if (!validation.valid) {
        ...
        draft.relationships = this.linker.detectRelationships(draft, this.records.getAll(), understanding, analysis, strategy, audience);
    ...
    search(query) {
        ...
        if (query.industry) {
            ...
            results = results.filter((r) => r.visualDirection.designStyle.toLowerCase().includes(industryLower) ||
        ...
        if (query.text) {
            ...
            results = results.filter((r) => r.creativeId.toLowerCase().includes(textLower) ||
...
//# sourceMappingURL=creative-direction-processor.js.map