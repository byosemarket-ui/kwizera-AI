...
export class VideoProcessor {
    ...
    async analyze(input) {
        ...
        const scores = this.scorer.computeScores(analysis.structure, analysis.editing, analysis.audio, analysis.marketing, analysis.visual);
        ...
        for (const relatedVideoId of draft.relationships.similarVideos) {
            ...
            if (!related)
                continue;
            this.ensureGraphNode(graph, related.knowledgeId, related.videoName, this.buildKnowledgeDescription(related));
    ...
    filterLocal(records, query) {
        return records.filter((r) => {
            if (query.videoType && r.videoType !== query.videoType)
                return false;
            ...
            if (query.brand && !r.brandName.toLowerCase().includes(query.brand.toLowerCase()))
                return false;
            if (query.editingStyle && r.editing.editingStyle !== query.editingStyle)
                return false;
            if (query.cameraStyle && !r.camera.primaryShots.includes(query.cameraStyle))
                return false;
            if (query.music && r.audio.backgroundMusic !== query.music)
                return false;
            if (query.language && r.language !== query.language)
                return false;
            if (query.marketingGoal && r.marketing.marketingGoal !== query.marketingGoal)
                return false;
            if (query.minStorytelling && r.scores.storytellingScore < query.minStorytelling)
                return false;
            if (query.sceneType && !r.structure.sceneSequence.some((s) => s.scenePurpose === query.sceneType)) {
                return false;
            }
            if (query.transition && !r.editing.transitionTechniques.includes(query.transition))
                return false;
            return true;
        });
    }
