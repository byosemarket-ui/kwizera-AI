...
export class ScriptPlanningProcessor {
    ...
    async createScriptPlan(input) {
        ...
        if (!storyboard?.validated || !storyboard.productionReady) {
            ...
            if (!storyboardResult.success || !storyboardResult.record) {
                return this.reject(start, storyboardResult.message ?? "Production-ready storyboard required before script planning", storyboardResult.diagnostics.length > 0
        ...
        const scores = this.scorer.computeScores(scenePlans, scriptStructure, voicePreparation, subtitlePreparation, storyboard, creative, strategy);
        ...
        if (!validation.valid) {
            ...
            if (repaired.length > 0) {
                scenePlans = repaired;
                subtitlePreparation = this.analyzer.buildSubtitlePreparation(scenePlans, storyboard);
                const repairedScores = this.scorer.computeScores(scenePlans, scriptStructure, voicePreparation, subtitlePreparation, storyboard, creative, strategy);
                validation = this.scorer.isScriptPlanValid(repairedScores, scenePlans, storyboard, this.analyzer.validateSceneAlignment(scenePlans, storyboard).issues);
                if (validation.valid) {
        ...
        draft.relationships = this.linker.detectRelationships(draft, this.records.getAll(), storyboard, creative, strategy, understanding);
    ...
    search(query) {
        ...
        if (query.text) {
            ...
            results = results.filter((r) => r.scriptPlanId.toLowerCase().includes(textLower) ||
                r.scriptStructure.hook.toLowerCase().includes(textLower));
        }
        return results.slice(0, query.limit ?? 50);
    }
    applySceneRepairs(_scenePlans, storyboard, understanding, creative, language) {
        return this.analyzer.buildScenePlans(storyboard, understanding, creative, language);
    }
    applyScoreRepairs(scenePlans, scores) {
        if (scores.readabilityScore >= 50 && scores.scriptPlanningScore >= 55) {
            return [];
        }
        return scenePlans.map((plan) => ({
            ...plan,
            plannedNarration: plan.plannedNarration.length >= 20
...
//# sourceMappingURL=script-planning-processor.js.map