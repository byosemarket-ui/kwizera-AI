...
export class BrandVisualProcessor {
    ...
    async analyze(input) {
        ...
        const built = this.analyzer.buildFromIntelligence(analysis, understanding, detection, lightingColor, input.brandName, input.industry, input.visualStyle);
    ...
    search(query) {
        ...
        if (query.color) {
            ...
            results = results.filter((r) => r.colorAnalysis.primaryBrandColors.some((c) => c.toLowerCase().includes(q)) ||
                r.profile.primaryColors.some((c) => c.toLowerCase().includes(q)));
        }
        if (query.typography) {
            const q = query.typography.toLowerCase();
            results = results.filter((r) => r.typography.primaryFont.toLowerCase().includes(q) ||
...
//# sourceMappingURL=brand-visual-processor.js.map