...
export class QualityPredictionAnalyzer {
    buildProfile(input, productionPlan, version) {
    ...
    buildAnalysisSummary(understanding, audience, strategy, creative, storyboard, scriptPlan, visualPlan, audioPlan, productionPlan) {
        return {
    ...
    computeScores(understanding, audience, strategy, creative, storyboard, scriptPlan, visualPlan, audioPlan, productionPlan, checks) {
        const visualQualityScore = Math.round((visualPlan.scores.visualPlanningScore + visualPlan.scores.compositionScore + visualPlan.scores.brandConsistencyScore) / 3);
        const storytellingScore = Math.round((storyboard.scores.storytellingScore + scriptPlan.scores.storytellingScore + scriptPlan.scores.readabilityScore) / 3);
        const marketingEffectivenessScore = Math.round((strategy.scores.marketingReadinessScore + scriptPlan.scores.marketingScore + creative.scores.marketingAlignmentScore) / 3);
        const brandConsistencyScore = Math.round((creative.scores.brandConsistencyScore +
        ...
        const technicalReadinessScore = Math.round((productionPlan.scores.assetReadinessScore + productionPlan.scores.dependencyScore + audioPlan.scores.synchronizationScore) / 3);
        const productionReadinessScore = productionPlan.scores.productionReadinessScore;
        const checkBonus = checks.issues.length === 0 ? 5 : 0;
        const overallQualityScore = Math.min(100, Math.round((visualQualityScore +
            storytellingScore +
            ...
            7 +
            checkBonus));
        const aiConfidenceScore = Math.round((overallQualityScore +
    ...
    runQualityChecks(storyboard, scriptPlan, visualPlan, audioPlan, productionPlan, creative) {
        const issues = [];
        const brandConsistency = visualPlan.brandConsistency.brandColors &&
            visualPlan.brandConsistency.logoPlacement &&
            creative.validated;
        const storyConsistency = storyboard.scenes.length === scriptPlan.scenePlans.length &&
            scriptPlan.scenePlans.length === storyboard.scenes.length;
        const visualConsistency = visualPlan.scenePlans.length === storyboard.scenes.length &&
            visualPlan.validated;
        const audioConsistency = audioPlan.sceneAudioPlans.length === storyboard.scenes.length &&
            audioPlan.validated;
        ...
        const assetCompleteness = requiredAssets.length > 0 &&
            requiredAssets.every((a) => a.status === "planned" || a.status === "validated");
        const dependencyValidation = productionPlan.dependencies.issues.length === 0 &&
        ...
        if (!brandConsistency)
            issues.push("Brand consistency check failed");
        if (!storyConsistency)
            issues.push("Story consistency across storyboard and script");
        if (!visualConsistency)
            issues.push("Visual plan scene alignment issue");
        if (!audioConsistency)
            issues.push("Audio plan scene alignment issue");
        if (!assetCompleteness)
            issues.push("Required assets incomplete");
        if (!dependencyValidation)
            issues.push("Production dependencies not fully validated");
        return {
            brandConsistency,
            storyConsistency,
            visualConsistency,
    ...
    detectRisks(checks, productionPlan, storyboard, scriptPlan, scores, audience) {
        const risks = [];
    ...
    buildPredictions(scores, risks, storyboard) {
        ...
        let productionRisk = "low";
        if (unresolvedCritical > 0)
            productionRisk = "critical";
        else if (unresolvedHigh > 1)
            productionRisk = "high";
        else if (unresolvedHigh === 1)
            productionRisk = "medium";
        let productionComplexity = "low";
        if (storyboard.scenes.length > 8)
            productionComplexity = "medium";
        if (storyboard.scenes.length > 12)
            productionComplexity = "high";
        const successProbability = Math.max(0, Math.min(100, scores.overallQualityScore - unresolvedCritical * 25 - unresolvedHigh * 10));
        const opportunities = [];
        if (scores.storytellingScore < 80)
            opportunities.push("Strengthen hook and narrative arc");
        if (scores.visualQualityScore < 80)
            opportunities.push("Enhance visual composition consistency");
        if (scores.marketingEffectivenessScore < 80)
            opportunities.push("Sharpen CTA and offer presentation");
        if (scores.brandConsistencyScore < 85)
            opportunities.push("Tighten brand color and logo placement");
        if (opportunities.length === 0)
            opportunities.push("Maintain current quality — minor platform optimizations available");
        return {
            successProbability,
            productionRisk,
    ...
    buildRecommendations(scores, risks, storyboard, platform) {
        const platformConfig = PLATFORM_QUALITY[platform];
        return {
            storyImprovements: scores.storytellingScore < 85
            ...
            visualImprovements: scores.visualQualityScore < 85
            ...
            audioImprovements: scores.technicalReadinessScore < 85
            ...
            brandingImprovements: scores.brandConsistencyScore < 85
            ...
            marketingImprovements: scores.marketingEffectivenessScore < 85
            ...
            productionOptimization: risks.some((r) => r.severity === "high" || r.severity === "critical")
    ...
    buildPlatformQuality(storyboard, scores) {
...
//# sourceMappingURL=quality-prediction-analyzer.js.map