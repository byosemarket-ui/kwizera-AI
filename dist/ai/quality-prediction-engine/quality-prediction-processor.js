...
export class QualityPredictionProcessor {
    ...
    async predictQuality(input) {
        ...
        if (!productionPlan?.validated || !productionPlan.productionReady) {
            ...
            if (!productionResult.success || !productionResult.record) {
                return this.reject(start, productionResult.message ?? "Production-ready production plan required before quality prediction", productionResult.diagnostics.length > 0
        ...
        const analysis = this.analyzer.buildAnalysisSummary(understanding, audience, strategy, creative, storyboard, scriptPlan, visualPlan, audioPlan, productionPlan);
        let checks = this.analyzer.runQualityChecks(storyboard, scriptPlan, visualPlan, audioPlan, productionPlan, creative);
        let scores = this.analyzer.computeScores(understanding, audience, strategy, creative, storyboard, scriptPlan, visualPlan, audioPlan, productionPlan, checks);
        ...
        if (this.analyzer.hasUnresolvedCriticalRisks(risks)) {
            ...
            if (repairedPlan?.success && repairedPlan.record) {
                ...
                checks = this.analyzer.runQualityChecks(storyboard, scriptPlan, visualPlan, audioPlan, productionPlan, creative);
                scores = this.analyzer.computeScores(understanding, audience, strategy, creative, storyboard, scriptPlan, visualPlan, audioPlan, productionPlan, checks);
                risks = this.analyzer.applySafeRiskRepairs(this.analyzer.detectRisks(checks, productionPlan, storyboard, scriptPlan, scores, audience), checks);
            }
        }
        const predictions = this.analyzer.buildPredictions(scores, risks, storyboard);
        const recommendations = this.analyzer.buildRecommendations(scores, risks, storyboard, storyboard.profile.platform);
        const platformQuality = this.analyzer.buildPlatformQuality(storyboard, scores);
        let validation = this.scorer.isPredictionValid(scores, checks, risks, productionPlan.productionReady);
        ...
        const productionReady = this.scorer.isProductionReady(scores, risks, productionPlan.productionReady, checks);
        const draft = {
        ...
        draft.relationships = this.linker.detectRelationships(draft, storyboard, scriptPlan, visualPlan, audioPlan, productionPlan, creative, strategy, understanding);
    ...
    search(query) {
        ...
        if (query.predictionId)
            results = results.filter((r) => r.predictionId === query.predictionId);
        if (query.projectId)
        ...
        if (query.text) {
            ...
            results = results.filter((r) => r.predictionId.toLowerCase().includes(textLower) ||
...
//# sourceMappingURL=quality-prediction-processor.js.map