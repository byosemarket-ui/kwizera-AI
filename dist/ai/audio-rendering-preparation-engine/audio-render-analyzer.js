import { ALL_AUDIO_RENDER_ASSET_TYPES, ALL_AUDIO_RENDER_PLATFORMS, ALL_AUDIO_RENDER_TIMELINE_CHECKS, ALL_AUDIO_RENDER_TRACK_CHECKS, ALL_AUDIO_RENDER_VALIDATION_STAGES, AUDIO_RENDER_PLATFORM_CONFIG, AUDIO_RENDER_VALIDATION_MODULE_MAP, AudioRenderAssetType, AudioRenderChannelLayout, AudioRenderPlatform, AudioRenderTimelineCheck, AudioRenderTrackCheck, } from "./types.js";
export class AudioRenderAnalyzer {
    buildProfile(input, platform, version, context) {
        const productionId = input.productionId ?? context.productionPlan?.audioProductionId ?? `production-${context.productId ?? "unknown"}`;
    ...
    buildTrackValidation(context, tracks) {
    ...
    buildTimelineValidation(context, timeline) {
    ...
    buildTimelineStructure(context, tracks) {
    ...
    extractContext(input, productionPlan, analysis) {
        return {
    ...
    validateTrackCheck(check, tracks, context) {
        if (tracks.length < 3)
            return false;
        switch (check) {
    ...
    validateTimelineCheck(check, timeline, context) {
        if (timeline.length < 2 && !context.renderPrompt && !context.productionPlan)
            return false;
        switch (check) {
    ...
    resolveAssetId(assetType, context, input) {
        ...
        if (!id && context.productionPlan)
            return `production-${assetType}`;
        return id;
    }
    resolveAssetSource(assetType, context) {
        if (context.productionPlan)
            return "production-plan";
        if (assetType === AudioRenderAssetType.VoiceTrack || assetType === AudioRenderAssetType.MusicTrack)
            return "audio-plan";
        return "render-plan";
    }
}
//# sourceMappingURL=audio-render-analyzer.js.map