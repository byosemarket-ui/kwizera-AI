...
import { OUTPUT_PROFILE_CONFIG, RENDER_OUTPUT_PLATFORM_TARGETS, RenderPlanType, mapStoryboardToRenderOutput, } from "./types.js";
export class RenderingPreparationAnalyzer {
    buildRenderPlan(storyboard, upstream, foundation, version) {
    ...
    buildProfile(storyboard, productionPlan, version) {
        return {
    ...
    buildRenderValidation(storyboard, upstream) {
        const issues = [];
        ...
        if (!upstream.productionPlan.validated || !upstream.productionPlan.productionReady)
            issues.push("Production plan incomplete");
        return {
    ...
    buildTimelineValidation(upstream) {
        const sceneTimeline = upstream.scenes.map((s) => `Scene ${s.structure.sceneOrder}: ${s.structure.sceneDuration}`);
        const cameraTimeline = upstream.cameraPlans.flatMap((p) => p.shotPlans.map((s) => `Shot ${s.shotOrder}: ${s.shotDuration}`));
        ...
        return {
            ...
            effectTimeline,
            renderTimeline,
            allTimelinesValid: sceneTimelineValid &&
                cameraTimelineValid &&
                motionTimelineValid &&
                animationTimelineValid &&
                audioTimelineValid &&
    ...
    buildRenderSettings(platform, config, productionPlan) {
    ...
    buildOutputProfiles(primaryPlatform) {
    ...
    buildResourcePlanning(upstream) {
        const sceneCount = upstream.scenes.length;
    ...
    buildRenderJobs(profile, platform) {
...
//# sourceMappingURL=rendering-preparation-analyzer.js.map