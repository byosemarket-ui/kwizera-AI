export class RenderingPreparationScorer {
    computeScores(draft, storyboard, _upstream) {
        ...
        const aiConfidenceScore = Math.round((renderReadinessScore +
            assetQualityScore +
    ...
    isPlanValid(scores, draft) {
        ...
        if (!draft.timelineValidation.allTimelinesValid)
            diagnostics.push("Timeline validation incomplete");
        if (draft.timelineValidation.renderTimeline.length < 4)
            diagnostics.push("Render timeline incomplete");
    ...
    computeRenderReadiness(draft, storyboard) {
        let score = 45;
        if (draft.renderValidation.allValidated)
            score += 25;
        if (storyboard.productionReady)
            score += 15;
        if (draft.renderSettings.resolution.length > 3)
            score += 15;
        return Math.min(100, score);
    }
    computeAssetQuality(draft) {
        let score = 45;
        if (draft.assetValidation.allAssetsReady)
            score += 30;
    ...
    computeTimelineIntegrity(draft) {
        let score = 45;
        if (draft.timelineValidation.sceneTimelineValid)
            score += 10;
        if (draft.timelineValidation.audioTimelineValid)
            score += 10;
        if (draft.timelineValidation.renderTimelineValid)
            score += 15;
        if (draft.timelineValidation.effectTimelineValid)
            score += 10;
        if (draft.timelineValidation.allTimelinesValid)
            score += 10;
        return Math.min(100, score);
    ...
    computePerformance(draft) {
        let score = 70;
        if (draft.resourcePlanning.renderQueue.length > 5)
            score += 15;
...
//# sourceMappingURL=rendering-preparation-scorer.js.map