import { CreativeStyle, } from "./types.js";
...
export class ImageRelationshipLinker {
    detectSimilar(record, allRecords) {
        ...
        for (const other of allRecords) {
            if (other.imageId === record.imageId)
                continue;
            const sameBrand = record.productPresentation.branding === other.productPresentation.branding &&
                record.productPresentation.branding !== "unknown";
            const score = this.similarityScore(record, other);
            if (score < 40 && !sameBrand)
                continue;
    ...
    similarityScore(a, b) {
        let score = 0;
        if (a.design.creativeStyle === b.design.creativeStyle)
            score += 25;
        if (a.design.layout === b.design.layout)
            score += 20;
...
//# sourceMappingURL=image-recommender.js.map