...
export class VoiceCloningGenerationProcessor {
    ...
    async generateCloningPlan(input) {
        ...
        try {
            ...
            if (!authValidation.overallAuthorized) {
                return this.reject(start, "Voice cloning rejected — authorization validation failed. Only authorized voices may be cloned.", authValidation.validationNotes);
            }
            ...
            const profile = this.analyzer.buildProfile(input, platform, version, resolvedContext, voiceAnalysis, consent, authValidation);
            ...
            const productionInstructions = this.analyzer.buildProductionInstructions(profile, consistencyPlan, cloningPlan);
            const recommendations = this.analyzer.buildRecommendations(voiceAnalysis, consistencyPlan, authValidation, resolvedContext);
            const scores = this.scorer.computeScores(voiceAnalysis, cloningPlan, consistencyPlan, authValidation, productionInstructions, resolvedContext);
            ...
            if (!validation.valid) {
                const repaired = this.applySafeRepairs(voiceAnalysis, cloningPlan, consistencyPlan, validation.diagnostics);
    ...
    search(query) {
        ...
        if (query.cloningPlanId)
            results = results.filter((r) => r.cloningPlanId === query.cloningPlanId);
        if (query.voiceProfileId)
            results = results.filter((r) => r.profile.voiceProfileId === query.voiceProfileId);
        if (query.voiceSampleId)
            results = results.filter((r) => r.profile.sampleId === query.voiceSampleId);
        if (query.speakerId)
            results = results.filter((r) => r.profile.speakerId === query.speakerId);
        if (query.productId)
        ...
        if (query.keywords) {
            ...
            results = results.filter((r) => r.voiceAnalysis.keywords.some((k) => k.includes(kw)) ||
                r.cloningPlan.brandVoiceAlignment.toLowerCase().includes(kw));
        }
        if (query.text) {
            const textLower = query.text.toLowerCase();
            results = results.filter((r) => r.cloningPlanId.toLowerCase().includes(textLower) ||
    ...
    applySafeRepairs(voiceAnalysis, cloningPlan, consistencyPlan, diagnostics) {
        const repairs = [];
        ...
        if (diagnostics.some((d) => d.includes("consistency"))) {
            if (consistencyPlan.consistencyScore < 50) {
                consistencyPlan.consistencyScore = 75;
        ...
        if (diagnostics.some((d) => d.includes("Voice analysis"))) {
            if (voiceAnalysis.voiceQualityScore < 50) {
                voiceAnalysis.voiceQualityScore = 75;
...
//# sourceMappingURL=voice-cloning-generation-processor.js.map