export class VoiceCloningGenerationScorer {
    computeScores(voiceAnalysis, cloningPlan, consistencyPlan, authValidation, productionInstructions, context) {
        ...
        const productionReadinessScore = this.computeProductionReadiness(voiceAnalysis, productionInstructions);
        ...
        const aiConfidenceScore = Math.round((voiceSimilarityScore +
            voiceStabilityScore +
            pronunciationScore +
    ...
    isCloningPlanValid(scores, authValidation, record) {
        const diagnostics = [];
    ...
    isProductionReady(scores, record) {
        return (record.authorizationValidation.overallAuthorized &&
    ...
    isAuthorizationCompliant(authValidation) {
        return authValidation.overallAuthorized && authValidation.authorizationStatus === "authorized";
    }
    isBrandConsistent(context, cloningPlan) {
        if (!context.brandName)
            return cloningPlan.brandVoiceAlignment.length >= 10;
        return cloningPlan.brandVoiceAlignment.toLowerCase().includes(context.brandName.toLowerCase());
    }
    computeVoiceSimilarity(voiceAnalysis, plan) {
        let score = 45;
        if (voiceAnalysis.voiceQualityScore >= 80)
            score += 20;
        if (Object.keys(plan.voiceIdentityMapping).length >= 3)
            score += 15;
        if (plan.accentMapping.length >= 10)
            score += 10;
        if (voiceAnalysis.timbre.length >= 10)
            score += 10;
        return Math.min(100, score);
    }
    computeVoiceStability(plan) {
        let score = 45;
        if (plan.consistencyScore >= 80)
            score += 25;
        if (plan.voiceStability.length >= 10)
            score += 15;
        if (plan.naturalRhythm.length >= 5)
            score += 15;
        return Math.min(100, score);
    }
    computePronunciationScore(voiceAnalysis, plan) {
        let score = 45;
        if (Object.keys(plan.pronunciationMapping).length >= 1)
            score += 20;
        if (voiceAnalysis.pronunciation.length >= 10)
            score += 15;
        if (voiceAnalysis.properNames.length > 0 && Object.keys(plan.pronunciationMapping).length >= 1)
            score += 20;
        return Math.min(100, score);
    }
    computeEmotionPreservation(voiceAnalysis, plan) {
        let score = 45;
        if (plan.emotionMapping.source === plan.emotionMapping.target)
            score += 25;
        if (voiceAnalysis.detectedEmotion)
            score += 20;
        if (plan.emotionMapping.source)
            score += 10;
        return Math.min(100, score);
    }
    computeProductionReadiness(voiceAnalysis, instructions) {
        let score = 45;
        if (voiceAnalysis.durationMs > 0)
            score += 10;
        if (instructions.renderNotes.length >= 2)
            score += 15;
        if (instructions.identityGuidance.length >= 1)
            score += 15;
        if (instructions.qualityTargets.length >= 2)
            score += 15;
        return Math.min(100, score);
    }
    computeAuthorizationCompliance(authValidation) {
        if (!authValidation.overallAuthorized)
            return 0;
        let score = 70;
        if (authValidation.voiceConsentValid)
            score += 6;
        if (authValidation.usagePermissionValid)
            score += 6;
        if (authValidation.projectAuthorizationValid)
            score += 6;
        if (authValidation.licensingValid)
            score += 6;
        if (authValidation.expirationValid)
            score += 6;
        return Math.min(100, score);
    }
    computeBrandConsistency(context, plan) {
        let score = 45;
        if (plan.brandVoiceAlignment.length >= 15)
            score += 20;
...
function voiceAnalysisIncomplete(analysis) {
    return !analysis.pitch || !analysis.timbre || analysis.voiceQualityScore < 50;
}
//# sourceMappingURL=voice-cloning-generation-scorer.js.map