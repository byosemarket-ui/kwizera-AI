...
import { AudioGenerationFoundationError, AudioGenerationHealthLevel, AudioGenerationLifecycleState, } from "./types.js";
export class AiAudioGenerationFoundation {
    core = null;
    storageRoot = "";
    initialized = false;
    startupComplete = false;
    lifecycleState = AudioGenerationLifecycleState.Initializing;
    startupMs = 0;
    lastIntegrity = null;
    lastHealth = null;
    logger = new AudioGenerationFoundationLogger();
    history = new AudioGenerationHistoryStore();
    integration = new AudioGenerationIntegrationBridge(this.logger);
    storage = new AudioGenerationStorageManager(this.logger);
    registry = new AudioGenerationRegistry(this.logger);
    integrityVerifier = new AudioGenerationIntegrityVerifier(this.logger);
    healthMonitor = new AudioGenerationHealthMonitor(this.logger);
    assetRegistry = new GenerationAssetRegistry(this.logger);
    blueprintManager = new AudioGenerationBlueprintManager(this.logger);
    workflow = new NonDestructiveGenerationWorkflow(this.logger);
    projectManager = new GenerationProjectManager(this.logger);
    textToSpeechGenerationEngine = new AiTextToSpeechGenerationEngine();
    speechToSpeechGenerationEngine = new AiSpeechToSpeechGenerationEngine();
    voiceCloningGenerationEngine = new AiVoiceCloningGenerationEngine();
    musicGenerationEngine = new AiMusicGenerationEngine();
    soundEffectsGenerationEngine = new AiSoundEffectsGenerationEngine();
    ambientAudioGenerationEngine = new AiAmbientAudioGenerationEngine();
    audioEnhancementRestorationEngine = new AiAudioEnhancementRestorationEngine();
    audioMixingMasteringEngine = new AiAudioMixingMasteringEngine();
    audioProductionEngine = new AiAudioProductionEngine();
    audioRenderingPreparationEngine = new AiAudioRenderingPreparationEngine();
    audioQualityValidationEngine = new AiAudioQualityValidationEngine();
    accessCoordinator = null;
    qualityValidator = null;
    initialize(core, storageRoot, memoryFoundation, knowledgeFoundation, productIntelligenceFoundation, imageIntelligenceFoundation, videoIntelligenceFoundation, videoGenerationFoundation, imageGenerationFoundation, moduleManager, stateManager, recoveryEngine, systemHealthMonitor) {
        this.core = core;
        ...
        this.accessCoordinator = new AudioGenerationAccessCoordinator(this.logger, this.history, this.registry);
        this.qualityValidator = new AudioGenerationQualityValidator(this.logger, this.registry);
        this.integration.connect(core, memoryFoundation, knowledgeFoundation, productIntelligenceFoundation, imageIntelligenceFoundation, videoIntelligenceFoundation, videoGenerationFoundation, imageGenerationFoundation, moduleManager, stateManager, recoveryEngine, systemHealthMonitor);
    ...
    registerAudioGenerationModule(registration) {
        this.ensureReady();
        const full = {
            ...registration,
    ...
    refreshIntegration(memoryFoundation, knowledgeFoundation, productIntelligenceFoundation, imageIntelligenceFoundation, videoIntelligenceFoundation, videoGenerationFoundation, imageGenerationFoundation, moduleManager, stateManager, recoveryEngine, systemHealthMonitor) {
        if (!this.core)
            return;
        this.integration.connect(this.core, memoryFoundation, knowledgeFoundation, productIntelligenceFoundation, imageIntelligenceFoundation, videoIntelligenceFoundation, videoGenerationFoundation, imageGenerationFoundation, moduleManager, stateManager, recoveryEngine, systemHealthMonitor);
    }
    ...
    setLifecycleGenerating() {
        if (this.startupComplete)
            this.lifecycleState = AudioGenerationLifecycleState.Generating;
    }
    setLifecycleReady() {
        if (this.startupComplete)
            this.lifecycleState = AudioGenerationLifecycleState.Ready;
    ...
    ensureReady() {
        if (!this.initialized || !this.accessCoordinator || !this.qualityValidator) {
            throw new AudioGenerationFoundationError("AI Audio Generation Foundation not initialized", "NOT_INITIALIZED");
        }
    }
}
export { createDefaultGenerationAssetQuality };
//# sourceMappingURL=audio-generation-foundation.js.map