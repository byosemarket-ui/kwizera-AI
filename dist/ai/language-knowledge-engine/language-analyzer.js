import { KnowledgeSupportedLanguage, LanguageMarketingGoal, LanguageScriptType, LanguageWritingStyle, } from "./types.js";
...
export class LanguageAnalyzer {
    analyze(input) {
        ...
        const localization = {
            ...
            relatedLanguages: input.localization?.relatedLanguages ?? defaults.relatedLanguages,
    ...
    detectLanguage(text, hint) {
        ...
        if (/\b(murakoze|amahoro|ubuzima)\b/.test(lower))
            return KnowledgeSupportedLanguage.Kinyarwanda;
        if (/\b(bonjour|merci|créatif)\b/.test(lower))
            return KnowledgeSupportedLanguage.French;
        if (/\b(asante|karibu|ubunifu)\b/.test(lower))
            return KnowledgeSupportedLanguage.Swahili;
        if (/\b(hola|gracias|creativo)\b/.test(lower))
            return KnowledgeSupportedLanguage.Spanish;
