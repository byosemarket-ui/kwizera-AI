...
export class ImageIntelligenceOptimizationAnalyzer {
    buildProfile(input, qualityPrediction, productionPlan, version) {
        const optimizationId = input.optimizationId ?? `optimization-${input.imageId}-${qualityPrediction.profile.platform}`;
        return {
            optimizationId,
    ...
    analyzeModuleOptimizations(foundation, baseline) {
        return MODULE_DEFS.map((mod) => {
            ...
            if (mod.moduleId === "background-intelligence-engine" ||
    ...
    buildStrategies(moduleResults) {
        return {
            relationshipOptimization: "Consolidated cross-module image relationship indexes for faster dependency resolution",
            classificationOptimization: "Refined image type, product and campaign classification metadata for search accuracy",
            searchOptimization: "Built inverted search index across image intelligence module records",
            knowledgeRetrievalOptimization: "Cached knowledge record links from upstream image intelligence relationships",
            recommendationOptimization: "Pre-computed recommendation paths from quality prediction analysis",
            workflowOptimization: "Optimized image production workflow checkpoint ordering without altering module responsibilities",
            performanceOptimization: "Reduced redundant lookups via warm cache for frequently accessed image records",
            cacheOptimization: `Warmed cache for ${moduleResults.length} image intelligence module contexts`,
            metadataOptimization: "Indexed image, brand, product, platform and campaign metadata for fast retrieval",
        };
    }
    buildCacheOptimization(foundation, imageId, existingCache) {
        ...
        const brand = creativePlan?.profile.brand ??
            enhancementPlan?.profile.brand ??
            qualityPrediction?.profile.brand;
        const product = creativePlan?.profile.product ??
        ...
        const cache = {
            ...
            creativeStyles: merge(existingCache.creativeStyles, creativePlan ? [creativePlan.creativeStyle.primaryStyle] : []),
            templates: merge(existingCache.templates, creativePlan ? [creativePlan.layoutPlanning.layoutType] : []),
            campaigns: merge(existingCache.campaigns, campaign ? [campaign] : []),
            productionPlans: merge(existingCache.productionPlans, productionPlan ? [productionPlan.profile.productionImagePlanId] : []),
...
//# sourceMappingURL=image-intelligence-optimization-analyzer.js.map