import { CameraAngle, CameraMovement, PLATFORM_SCENE_CONFIG, SCENE_PLATFORM_TARGETS, StoryboardGenerationPlatform, mapPurposeToPriority, mapPurposeToSceneType, } from "./types.js";
export class SceneGenerationAnalyzer {
    buildSceneRecord(storyboard, sourceScene, version) {
    ...
    buildProfile(storyboard, sourceScene, version) {
        return {
    ...
    buildShots(sourceScene) {
        return sourceScene.shots.map((shot) => ({
            shotId: shot.shotId,
    ...
    buildVisualPlan(storyboard, sourceScene) {
    ...
    buildCharacterPlanning(sourceScene, storyboard) {
    ...
    buildObjectPlanning(sourceScene, storyboard) {
    ...
    buildAudioPlanning(storyboard, sourceScene) {
    ...
    buildTransitionPlanning(sourceScene, storyboard) {
    ...
    buildMotionPlanning(sourceScene, shots) {
        return {
    ...
    buildLayout(sourceScene, storyboard) {
        return {
    ...
    buildPlatformOptimizations(primaryPlatform, sourceScene) {
        return SCENE_PLATFORM_TARGETS.map((platform) => {
            ...
            return {
                ...
                safeZoneNotes: platform === StoryboardGenerationPlatform.Television
    ...
    buildRecommendations(record) {
        const recs = [];
        if (record.shots.length < 2)
            recs.push("Consider adding B-roll shot for visual variety");
    ...
    resolveFocusPoint(purpose, shotType) {
        if (purpose === "product-showcase")
            return "Product hero — primary feature detail";
        if (purpose === "call-to-action")
            return "CTA element and brand logo";
        if (shotType === "close-up")
            return "Subject face or product detail";
        return "Scene subject center of interest";
    }
}
//# sourceMappingURL=scene-generation-analyzer.js.map