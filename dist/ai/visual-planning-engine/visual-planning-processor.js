...
export class VisualPlanningProcessor {
    ...
    async createVisualPlan(input) {
        ...
        if (!storyboard?.validated || !storyboard.productionReady) {
            ...
            if (!storyboardResult.success || !storyboardResult.record) {
                return this.reject(start, storyboardResult.message ?? "Production-ready storyboard required before visual planning", storyboardResult.diagnostics.length > 0
        ...
        if (!scriptPlan?.validated || !scriptPlan.productionReady) {
            ...
            if (!scriptResult.success || !scriptResult.record) {
                return this.reject(start, scriptResult.message ?? "Production-ready script plan required before visual planning", scriptResult.diagnostics.length > 0
        ...
        const scores = this.scorer.computeScores(scenePlans, backgroundPlanning, cameraPlanning, brandConsistency, storyboard, scriptPlan, creative, strategy);
        let validation = this.scorer.isVisualPlanValid(scores, scenePlans, storyboard, scriptPlan, brandConsistency, alignment.issues);
        if (!validation.valid) {
            const repaired = this.applyScoreRepairs(scenePlans, brandConsistency, creative);
            if (repaired.length > 0) {
                scenePlans = repaired;
                brandConsistency = this.analyzer.buildBrandConsistency(creative, scenePlans);
                const repairedScores = this.scorer.computeScores(scenePlans, backgroundPlanning, cameraPlanning, brandConsistency, storyboard, scriptPlan, creative, strategy);
                validation = this.scorer.isVisualPlanValid(repairedScores, scenePlans, storyboard, scriptPlan, brandConsistency, this.analyzer.validateSceneAlignment(scenePlans, storyboard, scriptPlan).issues);
                if (validation.valid) {
        ...
        if (!validation.valid) {
            ...
            return {
                ...
                diagnostics: validation.diagnostics,
                message: "Visual planning validation failed — plan must fully align with storyboard, script plan, brand and strategy",
            };
        }
        ...
        draft.relationships = this.linker.detectRelationships(draft, storyboard, scriptPlan, creative, strategy, understanding);
        ...
        this.logger.log("info", "relationship", "Visual planning relationships updated", {
            visualPlanId: draft.visualPlanId,
            relationshipCount: draft.relationships.audioPlans.length + draft.relationships.productionPlans.length,
        });
    ...
    search(query) {
        ...
        if (query.text) {
            ...
            results = results.filter((r) => r.visualPlanId.toLowerCase().includes(textLower) ||
                r.scenePlans.some((s) => s.visualGoal.toLowerCase().includes(textLower)));
        }
        return results.slice(0, query.limit ?? 50);
    }
    applySceneRepairs(storyboard, scriptPlan, creative, understanding) {
        return this.analyzer.buildScenePlans(storyboard, scriptPlan, creative, understanding);
    }
    applyScoreRepairs(scenePlans, brandConsistency, creative) {
        if (brandConsistency.logoPlacement && brandConsistency.brandColors) {
            return [];
        }
        return scenePlans.map((plan, index) => ({
            ...plan,
            logoPlacement: plan.logoPlacement !== "none" && plan.logoPlacement.length > 5
                ? plan.logoPlacement
                : index === scenePlans.length - 1
                    ? `Plan logo — ${creative.brandDirection.logoPlacement}`
                    : plan.logoPlacement,
            colorPalette: plan.colorPalette.includes(creative.profile.brand) || plan.colorPalette.length > 15
                ? plan.colorPalette
...
//# sourceMappingURL=visual-planning-processor.js.map