export class VisualPlanningScorer {
    computeScores(scenePlans, background, camera, brandConsistency, storyboard, scriptPlan, creative, strategy) {
        ...
        const aiConfidenceScore = Math.round((visualPlanningScore + compositionScore + brandConsistencyScore + creativeScore + marketingScore) / 5);
        return {
            visualPlanningScore,
            compositionScore,
    ...
    isVisualPlanValid(scores, scenePlans, storyboard, scriptPlan, brandConsistency, alignmentIssues) {
        const diagnostics = [];
    ...
    isProductionReady(scenePlans, storyboard, scriptPlan, scores) {
    ...
    computePlanningScore(scenePlans, storyboard, scriptPlan) {
        let score = 50;
        if (scenePlans.length === storyboard.scenes.length)
            score += 15;
        if (scenePlans.length === scriptPlan.scenePlans.length)
            score += 15;
        if (scenePlans.every((s) => s.productPlacement.startsWith("Plan")))
            score += 10;
        if (scenePlans.every((s) => s.lightingDirection.startsWith("Plan lighting")))
            score += 5;
        if (scenePlans.every((s) => s.cameraAngle.startsWith("Plan camera")))
            score += 5;
        return Math.min(100, score);
    }
    computeCompositionScore(scenePlans) {
        let score = 45;
        if (scenePlans.every((s) => s.composition.length >= 15))
            score += 20;
        if (scenePlans.every((s) => s.depth.length >= 10))
            score += 15;
        if (scenePlans.every((s) => s.transitionDirection.startsWith("Plan transition")))
            score += 10;
        if (scenePlans.some((s) => s.cameraDistance.includes("hero") || s.cameraDistance.includes("close-up")))
            score += 10;
        return Math.min(100, score);
    }
    computeBrandScore(brand, creative) {
        let score = 40;
        if (brand.logoPlacement)
            score += 15;
        ...
        if (brand.brandIdentity)
            score += 10;
        if (brand.visualConsistency)
            score += 10;
        if (brand.issues.length === 0)
            score += 10;
        return Math.min(100, score);
    }
}
//# sourceMappingURL=visual-planning-scorer.js.map