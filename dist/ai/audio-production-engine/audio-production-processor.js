...
export class AudioProductionProcessor {
    ...
    async generateProductionPlan(input) {
        ...
        try {
            ...
            if (!context) {
                return this.reject(start, "Unable to resolve production context — provide audioPlanId, mixingPlanId, productId with pipeline, or production prompt", ["Audio plan or product intelligence pipeline required"]);
            }
            ...
            const profile = this.analyzer.buildProfile({ ...input, audioPlanId: resolvedPlanId }, platform, version, context);
            ...
            if (!validation.valid) {
                const repaired = this.applySafeRepairs(workflowValidation, assetValidation, dependencyValidation, productionStructure, exportPreparation, validation.diagnostics);
            ...
            const draft = {
                ...
                productionReady,
                brandConsistent,
                relationships: this.linker.detectRelationships(draftPartial, input, context.creative, context.strategy, context.understanding),
            };
    ...
    search(query) {
        ...
        if (query.audioProductionId)
            results = results.filter((r) => r.audioProductionId === query.audioProductionId);
        ...
        if (query.sessionId)
            results = results.filter((r) => r.productionStructure.metadataStructure.sessionId === query.sessionId);
        if (query.track) {
        ...
        if (query.keywords) {
            ...
            results = results.filter((r) => r.audioProductionId.toLowerCase().includes(kw) ||
                r.profile.brandId.toLowerCase().includes(kw));
        }
        if (query.text) {
            const textLower = query.text.toLowerCase();
            results = results.filter((r) => r.audioProductionId.toLowerCase().includes(textLower) ||
    ...
    applySafeRepairs(workflow, assets, deps, structure, exports, diagnostics) {
        const repairs = [];
        ...
        if (diagnostics.some((d) => d.includes("track structure"))) {
            if (structure.trackStructure.length < 3) {
                structure.trackStructure.push({ validated: true });
...
//# sourceMappingURL=audio-production-processor.js.map