import { ALL_AUDIO_PRODUCTION_ASSET_TYPES, ALL_AUDIO_PRODUCTION_DEPENDENCIES, ALL_AUDIO_PRODUCTION_EXPORT_FORMATS, ALL_AUDIO_PRODUCTION_WORKFLOW_STAGES, AudioProductionAssetType, AudioProductionDependency, AudioProductionExportFormat, AudioProductionPlatform, AudioProductionWorkflowStage, AUDIO_PRODUCTION_PLATFORM_CONFIG, DEPENDENCY_MODULE_MAP, WORKFLOW_MODULE_MAP, } from "./types.js";
export class AudioProductionAnalyzer {
    buildProfile(input, platform, version, context) {
        ...
        const audioPlanId = input.audioPlanId ?? input.mixingPlanId ?? context.mixingPlanId ?? `audio-plan-${productId}`;
        return {
    ...
    resolveAudioPlanId(input, context) {
    ...
    buildTrackValidation(structure) {
    ...
    buildExportPreparation(input) {
        const exports = ALL_AUDIO_PRODUCTION_EXPORT_FORMATS.map((format) => ({
            format,
    ...
    buildPlatformRules(input, profile) {
        const config = AUDIO_PRODUCTION_PLATFORM_CONFIG[profile.platform];
        return {
            platform: profile.platform,
    ...
    buildRecommendations(context, profile) {
        const recs = [];
        if (context.mixingPlanId)
            recs.push("Link mixing plan to production execution blueprint");
        if (profile.platform === AudioProductionPlatform.Film)
            recs.push("Prepare surround and Atmos deliverables");
        if (context.brandName)
            recs.push(`Align production with ${context.brandName} brand guidelines`);
    ...
    resolvePlatform(input, context) {
        ...
        if (context.creative?.profile.platform === "youtube")
            return AudioProductionPlatform.YouTube;
        if (context.creative?.profile.platform === "tiktok")
            return AudioProductionPlatform.TikTok;
        if (context.industry === "health")
            return AudioProductionPlatform.Podcast;
    ...
    resolveAssetId(assetType, context, input) {
        ...
        if (!id && hasPrompt)
            return `planned-${assetType}`;
        return id;
    }
    resolveAssetSource(assetType, context, input) {
        if (input.voicePlanIds?.length && assetType === AudioProductionAssetType.VoiceTrack)
            return "voice-plan";
        if (input.musicPlanIds?.length && assetType === AudioProductionAssetType.MusicTrack)
            return "music-plan";
        if (context.mixingPlanId && assetType === AudioProductionAssetType.MultiTrackSession)
            return "mixing-plan";
        return "production-plan";
    }
}
//# sourceMappingURL=audio-production-analyzer.js.map