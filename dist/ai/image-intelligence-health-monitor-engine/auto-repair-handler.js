import { ImageIntelligenceHealthScoreLevel, ImageIntelligenceWarningType, } from "./types.js";
export class ImageIntelligenceAutoRepairHandler {
    ...
    async attemptRepairs(warnings) {
        const repairs = [];
        let success = true;
        const critical = warnings.some((w) => w.severity === ImageIntelligenceHealthScoreLevel.Critical ||
            w.severity === ImageIntelligenceHealthScoreLevel.Failed);
        const hasCriticalWarning = warnings.some((w) => w.type === ImageIntelligenceWarningType.DatabaseProblems ||
        ...
        if (critical || hasCriticalWarning || warnings.length > 3) {
            ...
            this.foundation.integration.reportCriticalIssue(`Critical image intelligence health: ${warnings.map((w) => w.message).join("; ")}`);
