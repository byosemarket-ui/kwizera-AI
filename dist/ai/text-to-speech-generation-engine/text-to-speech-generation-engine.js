...
import { TextToSpeechGenerationEngineError, } from "./types.js";
/**
...
export class AiTextToSpeechGenerationEngine {
    ...
    logger = new TextToSpeechGenerationLogger();
    records = new TextToSpeechGenerationRecordStore();
    analyzer = new TextToSpeechGenerationAnalyzer();
    scorer = new TextToSpeechGenerationScorer();
    linker = new TextToSpeechGenerationLinker();
    processor = null;
    generationTimes = [];
    searchTimes = [];
    blueprintTimes = [];
    initialize(foundation, storageRoot) {
        ...
        this.processor = new TextToSpeechGenerationProcessor(foundation, this.analyzer, this.scorer, this.linker, this.records, this.logger);
    ...
    async repairSpeechPlan(productId, platform) {
        this.ensureReady();
    ...
    buildStatusReport() {
        ...
        const avgPronunciation = all.length > 0
    ...
    ensureReady() {
        if (!this.initialized || !this.foundation || !this.processor) {
            throw new TextToSpeechGenerationEngineError("Text-to-Speech Generation Engine not initialized", "NOT_INITIALIZED");
        }
    }
}
//# sourceMappingURL=text-to-speech-generation-engine.js.map