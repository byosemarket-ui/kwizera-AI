...
export class TextToSpeechGenerationProcessor {
    ...
    async generateSpeechPlan(input) {
        ...
        try {
            ...
            const scores = this.scorer.computeScores(textAnalysis, voicePlan, pronunciationPlan, emotionPlan, naturalnessPlan, platformOptimizations, context);
    ...
    search(query) {
        ...
        if (query.speechPlanId)
            results = results.filter((r) => r.speechPlanId === query.speechPlanId);
        if (query.scriptId)
            results = results.filter((r) => r.profile.scriptId === query.scriptId);
        if (query.productId)
        ...
        if (query.keywords) {
            ...
            results = results.filter((r) => r.textAnalysis.keywords.some((k) => k.includes(kw)) ||
                r.voicePlan.voiceDescription.toLowerCase().includes(kw));
        }
        if (query.text) {
            const textLower = query.text.toLowerCase();
            results = results.filter((r) => r.speechPlanId.toLowerCase().includes(textLower) ||
    ...
    applySafeRepairs(textAnalysis, pronunciationPlan, emotionPlan, naturalnessPlan, diagnostics) {
        const repairs = [];
...
//# sourceMappingURL=text-to-speech-generation-processor.js.map