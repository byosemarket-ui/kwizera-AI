import { ALL_TTS_PLATFORMS, EmotionType, PLATFORM_SPEECH_CONFIG, TtsLanguage, TtsOutputUseCase, TtsPlatform, VoiceType, } from "./types.js";
...
export class TextToSpeechGenerationAnalyzer {
    ...
    buildProfile(input, platform, version, context, textAnalysis) {
    ...
    buildVoicePlan(input, context, textAnalysis) {
    ...
    buildPronunciationPlan(textAnalysis, context) {
        const brand = context.brandName ?? "Brand";
        const dict = {};
        ...
        return {
            ...
            technicalVocabulary: Object.fromEntries(textAnalysis.technicalTerms.map((t) => [t, this.phoneticHint(t, textAnalysis.language)])),
        };
    }
    buildEmotionPlan(input, context, textAnalysis) {
        const industry = context.industry ?? "default";
    ...
    buildNaturalnessPlan(textAnalysis, voicePlan, platform) {
    ...
    buildProductionInstructions(profile, naturalnessPlan, voicePlan) {
        return {
            renderNotes: [
    ...
    buildRecommendations(textAnalysis, emotionPlan, context) {
        const recs = [];
    ...
    resolveTextContent(input, context) {
        return (input.text ??
            input.script ??
    ...
    detectLanguage(content, input) {
        if (input.language)
            return input.language;
        if (/\b(murakoze|amakuru|ubuzima)\b/i.test(content))
            return TtsLanguage.Kinyarwanda;
        if (/\b(bonjour|merci|nous)\b/i.test(content))
            return TtsLanguage.French;
        if (/\b(asante|habari|karibu)\b/i.test(content))
            return TtsLanguage.Swahili;
        return TtsLanguage.English;
    }
    analyzeGrammar(content, language) {
    ...
    extractAbbreviations(content) {
        return [...content.matchAll(/\b[A-Z]{2,6}\b/g)].map((m) => m[0]).slice(0, 10);
    }
    extractProperNames(content, context) {
        const names = [];
        if (context.brandName)
