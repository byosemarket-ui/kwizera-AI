import { VideoIntelligenceHealthScoreLevel, VideoIntelligenceWarningType, } from "./types.js";
export class VideoIntelligenceAutoRepairHandler {
    ...
    async attemptRepairs(warnings) {
        const repairs = [];
        let success = true;
        const critical = warnings.some((w) => w.severity === VideoIntelligenceHealthScoreLevel.Critical ||
            w.severity === VideoIntelligenceHealthScoreLevel.Failed);
        const hasCriticalWarning = warnings.some((w) => w.type === VideoIntelligenceWarningType.DatabaseProblems ||
        ...
        if (critical || hasCriticalWarning || warnings.length > 3) {
            ...
            this.foundation.integration.reportCriticalIssue(`Critical video intelligence health: ${warnings.map((w) => w.message).join("; ")}`);
