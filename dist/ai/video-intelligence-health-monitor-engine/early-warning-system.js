...
import { deriveVideoIntelligencePerformanceIssues, } from "./resource-monitor.js";
import { VideoIntelligenceHealthScoreLevel, VideoIntelligenceWarningType, MonitoredVideoIntelligenceModule, } from "./types.js";
...
export class VideoIntelligenceEarlyWarningSystem {
    ...
    async detect(moduleScores, metrics) {
        ...
        if (!persistence.persistenceStatus.includes("survives")) {
            warnings.push(this.warn(VideoIntelligenceWarningType.DatabaseProblems, MonitoredVideoIntelligenceModule.VideoIntelligenceDatabase, "Video intelligence database persistence unverified", "Run foundation integrity verification and recovery"));
        }
        const registry = this.foundation.getRegistry();
        if (!registry.verifyChecksum()) {
            warnings.push(this.warn(VideoIntelligenceWarningType.RegistryProblems, MonitoredVideoIntelligenceModule.VideoIntelligenceRegistry, "Video intelligence registry checksum invalid", "Re-persist registry and verify module registrations"));
        }
        const integrity = this.foundation.getLastIntegrityResult();
        if (integrity && !integrity.verified) {
            warnings.push(this.warn(VideoIntelligenceWarningType.BrokenDependencies, MonitoredVideoIntelligenceModule.VideoIntelligenceFoundation, "Video intelligence integrity issues detected", "Run dependency validation and foundation recovery"));
        }
        ...
        for (const check of storageChecks) {
            ...
            try {
                ...
                warnings.push(this.warn(check.type, check.module, `${check.label} storage corrupted or invalid`, check.recommendation));
            }
        }
        ...
        if (missingDeps.length > 0) {
            warnings.push(this.warn(VideoIntelligenceWarningType.BrokenDependencies, MonitoredVideoIntelligenceModule.VideoIntelligenceFoundation, `${missingDeps.length} broken dependency reference(s)`, "Validate module registration order and dependencies"));
        }
        const sceneReport = this.foundation.getSceneDetectionEngine().buildStatusReport();
        if (sceneReport.knownIssues.length > 0) {
            warnings.push(this.warn(VideoIntelligenceWarningType.SceneDetectionFailure, MonitoredVideoIntelligenceModule.SceneDetection, sceneReport.knownIssues.join("; "), "Review scene detection records"));
        }
        ...
        if (timelineReport.knownIssues.length > 0) {
            warnings.push(this.warn(VideoIntelligenceWarningType.TimelineProblems, MonitoredVideoIntelligenceModule.TimelineIntelligence, timelineReport.knownIssues.join("; "), "Repair timeline intelligence records"));
        }
        const motionReport = this.foundation.getMotionIntelligenceEngine().buildStatusReport();
        if (motionReport.knownIssues.length > 0) {
            warnings.push(this.warn(VideoIntelligenceWarningType.MotionProblems, MonitoredVideoIntelligenceModule.MotionIntelligence, motionReport.knownIssues.join("; "), "Review motion intelligence analysis"));
        }
        const cameraReport = this.foundation.getCameraMovementEngine().buildStatusReport();
        if (cameraReport.knownIssues.length > 0) {
            warnings.push(this.warn(VideoIntelligenceWarningType.CameraProblems, MonitoredVideoIntelligenceModule.CameraMovement, cameraReport.knownIssues.join("; "), "Review camera movement intelligence"));
        }
        ...
        if (styleReport.averageStyleConsistencyScore > 0 && styleReport.averageStyleConsistencyScore < 55) {
            warnings.push(this.warn(VideoIntelligenceWarningType.StyleProblems, MonitoredVideoIntelligenceModule.VideoStyle, `Style consistency below threshold (${styleReport.averageStyleConsistencyScore})`, "Re-analyze video style intelligence"));
        }
        ...
        if (enhancementReport.knownIssues.length > 0) {
            warnings.push(this.warn(VideoIntelligenceWarningType.EnhancementPlanningProblems, MonitoredVideoIntelligenceModule.VideoEnhancementPlanning, enhancementReport.knownIssues.join("; "), "Review video enhancement planning records"));
        }
        ...
        if (creativeReport.knownIssues.length > 0) {
            warnings.push(this.warn(VideoIntelligenceWarningType.CreativePlanningProblems, MonitoredVideoIntelligenceModule.CreativeVideoIntelligence, creativeReport.knownIssues.join("; "), "Review creative video planning records"));
        }
        ...
        if (productionReport.knownIssues.length > 0) {
            warnings.push(this.warn(VideoIntelligenceWarningType.ProductionPlanningProblems, MonitoredVideoIntelligenceModule.ProductionVideoPlanning, productionReport.knownIssues.join("; "), "Run production video planning repair"));
        }
        const analysisReport = this.foundation.getVideoAnalysisEngine().buildStatusReport();
        if (analysisReport.knownIssues.length > 0) {
            warnings.push(this.warn(VideoIntelligenceWarningType.VideoAnalysisFailure, MonitoredVideoIntelligenceModule.VideoAnalysis, analysisReport.knownIssues.join("; "), "Re-analyze affected videos"));
        }
        if (metrics.searchPerformanceMs > 150) {
            warnings.push(this.warn(VideoIntelligenceWarningType.SearchFailure, MonitoredVideoIntelligenceModule.VideoSearch, `Video search averaging ${metrics.searchPerformanceMs}ms`, "Run video intelligence optimization for search performance"));
        }
        if (metrics.planningPerformanceMs > 120000) {
            warnings.push(this.warn(VideoIntelligenceWarningType.ProductionPlanningProblems, MonitoredVideoIntelligenceModule.ProductionVideoPlanning, `Planning pipeline averaging ${metrics.planningPerformanceMs}ms`, "Optimize production video planning workflow"));
        }
        const cache = this.foundation.getVideoIntelligenceOptimizationEngine().getCache();
        if (cache.hitRate < 5 && cache.videos.length === 0) {
            warnings.push(this.warn(VideoIntelligenceWarningType.CacheProblems, MonitoredVideoIntelligenceModule.VideoCache, "Video intelligence cache not warmed", "Run video intelligence optimization"));
        }
        if (metrics.diskUsageMb > 3000) {
            warnings.push(this.warn(VideoIntelligenceWarningType.HighResourceUsage, MonitoredVideoIntelligenceModule.VideoIntelligenceDatabase, `${metrics.diskUsageMb}MB disk used by video intelligence`, "Archive inactive video intelligence records"));
        }
        if (metrics.memoryUsageMb > 400) {
            warnings.push(this.warn(VideoIntelligenceWarningType.HighResourceUsage, MonitoredVideoIntelligenceModule.VideoCache, `${metrics.memoryUsageMb}MB heap used`, "Optimize video intelligence cache"));
        }
        if (metrics.gpuUsagePercent > 80) {
            warnings.push(this.warn(VideoIntelligenceWarningType.HighResourceUsage, MonitoredVideoIntelligenceModule.VideoIntelligenceFoundation, `GPU usage at ${metrics.gpuUsagePercent}%`, "Reduce concurrent video intelligence workloads"));
        }
        for (const issue of deriveVideoIntelligencePerformanceIssues(metrics)) {
            if (!warnings.some((w) => w.message.includes(issue))) {
                warnings.push(this.warn(VideoIntelligenceWarningType.HighResourceUsage, MonitoredVideoIntelligenceModule.VideoIntelligenceFoundation, issue, "Monitor video intelligence performance trends"));
            }
        }
        for (const mod of moduleScores) {
            if (mod.level === VideoIntelligenceHealthScoreLevel.Critical ||
                mod.level === VideoIntelligenceHealthScoreLevel.Failed) {
                warnings.push(this.warn(VideoIntelligenceWarningType.VideoAnalysisFailure, mod.module, `${mod.module} health critical (${mod.score})`, `Inspect ${mod.module} diagnostics`));
            }
        }
        return warnings;
    }
