export class VideoQualityPredictionScorer {
    computeScores(ctx, checks) {
        const { analysis, understanding, motion, camera, style, creativePlan, productionPlan } = ctx;
        const visualQualityScore = Math.round((analysis.scores.visualQualityScore +
        ...
        const storytellingScore = Math.round((understanding.scores.storytellingScore +
            creativePlan.scores.storytellingScore +
            ctx.timeline.scores.storyFlowScore) /
            3);
        const motionScore = Math.round((motion.scores.motionQualityScore +
            motion.scores.motionStabilityScore +
            motion.motionPlan.motionContinuity) /
            3);
        const cameraScore = Math.round((camera.scores.cameraMovementScore +
            camera.scores.cinematicScore +
            camera.scores.stabilityScore) /
            3);
        const styleScore = Math.round((style.scores.styleConsistencyScore +
            style.scores.editingQualityScore +
            style.scores.brandStyleScore) /
            3);
        const brandConsistencyScore = Math.round((understanding.scores.brandConsistencyScore +
            creativePlan.scores.brandConsistencyScore +
            style.scores.brandStyleScore) /
            3);
        const marketingEffectivenessScore = Math.round((understanding.scores.marketingScore +
        ...
        const overallVideoQualityScore = Math.round((visualQualityScore +
            audioQualityScore +
            storytellingScore +
            motionScore +
            cameraScore +
        ...
        const aiConfidenceScore = Math.round((overallVideoQualityScore +
    ...
    isPredictionValid(scores, risks, checks) {
        const diagnostics = [];
        const unresolvedCritical = risks.filter((r) => !r.resolved && r.severity === "critical");
        if (unresolvedCritical.length > 0) {
            diagnostics.push(`Unresolved critical risk(s): ${unresolvedCritical.map((r) => r.category).join(", ")}`);
        }
...
//# sourceMappingURL=video-quality-prediction-scorer.js.map