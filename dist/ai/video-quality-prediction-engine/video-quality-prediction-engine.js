...
import { VideoQualityPredictionEngineError, VideoQualityPredictionPlatform, } from "./types.js";
/**
...
export class AiVideoQualityPredictionEngine {
    ...
    logger = new VideoQualityPredictionLogger();
    records = new VideoQualityPredictionRecordStore();
    analyzer = new VideoQualityPredictionAnalyzer();
    scorer = new VideoQualityPredictionScorer();
    linker = new VideoQualityPredictionLinker();
    processor = null;
    predictionTimes = [];
    searchTimes = [];
    relationshipTimes = [];
    initialize(foundation, storageRoot) {
        ...
        this.processor = new VideoQualityPredictionProcessor(foundation, this.analyzer, this.scorer, this.linker, this.records, this.logger);
    ...
    async predictVideoQuality(input) {
        this.ensureReady();
    ...
    detectRelationships(videoId) {
        ...
        const updated = this.linker.detectRelationships(record, this.records.getAll(), analysis, understanding, productionPlan, creativePlan, enhancementPlan, record.relationships.relatedProjects, record.relationships.relatedKnowledge, record.relationships.relatedScripts);
        this.relationshipTimes.push(Date.now() - start);
        return updated;
    }
    async repairQualityPrediction(videoId) {
        this.ensureReady();
    ...
    buildStatusReport() {
        ...
        if (!this.startupComplete)
            readinessScore -= 25;
        if (!this.foundation?.getProductionVideoPlanningEngine().isStartupComplete())
            readinessScore -= 10;
        if (!integration?.knowledgeEngine)
            readinessScore -= 10;
        return {
            engineStatus: this.startupComplete ? "operational" : "initializing",
            qualityAnalysisStatus: "full video intelligence stack analyzed for quality prediction",
            predictionStatus: "production success, engagement, retention, marketing impact and rendering complexity predicted",
            riskDetectionStatus: "storytelling, visual, audio, motion, camera, style, asset and rendering risks evaluated",
    ...
    ensureReady() {
        if (!this.initialized || !this.foundation || !this.processor) {
            throw new VideoQualityPredictionEngineError("Video Quality Prediction Engine not initialized", "NOT_INITIALIZED");
        }
    }
}
export { VideoQualityPredictionPlatform };
//# sourceMappingURL=video-quality-prediction-engine.js.map