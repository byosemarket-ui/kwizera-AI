import { VideoQualityPredictionPlatform, } from "./types.js";
export class VideoQualityPredictionAnalyzer {
    ...
    buildQualityChecks(ctx) {
        ...
        return {
            ...
            audioSynchronization: timeline.scores.synchronizationScore >= 55 &&
            ...
            brandConsistency: style.scores.brandStyleScore >= 55 && creativePlan.scores.brandConsistencyScore >= 55,
    ...
    buildPredictions(ctx, checks, risks) {
        ...
        const productionSuccessProbability = Math.max(0, Math.min(100, Math.round(ctx.productionPlan.scores.productionReadinessScore * 0.35 +
            checkRatio * 100 * 0.25 +
        ...
        if (!checks.storyConsistency)
            opportunities.push("Strengthen storytelling flow before production");
        if (!checks.motionContinuity)
            opportunities.push("Improve motion continuity across scenes");
        if (!checks.cameraContinuity)
            opportunities.push("Stabilize camera movement for smoother delivery");
        if (!checks.audioSynchronization)
            opportunities.push("Align audio synchronization with scene timing");
        if (!checks.brandConsistency)
            opportunities.push("Align visual style with brand guidelines");
        ...
        const viewerEngagement = Math.round((ctx.creativePlan.scores.visualImpactScore +
            ctx.understanding.scores.marketingScore +
            ctx.style.scores.marketingReadinessScore) /
            3);
        const viewerRetention = Math.round((ctx.understanding.scores.storytellingScore +
        ...
        return {
            ...
            viewerEngagement,
            viewerRetention,
            marketingImpact: Math.round((ctx.understanding.scores.marketingScore +
                ctx.creativePlan.scores.marketingScore +
                ctx.style.scores.marketingReadinessScore) /
                3),
            conversionPotential: Math.round((ctx.creativePlan.scores.marketingScore +
                ctx.understanding.scores.audienceAlignmentScore +
                productionSuccessProbability) /
                3),
            renderingComplexity: Math.min(100, Math.round(25 + criticalCount * 20 + highCount * 10 + risks.length * 3)),
            improvementOpportunities: opportunities,
        };
    }
    buildPlatformQuality(ctx, targetPlatform) {
        ...
        return platforms.map((platform) => {
            let readiness = base;
            if ((platform === VideoQualityPredictionPlatform.TikTok ||
    ...
    platformRuleNote(productionPlan, platform) {
    ...
    buildRecommendations(ctx, checks, risks, predictions) {
        const recs = [];
    ...
    highestRiskLevel(risks) {
        const order = ["critical", "high", "medium", "low"];
        for (const level of order) {
            if (risks.some((r) => !r.resolved && r.severity === level))
                return level;
        }
        return "low";
    }
}
//# sourceMappingURL=video-quality-prediction-analyzer.js.map