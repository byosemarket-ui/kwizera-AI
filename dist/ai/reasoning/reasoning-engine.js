...
import { ReasoningEngineError, ReasoningStatus, ReasoningStep, } from "./types.js";
/**
...
export class AiReasoningEngine {
    logger = new ReasoningLogger();
    history = new ReasoningHistoryStore();
    contextAnalyzer = new ContextAnalyzer();
    approachGenerator = new ApproachGenerator();
    approachComparator = new ApproachComparator();
    confidenceCalculator = new ConfidenceCalculator();
    riskEvaluator = new RiskEvaluator();
    missingDetector = new MissingInformationDetector();
    ...
    async reason(request) {
        ...
        try {
            ...
            const memoryResult = await this.memoryProvider.search(request.userObjective, request.inputs);
            // Step 5 — Search Knowledge
            stepsCompleted.push(ReasoningStep.SearchKnowledge);
            const knowledgeResult = await this.knowledgeProvider.search(request.userObjective, request.inputs);
            ...
            contextAnalysis.factors.push(`memory:${memoryResult.found}`, `knowledge:${knowledgeResult.found}`, `inputs:${inputKeys.join(",") || "none"}`);
            ...
            const confidence = this.confidenceCalculator.calculate(contextAnalysis, approaches, missingInformation);
    ...
    buildStatusReport() {
        ...
        const averageReasoningMs = total > 0
            ? Math.round(this.reasoningDurations.reduce((a, b) => a + b, 0) / total)
            : 0;
        const complete = this.history
            .getAll()
            .filter((r) => r.confidenceScore >= 45);
        const accuracy = this.history.getCount() > 0
            ? Math.round((complete.length / this.history.getCount()) * 100)
            : 100;
        const avgConfidence = this.history.getCount() > 0
            ? Math.round(this.history.getAll().reduce((sum, r) => sum + r.confidenceScore, 0) /
                this.history.getCount())
            : 0;
        const confidenceQuality = avgConfidence >= 75 ? "high" : avgConfidence >= 50 ? "medium" : "low";
        const knownIssues = [];
        if (!this.initialized) {
    ...
    buildImprovements(missing, risk, context) {
        const improvements = [];
