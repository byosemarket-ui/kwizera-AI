export class ProductUnderstandingScorer {
    computeScores(purpose, customer, value, unique, marketing) {
        let understandingScore = 60;
        if (purpose.primaryPurpose.length >= 20)
            understandingScore += 10;
        if (purpose.secondaryFunctions.length >= 1)
            understandingScore += 5;
        if (customer.customerNeeds.length >= 3)
            understandingScore += 10;
        if (customer.customerPainPoints.length >= 2)
            understandingScore += 5;
        if (unique.uniqueSellingPoints.length >= 2)
            understandingScore += 10;
        understandingScore = Math.min(100, understandingScore);
        const businessValueScore = Math.round((value.commercialValue + value.marketValue + value.brandValue) / 3);
        const customerValueScore = Math.round((value.functionalValue + value.emotionalValue + value.practicalValue) / 3);
        const readyCount = [
        ...
        const aiConfidenceScore = Math.round((understandingScore + businessValueScore + customerValueScore) / 3);
        return {
            understandingScore,
            businessValueScore,
            customerValueScore,
...
//# sourceMappingURL=product-understanding-scorer.js.map