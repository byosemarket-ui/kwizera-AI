...
import { CinematicStyleClass, StyleTemplatePlatform, VideoStyleIntelligenceEngineError, } from "./types.js";
/**
...
export class AiVideoStyleIntelligenceEngine {
    ...
    logger = new VideoStyleLogger();
    records = new VideoStyleRecordStore();
    templateLibrary = new VideoStyleTemplateLibrary();
    analyzer = new VideoStyleAnalyzer();
    scorer = new VideoStyleScorer();
    linker = new VideoStyleLinker();
    processor = null;
    analysisTimes = [];
    searchTimes = [];
    initialize(foundation, storageRoot) {
        ...
        this.processor = new VideoStyleProcessor(foundation, this.analyzer, this.scorer, this.linker, this.records, this.logger);
    ...
    getStyleAnalysis(videoId) {
        this.ensureReady();
        return this.records.get(videoId) ?? null;
    }
    searchStyleAnalysis(query) {
        this.ensureReady();
    ...
    async repairStyleAnalysis(videoId) {
        this.ensureReady();
    ...
    buildStatusReport() {
        ...
        if (!this.startupComplete)
            readinessScore -= 25;
        if (!this.foundation?.getVideoUnderstandingEngine().isStartupComplete())
            readinessScore -= 10;
        ...
        return {
            ...
            styleAnalysisStatus: "color grading, lighting, composition, camera, motion, background, typography, graphics, visual identity",
            editingStyleStatus: "rhythm, pacing, transitions, cuts, effects, animation, captions, audio sync",
            cinematicClassificationStatus: "commercial, documentary, corporate, luxury, modern, minimal, technology, fashion, social media",
    ...
    ensureReady() {
        if (!this.initialized || !this.foundation || !this.processor) {
            throw new VideoStyleIntelligenceEngineError("Video Style Intelligence Engine not initialized", "NOT_INITIALIZED");
        }
    }
}
export { CinematicStyleClass, StyleTemplatePlatform };
//# sourceMappingURL=video-style-intelligence-engine.js.map