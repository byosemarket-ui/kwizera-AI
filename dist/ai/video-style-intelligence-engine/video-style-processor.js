...
export class VideoStyleProcessor {
    ...
    async analyze(input) {
        ...
        const built = this.analyzer.analyze(analysis, sceneDetection, timeline, camera, motion, understanding, input.industry);
        const cinematicBase = camera?.scores.cinematicScore ??
            motion?.scores.cinematicMotionScore ??
            understanding?.scores.storytellingScore ??
            65;
        const marketingBase = understanding?.scores.marketingScore ?? analysis.scores.productionReadinessScore;
        const templateMatch = built.templates[0]?.matchScore ?? 60;
        const scores = this.scorer.computeScores(built.visualStyle, built.editingStyle, built.brandStyle, cinematicBase, marketingBase, templateMatch);
        const validation = this.scorer.isAnalysisValid(scores, built.cinematicStyles.length, built.templates.length);
        if (!validation.valid) {
        ...
        draft.relationships = this.linker.detectRelationships(draft, this.records.getAll(), analysis, timeline, camera, motion, input.relatedProjects, input.relatedKnowledge, input.relatedStoryboards, input.relatedCreativePlans);
    ...
    search(query) {
        ...
        if (query.style) {
            results = results.filter((r) => r.dominantCinematicStyle === query.style || r.cinematicStyles.includes(query.style));
        }
        if (query.industry) {
        ...
        if (query.text) {
            ...
            results = results.filter((r) => r.profile.styleName.toLowerCase().includes(q) ||
...
//# sourceMappingURL=video-style-processor.js.map