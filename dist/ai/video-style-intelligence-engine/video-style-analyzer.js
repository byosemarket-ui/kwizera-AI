...
import { CinematicStyleClass, StyleCategory, } from "./types.js";
export class VideoStyleAnalyzer {
    templateLibrary = new VideoStyleTemplateLibrary();
    analyze(analysis, sceneDetection, timeline, camera, motion, understanding, industry) {
        ...
        const templates = this.templateLibrary.matchTemplates(analysis.classification.videoType, cinematicStyles, platformHint);
        const recommendations = this.buildRecommendations(visualStyle, editingStyle, brandStyle, camera, motion, templates);
        const keywords = [
            ...analysis.keywords,
            profile.styleName,
    ...
    inferIndustry(videoType, category) {
        if (category)
            return category;
        if (videoType === VideoAnalysisType.Commercial)
            return "technology";
        if (videoType === VideoAnalysisType.SocialMedia)
            return "social-media";
        if (videoType === VideoAnalysisType.Tutorial)
            return "education";
        return "general";
    }
    analyzeVisualStyle(analysis, camera, motion, understanding) {
        ...
        return {
            ...
            backgroundStyle: analysis.frame.visualComplexity > 60 ? "rich layered environments" : "clean minimal backgrounds",
            typographyStyle: analysis.classification.videoType === VideoAnalysisType.SocialMedia
                ? "bold sans-serif social overlays"
                : "clean professional sans-serif",
            graphicStyle: understanding?.context.marketingContext?.includes("launch")
    ...
    analyzeEditingStyle(analysis, sceneDetection, timeline, motion) {
        ...
        const pacing = avgShotSec < 2 ? "fast" : avgShotSec < 4 ? "medium" : avgShotSec < 8 ? "moderate" : "slow";
        return {
            editingRhythm: shotCount > 6 ? "staccato multi-beat rhythm" : "flowing narrative rhythm",
            pacing,
            transitionStyle: sceneDetection.transitions.length > 2
            ...
            effectStyle: analysis.visual.sharpness > 85 ? "subtle polish effects" : "standard correction effects",
            animationStyle: motion?.dominantClassification === "animated-motion"
                ? "motion graphics driven"
                : "minimal kinetic typography",
            captionStyle: analysis.classification.videoType === VideoAnalysisType.SocialMedia
    ...
    classifyCinematicStyle(analysis, understanding) {
        ...
        if (sub.includes("luxury"))
            styles.add(CinematicStyleClass.Luxury);
        if (sub.includes("fashion"))
            styles.add(CinematicStyleClass.Fashion);
        if (sub.includes("food"))
            styles.add(CinematicStyleClass.Food);
        if (sub.includes("beauty"))
            styles.add(CinematicStyleClass.Beauty);
        if (sub.includes("health"))
            styles.add(CinematicStyleClass.Healthcare);
        if (sub.includes("real-estate"))
            styles.add(CinematicStyleClass.RealEstate);
    ...
    analyzeBrandStyle(analysis, understanding, visual) {
        const brandColors = analysis.visual.dominantColors.length > 0
        ...
        return {
            ...
            visualConsistency: Math.min(100, Math.round(analysis.visual.visualStability * 0.4 +
            ...
            marketingIdentity: understanding?.context.marketingContext ?? `${analysis.classification.useCase} marketing identity`,
        };
    }
    buildRecommendations(visual, editing, brand, camera, motion, templates) {
        const recs = [];
...
//# sourceMappingURL=video-style-analyzer.js.map