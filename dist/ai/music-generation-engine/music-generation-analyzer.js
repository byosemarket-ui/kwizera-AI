import { LoopType, MusicGenre, MusicMood, MusicPlatform, PLATFORM_MUSIC_CONFIG, SyncTarget, } from "./types.js";
const INDUSTRY_GENRE_MAP = {
...
export class MusicGenerationAnalyzer {
    ...
    buildCompositionPlan(analysis, context) {
    ...
    buildArrangementPlan(analysis) {
        const active = GENRE_ARRANGEMENT[analysis.genre] ?? ["piano", "synth", "bass", "percussion"];
        const inactive = (key) => (active.includes(key) ? "active" : "optional");
        return {
    ...
    buildMoodPlan(input, analysis, context) {
        const brand = context.brandName ?? "brand";
        return {
    ...
    buildProductionInstructions(profile, analysis, arrangement) {
        return {
            renderNotes: [
    ...
    buildRecommendations(analysis, moodPlan, context) {
        const recs = [];
    ...
    resolvePlatform(input, context) {
        ...
        if (context.creative?.profile.platform) {
            ...
            if (p.includes("tiktok"))
                return MusicPlatform.TikTok;
            if (p.includes("instagram"))
                return MusicPlatform.Instagram;
            if (p.includes("facebook"))
                return MusicPlatform.Facebook;
            if (p.includes("mobile"))
                return MusicPlatform.Mobile;
            if (p.includes("tv") || p.includes("television"))
                return MusicPlatform.Television;
        }
    ...
    detectGenre(context) {
        ...
        if (prompt.includes("cinematic") || prompt.includes("epic"))
            return MusicGenre.Cinematic;
        if (prompt.includes("afrobeat"))
            return MusicGenre.Afrobeat;
        if (prompt.includes("jazz"))
            return MusicGenre.Jazz;
        if (prompt.includes("lo-fi") || prompt.includes("lofi"))
    ...
    detectMood(context) {
        const industry = context.industry?.toLowerCase() ?? "default";
        const prompt = (context.musicPrompt ?? "").toLowerCase();
        if (prompt.includes("calm") || prompt.includes("relax"))
            return MusicMood.Calm;
        if (prompt.includes("epic") || prompt.includes("dramatic"))
            return MusicMood.Epic;
        if (prompt.includes("happy") || prompt.includes("upbeat"))
    ...
    detectEnergy(mood, genre) {
        if ([MusicMood.Energetic, MusicMood.Epic, MusicMood.Happy].includes(mood))
            return "high";
        if ([MusicMood.Calm, MusicMood.Relaxing].includes(mood))
            return "low";
        if (genre === MusicGenre.EDM || genre === MusicGenre.Rock)
            return "high";
        if (genre === MusicGenre.Ambient || genre === MusicGenre.LoFi)
            return "low";
        return "medium";
    }
    resolveDuration(platform, context) {
        if (platform && PLATFORM_MUSIC_CONFIG[platform]) {
            return Math.min(PLATFORM_MUSIC_CONFIG[platform].maxDurationSec, 180);
        }
        return 60;
    }
    buildChordProgression(analysis) {
...
//# sourceMappingURL=music-generation-analyzer.js.map