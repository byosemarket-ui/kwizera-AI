...
import { MusicGenerationEngineError, } from "./types.js";
/**
...
export class AiMusicGenerationEngine {
    ...
    logger = new MusicGenerationLogger();
    records = new MusicGenerationRecordStore();
    analyzer = new MusicGenerationAnalyzer();
    scorer = new MusicGenerationScorer();
    linker = new MusicGenerationLinker();
    processor = null;
    generationTimes = [];
    searchTimes = [];
    blueprintTimes = [];
    initialize(foundation, storageRoot) {
        ...
        this.processor = new MusicGenerationProcessor(foundation, this.analyzer, this.scorer, this.linker, this.records, this.logger);
    ...
    async repairMusicPlan(productId, platform) {
        this.ensureReady();
    ...
    buildStatusReport() {
        ...
        const avgComposition = all.length > 0 ? Math.round(all.reduce((s, r) => s + r.scores.compositionScore, 0) / all.length) : 0;
...
//# sourceMappingURL=music-generation-engine.js.map