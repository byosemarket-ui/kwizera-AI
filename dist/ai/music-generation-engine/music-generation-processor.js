...
export class MusicGenerationProcessor {
    ...
    async generateMusicPlan(input) {
        ...
        try {
            ...
            const productionInstructions = this.analyzer.buildProductionInstructions(profile, musicAnalysis, arrangementPlan);
            const recommendations = this.analyzer.buildRecommendations(musicAnalysis, moodPlan, resolvedContext);
            const scores = this.scorer.computeScores(musicAnalysis, compositionPlan, arrangementPlan, moodPlan, syncPreparation, productionInstructions, resolvedContext);
            ...
            if (!validation.valid) {
                const repaired = this.applySafeRepairs(compositionPlan, arrangementPlan, moodPlan, syncPreparation, validation.diagnostics);
    ...
    search(query) {
        ...
        if (query.musicPlanId)
            results = results.filter((r) => r.musicPlanId === query.musicPlanId);
        if (query.productId)
        ...
        if (query.genre)
            results = results.filter((r) => r.profile.genre === query.genre);
        if (query.mood)
            results = results.filter((r) => r.profile.mood === query.mood);
        if (query.platform)
        ...
        if (query.keywords) {
            ...
            results = results.filter((r) => r.musicAnalysis.keywords.some((k) => k.includes(kw)) ||
                r.moodPlan.brandMoodAlignment.toLowerCase().includes(kw));
        }
        if (query.text) {
            const textLower = query.text.toLowerCase();
            results = results.filter((r) => r.musicPlanId.toLowerCase().includes(textLower) ||
    ...
    applySafeRepairs(composition, arrangement, moodPlan, syncPlan, diagnostics) {
        const repairs = [];
        if (diagnostics.some((d) => d.includes("Composition structure"))) {
            if (!composition.intro)
                composition.intro = "8-bar intro";
            if (!composition.chorus)
                composition.chorus = "16-bar chorus";
...
//# sourceMappingURL=music-generation-processor.js.map