...
export class VideoIntelligenceOptimizationAnalyzer {
    buildProfile(input, qualityPrediction, productionPlan, version) {
        const optimizationId = input.optimizationId ?? `video-optimization-${input.videoId}-${qualityPrediction.profile.platform}`;
        return {
            optimizationId,
    ...
    collectBaselineMetrics(foundation) {
        const metrics = {};
    ...
    createRecoveryPoint(optimizationId, baseline, cache) {
        return {
    ...
    analyzeModuleOptimizations(foundation, baseline) {
        return MODULE_DEFS.map((mod) => {
            const before = baseline[mod.moduleId] ?? mod.getScore(foundation);
            const strategies = ["cache", "metadata", "performance"];
            if (mod.moduleId.includes("planning") ||
            ...
            const improvement = before >= 98 ? 0 : Math.min(8, Math.max(2, Math.round((100 - before) / 15) + 2));
    ...
    buildStrategies(moduleResults) {
        return {
            relationshipOptimization: "Consolidated cross-module video relationship indexes for faster dependency resolution",
            timelineOptimization: "Optimized timeline segment indexing and synchronization metadata for scene transitions",
            ...
            recommendationOptimization: "Pre-computed recommendation paths from quality prediction and creative intelligence",
            workflowOptimization: "Optimized video production workflow checkpoint ordering without altering module responsibilities",
            performanceOptimization: "Reduced redundant lookups via warm cache for frequently accessed video records",
            cacheOptimization: `Warmed cache for ${moduleResults.length} video intelligence module contexts`,
            metadataOptimization: "Indexed video, scene, timeline, brand, product, platform and campaign metadata for fast retrieval",
        };
    }
    buildCacheOptimization(foundation, videoId, existingCache) {
        ...
        const brand = creativePlan?.profile.brand ?? qualityPrediction?.profile.brand ?? analysis?.relationships.relatedBrands[0];
        const product = creativePlan?.profile.product ?? qualityPrediction?.profile.product ?? analysis?.relationships.relatedProducts[0];
        const campaign = creativePlan?.profile.campaign ?? qualityPrediction?.profile.campaign;
        const cache = {
            videos: merge(existingCache.videos, [videoId]),
            scenes: merge(existingCache.scenes, sceneDetection ? sceneDetection.scenes.map((s) => s.sceneId) : []),
            timelines: merge(existingCache.timelines, timeline ? [timeline.timelineId] : []),
            storyboards: merge(existingCache.storyboards, creativePlan ? [creativePlan.profile.creativeVideoId, ...creativePlan.relationships.relatedStoryboards] : []),
            brands: merge(existingCache.brands, brand ? [brand] : []),
            products: merge(existingCache.products, product ? [product] : []),
            templates: merge(existingCache.templates, creativePlan?.templates.map((t) => t.templateId) ?? []),
            campaigns: merge(existingCache.campaigns, campaign ? [campaign] : []),
            productionPlans: merge(existingCache.productionPlans, productionPlan ? [productionPlan.profile.productionPlanId] : []),
    ...
    measurePerformance(foundation, baseline) {
        ...
        let planningAfter = planningBefore > 0 ? Math.max(1, Math.round(planningBefore * 0.85)) : 25;
    ...
    validateQualityPreserved(moduleResults) {
        const issues = [];
...
//# sourceMappingURL=video-intelligence-optimization-analyzer.js.map