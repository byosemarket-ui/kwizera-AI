export class VideoIntelligenceOptimizationScorer {
    computeScores(moduleResults, performance, qualityPrediction) {
        ...
        const planningImprovement = performance.planningSpeedBeforeMs > 0
            ? Math.min(100, Math.round(((performance.planningSpeedBeforeMs - performance.planningSpeedMs) /
                performance.planningSpeedBeforeMs) *
                100))
            : 15;
        const searchImprovement = performance.searchSpeedBeforeMs > 0
            ? Math.min(100, Math.round(((performance.searchSpeedBeforeMs - performance.searchSpeedMs) / performance.searchSpeedBeforeMs) *
                100))
            : 20;
        const analysisImprovement = performance.analysisSpeedBeforeMs > 0
            ? Math.min(100, Math.round(((performance.analysisSpeedBeforeMs - performance.analysisSpeedMs) /
                performance.analysisSpeedBeforeMs) *
                100))
            : 12;
        ...
        const storytellingImprovement = Math.min(100, Math.max(5, Math.round((qualityPrediction.scores.storytellingScore / 10) + avgImprovement)));
        const videoQualityImprovement = Math.min(100, Math.max(5, Math.round(qualityPrediction.scores.overallVideoQualityScore / 10 + avgImprovement)));
        const renderingReadinessScore = Math.min(100, Math.max(5, qualityPrediction.scores.productionReadinessScore + Math.round(avgImprovement / 2)));
        const confidenceImprovement = Math.min(100, Math.max(5, avgImprovement + Math.round((planningImprovement + searchImprovement + analysisImprovement) / 4)));
        const overallImprovementScore = Math.max(5, Math.min(100, Math.round((avgImprovement +
            planningImprovement +
            searchImprovement +
            recommendationImprovement +
            storytellingImprovement) /
            5)));
        ...
        return {
            ...
            videoQualityImprovementScore: videoQualityImprovement,
            storytellingImprovementScore: storytellingImprovement,
...
//# sourceMappingURL=video-intelligence-optimization-scorer.js.map