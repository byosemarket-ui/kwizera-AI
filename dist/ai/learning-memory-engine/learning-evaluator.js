import { LearningOutcome, LearningSource } from "./types.js";
...
export class LearningEvaluator {
    ...
    estimateQuality(input) {
        let score = 60;
        if (input.description.length > 50)
            score += 10;
        if (input.relatedProject)
            score += 10;
        if (input.outcome === LearningOutcome.Success)
            score += 15;
        if (input.outcome === LearningOutcome.Failure && input.lessonLearned)
            score += 10;
        if (input.userFeedback)
            score += 5;
        return Math.min(100, score);
    }
    computeLearningValue(input, quality) {
        ...
        if (highValueSources.includes(input.source))
            value += 20;
        if (input.outcome === LearningOutcome.Success)
            value += 10;
        if (input.patterns?.length)
            value += input.patterns.length * 5;
        return Math.min(100, Math.round(value));
    }
}
//# sourceMappingURL=learning-evaluator.js.map