...
import { CreativeImageIntelligenceEngineError, } from "./types.js";
/**
...
export class AiCreativeImageIntelligenceEngine {
    ...
    logger = new CreativeImageLogger();
    records = new CreativeImageIntelligenceRecordStore();
    analyzer = new CreativeImageAnalyzer();
    scorer = new CreativeImageScorer();
    linker = new CreativeImageLinker();
    processor = null;
    planningTimes = [];
    searchTimes = [];
    relationshipTimes = [];
    initialize(foundation, storageRoot) {
        ...
        this.processor = new CreativeImageProcessor(foundation, this.analyzer, this.scorer, this.linker, this.records, this.logger);
    ...
    detectRelationships(imageId) {
        ...
        if (!analysis || !understanding || !composition || !brandVisual)
            return record.relationships;
        const updated = this.linker.detectRelationships(record, this.records.getAll(), analysis, understanding, composition, brandVisual, enhancementPlan, record.relationships.relatedProjects, record.relationships.relatedKnowledge);
        this.relationshipTimes.push(Date.now() - start);
        return updated;
    }
    async repairCreativePlan(imageId) {
        this.ensureReady();
    ...
    buildStatusReport() {
        ...
        if (!this.foundation?.getImageUnderstandingEngine().isStartupComplete())
            readinessScore -= 8;
        if (!this.foundation?.getCompositionIntelligenceEngine().isStartupComplete())
            readinessScore -= 8;
        if (!this.foundation?.getBrandVisualIntelligenceEngine().isStartupComplete())
            readinessScore -= 8;
        return {
            engineStatus: this.startupComplete ? "operational" : "initializing",
            creativePlanningStatus: "poster, advertisement, thumbnail, banner, social and branding layouts planned",
            layoutPlanningStatus: "visual hierarchy, product, logo, headline, CTA and safe area planning prepared",
    ...
    ensureReady() {
        if (!this.initialized || !this.foundation || !this.processor) {
            throw new CreativeImageIntelligenceEngineError("Creative Image Intelligence Engine not initialized", "NOT_INITIALIZED");
        }
    }
}
//# sourceMappingURL=creative-image-intelligence-engine.js.map