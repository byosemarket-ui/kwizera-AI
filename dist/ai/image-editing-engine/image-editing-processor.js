...
export class ImageEditingProcessor {
    ...
    async generateEditingPlan(input) {
        ...
        try {
            ...
            if (!context) {
                return this.reject(start, "Unable to resolve editing context — provide sourceImageId, productId with pipeline, productImagePlanId, or backgroundPlanId", ["Source image or product intelligence pipeline required"]);
            }
            ...
            const productionInstructions = this.analyzer.buildProductionInstructions(profile, editingOperations, maskManagement);
            const recommendations = this.analyzer.buildRecommendations(context, imageAnalysis);
            const scores = this.scorer.computeScores(imageAnalysis, editingOperations, inpaintingPlan, outpaintingPlan, identityPreservation, maskManagement, qualityImprovement, nonDestructiveEditing, platformOptimizations, context);
            ...
            if (!validation.valid) {
                const repaired = this.applySafeRepairs(inpaintingPlan, editingOperations, identityPreservation, nonDestructiveEditing, validation.diagnostics);
            ...
            const draft = {
                ...
                productionReady,
                brandConsistent,
                relationships: this.linker.detectRelationships(draftPartial, input, context.productImagePlan, context.backgroundPlan, context.creative, context.strategy, context.understanding),
            };
    ...
    search(query) {
        ...
        if (query.imageEditingPlanId)
            results = results.filter((r) => r.imageEditingPlanId === query.imageEditingPlanId);
        ...
        if (query.primaryOperation)
            results = results.filter((r) => r.profile.primaryOperation === query.primaryOperation);
        if (query.inpaintingType)
            results = results.filter((r) => r.profile.inpaintingType === query.inpaintingType);
        if (query.outpaintingType)
            results = results.filter((r) => r.profile.outpaintingType === query.outpaintingType);
        if (query.keywords) {
            const kw = query.keywords.toLowerCase();
            results = results.filter((r) => r.editingOperations.operationPrompts[r.profile.primaryOperation]?.toLowerCase().includes(kw) ||
                r.imageAnalysis.subject.toLowerCase().includes(kw) ||
                r.profile.productId.toLowerCase().includes(kw));
        }
        if (query.text) {
            const textLower = query.text.toLowerCase();
            results = results.filter((r) => r.imageEditingPlanId.toLowerCase().includes(textLower) ||
    ...
    async resolveContext(input) {
        ...
        if (input.productId && productFoundation) {
            ...
            if (analysis || understanding || productImagePlan || backgroundPlan) {
                return this.analyzer.extractContextFromProduct(analysis, understanding, creative, strategy, input, productImagePlan, backgroundPlan);
            }
        }
        if (input.sourceImageId || productImagePlan || backgroundPlan) {
            return this.analyzer.extractContextFromProduct(null, null, null, null, input, productImagePlan, backgroundPlan);
        }
        return null;
    }
    ...
    applySafeRepairs(inpainting, operations, preservation, nonDestructive, diagnostics) {
        const repairs = [];
...
//# sourceMappingURL=image-editing-processor.js.map