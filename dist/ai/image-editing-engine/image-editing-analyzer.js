import { ALL_IMAGE_EDIT_GEN_PLATFORMS, ALL_IMAGE_EDIT_IDENTITY_TARGETS, ALL_IMAGE_EDIT_MASK_TYPES, ImageEditGenPlatform, ImageEditIdentityTarget, ImageEditInpaintingType, ImageEditMaskType, ImageEditOperationType, ImageEditOutpaintingType, IMAGE_EDIT_PLATFORM_CONFIG, } from "./types.js";
const INDUSTRY_OPERATION_MAP = {
...
export class ImageEditingAnalyzer {
    ...
    buildProfile(input, platform, version, context, sourceImageId) {
        ...
        const primaryOperation = input.primaryOperation ??
            input.operations?.[0] ??
    ...
    buildInpaintingPlan(input, profile) {
    ...
    buildOutpaintingPlan(input, profile) {
    ...
    buildMaskManagement(input, context) {
    ...
    buildNonDestructiveEditing(profile, existing) {
    ...
    buildProductionInstructions(profile, operations, maskManagement) {
        return {
    ...
    resolveSourceImageId(input, context) {
        ...
        if (input.backgroundPlanId)
            return input.backgroundPlanId;
        if (context.productImagePlan)
            return context.productImagePlan.productImagePlanId;
        if (context.backgroundPlan)
            return context.backgroundPlan.profile.sourceImageId;
        return null;
    }
    extractContextFromProduct(analysis, understanding, creative, strategy, input, productImagePlan, backgroundPlan) {
    ...
    identityNote(target, subject, brand) {
        const map = {
...
//# sourceMappingURL=image-editing-analyzer.js.map