export class StoryboardScorer {
    computeScores(scenes, storyFlow, timing, continuity, creative, strategy) {
        ...
        const aiConfidenceScore = Math.round((storyboardQualityScore + storytellingScore + visualPlanningScore + marketingScore + brandConsistencyScore) / 5);
        return {
    ...
    isStoryboardValid(scores, scenes, continuity) {
        const diagnostics = [];
        if (scenes.length < 4)
            diagnostics.push("Insufficient scenes for production-ready storyboard (minimum 4)");
        if (!continuity.storyConsistency)
            diagnostics.push("Story flow sequence is not logically connected");
        if (!continuity.sceneConsistency)
            diagnostics.push("Scene numbering inconsistent");
        ...
        if (!hasHook)
            diagnostics.push("Hook scene missing");
        if (!hasCta)
            diagnostics.push("CTA scene missing");
        return { valid: diagnostics.length === 0, diagnostics };
    }
    isProductionReady(scenes, continuity, scores) {
    ...
    computeStoryboardQuality(scenes, continuity) {
        let score = 50;
        if (scenes.length >= 5)
            score += 15;
        if (scenes.every((s) => s.visualObjective.length >= 15))
            score += 10;
        if (continuity.issues.length === 0)
            score += 15;
        if (continuity.recommendations.length <= 1)
            score += 10;
        return Math.min(100, score);
    }
    computeStorytellingScore(storyFlow, scenes) {
        let score = 45;
        const flowFields = Object.values(storyFlow).filter((v) => v && !v.startsWith("N/A"));
        score += Math.min(30, flowFields.length * 3);
        if (scenes.some((s) => s.scenePurpose === "hook"))
            score += 10;
        if (scenes.some((s) => s.scenePurpose === "ending"))
            score += 10;
        return Math.min(100, score);
    }
    computeVisualPlanningScore(scenes, creative) {
        let score = 40;
        if (scenes.every((s) => s.cameraDirection && s.lightingDirection))
            score += 20;
        if (scenes.every((s) => s.composition && s.backgroundStyle))
            score += 20;
        if (creative.visualDirection.colorPalette.length >= 3)
            score += 10;
        if (scenes.every((s) => s.transitionIn && s.transitionOut))
            score += 10;
        return Math.min(100, score);
    }
    computeBrandScore(continuity, creative) {
        let score = 40;
        if (continuity.brandConsistency)
            score += 25;
        if (continuity.creativeConsistency)
            score += 15;
        if (creative.brandDirection.brandColors.length >= 2)
            score += 10;
        if (creative.brandDirection.brandConsistency.length >= 20)
            score += 10;
        return Math.min(100, score);
    }
}
//# sourceMappingURL=storyboard-scorer.js.map