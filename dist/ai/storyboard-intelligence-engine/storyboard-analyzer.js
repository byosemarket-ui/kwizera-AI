...
export class StoryboardAnalyzer {
    buildProfile(input, creative, version) {
    ...
    buildStoryFlow(creative, understanding, strategy, includeSocialProof) {
    ...
    buildScenes(profile, storyFlow, creative, understanding, includeSocialProof) {
    ...
    checkContinuity(scenes, creative, understanding) {
        const issues = [];
        ...
        if (!sceneConsistent)
            issues.push("Scene numbering sequence broken");
        const brandConsistent = scenes.every((s) => s.backgroundStyle === scenes[0].backgroundStyle ||
        ...
        const productConsistent = productScenes.length >= 3 && productScenes.every((s) => s.productFocus.includes(product));
    ...
    selectScenesForPlatform(blueprints, targetCount) {
        ...
        for (const bp of optional) {
            if (selected.length >= targetCount)
                break;
            if (!selected.some((s) => s.purpose === bp.purpose))
                selected.push(bp);
        }
        return selected
    ...
    productFocusText(level, product) {
        switch (level) {
            case "none":
    ...
    cameraForBeat(beat, creative) {
        const base = creative.cinematicDirection.cameraStyle;
        const movement = creative.cinematicDirection.cameraMovement;
        if (beat === "attention" || beat === "curiosity")
            return `dynamic close-up — ${movement}`;
        if (beat === "action")
            return `direct address — ${base}`;
        return `${base} — ${creative.cinematicDirection.framingStyle}`;
    }
}
//# sourceMappingURL=storyboard-analyzer.js.map