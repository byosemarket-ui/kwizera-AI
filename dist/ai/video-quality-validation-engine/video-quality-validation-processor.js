...
export class VideoQualityValidationProcessor {
    ...
    search(query) {
        ...
        if (query.validationId)
            results = results.filter((r) => r.validationId === query.validationId);
        ...
        if (query.validation) {
            ...
            results = results.filter((r) => r.videoQuality.sceneContinuity.toLowerCase().includes(v) ||
        ...
        if (query.text) {
            ...
            results = results.filter((r) => r.validationId.toLowerCase().includes(textLower) ||
    ...
    markIssuesRepaired(draft, category, maxSeverity = QualityIssueSeverity.Medium) {
...
//# sourceMappingURL=video-quality-validation-processor.js.map