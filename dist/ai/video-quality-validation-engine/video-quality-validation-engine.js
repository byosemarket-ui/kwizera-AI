...
import { VideoQualityValidationEngineError, } from "./types.js";
/**
...
export class AiVideoQualityValidationEngine {
    ...
    logger = new VideoQualityValidationLogger();
    records = new QualityValidationRecordStore();
    analyzer = new VideoQualityValidationAnalyzer();
    scorer = new VideoQualityValidationScorer();
    linker = new VideoQualityValidationLinker();
    processor = null;
    validationTimes = [];
    searchTimes = [];
    repairTimes = [];
    initialize(foundation, storageRoot) {
        ...
        this.processor = new VideoQualityValidationProcessor(foundation, this.analyzer, this.scorer, this.linker, this.records, this.logger);
    ...
    async validateVideoQuality(input) {
        this.ensureReady();
    ...
    getValidationRecord(validationId) {
        this.ensureReady();
        return this.records.get(validationId) ?? null;
    }
    getValidationsByStoryboard(storyboardId) {
        this.ensureReady();
    ...
    async repairValidation(storyboardId, platform) {
        this.ensureReady();
    ...
    buildStatusReport() {
        ...
        if (!this.startupComplete)
            readinessScore -= 25;
        if (!this.foundation?.getRenderingPreparationEngine().isStartupComplete())
            readinessScore -= 10;
...
//# sourceMappingURL=video-quality-validation-engine.js.map