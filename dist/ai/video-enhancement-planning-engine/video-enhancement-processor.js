...
export class VideoEnhancementProcessor {
    ...
    async planEnhancement(input) {
        ...
        const built = this.analyzer.analyze(analysis, sceneDetection, timeline, camera, motion, style, understanding, input.projectId, input.platform);
        ...
        const scores = this.scorer.computeScores(built.qualityAnalysis, built.recommendations.length, built.platformOptimizations.length, productionBase, styleConsistency);
        const nonDestructiveValid = built.nonDestructive.preserveOriginal &&
            built.nonDestructive.supportsUndo &&
            built.nonDestructive.supportsRedo &&
            built.nonDestructive.supportsRecovery;
        const validation = this.scorer.isPlanValid(scores, built.recommendations.length, built.platformOptimizations.length, nonDestructiveValid);
        if (!validation.valid) {
        ...
        draft.relationships = this.linker.detectRelationships(draft, this.records.getAll(), analysis, sceneDetection, timeline, camera, motion, style, input.relatedProjects, input.relatedKnowledge);
    ...
    search(query) {
        ...
        if (query.enhancementType) {
            results = results.filter((r) => r.recommendations.some((rec) => rec.category === query.enhancementType));
        }
        if (query.platform) {
        ...
        if (query.text) {
            ...
            results = results.filter((r) => r.profile.enhancementPlanId.toLowerCase().includes(q) ||
...
//# sourceMappingURL=video-enhancement-processor.js.map