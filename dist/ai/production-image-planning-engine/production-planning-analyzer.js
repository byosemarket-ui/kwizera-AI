import { ProductionImagePlatform, } from "./types.js";
export class ProductionPlanningAnalyzer {
    ...
    inferPlatform(creativePlatform) {
        if (creativePlatform.includes("instagram"))
            return ProductionImagePlatform.Instagram;
        if (creativePlatform.includes("facebook"))
            return ProductionImagePlatform.Facebook;
        if (creativePlatform.includes("tiktok"))
            return ProductionImagePlatform.TikTok;
        if (creativePlatform.includes("youtube"))
            return ProductionImagePlatform.YouTube;
        if (creativePlatform.includes("whatsapp"))
            return ProductionImagePlatform.WhatsApp;
    ...
    buildAssetInventory(ctx) {
        ...
        return {
            originalImages: [
                this.assetItem("original", analysis.imageId, analysis.technical.filePath, hasOriginal, hasOriginal ? "Original image source validated" : "Original image path missing"),
            ],
            enhancedImages: [
                this.assetItem("enhanced", enhancementPlan.profile.enhancementPlanId, "enhancement-plan", enhancementPlan.validated, "Enhanced variant planned — source preserved"),
            ],
            logos: analysis.content.logos.map((logo, i) => this.assetItem("logo", `logo-${i}`, logo, true, brandVisual.logoAnalysis.logoPosition)),
            fonts: [
                this.assetItem("font-primary", brandVisual.typography.primaryFont, "brand-visual", Boolean(brandVisual.typography.primaryFont), brandVisual.typography.headingStyle),
                this.assetItem("font-secondary", brandVisual.typography.secondaryFont, "brand-visual", Boolean(brandVisual.typography.secondaryFont), brandVisual.typography.bodyStyle),
            ],
            icons: [
                this.assetItem("icon-style", brandVisual.profile.iconStyle, "brand-profile", true, "Brand icon style defined"),
            ],
            backgrounds: [
                this.assetItem("background", ctx.background.backgroundId, analysis.content.background ?? "detected", ctx.background.validated, ctx.background.replacementPlan.replacementStrategy),
            ],
            templates: [
                this.assetItem("template", creativePlan.profile.creativeImageId, "creative-plan", creativePlan.productionReady, creativePlan.layoutPlanning.layoutType),
            ],
            graphicElements: [
                this.assetItem("graphic", brandVisual.profile.graphicStyle, "brand-visual", true, brandVisual.planning.visualStylePlan),
            ],
            qrCodes: [
                this.assetItem("qr", `qr-${analysis.imageId}`, "planned", false, creativePlan.layoutPlanning.qrCodePlacement),
            ],
            ctaAssets: [
                this.assetItem("cta", `cta-${analysis.imageId}`, "creative-plan", creativePlan.layoutPlanning.ctaPlacement.length > 0, creativePlan.productionInstructions.ctaGuidance),
            ],
            brandAssets: [
