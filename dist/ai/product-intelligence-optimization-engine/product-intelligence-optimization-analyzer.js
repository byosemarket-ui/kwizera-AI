...
export class ProductIntelligenceOptimizationAnalyzer {
    ...
    buildCacheOptimization(foundation, productId, existingCache) {
        ...
        const cache = {
            ...
            creativeStyles: merge(existingCache.creativeStyles, creatives.map((c) => c.profile.creativeStyle)),
            campaignTypes: merge(existingCache.campaignTypes, strategies.map((s) => s.marketingObjective)),
            audienceProfiles: merge(existingCache.audienceProfiles, audiences.map((a) => a.profile.audienceId)),
            storyboards: merge(existingCache.storyboards, storyboards.map((s) => s.storyboardId)),
            visualPlans: merge(existingCache.visualPlans, visualPlans.map((v) => v.visualPlanId)),
            audioPlans: merge(existingCache.audioPlans, audioPlans.map((a) => a.audioPlanId)),
...
//# sourceMappingURL=product-intelligence-optimization-analyzer.js.map