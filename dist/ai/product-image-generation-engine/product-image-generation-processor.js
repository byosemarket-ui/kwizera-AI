...
export class ProductImageGenerationProcessor {
    ...
    async generateProductImagePlan(input) {
        ...
        try {
            ...
            const productionInstructions = this.analyzer.buildProductionInstructions(profile, presentationPlan, photographyPlan, lightingPlan);
            const recommendations = this.analyzer.buildRecommendations(context, consistencyPlan);
            const scores = this.scorer.computeScores(presentationPlan, photographyPlan, backgroundPlan, lightingPlan, consistencyPlan, marketingVariations, platformOptimizations, context);
    ...
    applySafeRepairs(lightingPlan, backgroundPlan, consistencyPlan, diagnostics) {
        const repairs = [];
...
//# sourceMappingURL=product-image-generation-processor.js.map