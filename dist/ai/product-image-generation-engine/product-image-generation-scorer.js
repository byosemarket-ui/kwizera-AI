export class ProductImageGenerationScorer {
    computeScores(presentationPlan, photographyPlan, backgroundPlan, lightingPlan, consistencyPlan, marketingVariations, platformOptimizations, context) {
        ...
        const marketplaceReadinessScore = this.computeMarketplaceReadiness(platformOptimizations, marketingVariations, consistencyPlan);
        const productionReadinessScore = this.computeProductionReadiness(presentationPlan, photographyPlan, lightingPlan, consistencyPlan);
        const aiConfidenceScore = Math.round((productPresentationScore +
            photographyScore +
    ...
    isProductImagePlanValid(scores, record) {
        const diagnostics = [];
    ...
    isMarketplaceReady(scores, record) {
        return (scores.marketplaceReadinessScore >= 55 &&
            record.platformOptimizations.length >= 1 &&
            record.marketingVariations.length >= 4);
    }
    isBrandConsistent(context, consistencyPlan) {
        if (!context.brandName)
            return consistencyPlan.rules.length >= 4;
        return (consistencyPlan.logoPlacement.toLowerCase().includes(context.brandName.toLowerCase()) ||
            consistencyPlan.colorLock);
    }
    computePresentationScore(plan) {
        let score = 45;
        if (plan.views.length >= 10)
            score += 25;
        else if (plan.views.length >= 8)
            score += 20;
        if (plan.showcaseLayout.length >= 15)
            score += 10;
        if (plan.heroPlacement.length >= 10)
            score += 10;
        if (plan.catalogueStructure.length >= 3)
            score += 10;
        return Math.min(100, score);
    }
    computePhotographyScore(plan, lighting) {
        let score = 45;
        if (plan.modes.length >= 5)
            score += 20;
        if (plan.studioSetup.length >= 15)
            score += 10;
        if (plan.commercialStyle.length >= 10)
            score += 10;
        if (lighting.productHighlight && lighting.reflectionControl)
            score += 15;
        return Math.min(100, score);
    }
    computeBrandConsistency(context, consistency, background) {
        let score = 45;
        if (consistency.rules.length >= 6)
            score += 20;
        if (consistency.colorLock && consistency.shapeLock)
            score += 15;
        if (context.brandGuidelines)
            score += 10;
        if (context.brandName && consistency.logoPlacement.length >= 10)
            score += 10;
        if (background.colorHarmony.length >= 10)
            score += 10;
        return Math.min(100, score);
    }
    computeMarketplaceReadiness(platforms, variations, consistency) {
        let score = 45;
        if (platforms.length >= 4)
            score += 20;
        if (variations.length >= 6)
            score += 15;
        if (consistency.packagingNotes.length >= 5)
            score += 10;
        if (platforms.some((p) => p.platform === "ecommerce"))
            score += 10;
        return Math.min(100, score);
    }
    computeProductionReadiness(presentation, photography, lighting, consistency) {
        let score = 45;
        if (presentation.views.every((v) => v.description.length >= 10))
            score += 15;
        if (photography.notes.length >= 2)
            score += 10;
        if (lighting.shadowPlanning && lighting.reflectionControl)
            score += 15;
        if (consistency.shapeLock && consistency.colorLock)
            score += 15;
        return Math.min(100, score);
    }
}
//# sourceMappingURL=product-image-generation-scorer.js.map