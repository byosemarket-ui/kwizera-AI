export class SpeechToSpeechGenerationScorer {
    computeScores(speechAnalysis, voiceTransformation, emotionPreservation, pronunciationAdaptation, timingPreservation, platformOptimizations, context) {
        ...
        const productionReadinessScore = this.computeProductionReadiness(speechAnalysis, pronunciationAdaptation, timingPreservation, platformOptimizations);
    ...
    isTransformationValid(scores, record) {
        const diagnostics = [];
    ...
    computeTransformationQuality(speechAnalysis, voiceTransformation) {
        let score = 45;
        if (speechAnalysis.audioQualityScore >= 70)
            score += 15;
        if (speechAnalysis.speakerSegments.length >= 1)
            score += 15;
        if (voiceTransformation.accentAdaptation.length >= 10)
            score += 10;
        if (voiceTransformation.pitchAdaptation.length >= 10)
            score += 10;
        if (voiceTransformation.speakingRateAdaptation.length >= 10)
            score += 5;
        return Math.min(100, score);
    }
    computePronunciationScore(speechAnalysis, plan) {
        let score = 45;
        ...
        if (Object.keys(plan.pronunciationDictionary).length >= 1)
            score += 15;
        if (Object.keys(plan.namePreservation).length >= 1)
            score += 10;
        if (speechAnalysis.properNames.length > 0 && Object.keys(plan.namePreservation).length >= 1)
            score += 10;
        return Math.min(100, score);
    }
    computeEmotionPreservation(plan) {
        let score = 45;
        if (plan.emotionalArc.length >= 1)
            score += 20;
        if (plan.sceneEmotionNotes.length >= 1)
            score += 15;
        if (plan.preservationScore >= 80)
            score += 15;
        if (plan.intensityPreservation >= 50)
            score += 5;
        return Math.min(100, score);
    }
    computeTimingPreservation(plan, speechAnalysis) {
        let score = 45;
        if (plan.naturalPauses.length >= 3)
            score += 15;
        if (plan.segmentTiming.length >= speechAnalysis.speakerSegments.length)
            score += 20;
        if (plan.breathPlanning.length >= 2)
            score += 10;
        if (plan.speechTiming.length >= 10)
            score += 10;
        return Math.min(100, score);
    }
    ...
    computeProductionReadiness(speechAnalysis, pronunciationPlan, timingPlan, platformOptimizations) {
        let score = 45;
        if (speechAnalysis.durationMs > 0)
            score += 10;
        if (Object.keys(pronunciationPlan.phonemeMapping).length >= 0)
            score += 10;
        if (timingPlan.segmentTiming.length >= 1)
            score += 15;
        if (platformOptimizations.length >= 1)
            score += 15;
        if (timingPlan.breathPlanning.length >= 2)
            score += 15;
        return Math.min(100, score);
    }
}
//# sourceMappingURL=speech-to-speech-generation-scorer.js.map