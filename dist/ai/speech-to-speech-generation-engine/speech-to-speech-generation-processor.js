...
export class SpeechToSpeechGenerationProcessor {
    ...
    async generateTransformationPlan(input) {
        ...
        try {
            ...
            const productionInstructions = this.analyzer.buildProductionInstructions(profile, timingPreservation, voiceTransformation);
            const recommendations = this.analyzer.buildRecommendations(speechAnalysis, emotionPreservation, context);
            const scores = this.scorer.computeScores(speechAnalysis, voiceTransformation, emotionPreservation, pronunciationAdaptation, timingPreservation, platformOptimizations, context);
            ...
            if (!validation.valid) {
                const repaired = this.applySafeRepairs(speechAnalysis, voiceTransformation, emotionPreservation, pronunciationAdaptation, timingPreservation, validation.diagnostics);
            ...
            draft.relationships = this.linker.detectRelationships(draft, input, context.creative, context.strategy, context.understanding);
    ...
    search(query) {
        ...
        if (query.transformationId)
            results = results.filter((r) => r.transformationId === query.transformationId);
        if (query.sourceAudioId)
            results = results.filter((r) => r.profile.sourceAudioId === query.sourceAudioId);
        if (query.sourceVoiceId)
            results = results.filter((r) => r.profile.sourceVoiceId === query.sourceVoiceId);
        if (query.targetVoiceId)
            results = results.filter((r) => r.profile.targetVoiceId === query.targetVoiceId);
        if (query.productId)
        ...
        if (query.keywords) {
            ...
            results = results.filter((r) => r.speechAnalysis.keywords.some((k) => k.includes(kw)) ||
                r.voiceTransformation.brandVoiceAlignment.toLowerCase().includes(kw));
        }
        if (query.text) {
            const textLower = query.text.toLowerCase();
            results = results.filter((r) => r.transformationId.toLowerCase().includes(textLower) ||
    ...
    applySafeRepairs(speechAnalysis, voiceTransformation, emotionPreservation, pronunciationAdaptation, timingPreservation, diagnostics) {
        const repairs = [];
        if (diagnostics.some((d) => d.includes("speaker segments")) && speechAnalysis.speakerSegments.length < 1) {
            speechAnalysis.speakerSegments = [
        ...
        if (diagnostics.some((d) => d.includes("Timing preservation"))) {
            if (timingPreservation.segmentTiming.length < 1) {
                timingPreservation.segmentTiming = [
                    { segmentId: "seg-1", startMs: 0, endMs: 5000 },
                ];
...
//# sourceMappingURL=speech-to-speech-generation-processor.js.map