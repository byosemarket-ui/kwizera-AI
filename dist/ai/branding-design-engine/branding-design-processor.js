...
export class BrandingDesignProcessor {
    ...
    async generateBrandingPlan(input) {
        ...
        try {
            ...
            if (!context) {
                return this.reject(start, "Unable to resolve branding context — provide productId with pipeline, brandGuidelines, or productImagePlanId", ["Product intelligence pipeline or brand context required"]);
            }
            ...
            const scores = this.scorer.computeScores(designPlanning, logoPlanning, marketingMaterials, socialMediaDesign, printDesign, brandConsistency, colorManagement, platformOptimizations, context);
            ...
            if (!validation.valid) {
                const repaired = this.applySafeRepairs(designPlanning, logoPlanning, brandConsistency, colorManagement, validation.diagnostics);
            ...
            const draft = {
                ...
                printReady,
                brandConsistent,
                relationships: this.linker.detectRelationships(draftPartial, input, context.productImagePlan, context.enhancementPlan, context.creative, context.strategy, context.understanding),
            };
    ...
    search(query) {
        ...
        if (query.brandDesignId)
            results = results.filter((r) => r.brandDesignId === query.brandDesignId);
        if (query.brandId)
        ...
        if (query.designType)
            results = results.filter((r) => r.profile.designType === query.designType);
        if (query.templateId)
            results = results.filter((r) => r.relationships.templates.includes(query.templateId));
        if (query.keywords) {
            const kw = query.keywords.toLowerCase();
            results = results.filter((r) => r.designPlanning.layoutStructure.toLowerCase().includes(kw) ||
        ...
        if (query.text) {
            ...
            results = results.filter((r) => r.brandDesignId.toLowerCase().includes(textLower) ||
    ...
    async resolveContext(input) {
        ...
        if (input.productId && productFoundation) {
            ...
            if (analysis || understanding || productImagePlan) {
                return this.analyzer.extractContextFromProduct(analysis, understanding, creative, strategy, input, productImagePlan, enhancementPlan);
            }
        }
        if (input.brandGuidelines || input.brandName || productImagePlan) {
            return this.analyzer.extractContextFromProduct(null, null, null, null, input, productImagePlan, enhancementPlan);
        }
        return null;
    }
    ...
    applySafeRepairs(design, logo, consistency, color, diagnostics) {
        const repairs = [];
...
//# sourceMappingURL=branding-design-processor.js.map