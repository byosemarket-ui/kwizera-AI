...
import { AudioEnhancementRestorationEngineError, } from "./types.js";
/**
...
export class AiAudioEnhancementRestorationEngine {
    ...
    logger = new AudioEnhancementRestorationLogger();
    records = new AudioEnhancementRestorationRecordStore();
    analyzer = new AudioEnhancementRestorationAnalyzer();
    scorer = new AudioEnhancementRestorationScorer();
    linker = new AudioEnhancementRestorationLinker();
    processor = null;
    generationTimes = [];
    searchTimes = [];
    blueprintTimes = [];
    initialize(foundation, storageRoot) {
        ...
        this.processor = new AudioEnhancementRestorationProcessor(foundation, this.analyzer, this.scorer, this.linker, this.records, this.logger);
    ...
    getEnhancementPlansByType(enhancementType) {
        this.ensureReady();
    ...
    async repairEnhancementPlan(productId, platform) {
        this.ensureReady();
    ...
    buildStatusReport() {
        ...
        const avgClarity = all.length > 0 ? Math.round(all.reduce((s, r) => s + r.scores.audioClarityScore, 0) / all.length) : 0;
    ...
    ensureReady() {
        if (!this.initialized || !this.foundation || !this.processor) {
            throw new AudioEnhancementRestorationEngineError("Audio Enhancement & Restoration Engine not initialized", "NOT_INITIALIZED");
        }
    }
}
//# sourceMappingURL=audio-enhancement-restoration-engine.js.map