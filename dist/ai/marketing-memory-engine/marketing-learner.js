...
export class MarketingLearner {
    ...
    async learnFromCompletedCampaign(campaign, patternsStored) {
    ...
    identifyStrengths(campaign) {
        const s = [];
        if (campaign.scores.qualityScore >= 70)
            s.push("High-quality campaign content");
        if (campaign.content.hooks.length > 0)
            s.push(`Strong hooks (${campaign.content.hooks.length})`);
        if (campaign.content.callToActions.length > 0)
            s.push("Clear call-to-action strategy");
        if (campaign.campaign.campaignStructure)
            s.push("Well-defined campaign structure");
        if (campaign.branding.brandVoice)
            s.push(`Consistent brand voice: ${campaign.branding.brandVoice}`);
        if (campaign.patterns.length > 0)
            s.push(`${campaign.patterns.length} reusable pattern(s)`);
        return s;
    }
    identifyWeaknesses(campaign) {
        const w = [];
        if (!campaign.content.hooks.length)
            w.push("Missing attention hooks");
        if (!campaign.content.callToActions.length)
            w.push("No call-to-action defined");
        if (!campaign.campaign.customerProblem)
            w.push("Customer problem not articulated");
        if (!campaign.branding.brandVoice)
            w.push("Brand voice undefined");
        if (campaign.scores.engagementScore < 50)
            w.push("Low engagement potential");
        return w;
    }
    buildRecommendations(campaign, weaknesses) {
        const recs = [];
    ...
    buildDescription(campaign, strengths, weaknesses) {
        return [
