...
export class MarketingRelationshipLinker {
    ...
    link(campaignId, projectId, brand, product, tags = []) {
        ...
        if (brand) {
            ...
            for (const id of byBrand.memoryIds) {
                if (id !== campaignId)
                    relationships.relatedBrands.push(id);
            }
        }
        for (const id of relationships.relatedMemories) {
            ...
            switch (entry.memoryType) {
                case MemoryStorageType.Marketing:
                    if (id !== campaignId)
                        relationships.relatedCampaigns.push(id);
                    break;
        ...
        for (const tag of tags) {
            ...
            for (const id of byTag.memoryIds) {
                ...
                if (tag.includes("style"))
                    relationships.relatedStyles.push(id);
                if (tag.includes("customer"))
                    relationships.relatedCustomerTypes.push(id);
            }
        }
...
//# sourceMappingURL=marketing-relationship-linker.js.map