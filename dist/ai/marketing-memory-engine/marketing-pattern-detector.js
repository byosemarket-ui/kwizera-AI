...
export class MarketingPatternDetector {
    ...
    detect(campaign) {
        ...
        if (campaign.campaign.campaignStructure && campaign.campaign.campaignFlow) {
            detected.push(this.create("campaign-structure", `Structure: ${campaign.campaign.campaignStructure} → ${campaign.campaign.campaignFlow}`, id, 78, now));
        }
        ...
        if (campaign.campaign.customerProblem && campaign.campaign.solution) {
            detected.push(this.create("storytelling", `Problem→Solution: ${campaign.campaign.customerProblem.slice(0, 40)} → ${campaign.campaign.solution.slice(0, 40)}`, id, 76, now));
        }
        if (campaign.campaign.productPresentation && campaign.content.sellingPoints.length > 0) {
            detected.push(this.create("product-positioning", `Positioning: ${campaign.campaign.productPresentation} with ${campaign.content.sellingPoints.length} selling point(s)`, id, 74, now));
        }
    ...
    create(type, description, sourceCampaignId, confidence, detectedAt) {
        return {
...
//# sourceMappingURL=marketing-pattern-detector.js.map