...
import { CampaignStatus, MarketingMemoryEngineError, } from "./types.js";
/**
...
export class AiMarketingMemoryEngine {
    ...
    logger = new MarketingMemoryLogger();
    history = new MarketingHistoryStore();
    patterns = new MarketingPatternStore();
    customers = new MarketingCustomerStore();
    campaigns = new Map();
    ...
    initialize(foundation, storageRoot) {
        ...
        this.processor = new MarketingProcessor(foundation, this.history, this.customers, this.scorer, this.patternDetector, this.linker, this.learner, this.logger, this.campaigns);
    ...
    async completeCampaign(campaignId, effectivenessRating) {
        this.ensureReady();
    ...
    getCampaignRelationships(campaignId) {
        const campaign = this.campaigns.get(campaignId);
        if (!campaign || !this.linker)
            return null;
        return this.linker.link(campaign.campaignId, campaign.projectId, campaign.brand, campaign.product, campaign.tags);
    }
    getDetectedPatterns() {
    ...
    searchCampaigns(query) {
        ...
        if (query.cta) {
            ...
            results = results.filter((c) => c.content.callToActions.some((cta) => cta.toLowerCase().includes(lower)));
        }
        if (query.hook) {
            const lower = query.hook.toLowerCase();
            results = results.filter((c) => c.content.hooks.some((h) => h.toLowerCase().includes(lower)));
        }
        if (query.style) {
            const lower = query.style.toLowerCase();
            results = results.filter((c) => c.branding.brandStyle.toLowerCase().includes(lower) ||
                c.tags.some((t) => t.toLowerCase().includes(lower)));
        }
        if (query.keywords?.length) {
            results = results.filter((c) => query.keywords.some((kw) => c.keywords.includes(kw) ||
    ...
    ensureReady() {
        if (!this.initialized || !this.foundation) {
            throw new MarketingMemoryEngineError("Marketing Memory Engine not initialized", "NOT_INITIALIZED");
        }
    }
}
//# sourceMappingURL=marketing-memory-engine.js.map