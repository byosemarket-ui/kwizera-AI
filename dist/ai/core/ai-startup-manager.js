    ...
    async start(deps, options = {}) {
        ...
        try {
            ...
            const recoveryPath = options.storageRootOverride
                ...
                : path.join(options.storageRootOverride ?? "D:\\KWIZERA-AI-STUDIO", "logs");
            if (!fs.existsSync(recoveryPath)) {
                fs.mkdirSync(recoveryPath, { recursive: true });
            }
            throw new AiCoreError(`AI Core startup failed: ${message}`, "STARTUP_FAILED", this.diagnostics[this.diagnostics.length - 1]);
        }
    }
}
//# sourceMappingURL=ai-startup-manager.js.map