...
export class AiModuleRegistry {
    entries = new Map();
    initializeSlots(definitions, logger) {
        this.entries.clear();
    ...
    async initializeModule(id, logger) {
    ...
    async shutdownModule(id, logger) {
        const entry = this.entries.get(id);
        if (!entry?.plugin) {
            return;
        }
    ...
    updateStatus(id, status, lastError) {
        ...
        this.entries.set(id, {
            ...entry,
            status,
            initializedAt: status === "initialized" ? new Date().toISOString() : entry.initializedAt,
            lastError,
        });
    }
}
