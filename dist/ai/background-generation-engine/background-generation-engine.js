...
import { BackgroundGenerationEngineError, } from "./types.js";
/**
...
export class AiBackgroundGenerationEngine {
    ...
    logger = new BackgroundGenerationLogger();
    records = new BackgroundGenerationRecordStore();
    analyzer = new BackgroundGenerationAnalyzer();
    scorer = new BackgroundGenerationScorer();
    linker = new BackgroundGenerationLinker();
    processor = null;
    generationTimes = [];
    searchTimes = [];
    analysisTimes = [];
    initialize(foundation, storageRoot) {
        ...
        this.processor = new BackgroundGenerationProcessor(foundation, this.analyzer, this.scorer, this.linker, this.records, this.logger);
    ...
    getBackgroundPlan(backgroundPlanId) {
        this.ensureReady();
        return this.records.get(backgroundPlanId) ?? null;
    }
    getBackgroundPlansByProduct(productId) {
        this.ensureReady();
        return this.records.getByProduct(productId);
    }
    getBackgroundPlansBySourceImage(sourceImageId) {
        this.ensureReady();
        return this.records.getBySourceImage(sourceImageId);
    }
    searchBackgroundPlans(query) {
        this.ensureReady();
    ...
    async repairBackgroundPlan(sourceImageId, platform) {
        this.ensureReady();
    ...
    ensureReady() {
        if (!this.initialized || !this.foundation || !this.processor) {
            throw new BackgroundGenerationEngineError("Background Generation & Replacement Engine not initialized", "NOT_INITIALIZED");
        }
    }
}
//# sourceMappingURL=background-generation-engine.js.map