...
export class BackgroundGenerationProcessor {
    ...
    async generateBackgroundPlan(input) {
        ...
        try {
            ...
            const productionInstructions = this.analyzer.buildProductionInstructions(profile, generationPlan, lightingMatching);
            const recommendations = this.analyzer.buildRecommendations(context, backgroundAnalysis);
            const scores = this.scorer.computeScores(backgroundAnalysis, subjectPreservation, generationPlan, lightingMatching, depthPlanning, qualityImprovement, replacementPlan, platformOptimizations, context);
            ...
            const draft = {
                ...
                productionReady,
                brandConsistent,
                relationships: this.linker.detectRelationships(draftPartial, input, context.productImagePlan, context.creative, context.strategy, context.understanding),
            };
    ...
    search(query) {
        ...
        if (query.backgroundPlanId)
            results = results.filter((r) => r.backgroundPlanId === query.backgroundPlanId);
        ...
        if (query.targetBackground)
            results = results.filter((r) => r.profile.targetBackground === query.targetBackground);
        if (query.marketingPreset)
            results = results.filter((r) => r.profile.marketingPreset === query.marketingPreset);
        if (query.keywords) {
            const kw = query.keywords.toLowerCase();
            results = results.filter((r) => r.generationPlan.generationPrompt.toLowerCase().includes(kw) ||
                r.backgroundAnalysis.sceneEnvironment.toLowerCase().includes(kw) ||
                r.profile.productId.toLowerCase().includes(kw));
        }
        if (query.text) {
            const textLower = query.text.toLowerCase();
            results = results.filter((r) => r.backgroundPlanId.toLowerCase().includes(textLower) ||
    ...
    async resolveContext(input) {
        ...
        if (input.productId && productFoundation) {
            ...
            if (analysis || understanding || productImagePlan) {
                return this.analyzer.extractContextFromProduct(analysis, understanding, creative, strategy, input, productImagePlan);
            }
        }
    ...
    applySafeRepairs(lighting, generation, preservation, diagnostics) {
        const repairs = [];
...
//# sourceMappingURL=background-generation-processor.js.map