...
import { GENERATION_BLUEPRINT_STAGES, PREPARED_VIDEO_GENERATION_MODULES, SUPPORTED_GENERATION_ASSET_TYPES, } from "./video-generation-categories.js";
export class VideoGenerationIntegrityVerifier {
    logger;
    constructor(logger) {
        this.logger = logger;
    }
    verify(storage, registry, blueprintManager) {
        ...
        if (!fs.existsSync(generationRoot)) {
            issues.push(`Video generation root missing: ${generationRoot}`);
        }
        else {
            checkedPaths++;
        }
        ...
        for (const module of PREPARED_VIDEO_GENERATION_MODULES) {
        ...
        this.logger.log(verified ? "info" : "warn", "integrity", "Video generation integrity verification complete", {
            verified,
            checkedPaths,
    ...
    writeManifest(storage, storageRoot) {
        const manifest = {
            ...
            modules: PREPARED_VIDEO_GENERATION_MODULES.map((m) => m.moduleId),
            assetTypes: SUPPORTED_GENERATION_ASSET_TYPES.length,
            blueprintStages: GENERATION_BLUEPRINT_STAGES.length,
            createdAt: new Date().toISOString(),
        };
        fs.writeFileSync(path.join(storage.getGenerationRoot(), "foundation-manifest.json"), JSON.stringify(manifest, null, 2), "utf8");
    }
}
//# sourceMappingURL=video-generation-integrity-verifier.js.map