...
export class ImageGenerationResourceMonitor {
    ...
    measure() {
        ...
        return {
            ...
            cpuUsagePercent: 0,
            gpuUsagePercent: 0,
            searchPerformanceMs: Math.max(prompt.performance.averageSearchMs, production.performance.averageSearchMs ?? 0),
...
export function deriveImageGenerationPerformanceIssues(metrics) {
    const issues = [];
    if (metrics.planningPerformanceMs > 120000)
        issues.push("Slow production planning detected");
    if (metrics.validationPerformanceMs > 120000)
        issues.push("Slow quality validation detected");
    if (metrics.optimizationPerformanceMs > 120000)
        issues.push("Slow optimization detected");
    if (metrics.searchPerformanceMs > 200)
        issues.push("Slow image generation search detected");
    if (metrics.diskUsageMb > 5000)
        issues.push("High image generation disk usage");
