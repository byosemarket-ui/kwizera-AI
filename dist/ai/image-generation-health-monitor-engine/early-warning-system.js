...
import { deriveImageGenerationPerformanceIssues, } from "./resource-monitor.js";
import { ImageGenerationHealthScoreLevel, ImageGenerationWarningType, MonitoredImageGenerationModule, } from "./types.js";
export class ImageGenerationEarlyWarningSystem {
    ...
    async detect(moduleScores, metrics) {
        ...
        if (!persistence.persistenceStatus.includes("survives")) {
            warnings.push(this.warn(ImageGenerationWarningType.DatabaseProblems, MonitoredImageGenerationModule.ImageGenerationFoundation, "Image generation database persistence unverified", "Run foundation integrity verification and recovery"));
        }
        const registry = this.foundation.getRegistry();
        if (!registry.verifyChecksum()) {
            warnings.push(this.warn(ImageGenerationWarningType.RegistryProblems, MonitoredImageGenerationModule.ProductionRegistry, "Image generation registry checksum invalid", "Re-persist registry and verify module registrations"));
        }
        const integrity = this.foundation.getLastIntegrityResult();
        if (integrity && !integrity.verified) {
            warnings.push(this.warn(ImageGenerationWarningType.BrokenDependencies, MonitoredImageGenerationModule.ImageGenerationFoundation, "Image generation integrity issues detected", "Run dependency validation and foundation recovery"));
        }
        const storageChecks = [
            {
        ...
        if (fs.existsSync(assetCatalog)) {
            try {
                ...
                warnings.push(this.warn(ImageGenerationWarningType.RegistryProblems, MonitoredImageGenerationModule.AssetRegistry, "Asset registry catalog corrupted or invalid", "Repair asset registry from recovery point"));
            }
        }
        ...
        if (missingDeps.length > 0) {
            warnings.push(this.warn(ImageGenerationWarningType.BrokenDependencies, MonitoredImageGenerationModule.ImageGenerationFoundation, `${missingDeps.length} broken dependency reference(s)`, "Validate module registration order and dependencies"));
        }
        const promptReport = this.foundation.getTextToImageGenerationEngine().buildStatusReport();
        if (promptReport.knownIssues.length > 0) {
            warnings.push(this.warn(ImageGenerationWarningType.PromptProblems, MonitoredImageGenerationModule.TextToImageGeneration, promptReport.knownIssues.join("; "), "Review text-to-image prompt records"));
        }
        ...
        if (productReport.knownIssues.length > 0) {
            warnings.push(this.warn(ImageGenerationWarningType.ImageProblems, MonitoredImageGenerationModule.ProductImageGeneration, productReport.knownIssues.join("; "), "Review product image generation records"));
        }
        ...
        if (renderReport.knownIssues.length > 0) {
            warnings.push(this.warn(ImageGenerationWarningType.LayerProblems, MonitoredImageGenerationModule.ImageRenderingPreparation, renderReport.knownIssues.join("; "), "Review layer and mask integrity in render plans"));
        }
        ...
        if (productionReport.knownIssues.length > 0) {
            warnings.push(this.warn(ImageGenerationWarningType.ProductionProblems, MonitoredImageGenerationModule.ImageProduction, productionReport.knownIssues.join("; "), "Run image production plan repair"));
        }
        const brandingReport = this.foundation.getBrandingDesignEngine().buildStatusReport();
        if (brandingReport.knownIssues.length > 0) {
            warnings.push(this.warn(ImageGenerationWarningType.BrandingProblems, MonitoredImageGenerationModule.BrandingDesign, brandingReport.knownIssues.join("; "), "Review branding design consistency"));
        }
        if (metrics.searchPerformanceMs > 150) {
            warnings.push(this.warn(ImageGenerationWarningType.SearchFailure, MonitoredImageGenerationModule.AssetRegistry, `Image generation search averaging ${metrics.searchPerformanceMs}ms`, "Run image generation optimization for search performance"));
        }
        if (metrics.diskUsageMb > 3000) {
            warnings.push(this.warn(ImageGenerationWarningType.HighResourceUsage, MonitoredImageGenerationModule.ImageGenerationFoundation, `${metrics.diskUsageMb}MB disk used by image generation`, "Archive inactive generation records"));
        }
        if (metrics.memoryUsageMb > 400) {
            warnings.push(this.warn(ImageGenerationWarningType.HighResourceUsage, MonitoredImageGenerationModule.AssetRegistry, `${metrics.memoryUsageMb}MB heap used`, "Optimize generation asset cache"));
        }
        if (metrics.gpuUsagePercent > 80) {
            warnings.push(this.warn(ImageGenerationWarningType.HighResourceUsage, MonitoredImageGenerationModule.ImageGenerationFoundation, `GPU usage at ${metrics.gpuUsagePercent}%`, "Reduce concurrent image generation workloads"));
        }
        for (const issue of deriveImageGenerationPerformanceIssues(metrics)) {
            if (!warnings.some((w) => w.message.includes(issue))) {
                warnings.push(this.warn(ImageGenerationWarningType.HighResourceUsage, MonitoredImageGenerationModule.ImageGenerationFoundation, issue, "Monitor image generation performance trends"));
            }
        }
        for (const mod of moduleScores) {
            if (mod.level === ImageGenerationHealthScoreLevel.Critical ||
                mod.level === ImageGenerationHealthScoreLevel.Failed) {
                warnings.push(this.warn(ImageGenerationWarningType.ProductionProblems, mod.module, `${mod.module} health critical (${mod.score})`, `Inspect ${mod.module} diagnostics`));
            }
        }
        return warnings;
    }
