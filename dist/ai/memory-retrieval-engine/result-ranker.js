...
export class ResultRanker {
    ...
    computeFactors(entry, query, usage, relatedToId, mode) {
        ...
        const compositeScore = Math.round(relevanceScore * 0.3 +
            qualityScore * 0.25 +
    ...
    computeLearningImportance(entry) {
        if (entry.memoryType === MemoryStorageType.Learning)
            return 90;
        if (entry.memoryType === MemoryStorageType.Reasoning)
            return 75;
        if (entry.memoryType === MemoryStorageType.Decision)
            return 70;
        return 40;
    }
    computeRelationshipStrength(entry, relatedToId) {
        if (entry.memoryId === relatedToId)
            return 0;
        if (entry.searchableText.includes(relatedToId.toLowerCase()))
            return 80;
        return 30;
    }
    computeRecencyBonus(lastUpdate, lastAccess) {
