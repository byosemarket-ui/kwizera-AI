...
export class AudioPlanningProcessor {
    ...
    async createAudioPlan(input) {
        ...
        if (!storyboard?.validated || !storyboard.productionReady) {
            ...
            if (!storyboardResult.success || !storyboardResult.record) {
                return this.reject(start, storyboardResult.message ?? "Production-ready storyboard required before audio planning", storyboardResult.diagnostics.length > 0
        ...
        if (!scriptPlan?.validated || !scriptPlan.productionReady) {
            ...
            if (!scriptResult.success || !scriptResult.record) {
                return this.reject(start, scriptResult.message ?? "Production-ready script plan required before audio planning", scriptResult.diagnostics.length > 0
        ...
        if (!visualPlan?.validated || !visualPlan.productionReady) {
            ...
            if (!visualResult.success || !visualResult.record) {
                return this.reject(start, visualResult.message ?? "Production-ready visual plan required before audio planning", visualResult.diagnostics.length > 0
        ...
        const scores = this.scorer.computeScores(sceneAudioPlans, voicePlanning, musicPlanning, synchronization, storyboard, scriptPlan, visualPlan, creative, strategy);
        let validation = this.scorer.isAudioPlanValid(scores, sceneAudioPlans, storyboard, scriptPlan, visualPlan, alignment.issues);
        if (!validation.valid) {
            const repaired = this.applyScoreRepairs(sceneAudioPlans);
            if (repaired.length > 0) {
                sceneAudioPlans = repaired;
                const repairedScores = this.scorer.computeScores(sceneAudioPlans, voicePlanning, musicPlanning, synchronization, storyboard, scriptPlan, visualPlan, creative, strategy);
                validation = this.scorer.isAudioPlanValid(repairedScores, sceneAudioPlans, storyboard, scriptPlan, visualPlan, this.analyzer.validateAlignment(sceneAudioPlans, storyboard, scriptPlan, visualPlan).issues);
                if (validation.valid) {
        ...
        if (!validation.valid) {
            ...
            return {
                ...
                diagnostics: validation.diagnostics,
                message: "Audio planning validation failed — plan must fully align with storyboard, script, visual plan, brand and strategy",
            };
        }
        const productionReady = this.scorer.isProductionReady(sceneAudioPlans, storyboard, scriptPlan, visualPlan, scores);
        const draft = {
        ...
        draft.relationships = this.linker.detectRelationships(draft, storyboard, scriptPlan, visualPlan, creative, strategy, understanding);
    ...
    search(query) {
        ...
        if (query.language)
            results = results.filter((r) => r.profile.language === query.language);
        if (query.voiceStyle) {
        ...
        if (query.text) {
            ...
            results = results.filter((r) => r.audioPlanId.toLowerCase().includes(textLower) ||
                r.musicPlanning.musicMood.toLowerCase().includes(textLower));
        }
        return results.slice(0, query.limit ?? 50);
    }
    applySceneRepairs(storyboard, scriptPlan, creative) {
        return this.analyzer.buildSceneAudioPlans(storyboard, scriptPlan, creative);
    }
    applyScoreRepairs(sceneAudioPlans) {
...
//# sourceMappingURL=audio-planning-processor.js.map