import { BusCommunicationState, BusMessageType, } from "./types.js";
export class MessageRouter {
    ...
    totalLatencyMs = 0;
    routedCount = 0;
    constructor(deps, logger, history, retryHandler) {
        this.deps = deps;
        this.logger = logger;
        this.history = history;
        this.retryHandler = retryHandler;
    }
    async route(message, handler) {
        ...
        const deliver = async () => {
            ...
            try {
                ...
                else if (message.messageType === BusMessageType.HealthCheck ||
                    message.messageType === BusMessageType.Request) {
                    const core = this.deps.getCore();
                    const plugin = core.registry.getPlugin(message.receiver) ??
        ...
        this.logger.log(success ? "info" : "error", success ? "response" : "error", `Routed ${message.messageType}: ${message.sender} → ${message.receiver}`, {
            messageId: message.messageId,
