import { AmbientPlatform, AmbientSyncTarget, EnvironmentCategory, IndoorAmbienceType, NatureAmbienceType, PLATFORM_AMBIENT_CONFIG, UrbanAmbienceType, WeatherType, } from "./types.js";
...
export class AmbientAudioGenerationAnalyzer {
    analyzeEnvironment(input, context) {
    ...
    buildProfile(input, platform, version, context, analysis) {
        ...
        const category = input.environmentCategory ?? analysis.environmentType;
    ...
    buildAmbientSoundPlan(analysis, category) {
        const types = [];
        if (category === EnvironmentCategory.Nature || category === EnvironmentCategory.Mixed) {
            ...
            if (analysis.weather.includes("rain"))
                types.push(NatureAmbienceType.Rain);
            if (analysis.indoorOutdoor === "outdoor")
                types.push(NatureAmbienceType.Wind);
        }
        if (types.length === 0)
            types.push(NatureAmbienceType.Wind);
        const layers = {};
    ...
    buildUrbanAmbiencePlan(analysis, category) {
        const types = category === EnvironmentCategory.Urban || category === EnvironmentCategory.Mixed
            ? [UrbanAmbienceType.City, UrbanAmbienceType.Traffic]
            : [];
        if (analysis.keywords.some((k) => k.includes("market")))
            types.push(UrbanAmbienceType.Market);
        const layerDetails = {};
        for (const t of types) {
    ...
    buildIndoorAmbiencePlan(analysis, category) {
        const types = category === EnvironmentCategory.Indoor || category === EnvironmentCategory.Mixed
        ...
        if (analysis.keywords.some((k) => k.includes("restaurant")))
            types.push(IndoorAmbienceType.Restaurant);
        const layerDetails = {};
        for (const t of types) {
    ...
    buildWeatherAmbiencePlan(analysis) {
        const types = [];
        const weather = analysis.weather.toLowerCase();
        if (weather.includes("rain"))
            types.push(WeatherType.LightRain);
        if (weather.includes("storm"))
            types.push(WeatherType.Storm, WeatherType.Thunder);
        if (weather.includes("wind"))
            types.push(WeatherType.Wind);
        if (weather.includes("snow"))
            types.push(WeatherType.Snow);
        if (analysis.timeOfDay === "night")
            types.push(WeatherType.Night);
        if (analysis.timeOfDay === "sunrise")
            types.push(WeatherType.Sunrise);
        if (analysis.timeOfDay === "sunset")
            types.push(WeatherType.Sunset);
        if (types.length === 0)
            types.push(WeatherType.Dawn);
        const weatherLayers = {};
        for (const t of types) {
    ...
    buildSpatialAudioPlan(analysis) {
        return {
    ...
    buildTimelinePlan(analysis, ambientPlan, platform) {
    ...
    buildProductionInstructions(profile, analysis, spatial) {
        return {
            renderNotes: [
    ...
    resolvePlatform(input, context) {
        ...
        if (context.creative?.profile.platform) {
            ...
            if (p.includes("youtube"))
                return AmbientPlatform.YouTube;
            if (p.includes("tiktok"))
                return AmbientPlatform.TikTok;
            if (p.includes("instagram"))
                return AmbientPlatform.Instagram;
            if (p.includes("facebook"))
                return AmbientPlatform.Facebook;
            if (p.includes("mobile"))
                return AmbientPlatform.Mobile;
            if (p.includes("tv") || p.includes("television"))
                return AmbientPlatform.Television;
        }
    ...
    detectCategory(context) {
        ...
        if (prompt.includes("forest") || prompt.includes("nature") || prompt.includes("ocean"))
            return EnvironmentCategory.Nature;
        if (prompt.includes("city") || prompt.includes("traffic") || prompt.includes("urban"))
            return EnvironmentCategory.Urban;
        if (prompt.includes("office") || prompt.includes("indoor") || prompt.includes("hospital"))
            return EnvironmentCategory.Indoor;
        if (prompt.includes("rain") || prompt.includes("storm") || prompt.includes("weather"))
            return EnvironmentCategory.Weather;
        const industry = context.industry?.toLowerCase() ?? "default";
        return INDUSTRY_CATEGORY_MAP[industry] ?? EnvironmentCategory.Mixed;
    }
    detectIndoorOutdoor(prompt, category) {
        if (prompt.includes("outdoor") || category === EnvironmentCategory.Nature || category === EnvironmentCategory.Urban)
            return "outdoor";
        if (prompt.includes("indoor") || category === EnvironmentCategory.Indoor)
            return "indoor";
        return "mixed";
    }
    detectTimeOfDay(prompt) {
        if (prompt.includes("night"))
            return "night";
        if (prompt.includes("sunrise") || prompt.includes("dawn"))
            return "sunrise";
    ...
    detectWeather(prompt) {
        if (prompt.includes("storm"))
            return "storm";
        if (prompt.includes("heavy rain"))
            return "heavy rain";
        if (prompt.includes("rain"))
            return "light rain";
        if (prompt.includes("wind"))
            return "windy";
        if (prompt.includes("snow"))
            return "snow";
        return "clear";
    }
    detectSeason(prompt) {
        if (prompt.includes("winter") || prompt.includes("snow"))
            return "winter";
        if (prompt.includes("summer"))
            return "summer";
        if (prompt.includes("autumn") || prompt.includes("fall"))
            return "autumn";
        return "spring";
    }
    detectMood(context, category) {
        if (context.industry === "health")
            return "calm";
        if (category === EnvironmentCategory.Nature)
            return "peaceful";
        if (category === EnvironmentCategory.Urban)
            return "energetic";
...
//# sourceMappingURL=ambient-audio-generation-analyzer.js.map