...
import { NatureAmbienceType, WeatherType, } from "./types.js";
export class AmbientAudioGenerationProcessor {
    ...
    async generateAmbientPlan(input) {
        ...
        try {
            ...
            const scores = this.scorer.computeScores(environmentAnalysis, ambientSoundPlan, urbanAmbiencePlan, indoorAmbiencePlan, weatherAmbiencePlan, spatialAudioPlan, timelinePlan, syncPreparation, productionInstructions, resolvedContext);
    ...
    search(query) {
        ...
        if (query.ambientPlanId)
            results = results.filter((r) => r.ambientPlanId === query.ambientPlanId);
        if (query.productId)
        ...
        if (query.environmentCategory)
            results = results.filter((r) => r.profile.environmentCategory === query.environmentCategory);
        if (query.platform)
        ...
        if (query.keywords) {
            ...
            results = results.filter((r) => r.environmentAnalysis.keywords.some((k) => k.includes(kw)) ||
                r.environmentAnalysis.location.toLowerCase().includes(kw));
        }
        if (query.text) {
            const textLower = query.text.toLowerCase();
            results = results.filter((r) => r.ambientPlanId.toLowerCase().includes(textLower) ||
    ...
    applySafeRepairs(ambient, weather, spatial, timeline, sync, diagnostics) {
        const repairs = [];
...
//# sourceMappingURL=ambient-audio-generation-processor.js.map