...
import { deriveVideoGenerationPerformanceIssues, } from "./resource-monitor.js";
import { VideoGenerationHealthScoreLevel, VideoGenerationWarningType, MonitoredVideoGenerationModule, } from "./types.js";
export class VideoGenerationEarlyWarningSystem {
    ...
    async detect(moduleScores, metrics) {
        ...
        if (!persistence.persistenceStatus.includes("survives")) {
            warnings.push(this.warn(VideoGenerationWarningType.DatabaseProblems, MonitoredVideoGenerationModule.VideoGenerationFoundation, "Video generation database persistence unverified", "Run foundation integrity verification and recovery"));
        }
        const registry = this.foundation.getRegistry();
        if (!registry.verifyChecksum()) {
            warnings.push(this.warn(VideoGenerationWarningType.RegistryProblems, MonitoredVideoGenerationModule.ProductionRegistry, "Video generation registry checksum invalid", "Re-persist registry and verify module registrations"));
        }
        const integrity = this.foundation.getLastIntegrityResult();
        if (integrity && !integrity.verified) {
            warnings.push(this.warn(VideoGenerationWarningType.BrokenDependencies, MonitoredVideoGenerationModule.VideoGenerationFoundation, "Video generation integrity issues detected", "Run dependency validation and foundation recovery"));
        }
        const storageChecks = [
            {
        ...
        if (missingDeps.length > 0) {
            warnings.push(this.warn(VideoGenerationWarningType.BrokenDependencies, MonitoredVideoGenerationModule.VideoGenerationFoundation, `${missingDeps.length} broken dependency reference(s)`, "Validate module registration order and dependencies"));
        }
        const storyReport = this.foundation.getStoryGenerationEngine().buildStatusReport();
        if (storyReport.knownIssues.length > 0) {
            warnings.push(this.warn(VideoGenerationWarningType.StoryboardProblems, MonitoredVideoGenerationModule.StoryboardGeneration, storyReport.knownIssues.join("; "), "Review storyboard generation records"));
        }
        const sceneReport = this.foundation.getSceneGenerationEngine().buildStatusReport();
        if (sceneReport.knownIssues.length > 0) {
            warnings.push(this.warn(VideoGenerationWarningType.SceneProblems, MonitoredVideoGenerationModule.SceneGeneration, sceneReport.knownIssues.join("; "), "Review scene generation records"));
        }
        ...
        if (productionReport.knownIssues.length > 0) {
            warnings.push(this.warn(VideoGenerationWarningType.ProductionProblems, MonitoredVideoGenerationModule.VideoProduction, productionReport.knownIssues.join("; "), "Run video production plan repair"));
        }
        if (metrics.searchPerformanceMs > 150) {
            warnings.push(this.warn(VideoGenerationWarningType.SearchFailure, MonitoredVideoGenerationModule.AssetRegistry, `Video generation search averaging ${metrics.searchPerformanceMs}ms`, "Run video generation optimization for search performance"));
        }
        if (metrics.diskUsageMb > 3000) {
            warnings.push(this.warn(VideoGenerationWarningType.HighResourceUsage, MonitoredVideoGenerationModule.VideoGenerationFoundation, `${metrics.diskUsageMb}MB disk used by video generation`, "Archive inactive generation records"));
        }
        if (metrics.memoryUsageMb > 400) {
            warnings.push(this.warn(VideoGenerationWarningType.HighResourceUsage, MonitoredVideoGenerationModule.AssetRegistry, `${metrics.memoryUsageMb}MB heap used`, "Optimize generation asset cache"));
        }
        if (metrics.gpuUsagePercent > 80) {
            warnings.push(this.warn(VideoGenerationWarningType.HighResourceUsage, MonitoredVideoGenerationModule.VideoGenerationFoundation, `GPU usage at ${metrics.gpuUsagePercent}%`, "Reduce concurrent video generation workloads"));
        }
        for (const issue of deriveVideoGenerationPerformanceIssues(metrics)) {
            if (!warnings.some((w) => w.message.includes(issue))) {
                warnings.push(this.warn(VideoGenerationWarningType.HighResourceUsage, MonitoredVideoGenerationModule.VideoGenerationFoundation, issue, "Monitor video generation performance trends"));
            }
        }
        for (const mod of moduleScores) {
            if (mod.level === VideoGenerationHealthScoreLevel.Critical ||
                mod.level === VideoGenerationHealthScoreLevel.Failed) {
                warnings.push(this.warn(VideoGenerationWarningType.ProductionProblems, mod.module, `${mod.module} health critical (${mod.score})`, `Inspect ${mod.module} diagnostics`));
            }
        }
        return warnings;
    }
