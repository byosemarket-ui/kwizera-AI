...
export class VideoGenerationAuditor {
    ...
    async runAudit() {
        ...
        const brandConsistent = storyReport.averageProductionReadinessScore === 0 ||
        ...
        for (const mod of implemented) {
            for (const dep of mod.dependencies) {
                if (EXTERNAL_VG_DEPENDENCIES.has(dep)) {
                    if (dep === "knowledge-engine" && !integration.knowledgeEngine)
                        return false;
                    if (dep === "memory-engine" && !integration.memoryEngine)
                        return false;
                    if (dep === "product-intelligence-engine" && !integration.productIntelligenceEngine)
                        return false;
                    if (dep === "image-intelligence-engine" && !integration.imageIntelligenceEngine)
                        return false;
                    if (dep === "video-intelligence-engine" && !integration.videoIntelligenceEngine)
                        return false;
                    continue;
                }
...
//# sourceMappingURL=video-generation-auditor.js.map