import { CameraStabilityLevel, ShotFraming } from "./types.js";
export class CameraMovementScorer {
    computeScores(shots, avgContinuity, productionReadinessFromTimeline, storytellingScore) {
        let cameraMovementScore = 55;
        if (shots.length >= 3)
            cameraMovementScore += 15;
        const uniqueMovements = new Set(shots.map((s) => s.movement)).size;
        if (uniqueMovements >= 2)
            cameraMovementScore += 10;
        if (shots.every((s) => s.confidence >= 70))
            cameraMovementScore += 10;
        cameraMovementScore = Math.min(100, cameraMovementScore);
        let cinematicScore = 60;
        if (uniqueMovements >= 3)
            cinematicScore += 15;
        if (shots.some((s) => s.framing === ShotFraming.HeroShot || s.framing === ShotFraming.CloseUp))
            cinematicScore += 10;
        ...
        const slightWeight = shots.filter((s) => s.stability === CameraStabilityLevel.SlightShake).length * 0.75;
        let stabilityScore = shots.length > 0
            ? Math.round(((stableWeight + slightWeight) / shots.length) * 100)
            : 50;
        const avgSmoothness = shots.length > 0
        ...
        const productionReadinessScore = Math.round((productionReadinessFromTimeline + stabilityScore + avgContinuity) / 3);
        const aiConfidenceScore = Math.round((cameraMovementScore + cinematicScore + stabilityScore + storytellingScore + productionReadinessScore) / 5);
        return {
            cameraMovementScore,
            cinematicScore,
            stabilityScore,
    ...
    isAnalysisValid(scores, shotCount) {
        const diagnostics = [];
        if (shotCount < 1)
            diagnostics.push("At least 1 shot required for camera analysis");
...
//# sourceMappingURL=camera-movement-scorer.js.map