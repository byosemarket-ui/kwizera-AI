...
import { CameraAngle, CameraMovementType, CameraStabilityLevel, CinematicPurpose, ShotFraming, } from "./types.js";
...
export class CameraMovementAnalyzer {
    analyze(analysis, sceneDetection, timeline, understanding) {
        const motionDensity = analysis.frame.motionDensity;
        const visualStability = analysis.visual.visualStability;
        const shotAnalyses = sceneDetection.shots.map((shot, i) => this.analyzeShot(shot, sceneDetection, analysis, motionDensity, visualStability, i));
        ...
        const movementPlan = this.buildMovementPlan(shotAnalyses, analysis, understanding, timeline, visualStability);
    ...
    analyzeShot(shot, sceneDetection, analysis, motionDensity, visualStability, index) {
    ...
    inferMovement(shot, motionDensity, index, analysis) {
        if (motionDensity < 25 && !shot.cameraChange)
        ...
        if (analysis.technical.metadata?.drone === "true" ||
            analysis.technical.videoName.toLowerCase().includes("drone")) {
            return CameraMovementType.Drone;
        }
        if (motionDensity > 70) {
            if (index % 4 === 0)
                return CameraMovementType.TrackingShot;
            if (index % 4 === 1)
                return CameraMovementType.FollowShot;
        ...
        if (index % 5 === 0)
            return CameraMovementType.Orbit;
        if (index % 5 === 1)
            return CameraMovementType.TruckLeft;
        if (index % 5 === 2)
            return CameraMovementType.TruckRight;
        if (index % 5 === 3)
            return CameraMovementType.Tilt;
        if (motionDensity > 45)
            return CameraMovementType.ZoomIn;
        return CameraMovementType.Static;
    }
    inferAngle(shotType, orientation, index) {
        if (orientation === VideoOrientation.Portrait && index === 0)
            return CameraAngle.EyeLevel;
        if (shotType === ShotType.Establishing)
            return CameraAngle.HighAngle;
        if (shotType === ShotType.CloseUp)
            return CameraAngle.EyeLevel;
        if (shotType === ShotType.ExtremeCloseUp)
        ...
        if (index % 7 === 3)
            return CameraAngle.SideView;
        if (index % 7 === 4)
            return CameraAngle.FrontView;
        if (index % 7 === 5)
            return CameraAngle.RearView;
        return CameraAngle.EyeLevel;
    }
    inferFraming(shotType, classification) {
        if (classification === SceneClassification.ProductDemo)
            return ShotFraming.HeroShot;
        if (classification === SceneClassification.Hook)
            return ShotFraming.MediumCloseUp;
        if (classification === SceneClassification.Cta)
            return ShotFraming.CloseUp;
        return SHOT_TO_FRAMING[shotType] ?? ShotFraming.MediumShot;
    }
    inferStability(visualStability, motionDensity) {
        if (visualStability >= 75 && motionDensity <= 65)
            return CameraStabilityLevel.Stable;
        if (visualStability >= 55)
            return CameraStabilityLevel.SlightShake;
        return CameraStabilityLevel.HeavyShake;
    }
    inferPurpose(classification, framing, movement) {
        if (classification === SceneClassification.ProductDemo || framing === ShotFraming.HeroShot) {
            return CinematicPurpose.ProductShowcase;
        }
        if (classification === SceneClassification.Cta)
            return CinematicPurpose.CtaFocus;
        if (classification === SceneClassification.Hook)
            return CinematicPurpose.AudienceAttention;
        if (classification === SceneClassification.BrandScene)
            return CinematicPurpose.MarketingFocus;
    ...
    analyzeTransitions(shots, sceneDetection) {
        const transitions = [];
        for (let i = 0; i < shots.length - 1; i++) {
            const from = shots[i];
            const to = shots[i + 1];
            const continuity = from.movement === to.movement ? 90 : from.stability === to.stability ? 75 : 60;
            transitions.push({
    ...
    buildMovementPlan(shots, analysis, understanding, timeline, visualStability) {
        const dominant = this.getDominantMovement(shots);
        const style = understanding?.context.creativeContext ??
            `${analysis.classification.creativeStyle} cinematic`;
        const avgContinuity = shots.length > 1
            ? Math.round(shots.slice(0, -1).reduce((s, shot, i) => {
    ...
    getDominantMovement(shots) {
        const counts = new Map();
        for (const s of shots)
            counts.set(s.movement, (counts.get(s.movement) ?? 0) + 1);
    ...
    buildRecommendations(shots, analysis, visualStability) {
        const recs = [];
...
//# sourceMappingURL=camera-movement-analyzer.js.map