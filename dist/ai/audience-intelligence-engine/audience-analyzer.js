...
import { AudienceCategory, AudiencePlatform, } from "./types.js";
const INDUSTRY_PLATFORM_MAP = {
...
export class AudienceAnalyzer {
    buildProfile(input, understanding, analysis) {
        ...
        const audienceName = input.audienceName ??
        ...
        const platforms = input.preferredPlatforms ??
            INDUSTRY_PLATFORM_MAP[industry] ??
            INDUSTRY_PLATFORM_MAP.general;
        return {
            audienceId,
            audienceName,
            audienceCategory: BUSINESS_TYPE_CATEGORY[analysis.classification.businessType] ?? AudienceCategory.General,
            industry,
            productCategory: analysis.profile.category,
            preferredLanguage: input.preferredLanguage ?? input.demographics?.language,
            preferredCommunicationStyle: this.inferCommunicationStyle(analysis.classification.businessType, industry),
            preferredPlatforms: platforms,
            marketingGoal,
        };
    }
    buildDemographics(input, analysis) {
        const demographics = {
    ...
    buildPsychologicalUnderstanding(understanding, marketingGoal) {
    ...
    buildSegmentation(understanding, analysis, psychological, profile) {
        const businessGoals = [
    ...
    buildMarketingPreparation(profile, psychological, segmentation) {
        const grounded = psychological.customerNeeds.length >= 2 &&
        ...
        return {
            ...
            productionPlanningReady: grounded &&
    ...
    deriveCustomerGoals(understanding) {
        const goals = [];
    ...
    deriveCustomerInterests(understanding) {
        const interests = [
    ...
    deriveDecisionFactors(understanding) {
        const factors = [
