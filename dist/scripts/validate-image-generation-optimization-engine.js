...
import { ALL_PIPELINE_OPTIMIZATION_AREAS, BrandDesignGenPlatform, BrandDesignType, createAiCore, CreativePlatform, ImageProductionPlatform, ImageRenderPlatform, MarketingObjective, MultiStyleGenPlatform, MultiStyleImageCategory, OptimizationPlatform, ProductAnalysisCategory, ProductAvailabilityStatus, ProductBusinessType, ProductImageGenPlatform, ProductUnderstandingMarketingGoal, QualityValidationPlatform, } from "../ai/index.js";
...
async function preparePipeline(piFoundation, sample, objective, platform) {
...
async function prepareFullImagePipeline(imgFoundation, key, brandingPlanId) {
    const s = SAMPLES[key];
...
async function main() {
    ...
    try {
        ...
        results.pipelineOptimization = {
            passed: tech.record?.pipelineOptimization.allPipelineOptimized === true &&
        ...
        results.qualityOptimization = {
            passed: tech.record?.qualityOptimization.qualityMaintainedOrImproved === true &&
        ...
        results.componentOptimization = {
            passed: tech.record?.componentOptimization.productionOptimized === true &&
        ...
        results.optimizationScores = {
            passed: (tech.record?.scores.optimizationScore ?? 0) >= 55 &&
        ...
        results.relationships = {
            passed: (tech.record?.relationships.validationReports.length ?? 0) >= 1 &&
...
function buildPipelineReport(tech, fashion, food) {
...
function buildPerformanceReport(tech, fashion, food) {
    const rows = [tech, fashion, food].filter(Boolean);
...
function buildResourceReport(tech, fashion, food) {
    const rows = [tech, fashion, food].filter(Boolean);
...
function buildProductionReport(status, tech, fashion, food) {
    const rows = [tech, fashion, food].filter(Boolean);
    return [
        ...
        "|---------|------------------|-----------------|------------|-------------------|",
        ...rows.map((r) => `| ${r.profile.productId} | ${r.productionReady ? "✅" : "❌"} | ${r.scores.productionReadinessScore}/100 | ${r.scores.aiConfidenceScore}/100 | ${r.qualityOptimization.qualityMaintainedOrImproved ? "✅" : "❌"} |`),
        "",
        "## Performance",
        "",
...
void main();
//# sourceMappingURL=validate-image-generation-optimization-engine.js.map