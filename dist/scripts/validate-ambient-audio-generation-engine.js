...
import { AmbientPlatform, AmbientSyncTarget, createAiCore, CreativePlatform, EnvironmentCategory, MarketingObjective, NATURE_AMBIENCE_TYPES, ProductAnalysisCategory, ProductAvailabilityStatus, ProductBusinessType, ProductUnderstandingMarketingGoal, SUPPORTED_ENVIRONMENT_CATEGORIES, WEATHER_TYPES, } from "../ai/index.js";
...
async function main() {
    ...
    try {
        ...
        results.environmentAnalysis = {
            passed: Boolean(tech.record?.environmentAnalysis.environmentType &&
        ...
        results.ambientPlanning = {
            passed: Boolean(nature.record?.ambientSoundPlan.natureAmbience.length >= 1 &&
        ...
        results.weatherPlanning = {
            passed: Boolean(nature.record?.weatherAmbiencePlan.weatherTypes.length >= 1 &&
        ...
        results.spatialAudioPlanning = {
            passed: Boolean(tech.record?.spatialAudioPlan.surroundPreparation &&
        ...
        results.timelinePlanning = {
            passed: Boolean(tech.record?.timelinePlan.cuePoints.length >= 3 &&
        ...
        results.synchronization = {
            passed: Boolean(tech.record?.syncPreparation.hitPoints.length >= 1 &&
        ...
        results.ambientScores = {
            passed: (tech.record?.scores.environmentalRealismScore ?? 0) >= 55 &&
        ...
        results.relationships = {
            passed: (tech.record?.relationships.ambientPlans.length ?? 0) >= 1 &&
        ...
        fs.writeFileSync(path.join(projectStateDir, "AI-Ambient-Audio-Report.md"), buildMainReport(status, results, storageRoot, allPassed, tech.record, health.record, finance.record, nature.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Environment-Analysis-Report.md"), buildEnvironmentReport(tech.record, health.record, finance.record, nature.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Spatial-Audio-Report.md"), buildSpatialReport(tech.record, health.record, finance.record, nature.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Timeline-Planning-Report.md"), buildTimelineReport(tech.record, health.record, finance.record, nature.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Ambient-Audio-Readiness-Report.md"), buildReadinessReport(status, tech.record, health.record, finance.record, nature.record), "utf8");
        fs.writeFileSync(path.join(process.cwd(), "STEP-10G-VALIDATION-REPORT.md"), buildMainReport(status, results, storageRoot, allPassed, tech.record, health.record, finance.record, nature.record), "utf8");
...
function buildMainReport(status, results, storageRoot, allPassed, tech, health, finance, nature) {
    return [
...
function buildEnvironmentReport(...records) {
    ...
    return [
        ...
        "|------|----------|----------|---------|------|----------------|",
        ...rows.map((r) => `| ${r.profile.environmentCategory} | ${r.profile.environmentCategory} | ${r.environmentAnalysis.location.slice(0, 20)} | ${r.environmentAnalysis.weather} | ${r.environmentAnalysis.intendedMood} | ${r.environmentAnalysis.indoorOutdoor} |`),
    ].join("\n");
}
function buildSpatialReport(...records) {
    const rows = records.filter(Boolean);
    return [
        ...
        "|------|----------|----------|-------|---------------|",
        ...rows.map((r) => `| ${r.profile.platform} | ${r.spatialAudioPlan.surroundPreparation.slice(0, 25)}... | ${r.spatialAudioPlan.binauralPreparation.slice(0, 25)}... | ${r.spatialAudioPlan.depth.slice(0, 20)}... | ${r.scores.spatialAudioScore}/100 |`),
    ].join("\n");
}
function buildTimelineReport(...records) {
    ...
    return [
        ...
        "|------|------|----------|------|------------|",
        ...rows.map((r) => `| ${r.profile.environmentCategory} | ${r.timelinePlan.cuePoints.length} | ${r.timelinePlan.totalDurationSec}s | ${r.timelinePlan.loopPlanning.slice(0, 20)}... | ${r.scores.synchronizationScore}/100 |`),
    ].join("\n");
}
function buildReadinessReport(status, ...records) {
    ...
    return [
        ...
        "|------|---------|-----------|---------|------|------------|------------|-------|",
        ...rows.map((r) => `| ${r.profile.environmentCategory} | ${r.scores.environmentalRealismScore} | ${r.scores.immersionScore} | ${r.scores.spatialAudioScore} | ${r.scores.synchronizationScore} | ${r.scores.productionReadinessScore} | ${r.scores.aiConfidenceScore} | ${r.productionReady ? "✅" : "❌"} |`),
    ].join("\n");
}
void main();
//# sourceMappingURL=validate-ambient-audio-generation-engine.js.map