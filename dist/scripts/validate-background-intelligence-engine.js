...
import { BackgroundType, BackgroundMarketingGoal, createAiCore, ImageAnalysisType, ImageColorSpace, ImageCompressionType, ImageFileFormat, ImageUnderstandingMarketingGoal, ImageUnderstandingPlatform, } from "../ai/index.js";
...
async function main() {
    ...
    try {
        ...
        results.backgroundClassification = {
            passed: product.record?.classification.classificationTags.length !== undefined &&
        ...
        results.backgroundQuality = {
            passed: (product.record?.quality.visualQuality ?? 0) >= 50 &&
        ...
        results.backgroundSuitability = {
            passed: (product.record?.suitability.productShowcase ?? 0) >= 55 &&
        ...
        results.qualityScores = {
            passed: (product.record?.scores.backgroundQualityScore ?? 0) >= 55 &&
        ...
        fs.writeFileSync(path.join(projectStateDir, "Background-Analysis-Report.md"), buildAnalysisReport(status, results, storageRoot, allPassed, product.record, lifestyle.record, banner.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Background-Suitability-Report.md"), buildSuitabilityReport(product.record, lifestyle.record, banner.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Background-Quality-Report.md"), buildQualityReport(product.record, lifestyle.record, banner.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Background-Readiness-Report.md"), buildReadinessReport(status, allPassed), "utf8");
        fs.writeFileSync(path.join(process.cwd(), "STEP-6E-VALIDATION-REPORT.md"), buildAnalysisReport(status, results, storageRoot, allPassed, product.record, lifestyle.record, banner.record), "utf8");
...
function buildSuitabilityReport(product, lifestyle, banner) {
    ...
    return [
        ...
        "|-------|----------|---------------|--------|--------|--------|-----------|-------|",
        ...rows.map((r) => `| ${r.imageId} | ${r.suitability.productShowcase} | ${r.suitability.advertisement} | ${r.suitability.socialMedia} | ${r.suitability.poster} | ${r.suitability.banner} | ${r.suitability.thumbnail} | ${r.suitability.videoProduction} |`),
        "",
        "## Suitability Scores",
        "",
...
function buildQualityReport(product, lifestyle, banner) {
    ...
    return [
        ...
        "|-------|------|------------|----------------|-------------|---------------|--------------|-------------|",
        ...rows.map((r) => `| ${r.imageId} | ${r.analysis.backgroundType} | ${r.analysis.backgroundComplexity} | ${r.quality.visualQuality} | ${r.quality.backgroundDistraction} | ${r.quality.colorHarmony} | ${r.quality.brandCompatibility} | ${r.analysis.backgroundCleanliness} |`),
        "",
        "## Quality Scores",
        "",
        ...rows.map((r) => `- ${r.imageId}: quality ${r.scores.backgroundQualityScore}/100, brand ${r.scores.brandCompatibilityScore}/100, creative ${r.scores.creativeReadinessScore}/100`),
        "",
    ].join("\n");
}
...
void main();
//# sourceMappingURL=validate-background-intelligence-engine.js.map