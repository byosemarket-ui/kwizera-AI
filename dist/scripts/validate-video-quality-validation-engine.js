...
import { createAiCore, CreativePlatform, MarketingObjective, QUALITY_PLATFORM_TARGETS, ProductAnalysisCategory, ProductAvailabilityStatus, ProductBusinessType, ProductUnderstandingMarketingGoal, QualityIssueSeverity, StoryboardGenerationPlatform, } from "../ai/index.js";
...
async function main() {
    ...
    try {
        ...
        results.qualityScores = {
            passed: (firstRecord?.scores.overallQualityScore ?? 0) >= 55 &&
        ...
        fs.writeFileSync(path.join(projectStateDir, "AI-Video-Quality-Validation-Report.md"), buildMainReport(status, results, storageRoot, allPassed, techValidation.validations, fashionValidation.validations, beautyValidation.validations), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Visual-Validation-Report.md"), buildVisualReport(techValidation.validations), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Audio-Validation-Report.md"), buildAudioReport(techValidation.validations), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Brand-Validation-Report.md"), buildBrandReport(techValidation.validations), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Render-Readiness-Report.md"), buildReadinessReport(status, techValidation.validations, fashionValidation.validations, beautyValidation.validations), "utf8");
        fs.writeFileSync(path.join(process.cwd(), "STEP-8L-VALIDATION-REPORT.md"), buildMainReport(status, results, storageRoot, allPassed, techValidation.validations, fashionValidation.validations, beautyValidation.validations), "utf8");
...
function buildVisualReport(validations) {
    ...
    for (const record of validations ?? []) {
        ...
        lines.push(`## ${record.validationId}`, "", `- Scene continuity: ${v.sceneContinuity}`, `- Camera continuity: ${v.cameraContinuity}`, `- Visual consistency: ${v.visualConsistency}`, `- Color consistency: ${v.colorConsistency}`, `- All visual checks passed: ${v.allVisualChecksPassed}`, `- Visual score: ${record.scores.visualQualityScore}`, "");
    }
    return lines.join("\n");
}
function buildAudioReport(validations) {
    const lines = ["# Audio Validation Report — Step 8L", "", `**Date:** ${new Date().toISOString()}`, ""];
    for (const record of validations ?? []) {
        const a = record.audioQuality;
        lines.push(`## ${record.validationId}`, "", `- Voice quality: ${a.voiceQuality}`, `- Music quality: ${a.musicQuality}`, `- Lip sync: ${a.lipSync}`, `- Audio balance: ${a.audioBalance}`, `- All audio checks passed: ${a.allAudioChecksPassed}`, `- Audio score: ${record.scores.audioQualityScore}`, "");
    }
    return lines.join("\n");
}
function buildBrandReport(validations) {
    const lines = ["# Brand Validation Report — Step 8L", "", `**Date:** ${new Date().toISOString()}`, ""];
    for (const record of validations ?? []) {
        const b = record.brandQuality;
        lines.push(`## ${record.validationId}`, "", `- Logo usage: ${b.logoUsage}`, `- Brand colors: ${b.brandColors}`, `- Brand typography: ${b.brandTypography}`, `- Marketing consistency: ${b.marketingConsistency}`, `- All brand checks passed: ${b.allBrandChecksPassed}`, `- Brand score: ${record.scores.brandConsistencyScore}`, "");
    }
    return lines.join("\n");
}
...
void main();
//# sourceMappingURL=validate-video-quality-validation-engine.js.map