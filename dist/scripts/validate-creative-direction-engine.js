...
import { createAiCore, CreativePlatform, CreativeDirectionStyle, MarketingObjective, ProductAnalysisCategory, ProductAnalysisIndustry, ProductAvailabilityStatus, ProductBusinessType, ProductUnderstandingMarketingGoal, } from "../ai/index.js";
...
async function preparePipeline(foundation, sample, objective) {
...
async function main() {
    ...
    try {
        ...
        results.creativeProfile = {
            passed: Boolean(tech.record?.profile.creativeId) &&
        ...
        results.visualDirection = {
            passed: (tech.record?.visualDirection.colorPalette.length ?? 0) >= 3 &&
        ...
        results.cinematicDirection = {
            passed: Boolean(tech.record?.cinematicDirection.cameraStyle) &&
        ...
        results.brandDirection = {
            passed: (tech.record?.brandDirection.brandColors.length ?? 0) >= 2 &&
        ...
        results.marketingDirection = {
            passed: Boolean(tech.record?.marketingDirection.hookDirection) &&
        ...
        results.creativeScores = {
            passed: (tech.record?.scores.creativeQualityScore ?? 0) >= 55 &&
        ...
        fs.writeFileSync(path.join(projectStateDir, "Creative-Direction-Report.md"), buildCreativeDirectionReport(status, results, storageRoot, allPassed, tech.record, fashion.record, beauty.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Creative-Style-Report.md"), buildCreativeStyleReport(tech.record, fashion.record, beauty.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Creative-Readiness-Report.md"), buildCreativeReadinessReport(status, tech.record, fashion.record, beauty.record), "utf8");
        fs.writeFileSync(path.join(process.cwd(), "STEP-5F-VALIDATION-REPORT.md"), buildCreativeDirectionReport(status, results, storageRoot, allPassed, tech.record, fashion.record, beauty.record), "utf8");
...
function buildCreativeDirectionReport(status, results, storageRoot, allPassed, tech, fashion, beauty) {
    return [
...
function buildCreativeStyleReport(tech, fashion, beauty) {
    ...
    return [
        ...
        "|---------|-------|-------|------|------|----------|-------------------|",
        ...rows.map((r) => `| ${r.productId} | ${r.profile.creativeStyle} | ${r.profile.creativeTheme.slice(0, 40)}... | ${r.profile.mood} | ${r.profile.tone.slice(0, 30)} | ${r.profile.platform} | ${r.scores.brandConsistencyScore}/100 |`),
        "",
    ].join("\n");
}
function buildCreativeReadinessReport(status, tech, fashion, beauty) {
    const rows = [tech, fashion, beauty].filter(Boolean);
    return [
        ...
        "|---------|---------|-----------|--------|----------|------------|-----------|-----------|",
        ...rows.map((r) => `| ${r.productId} | ${r.scores.creativeQualityScore}/100 | ${r.scores.marketingAlignmentScore}/100 | ${r.scores.visualDirectionScore}/100 | ${r.scores.audienceAlignmentScore}/100 | ${r.scores.aiConfidenceScore}/100 | ${r.platformDirections.length} | ${r.validated ? "✅" : "❌"} |`),
        "",
        "## Performance",
        "",
...
void main();
//# sourceMappingURL=validate-creative-direction-engine.js.map