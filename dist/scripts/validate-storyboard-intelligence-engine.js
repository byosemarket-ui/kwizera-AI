...
async function main() {
    ...
    try {
        ...
        results.storyboardProfile = {
            passed: Boolean(tech.record?.profile.storyboardId) &&
        ...
        results.scenePlanning = {
            passed: (tech.record?.scenes.length ?? 0) >= 5 &&
        ...
        results.storyFlow = {
            passed: Boolean(tech.record?.storyFlow.hook) &&
        ...
        results.timingIntelligence = {
            passed: Boolean(tech.record?.timing.hookTiming) &&
        ...
        results.continuityValidation = {
            passed: tech.record?.continuity.storyConsistency === true &&
        ...
        results.storyboardScores = {
            passed: (tech.record?.scores.storyboardQualityScore ?? 0) >= 55 &&
        ...
        results.relationshipDetection = {
            passed: (tech.record?.relationships.creativeDirections.length ?? 0) >= 1 &&
        ...
        fs.writeFileSync(path.join(projectStateDir, "Storyboard-Report.md"), buildStoryboardReport(status, results, storageRoot, allPassed, tech.record, fashion.record, beauty.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Scene-Planning-Report.md"), buildScenePlanningReport(tech.record, fashion.record, beauty.record), "utf8");
        ...
        fs.writeFileSync(path.join(process.cwd(), "STEP-5G-VALIDATION-REPORT.md"), buildStoryboardReport(status, results, storageRoot, allPassed, tech.record, fashion.record, beauty.record), "utf8");
...
function buildStoryboardReport(status, results, storageRoot, allPassed, tech, fashion, beauty) {
    return [
...
function buildScenePlanningReport(tech, fashion, beauty) {
    ...
    for (const record of rows) {
        ...
        for (const scene of record.scenes) {
            lines.push(`| ${scene.sceneNumber} | ${scene.scenePurpose} | ${scene.estimatedDuration} | ${scene.cameraDirection.slice(0, 30)}... | ${scene.ctaPlacement === "none" ? "—" : "✓"} |`);
        }
        lines.push("");
    }
    return lines.join("\n");
...
function buildReadinessReport(status, tech, fashion, beauty) {
    ...
    return [
        ...
        "|---------|---------|--------------|--------|-------|------------------|-----------|",
        ...rows.map((r) => `| ${r.productId} | ${r.scores.storyboardQualityScore}/100 | ${r.scores.storytellingScore}/100 | ${r.scores.visualPlanningScore}/100 | ${r.scores.brandConsistencyScore}/100 | ${r.productionReady ? "✅" : "❌"} | ${r.validated ? "✅" : "❌"} |`),
        "",
        "## Performance",
        "",
...
void main();
//# sourceMappingURL=validate-storyboard-intelligence-engine.js.map