...
import { createAiCore, VideoAnalysisType, VideoCodec, AudioCodec, VideoContainer, VideoFileFormat, FrameRateMode, VideoColorSpace, SceneClassification, TransitionType, } from "../ai/index.js";
...
async function main() {
    ...
    try {
        ...
        results.transitionDetection = {
            passed: (commercial.record?.transitionCount ?? 0) >= 2 &&
        ...
        results.sceneClassification = {
            passed: commercial.record?.scenes.some((s) => s.classification === SceneClassification.Intro) === true &&
        ...
        results.indexCreation = {
            passed: (commercial.record?.indexes.sceneIndexIds.length ?? 0) >= 3 &&
        ...
        const logFile = path.join(storageRoot, "logs", `scene-detection-intelligence-engine-${logDate}.jsonl`);
        results.logging = {
        ...
        fs.writeFileSync(path.join(projectStateDir, "Scene-Detection-Report.md"), buildSceneReport(status, results, storageRoot, allPassed), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Shot-Detection-Report.md"), buildShotReport(commercial.record, social.record, tutorial.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Transition-Report.md"), buildTransitionReport(commercial.record, social.record, tutorial.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Timeline-Detection-Report.md"), buildTimelineReport(commercial.record, social.record, tutorial.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Scene-Readiness-Report.md"), buildReadinessReport(status, commercial.record, social.record, tutorial.record, allPassed), "utf8");
        fs.writeFileSync(path.join(process.cwd(), "STEP-7D-VALIDATION-REPORT.md"), buildSceneReport(status, results, storageRoot, allPassed), "utf8");
...
function buildSceneReport(status, results, storageRoot, allPassed) {
    return [
...
function buildShotReport(commercial, social, tutorial) {
    ...
    return [
        ...
        "|-------|-------|------------|----------------|------------|",
        ...rows.map((r) => `| ${r.videoId} | ${r.shotCount} | ${r.scores.shotDetectionScore} | ${r.shots.filter((s) => s.cameraChange).length} | ${[...new Set(r.shots.map((s) => s.shotType))].join(", ")} |`),
        "",
    ].join("\n");
}
function buildTransitionReport(commercial, social, tutorial) {
    const rows = [commercial, social, tutorial].filter(Boolean);
    return [
        ...
        "|-------|-------------|------------------|-------|",
        ...rows.map((r) => `| ${r.videoId} | ${r.transitionCount} | ${r.scores.transitionScore} | ${[...new Set(r.transitions.map((t) => t.type))].join(", ")} |`),
        "",
...
function buildTimelineReport(commercial, social, tutorial) {
    ...
    return [
        ...
        "|-------|-----------------|--------|-------------------|------------------|",
        ...rows.map((r) => `| ${r.videoId} | ${(r.timelineLengthMs / 1000).toFixed(1)}s | ${r.sceneCount} | ${r.scores.timelineAccuracyScore} | ${r.indexes.timelineIndexIds.length} |`),
        "",
    ].join("\n");
}
function buildReadinessReport(status, commercial, social, tutorial, allPassed) {
    ...
    return [
        ...
        "|-------|-------|------|------------|----------|------------|",
        ...rows.map((r) => `| ${r.videoId} | ${r.scores.sceneDetectionScore} | ${r.scores.shotDetectionScore} | ${r.scores.transitionScore} | ${r.scores.timelineAccuracyScore} | ${r.scores.aiConfidenceScore} |`),
        "",
        "## Index Summary",
        "",
        ...rows.map((r) => `- **${r.videoId}:** ${r.indexes.sceneIndexIds.length} scene, ${r.indexes.shotIndexIds.length} shot, ${r.indexes.transitionIndexIds.length} transition indexes`),
        "",
    ].join("\n");
}
void main();
//# sourceMappingURL=validate-scene-detection-intelligence-engine.js.map