...
import { createAiCore, VideoAnalysisType, VideoCodec, AudioCodec, VideoContainer, VideoFileFormat, ProductionVideoPlatform, ProductionVideoWorkflowStep, } from "../ai/index.js";
...
async function main() {
    ...
    try {
        ...
        results.workflowPlanning = {
            passed: Boolean(commercial.record?.workflow.analysisValidation) &&
        ...
        results.assetValidation = {
            passed: (commercial.record?.assets.sourceVideos.every((a) => a.status !== "missing") ?? false) &&
        ...
        results.dependencyValidation = {
            passed: commercial.record?.dependencies.allRequiredPassed === true &&
        ...
        results.renderPreparation = {
            passed: Boolean(commercial.record?.renderPreparation.resolution) &&
        ...
        results.exportPreparation = {
            passed: Boolean(commercial.record?.exportPreparation.mp4) &&
        ...
        const logFile = path.join(storageRoot, "logs", `production-video-planning-engine-${new Date().toISOString().slice(0, 10)}.jsonl`);
        ...
        fs.writeFileSync(path.join(projectStateDir, "Production-Video-Planning-Report.md"), buildMainReport(status, results, storageRoot, allPassed), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Workflow-Validation-Report.md"), buildWorkflowReport(commercial.record, social.record, tutorial.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Asset-Validation-Report.md"), buildAssetReport(commercial.record, social.record, tutorial.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Production-Readiness-Report.md"), buildReadinessReport(status, commercial.record, social.record, tutorial.record, allPassed), "utf8");
        fs.writeFileSync(path.join(process.cwd(), "STEP-7K-VALIDATION-REPORT.md"), buildMainReport(status, results, storageRoot, allPassed), "utf8");
...
function buildWorkflowReport(a, b, c) {
    ...
    return [
        ...
        "|-------|----------|----------|-------------|-------------|----------|",
        ...rows.map((r) => `| ${r.videoId} | ${r.workflow.analysisValidation.slice(0, 25)} | ${r.workflow.creativeValidation.slice(0, 25)} | ${r.workflow.renderingPreparation.slice(0, 25)} | ${r.workflow.exportPreparation.slice(0, 25)} | ${r.workflow.deliveryPreparation.slice(0, 25)} |`),
        "",
    ].join("\n");
}
function buildAssetReport(a, b, c) {
    const rows = [a, b, c].filter(Boolean);
    return [
        ...
        "|-------|--------|-------|------|----------|----------|-------|-------------|",
        ...rows.map((r) => `| ${r.videoId} | ${r.assets.sourceVideos[0]?.status} | ${r.assets.audio[0]?.status} | ${r.assets.logos[0]?.status} | ${r.assets.templates[0]?.status} | ${r.assets.captions[0]?.status} | ${r.assets.brandAssets[0]?.status} | ${r.scores.assetReadinessScore} |`),
        "",
    ].join("\n");
}
function buildReadinessReport(status, a, b, c, allPassed) {
    ...
    return [
        ...
        "|-------|------------|-------|----------|------------|-------------|------------|-------|",
        ...rows.map((r) => `| ${r.videoId} | ${r.scores.productionReadinessScore} | ${r.scores.assetReadinessScore} | ${r.scores.workflowScore} | ${r.scores.dependencyScore} | ${r.scores.performanceScore} | ${r.scores.aiConfidenceScore} | ${r.productionReady ? "✅" : "❌"} |`),
        "",
    ].join("\n");
}
void main();
//# sourceMappingURL=validate-production-video-planning-engine.js.map