...
import { AiCore, PREPARED_VIDEO_INTELLIGENCE_MODULES, VideoAnalysisType, VideoCodec, AudioCodec, VideoContainer, VideoFileFormat, VideoIntelligenceAccessOperation, VideoIntelligenceCategory, VideoIntelligenceLifecycleState, VideoQualityPredictionPlatform, createAiCore, } from "../ai/index.js";
...
function stressVideoInput(index) {
...
function estimateFrames(videos) {
...
async function main() {
    ...
    try {
        ...
        for (const mod of MODULES_TO_CERTIFY) {
            if (mod.id === "video-intelligence-foundation")
                continue;
        ...
        integrationResults["product-intelligence-bridge"] = {
            passed: viIntegration.productIntelligenceEngine &&
        ...
        integrationResults["image-intelligence-bridge"] = {
            passed: viIntegration.imageIntelligenceEngine &&
        ...
        integrationResults["scene-timeline-chain"] = {
            passed: sceneDetection.buildStatusReport().engineStatus === "operational" &&
        ...
        integrationResults["planning-pipeline-chain"] = {
            passed: enhancement.buildStatusReport().engineStatus === "operational" &&
        ...
        liveResults.multiBrandCampaign = {
            passed: analysis.getVideo(LIVE_SOCIAL.videoId)?.validated === true &&
        ...
        const uniqueBrands = new Set(analysis
            .searchVideos({ limit: 10000 })
            .flatMap((v) => v.relationships.relatedBrands)
            .filter(Boolean));
        const uniqueCampaigns = new Set(analysis
            .searchVideos({ limit: 10000 })
        ...
        integrityResults.planningStagesComplete = {
            passed: Boolean(prodPlan?.analysisId &&
        ...
        integrityResults.noCorruptedPlanning = {
            passed: healthCheck.videoQualityIntegrity &&
        ...
        integrityResults.noMissingStages = {
            passed: Boolean(sceneDetection.getDetection(commercialId) &&
        ...
        healthResults.videoIntegrityHealth = {
            ...
            detail: healthCheck.videoQualityIntegrity && healthCheck.sceneIntegrity
        ...
        const reports = {
            certification: buildCertificationReport(moduleCertification, integrationResults, liveResults, stressResults, integrityResults, readinessResults, healthResults, performance, scores, storageRoot, stress, phase7Approved, healthStatus),
        ...
        fs.writeFileSync(path.join(certRecordDir, "phase-7-certification.json"), JSON.stringify({
            phase: 7,
            step: "7O",
        ...
        console.log(`Phase 7 Status: ${phase7Approved ? "✅ APPROVED — COMPLETE" : "❌ NOT APPROVED — ISSUES REMAIN"}`);
        if (!phase7Approved) {
            console.log("\nFailed checks:");
...
void main();
//# sourceMappingURL=validate-video-intelligence-certification.js.map