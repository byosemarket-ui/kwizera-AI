...
import { ALL_IMAGE_RENDER_ASSET_TYPES, ALL_IMAGE_RENDER_LAYER_CHECKS, ALL_IMAGE_RENDER_MASK_TYPES, ALL_IMAGE_RENDER_PLATFORMS, ALL_IMAGE_RENDER_VALIDATION_STAGES, BrandDesignGenPlatform, BrandDesignType, createAiCore, CreativePlatform, ImageProductionPlatform, ImageRenderColorSpace, ImageRenderPlatform, MarketingObjective, MultiStyleGenPlatform, MultiStyleImageCategory, ProductAnalysisCategory, ProductAvailabilityStatus, ProductBusinessType, ProductImageGenPlatform, ProductUnderstandingMarketingGoal, } from "../ai/index.js";
...
async function main() {
    ...
    try {
        ...
        results.upstreamPreparation = {
            passed: techProduction.success && fashionProduction.success && foodProduction.success,
        ...
        results.renderValidation = {
            passed: (tech.record?.renderValidation.length ?? 0) === ALL_IMAGE_RENDER_VALIDATION_STAGES.length &&
        ...
        results.layerValidation = {
            passed: (tech.record?.layerValidation.length ?? 0) === ALL_IMAGE_RENDER_LAYER_CHECKS.length &&
        ...
        results.maskValidation = {
            passed: (tech.record?.maskValidation.length ?? 0) === ALL_IMAGE_RENDER_MASK_TYPES.length &&
        ...
        results.assetValidation = {
            passed: (tech.record?.assetValidation.length ?? 0) === ALL_IMAGE_RENDER_ASSET_TYPES.length &&
        ...
        results.renderSettings = {
            passed: Boolean(tech.record?.renderSettings.resolution) &&
...
function buildRenderProfileReport(tech, fashion, food) {
    ...
    for (const record of rows) {
        lines.push(`| ${record.relationships.products[0] ?? "n/a"} | ${record.profile.platform} | ${record.renderSettings.resolution} | ${record.renderSettings.dpi} | ${record.renderSettings.colorSpace} | ${record.renderSettings.bitDepth} | ${record.renderSettings.alphaChannel ? "✅" : "❌"} |`);
    }
    return lines.join("\n");
}
function buildLayerReport(tech, fashion, food) {
    ...
    for (const record of rows) {
        ...
        lines.push(`| ${record.relationships.products[0] ?? "n/a"} | ${record.layerStructure.length} | ${checks}/${record.layerValidation.length} | ${record.scores.layerIntegrityScore}/100 | ${record.renderReady ? "✅" : "❌"} |`);
    }
    return lines.join("\n");
}
function buildMaskReport(tech, fashion, food) {
    ...
    for (const record of rows) {
        ...
        lines.push(`| ${record.relationships.products[0] ?? "n/a"} | ${record.maskValidation.length} | ${validated}/${record.maskValidation.length} | ${record.scores.maskIntegrityScore}/100 |`);
    }
    return lines.join("\n");
}
function buildReadinessReport(status, tech, fashion, food) {
    ...
    return [
        ...
        "|---------|--------------|-----------|--------|--------|-------|----------|-------------|------------|",
        ...rows.map((r) => `| ${r.relationships.products[0] ?? "n/a"} | ${r.renderReady ? "✅" : "❌"} | ${r.scores.renderReadinessScore} | ${r.scores.assetQualityScore} | ${r.scores.layerIntegrityScore} | ${r.scores.maskIntegrityScore} | ${r.scores.platformCompatibilityScore} | ${r.scores.performanceScore} | ${r.scores.aiConfidenceScore} |`),
        "",
        "## Performance",
        "",
...
void main();
//# sourceMappingURL=validate-image-rendering-preparation-engine.js.map