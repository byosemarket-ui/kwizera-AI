...
async function main() {
    ...
    try {
        ...
        results.qualityProfile = {
            passed: Boolean(tech.record?.profile.predictionId) &&
        ...
        results.qualityAnalysis = {
            passed: Boolean(tech.record?.analysis.storyboard) &&
        ...
        results.qualityChecks = {
            passed: tech.record?.checks.brandConsistency === true &&
        ...
        results.predictionEngine = {
            passed: (tech.record?.predictions.successProbability ?? 0) >= 55 &&
        ...
        results.platformQuality = {
            passed: Boolean(tech.record?.platformQuality.platform) &&
        ...
        fs.writeFileSync(path.join(projectStateDir, "Quality-Prediction-Report.md"), buildQualityPredictionReport(status, results, storageRoot, allPassed, tech.record, fashion.record, beauty.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Risk-Analysis-Report.md"), buildRiskAnalysisReport(tech.record, fashion.record, beauty.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Production-Quality-Report.md"), buildProductionQualityReport(tech.record, fashion.record, beauty.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Quality-Recommendations.md"), buildRecommendationsReport(tech.record, fashion.record, beauty.record), "utf8");
        fs.writeFileSync(path.join(process.cwd(), "STEP-5L-VALIDATION-REPORT.md"), buildQualityPredictionReport(status, results, storageRoot, allPassed, tech.record, fashion.record, beauty.record), "utf8");
...
function buildQualityPredictionReport(status, results, storageRoot, allPassed, tech, fashion, beauty) {
    return [
...
function buildRiskAnalysisReport(tech, fashion, beauty) {
    ...
    for (const record of rows) {
        lines.push(`## ${record.productId} — ${record.profile.platform}`, "", `- **Production Risk:** ${record.predictions.productionRisk}`, `- **Success Probability:** ${record.predictions.successProbability}%`, `- **Complexity:** ${record.predictions.productionComplexity}`, "", "| Category | Description | Severity | Resolved |", "|----------|-------------|----------|----------|");
...
function buildProductionQualityReport(tech, fashion, beauty) {
    const rows = [tech, fashion, beauty].filter(Boolean);
    return [
...
function buildRecommendationsReport(tech, fashion, beauty) {
    ...
    for (const record of rows) {
        ...
        lines.push(`## ${record.productId}`, "", "### Story", ...r.storyImprovements.map((i) => `- ${i}`), "", "### Visual", ...r.visualImprovements.map((i) => `- ${i}`), "", "### Audio", ...r.audioImprovements.map((i) => `- ${i}`), "", "### Branding", ...r.brandingImprovements.map((i) => `- ${i}`), "", "### Marketing", ...r.marketingImprovements.map((i) => `- ${i}`), "", "### Platform", ...r.platformOptimization.map((i) => `- ${i}`), "", "### Production", ...r.productionOptimization.map((i) => `- ${i}`), "");
    }
    return lines.join("\n");
}
void main();
//# sourceMappingURL=validate-quality-prediction-engine.js.map