...
import { createAiCore, EditingStyle, VideoType, } from "../ai/index.js";
function createTempStorageRoot() {
    return fs.mkdtempSync(path.join(os.tmpdir(), "kwizera-validate-video-knowledge-"));
}
const SAMPLE_PROMOTIONAL = {
...
async function main() {
    ...
    try {
        ...
        results.qualityScoring = {
            passed: (promo.record?.scores.storytellingScore ?? 0) >= (basic.record?.scores.storytellingScore ?? 0) &&
        ...
        results.knowledgeStorage = {
            passed: Boolean(promo.record?.knowledgeId) &&
...
void main();
//# sourceMappingURL=validate-video-knowledge-engine.js.map