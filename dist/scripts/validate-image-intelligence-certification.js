...
import { AiCore, CreativeImagePlatform, CreativeLayoutType, EnhancementPlatform, ImageAnalysisType, ImageColorSpace, ImageCompressionType, ImageFileFormat, ImageIntelligenceAccessOperation, ImageIntelligenceCategory, ImageIntelligenceLifecycleState, ImageQualityPredictionPlatform, ImageUnderstandingMarketingGoal, ImageUnderstandingPlatform, PREPARED_IMAGE_INTELLIGENCE_MODULES, ProductionImagePlatform, createAiCore, } from "../ai/index.js";
...
function stressImageInput(index) {
...
async function main() {
    ...
    try {
        ...
        for (const mod of MODULES_TO_CERTIFY) {
            if (mod.id === "image-intelligence-foundation")
                continue;
        ...
        integrationResults["product-intelligence-bridge"] = {
            passed: iiIntegration.productIntelligenceEngine &&
        ...
        integrationResults["detection-background-chain"] = {
            passed: objectDetection.buildStatusReport().engineStatus === "operational" &&
        ...
        integrationResults["composition-lighting-brand-chain"] = {
            passed: composition.buildStatusReport().engineStatus === "operational" &&
        ...
        liveResults.multiBrandCampaign = {
            passed: analysis.getImage(LIVE_CAMPAIGN.imageId)?.validated === true &&
        ...
        const reports = {
            certification: buildCertificationReport(moduleCertification, integrationResults, liveResults, stressResults, integrityResults, readinessResults, healthResults, performance, scores, storageRoot, stress, phase6Approved, healthStatus),
        ...
        fs.writeFileSync(path.join(certRecordDir, "phase-6-certification.json"), JSON.stringify({
            phase: 6,
            step: "6O",
        ...
        console.log(`Phase 6 Status: ${phase6Approved ? "✅ APPROVED — COMPLETE" : "❌ NOT APPROVED — ISSUES REMAIN"}`);
        if (!phase6Approved) {
            console.log("\nFailed checks:");
...
void main();
//# sourceMappingURL=validate-image-intelligence-certification.js.map