...
import { createAiCore, CreativePlatform, MarketingObjective, ProductAnalysisCategory, ProductAvailabilityStatus, ProductBusinessType, ProductUnderstandingMarketingGoal, StoryboardGenerationPlatform, SCENE_PLATFORM_TARGETS, SceneType, } from "../ai/index.js";
...
async function preparePipelineAndStoryboard(piFoundation, genFoundation, sample, objective, platform, genPlatform) {
...
async function main() {
    ...
    try {
        ...
        const techStoryboard = await preparePipelineAndStoryboard(piFoundation, genFoundation, SAMPLE_TECH, MarketingObjective.ProductLaunch, CreativePlatform.YouTube, StoryboardGenerationPlatform.YouTubeLongForm);
        const fashionStoryboard = await preparePipelineAndStoryboard(piFoundation, genFoundation, SAMPLE_FASHION, MarketingObjective.ProductPromotion, CreativePlatform.InstagramReels, StoryboardGenerationPlatform.InstagramReels);
        const beautyStoryboard = await preparePipelineAndStoryboard(piFoundation, genFoundation, SAMPLE_BEAUTY, MarketingObjective.BrandAwareness, CreativePlatform.TikTok, StoryboardGenerationPlatform.TikTok);
        ...
        results.sceneStructure = {
            passed: Boolean(firstScene?.structure.sceneOrder &&
        ...
        results.visualPlanning = {
            passed: Boolean(firstScene?.visualPlan.background &&
        ...
        results.characterPlanning = {
            passed: Boolean(firstScene?.characterPlanning.characterPosition &&
        ...
        results.audioPlanning = {
            passed: Boolean(firstScene?.audioPlanning.voiceTiming &&
        ...
        results.transitionPlanning = {
            passed: Boolean(firstScene?.transitionPlanning.sceneTransition &&
        ...
        results.sceneScores = {
            passed: (firstScene?.scores.sceneQualityScore ?? 0) >= 55 &&
        ...
        results.relationships = {
            passed: (firstScene?.relationships.storyboards.length ?? 0) >= 1 &&
        ...
        fs.writeFileSync(path.join(projectStateDir, "AI-Scene-Generation-Report.md"), buildMainReport(status, results, storageRoot, allPassed, techScenes.scenes, fashionScenes.scenes, beautyScenes.scenes), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Scene-Structure-Report.md"), buildStructureReport(techScenes.scenes, fashionScenes.scenes, beautyScenes.scenes), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Shot-Generation-Report.md"), buildShotReport(techScenes.scenes), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Scene-Composition-Report.md"), buildCompositionReport(techScenes.scenes), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Scene-Readiness-Report.md"), buildReadinessReport(status, techScenes.scenes, fashionScenes.scenes, beautyScenes.scenes), "utf8");
        fs.writeFileSync(path.join(process.cwd(), "STEP-8C-VALIDATION-REPORT.md"), buildMainReport(status, results, storageRoot, allPassed, techScenes.scenes, fashionScenes.scenes, beautyScenes.scenes), "utf8");
...
function buildStructureReport(tech, fashion, beauty) {
    const lines = [
    ...
    for (const [label, scenes] of [
        ...
        for (const scene of scenes) {
            lines.push(`| ${scene.structure.sceneOrder} | ${scene.structure.sceneType} | ${scene.structure.scenePurpose} | ${scene.structure.sceneDuration} | ${scene.structure.scenePriority} | ${scene.structure.sceneMood} |`);
        }
        lines.push("");
    }
    return lines.join("\n");
}
function buildShotReport(scenes) {
    const lines = [
    ...
    for (const scene of scenes.slice(0, 5)) {
        ...
        for (const shot of scene.shots) {
            lines.push(`| ${shot.shotOrder} | ${shot.shotType} | ${shot.cameraAngle} | ${shot.cameraMovement} | ${shot.focusPoint.slice(0, 25)}... | ${shot.shotDuration} |`);
        }
        lines.push("");
    }
    return lines.join("\n");
}
function buildCompositionReport(scenes) {
    const lines = [
    ...
    for (const scene of scenes?.slice(0, 8) ?? []) {
        ...
        lines.push(`| ${scene.structure.scenePurpose} | ${v.composition.slice(0, 20)}... | ${v.lighting.slice(0, 15)}... | ${v.productPlacement.slice(0, 20)}... | ${v.logoPlacement.slice(0, 15)}... | ${v.typographyPlacement.slice(0, 15)}... |`);
    }
    lines.push("");
    return lines.join("\n");
}
function buildReadinessReport(status, tech, fashion, beauty) {
    ...
    const avgQuality = all.length > 0 ? Math.round(all.reduce((s, r) => s + r.scores.sceneQualityScore, 0) / all.length) : 0;
    return [
...
void main();
//# sourceMappingURL=validate-scene-generation-engine.js.map