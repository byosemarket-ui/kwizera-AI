...
import { CompositionMarketingGoal, CompositionPlatform, CompositionType, createAiCore, ImageAnalysisType, ImageColorSpace, ImageCompressionType, ImageFileFormat, ImageUnderstandingMarketingGoal, ImageUnderstandingPlatform, } from "../ai/index.js";
...
async function main() {
    ...
    try {
        ...
        results.visualHierarchy = {
            passed: (product.record?.visualHierarchy.mainSubjectVisibility ?? 0) >= 70 &&
        ...
        results.productPlacement = {
            passed: Boolean(product.record?.productPlacement.productPosition) &&
        ...
        results.qualityScores = {
            passed: (product.record?.scores.compositionQualityScore ?? 0) >= 55 &&
        ...
        fs.writeFileSync(path.join(projectStateDir, "Composition-Analysis-Report.md"), buildAnalysisReport(status, results, storageRoot, allPassed, product.record, lifestyle.record, banner.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Visual-Hierarchy-Report.md"), buildHierarchyReport(product.record, lifestyle.record, banner.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Composition-Quality-Report.md"), buildQualityReport(product.record, lifestyle.record, banner.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Composition-Readiness-Report.md"), buildReadinessReport(status, allPassed), "utf8");
        fs.writeFileSync(path.join(process.cwd(), "STEP-6F-VALIDATION-REPORT.md"), buildAnalysisReport(status, results, storageRoot, allPassed, product.record, lifestyle.record, banner.record), "utf8");
...
function buildAnalysisReport(status, results, storageRoot, allPassed, product, lifestyle, banner) {
    return [
...
function buildHierarchyReport(product, lifestyle, banner) {
    ...
    return [
        ...
        "|-------|--------------|-----------|------------------|-------|-----|--------------|",
        ...rows.map((r) => `| ${r.imageId} | ${r.visualHierarchy.mainSubjectVisibility} | ${r.visualHierarchy.secondarySubjectVisibility} | ${r.visualHierarchy.productPriority} | ${r.visualHierarchy.brandVisibility} | ${r.visualHierarchy.ctaVisibility} | ${r.visualHierarchy.readingFlow} |`),
        "",
        "## Hierarchy Scores",
        "",
...
function buildQualityReport(product, lifestyle, banner) {
    const rows = [product, lifestyle, banner].filter(Boolean);
    return [
        ...
        "|-------|------|---------|----------|----------------|---------|-------------------|",
        ...rows.map((r) => `| ${r.imageId} | ${r.compositionAnalysis.compositionType} | ${r.compositionAnalysis.balance} | ${r.compositionAnalysis.symmetry} | ${r.compositionAnalysis.negativeSpace} | ${r.compositionAnalysis.framing} | ${r.productPlacement.productVisibility} |`),
        "",
        "## Quality Scores",
        "",
        ...rows.map((r) => `- ${r.imageId}: composition ${r.scores.compositionQualityScore}/100, balance ${r.scores.visualBalanceScore}/100, marketing ${r.scores.marketingReadinessScore}/100`),
        "",
    ].join("\n");
}
...
void main();
//# sourceMappingURL=validate-composition-intelligence-engine.js.map