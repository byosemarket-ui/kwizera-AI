...
import { AudioMixingPlatform, AudioTrackType, createAiCore, CreativePlatform, MarketingObjective, ProductAnalysisCategory, ProductAvailabilityStatus, ProductBusinessType, ProductUnderstandingMarketingGoal, } from "../ai/index.js";
...
async function main() {
    ...
    try {
        ...
        results.multiTrackAnalysis = {
            passed: Boolean(tech.record?.multiTrackAnalysis.trackCount >= 2 &&
        ...
        results.mixingPlanning = {
            passed: Boolean(tech.record?.mixingPlan.busRouting.length >= 3 &&
        ...
        results.masteringPlanning = {
            passed: Boolean(tech.record?.masteringPlan.techniques.length >= 6 &&
        ...
        results.loudnessPlanning = {
            passed: Boolean(tech.record?.loudnessManagement.platformTarget &&
        ...
        results.frequencyBalance = {
            passed: Boolean(tech.record?.frequencyManagement.tonalBalance &&
        ...
        results.spatialAudio = {
            passed: Boolean(film.record?.spatialMixPlan.surroundPreparation &&
        ...
        results.mixMasterScores = {
            passed: (tech.record?.scores.mixingQualityScore ?? 0) >= 55 &&
                (tech.record?.scores.masteringQualityScore ?? 0) >= 55 &&
        ...
        results.relationships = {
            passed: (tech.record?.relationships.mixingPlans.length ?? 0) >= 1 &&
        ...
        const logFile = path.join(storageRoot, "logs", `audio-mixing-mastering-engine-${new Date().toISOString().slice(0, 10)}.jsonl`);
        ...
        fs.writeFileSync(path.join(projectStateDir, "AI-Audio-Mixing-Report.md"), buildMixingReport(tech.record, health.record, finance.record, film.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "AI-Audio-Mastering-Report.md"), buildMasteringReport(tech.record, health.record, finance.record, film.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Loudness-Planning-Report.md"), buildLoudnessReport(tech.record, health.record, finance.record, film.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Frequency-Balance-Report.md"), buildFrequencyReport(tech.record, health.record, finance.record, film.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Audio-Mastering-Readiness-Report.md"), buildReadinessReport(status, tech.record, health.record, finance.record, film.record), "utf8");
        fs.writeFileSync(path.join(process.cwd(), "STEP-10I-VALIDATION-REPORT.md"), buildMainReport(status, results, allPassed, tech.record, health.record, finance.record, film.record), "utf8");
...
function buildMixingReport(...records) {
    ...
    return [
        ...
        "|------|----------|--------|-------|--------------|",
        ...rows.map((r) => `| ${r.profile.sessionId.slice(0, 20)} | ${r.profile.platform} | ${r.multiTrackAnalysis.trackCount} | ${r.mixingPlan.busRouting.length} | ${r.scores.mixingQualityScore}/100 |`),
    ].join("\n");
}
function buildMasteringReport(...records) {
    const rows = records.filter(Boolean);
    return [
        ...
        "|------|----------|------------|-------------|-----------------|",
        ...rows.map((r) => `| ${r.masteringPlanId.slice(0, 30)}... | ${r.profile.platform} | ${r.masteringPlan.techniques.length} | ${r.masteringPlan.targetLufs} | ${r.scores.masteringQualityScore}/100 |`),
    ].join("\n");
}
function buildLoudnessReport(...records) {
    const rows = records.filter(Boolean);
    return [
        ...
        "|------|----------|--------|-----------|----------------|",
        ...rows.map((r) => `| ${r.profile.platform} | ${r.outputPreparation.loudnessTarget} | ${r.loudnessManagement.platformTarget.slice(0, 25)}... | ${r.loudnessManagement.streamingLoudness.slice(0, 20)}... | ${r.scores.loudnessScore}/100 |`),
    ].join("\n");
}
function buildFrequencyReport(...records) {
    const rows = records.filter(Boolean);
    return [
        ...
        "|------|-----|-----|------|---------------|-----------------|",
        ...rows.map((r) => `| ${r.profile.platform} | ${r.frequencyManagement.lowFrequencies.slice(0, 20)}... | ${r.frequencyManagement.midFrequencies.slice(0, 20)}... | ${r.frequencyManagement.highFrequencies.slice(0, 20)}... | ${r.frequencyManagement.frequencyMasking.length} | ${r.scores.frequencyBalanceScore}/100 |`),
    ].join("\n");
}
function buildReadinessReport(status, ...records) {
    ...
    return [
        ...
        "|------|-----|--------|----------|-----------|------------|------------|-------|",
        ...rows.map((r) => `| ${r.profile.platform} | ${r.scores.mixingQualityScore} | ${r.scores.masteringQualityScore} | ${r.scores.loudnessScore} | ${r.scores.frequencyBalanceScore} | ${r.scores.productionReadinessScore} | ${r.scores.aiConfidenceScore} | ${r.productionReady ? "✅" : "❌"} |`),
    ].join("\n");
}
void main();
//# sourceMappingURL=validate-audio-mixing-mastering-engine.js.map