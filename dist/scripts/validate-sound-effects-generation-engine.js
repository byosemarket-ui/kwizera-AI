...
import { createAiCore, CreativePlatform, MarketingObjective, ProductAnalysisCategory, ProductAvailabilityStatus, ProductBusinessType, ProductUnderstandingMarketingGoal, SfxPlatform, SfxSyncTarget, SoundCategory, SUPPORTED_SOUND_CATEGORIES, FOLEY_TYPES, ENVIRONMENTAL_TYPES, } from "../ai/index.js";
...
async function main() {
    ...
    try {
        ...
        results.soundAnalysis = {
            passed: Boolean(tech.record?.soundAnalysis.scene &&
        ...
        results.foleyPlanning = {
            passed: Boolean(tech.record?.foleyPlan.foleyTypes.length >= 1 &&
        ...
        results.environmentalPlanning = {
            passed: Boolean(health.record?.environmentalPlan.ambientLayers.length >= 1 &&
        ...
        results.soundScores = {
            passed: (tech.record?.scores.realismScore ?? 0) >= 55 &&
                (tech.record?.scores.synchronizationScore ?? 0) >= 55 &&
        ...
        results.relationships = {
            passed: (tech.record?.relationships.soundPlans.length ?? 0) >= 1 &&
        ...
        fs.writeFileSync(path.join(projectStateDir, "AI-Sound-Effects-Generation-Report.md"), buildMainReport(status, results, storageRoot, allPassed, tech.record, health.record, finance.record, foley.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Foley-Planning-Report.md"), buildFoleyReport(tech.record, health.record, finance.record, foley.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Environmental-Sound-Report.md"), buildEnvironmentalReport(tech.record, health.record, finance.record, foley.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Timeline-Planning-Report.md"), buildTimelineReport(tech.record, health.record, finance.record, foley.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Sound-Effects-Readiness-Report.md"), buildReadinessReport(status, tech.record, health.record, finance.record, foley.record), "utf8");
        fs.writeFileSync(path.join(process.cwd(), "STEP-10F-VALIDATION-REPORT.md"), buildMainReport(status, results, storageRoot, allPassed, tech.record, health.record, finance.record, foley.record), "utf8");
...
function buildMainReport(status, results, storageRoot, allPassed, tech, health, finance, foley) {
    return [
...
function buildFoleyReport(...records) {
    ...
    for (const r of rows) {
        lines.push(`| ${r.soundPlanId.slice(0, 28)}... | ${r.profile.soundCategory} | ${r.foleyPlan.foleyTypes.join(", ")} | ${r.foleyPlan.footsteps.slice(0, 30)}... | ${r.scores.realismScore}/100 |`);
    }
    return lines.join("\n");
}
function buildEnvironmentalReport(...records) {
    const rows = records.filter(Boolean);
    const lines = [
    ...
    for (const r of rows) {
        lines.push(`| ${r.profile.soundCategory} | ${r.soundAnalysis.environment.slice(0, 25)}... | ${r.environmentalPlan.environmentalTypes.join(", ")} | ${r.environmentalPlan.ambientLayers.length} | ${r.scores.layerQualityScore}/100 |`);
    }
    return lines.join("\n");
}
function buildTimelineReport(...records) {
    ...
    for (const r of rows) {
        lines.push(`| ${r.profile.platform} | ${r.timelinePlan.cuePoints.length} | ${r.timelinePlan.totalDurationSec}s | ${r.syncPreparation.syncTarget} | ${r.scores.synchronizationScore}/100 |`);
    }
    return lines.join("\n");
}
function buildReadinessReport(status, ...records) {
    ...
    return [
        ...
        "|------|---------|------|--------|-------|------------|------------|-------|",
        ...rows.map((r) => `| ${r.profile.soundCategory} | ${r.scores.realismScore} | ${r.scores.synchronizationScore} | ${r.scores.layerQualityScore} | ${r.scores.brandConsistencyScore} | ${r.scores.productionReadinessScore} | ${r.scores.aiConfidenceScore} | ${r.productionReady ? "✅" : "❌"} |`),
        "",
        "## Performance",
        "",
...
void main();
//# sourceMappingURL=validate-sound-effects-generation-engine.js.map