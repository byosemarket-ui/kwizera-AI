...
import { createAiCore, VideoAnalysisType, VideoCodec, AudioCodec, VideoContainer, VideoFileFormat, FrameRateMode, VideoColorSpace, CinematicStyleClass, StyleTemplatePlatform, } from "../ai/index.js";
...
async function main() {
    ...
    try {
        ...
        results.editingStyle = {
            passed: Boolean(commercial.record?.editingStyle.pacing) &&
        ...
        results.relationshipDetection = {
            passed: (commercial.record?.relationships.relatedBrands.length ?? 0) >= 1 &&
        ...
        const logFile = path.join(storageRoot, "logs", `video-style-intelligence-engine-${new Date().toISOString().slice(0, 10)}.jsonl`);
        ...
        fs.writeFileSync(path.join(projectStateDir, "Video-Style-Report.md"), buildStyleReport(status, results, storageRoot, allPassed), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Editing-Style-Report.md"), buildEditingReport(commercial.record, social.record, tutorial.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Brand-Style-Report.md"), buildBrandReport(commercial.record, social.record, tutorial.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Style-Template-Report.md"), buildTemplateReport(commercial.record, social.record, tutorial.record, status), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Video-Style-Readiness.md"), buildReadinessReport(status, commercial.record, social.record, tutorial.record, allPassed), "utf8");
        fs.writeFileSync(path.join(process.cwd(), "STEP-7H-VALIDATION-REPORT.md"), buildStyleReport(status, results, storageRoot, allPassed), "utf8");
...
function buildStyleReport(status, results, storageRoot, allPassed) {
    return [
...
function buildEditingReport(a, b, c) {
    ...
    return [
        ...
        "|-------|--------|--------|-------------|------|------------|",
        ...rows.map((r) => `| ${r.videoId} | ${r.editingStyle.pacing} | ${r.editingStyle.editingRhythm} | ${r.editingStyle.transitionStyle} | ${r.editingStyle.cutStyle} | ${r.editingStyle.audioSyncStyle.slice(0, 30)} |`),
        "",
    ].join("\n");
}
function buildBrandReport(a, b, c) {
    const rows = [a, b, c].filter(Boolean);
    return [
        ...
        "|-------|-------|-------------|-----------|--------|",
        ...rows.map((r) => `| ${r.videoId} | ${r.profile.brand} | ${r.brandStyle.visualConsistency} | ${r.brandStyle.ctaStyle.slice(0, 35)} | ${r.brandStyle.brandColors.join(", ")} |`),
        "",
    ].join("\n");
}
function buildTemplateReport(a, b, c, status) {
    const rows = [a, b, c].filter(Boolean);
    return [
        ...
        "|-------|--------------|----------|-------------|-------------|",
        ...rows.map((r) => `| ${r.videoId} | ${r.templates[0]?.name ?? "—"} | ${r.templates[0]?.platform ?? "—"} | ${r.templates[0]?.matchScore ?? "—"} | ${r.templates.map((t) => t.platform).join(", ")} |`),
        "",
    ].join("\n");
}
function buildReadinessReport(status, a, b, c, allPassed) {
    ...
    return [
        ...
        "|-------|-------------|-----------|-------|---------|-----------|------------|",
        ...rows.map((r) => `| ${r.videoId} | ${r.scores.styleConsistencyScore} | ${r.scores.cinematicScore} | ${r.scores.brandStyleScore} | ${r.scores.editingQualityScore} | ${r.scores.marketingReadinessScore} | ${r.scores.aiConfidenceScore} |`),
        "",
    ].join("\n");
}
void main();
//# sourceMappingURL=validate-video-style-intelligence-engine.js.map