...
import { AudienceCategory, AudiencePlatform, createAiCore, ProductAnalysisCategory, ProductAnalysisIndustry, ProductAvailabilityStatus, ProductBusinessType, ProductUnderstandingMarketingGoal, } from "../ai/index.js";
...
async function main() {
    ...
    try {
        ...
        results.audienceProfile = {
            passed: Boolean(tech.record?.profile.audienceId) &&
        ...
        results.demographicPreparation = {
            passed: Boolean(tech.record?.demographics.businessType) &&
        ...
        results.psychologicalUnderstanding = {
            passed: (tech.record?.psychological.customerNeeds.length ?? 0) >= 2 &&
        ...
        results.audienceSegmentation = {
            passed: Boolean(tech.record?.segmentation.productType) &&
        ...
        results.platformPreferences = {
            passed: tech.record?.profile.preferredPlatforms.includes(AudiencePlatform.YouTube) === true &&
        ...
        results.audienceScores = {
            passed: (tech.record?.scores.audienceRelevanceScore ?? 0) >= 55 &&
        ...
        fs.writeFileSync(path.join(projectStateDir, "Target-Audience-Report.md"), buildTargetAudienceReport(status, results, storageRoot, allPassed, tech.record, fashion.record, beauty.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Audience-Segmentation-Report.md"), buildSegmentationReport(tech.record, fashion.record, beauty.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Audience-Readiness-Report.md"), buildReadinessReport(status, tech.record, fashion.record, beauty.record), "utf8");
        fs.writeFileSync(path.join(process.cwd(), "STEP-5D-VALIDATION-REPORT.md"), buildTargetAudienceReport(status, results, storageRoot, allPassed, tech.record, fashion.record, beauty.record), "utf8");
...
function buildTargetAudienceReport(status, results, storageRoot, allPassed, tech, fashion, beauty) {
    return [
...
function buildSegmentationReport(tech, fashion, beauty) {
    ...
    return [
        ...
        "|----------|----------|----------|--------------|-------|-----------|",
        ...rows.map((r) => `| ${r.profile.audienceName.slice(0, 30)} | ${r.profile.audienceCategory} | ${r.segmentation.industry} | ${r.segmentation.productType} | ${r.segmentation.customerNeeds.length} | ${r.profile.preferredPlatforms.join(", ")} |`),
        "",
    ].join("\n");
}
function buildReadinessReport(status, tech, fashion, beauty) {
    ...
    return [
        ...
        "|----------|-----------|------------|-----------|---------------|-----------|",
        ...rows.map((r) => `| ${r.productId} | ${r.scores.audienceRelevanceScore}/100 | ${r.scores.audienceConfidenceScore}/100 | ${r.scores.marketingReadinessScore}/100 | ${r.scores.communicationReadinessScore}/100 | ${r.validated ? "✅" : "❌"} |`),
        "",
        "## Performance",
        "",
...
void main();
//# sourceMappingURL=validate-audience-intelligence-engine.js.map