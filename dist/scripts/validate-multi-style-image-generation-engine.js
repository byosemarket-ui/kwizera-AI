...
import { ALL_MULTI_STYLE_GEN_PLATFORMS, ALL_MULTI_STYLE_IDENTITY_TARGETS, ALL_MULTI_STYLE_IMAGE_CATEGORIES, ALL_MULTI_STYLE_VARIATION_TYPES, BrandDesignType, BrandDesignGenPlatform, createAiCore, CreativePlatform, MarketingObjective, MultiStyleGenPlatform, MultiStyleImageCategory, ProductAnalysisCategory, ProductAvailabilityStatus, ProductBusinessType, ProductImageGenPlatform, ProductUnderstandingMarketingGoal, } from "../ai/index.js";
...
async function main() {
    ...
    try {
        ...
        results.styleMapping = {
            passed: Boolean(tech.record?.styleTransformation.styleMapping &&
        ...
        results.styleTransformation = {
            passed: Boolean(tech.record?.styleTransformation.compositionAdaptation) &&
        ...
        results.identityPreservation = {
            passed: (tech.record?.identityPreservation.targets.length ?? 0) === ALL_MULTI_STYLE_IDENTITY_TARGETS.length &&
        ...
        results.styleScores = {
            passed: (tech.record?.scores.styleQualityScore ?? 0) >= 55 &&
        ...
        fs.writeFileSync(path.join(projectStateDir, "AI-Multi-Style-Image-Generation-Report.md"), buildMainReport(status, results, storageRoot, allPassed, tech.record, fashion.record, food.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Style-Mapping-Report.md"), buildStyleMappingReport(tech.record, fashion.record, food.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Identity-Preservation-Report.md"), buildIdentityReport(tech.record, fashion.record, food.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Style-Validation-Report.md"), buildValidationReport(tech.record, fashion.record, food.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Multi-Style-Readiness-Report.md"), buildReadinessReport(status, tech.record, fashion.record, food.record), "utf8");
        fs.writeFileSync(path.join(process.cwd(), "STEP-9I-VALIDATION-REPORT.md"), buildMainReport(status, results, storageRoot, allPassed, tech.record, fashion.record, food.record), "utf8");
...
function buildStyleMappingReport(tech, fashion, food) {
    ...
    for (const record of rows) {
        lines.push(`| ${record.profile.productId} | ${record.profile.styleCategory} | ${record.styleTransformation.styleMapping.slice(0, 25)}... | ${record.styleTransformation.colorAdaptation.slice(0, 20)}... | ${record.styleTransformation.lightingAdaptation.slice(0, 20)}... | ${record.scores.styleQualityScore}/100 |`);
    }
    return lines.join("\n");
}
function buildIdentityReport(tech, fashion, food) {
    const rows = [tech, fashion, food].filter(Boolean);
    const lines = [
    ...
    for (const record of rows) {
        lines.push(`| ${record.profile.productId} | ${record.identityPreservation.targets.length} | ${record.identityPreservation.identityLock ? "✅" : "❌"} | ${record.identityPreservation.productLock ? "✅" : "❌"} | ${record.scores.identityPreservationScore}/100 |`);
    }
    return lines.join("\n");
}
function buildValidationReport(tech, fashion, food) {
    const rows = [tech, fashion, food].filter(Boolean);
...
function buildReadinessReport(status, tech, fashion, food) {
    ...
    return [
        ...
        "|---------|---------------|----------|--------------|-------|------------|------------|-------|",
        ...rows.map((r) => `| ${r.profile.productId} | ${r.scores.styleQualityScore} | ${r.scores.styleAccuracyScore} | ${r.scores.identityPreservationScore} | ${r.scores.brandConsistencyScore} | ${r.scores.productionReadinessScore} | ${r.scores.aiConfidenceScore} | ${r.productionReady ? "✅" : "❌"} |`),
        "",
        "## Performance",
        "",
...
void main();
//# sourceMappingURL=validate-multi-style-image-generation-engine.js.map