...
import { createAiCore, createDefaultAssetQuality, createDefaultProjectQuality, PREPARED_VIDEO_INTELLIGENCE_MODULES, SUPPORTED_VIDEO_ASSET_TYPES, SUPPORTED_VIDEO_INDEX_TYPES, SUPPORTED_VIDEO_INTELLIGENCE_SOURCES, VideoAspectRatio, VideoAssetType, VideoIntelligenceAccessOperation, VideoIntelligenceCategory, VideoIntelligenceHealthLevel, VideoIntelligenceLifecycleState, VideoIntelligenceModuleStatus, VideoIntelligenceSource, VideoIntelligenceVerificationStatus, VideoWorkflowActionType, } from "../ai/index.js";
...
async function main() {
    ...
    try {
        ...
        const registryPath = path.join(storageRoot, "video-intelligence", "registry", "video-intelligence-registry.json");
        ...
        results.integration = {
            passed: integration.readyCount >= 8 &&
        ...
        foundation.getFrameIndexManager().indexScene(project.projectId, videoId, scene.sceneId, scene.startMs, scene.endMs, timeline.timelineId);
        foundation.getFrameIndexManager().indexShot(project.projectId, videoId, "shot-intro-1", scene.sceneId, 2_000);
        ...
        results.frameIndexing = {
            passed: foundation.getFrameIndexManager().getCount() >= 6 &&
                Boolean(frameLookup) &&
        ...
        const edit = foundation.getWorkflow().recordEdit(project.projectId, videoId, VideoWorkflowActionType.Trim, "Trim intro segment", "state-v1", "state-v2", timeline.timelineId);
        ...
        results.multiTimelineSupport = {
            passed: foundation.getProjectManager().getTimelinesByProject(project.projectId).length >= 1 &&
...
function buildAssetRegistryReport(foundation, allPassed) {
...
${assets
        .map((a) => `- **${a.assetName}** (\`${a.assetId}\`) — ${a.assetType}, project \`${a.projectId}\`, v${a.version}`)
        .join("\n")}
...
function buildFrameIndexingReport(foundation, allPassed) {
...
void main();
//# sourceMappingURL=validate-video-intelligence-foundation.js.map