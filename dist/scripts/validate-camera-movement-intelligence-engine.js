...
import { createAiCore, VideoAnalysisType, VideoCodec, AudioCodec, VideoContainer, VideoFileFormat, FrameRateMode, VideoColorSpace, CameraMovementType, CameraAngle, ShotFraming, } from "../ai/index.js";
...
async function main() {
    ...
    try {
        ...
        results.cameraAngleDetection = {
            passed: commercial.record?.shotAnalyses.some((s) => s.angle === CameraAngle.EyeLevel) === true &&
        ...
        results.framingDetection = {
            passed: commercial.record?.shotAnalyses.some((s) => s.framing === ShotFraming.MediumShot || s.framing === ShotFraming.CloseUp) === true,
        ...
        results.cinematicPlanning = {
            passed: Boolean(commercial.record?.movementPlan.recommendedPath) &&
        ...
        results.relationshipDetection = {
            passed: (commercial.record?.relationships.relatedShots.length ?? 0) >= 3 &&
        ...
        const logFile = path.join(storageRoot, "logs", `camera-movement-intelligence-engine-${new Date().toISOString().slice(0, 10)}.jsonl`);
        ...
        fs.writeFileSync(path.join(projectStateDir, "Camera-Movement-Report.md"), buildMovementReport(status, results, storageRoot, allPassed), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Camera-Angles-Report.md"), buildAnglesReport(commercial.record, social.record, tutorial.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Framing-Report.md"), buildFramingReport(commercial.record, social.record, tutorial.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Cinematic-Planning-Report.md"), buildCinematicReport(commercial.record, social.record, tutorial.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Camera-Readiness-Report.md"), buildReadinessReport(status, commercial.record, social.record, tutorial.record, allPassed), "utf8");
        fs.writeFileSync(path.join(process.cwd(), "STEP-7F-VALIDATION-REPORT.md"), buildMovementReport(status, results, storageRoot, allPassed), "utf8");
...
function buildMovementReport(status, results, storageRoot, allPassed) {
    return [
...
function buildAnglesReport(a, b, c) {
    ...
    return [
        ...
        "|-------|----------------|-------------|",
        ...rows.map((r) => `| ${r.videoId} | ${r.dominantAngle} | ${[...new Set(r.shotAnalyses.map((s) => s.angle))].join(", ")} |`),
        "",
    ].join("\n");
}
function buildFramingReport(a, b, c) {
    const rows = [a, b, c].filter(Boolean);
    return [
        ...
        "|-------|------------------|---------------|",
        ...rows.map((r) => `| ${r.videoId} | ${r.dominantFraming} | ${[...new Set(r.shotAnalyses.map((s) => s.framing))].join(", ")} |`),
        "",
    ].join("\n");
}
function buildCinematicReport(a, b, c) {
    const rows = [a, b, c].filter(Boolean);
    return [
        ...
        "|-------|----------------------|-------|------------|----------|",
        ...rows.map((r) => `| ${r.videoId} | ${r.movementPlan.recommendedMovement} | ${r.movementPlan.cinematicStyle.slice(0, 40)} | ${r.movementPlan.motionContinuity} | ${r.cinematicPurposes.join(", ")} |`),
        "",
    ].join("\n");
}
function buildReadinessReport(status, a, b, c, allPassed) {
    ...
    return [
        ...
        "|-------|----------|-----------|-----------|--------------|------------|------------|",
        ...rows.map((r) => `| ${r.videoId} | ${r.scores.cameraMovementScore} | ${r.scores.cinematicScore} | ${r.scores.stabilityScore} | ${r.scores.storytellingScore} | ${r.scores.productionReadinessScore} | ${r.scores.aiConfidenceScore} |`),
        "",
    ].join("\n");
}
void main();
//# sourceMappingURL=validate-camera-movement-intelligence-engine.js.map