...
import { ALL_IMAGE_PRODUCTION_ASSET_TYPES, ALL_IMAGE_PRODUCTION_DEPENDENCIES, ALL_IMAGE_PRODUCTION_EXPORT_FORMATS, ALL_IMAGE_PRODUCTION_PLATFORMS, ALL_IMAGE_PRODUCTION_WORKFLOW_STAGES, BrandDesignGenPlatform, BrandDesignType, createAiCore, CreativePlatform, ImageProductionPlatform, MarketingObjective, MultiStyleGenPlatform, MultiStyleImageCategory, ProductAnalysisCategory, ProductAvailabilityStatus, ProductBusinessType, ProductImageGenPlatform, ProductUnderstandingMarketingGoal, } from "../ai/index.js";
...
async function main() {
    ...
    try {
        ...
        results.workflowValidation = {
            passed: (tech.record?.workflowValidation.length ?? 0) === ALL_IMAGE_PRODUCTION_WORKFLOW_STAGES.length &&
        ...
        results.assetValidation = {
            passed: (tech.record?.assetValidation.length ?? 0) === ALL_IMAGE_PRODUCTION_ASSET_TYPES.length &&
        ...
        results.dependencyValidation = {
            passed: (tech.record?.dependencyValidation.length ?? 0) === ALL_IMAGE_PRODUCTION_DEPENDENCIES.length &&
        ...
        results.layerValidation = {
            passed: (tech.record?.productionStructure.layerStructure.length ?? 0) >= 5 &&
        ...
        results.exportPreparation = {
            passed: (tech.record?.exportPreparation.exports.length ?? 0) === ALL_IMAGE_PRODUCTION_EXPORT_FORMATS.length &&
        ...
        fs.writeFileSync(path.join(projectStateDir, "AI-Image-Production-Report.md"), buildMainReport(status, results, storageRoot, allPassed, tech.record, fashion.record, food.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Production-Workflow-Report.md"), buildWorkflowReport(tech.record, fashion.record, food.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Asset-Validation-Report.md"), buildAssetReport(tech.record, fashion.record, food.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Layer-Validation-Report.md"), buildLayerReport(tech.record, fashion.record, food.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Production-Readiness-Report.md"), buildReadinessReport(status, tech.record, fashion.record, food.record), "utf8");
        fs.writeFileSync(path.join(process.cwd(), "STEP-9J-VALIDATION-REPORT.md"), buildMainReport(status, results, storageRoot, allPassed, tech.record, fashion.record, food.record), "utf8");
...
function buildWorkflowReport(tech, fashion, food) {
    ...
    for (const record of rows) {
        ...
        lines.push(`| ${record.profile.productId} | ${record.workflowValidation.length} | ${validated}/${record.workflowValidation.length} | ${record.scores.workflowScore}/100 |`);
    }
    return lines.join("\n");
}
function buildAssetReport(tech, fashion, food) {
    const rows = [tech, fashion, food].filter(Boolean);
    const lines = [
    ...
    for (const record of rows) {
        ...
        lines.push(`| ${record.profile.productId} | ${record.assetValidation.length} | ${validated}/${record.assetValidation.length} | ${record.scores.assetReadinessScore}/100 | ${deps}/${record.dependencyValidation.length} |`);
    }
    return lines.join("\n");
}
function buildLayerReport(tech, fashion, food) {
    const rows = [tech, fashion, food].filter(Boolean);
    const lines = [
    ...
    for (const record of rows) {
        lines.push(`| ${record.profile.productId} | ${record.productionStructure.layerStructure.length} | ${record.productionStructure.maskStructure.length} | ${record.scores.layerIntegrityScore}/100 | ${record.renderPreparation.instructions.length >= 2 ? "✅" : "❌"} |`);
    }
    return lines.join("\n");
}
function buildReadinessReport(status, tech, fashion, food) {
    ...
    return [
        ...
        "|---------|-----------|----------|--------|--------|--------------|------------|-------|",
        ...rows.map((r) => `| ${r.profile.productId} | ${r.scores.productionReadinessScore} | ${r.scores.workflowScore} | ${r.scores.assetReadinessScore} | ${r.scores.layerIntegrityScore} | ${r.scores.dependencyScore} | ${r.scores.aiConfidenceScore} | ${r.productionReady ? "✅" : "❌"} |`),
        "",
        "## Performance",
        "",
...
void main();
//# sourceMappingURL=validate-image-production-engine.js.map