...
import { createAiCore, CreativePlatform, MarketingObjective, MotionType, MOTION_PLATFORM_TARGETS, ProductAnalysisCategory, ProductAvailabilityStatus, ProductBusinessType, ProductUnderstandingMarketingGoal, StoryboardGenerationPlatform, } from "../ai/index.js";
...
async function main() {
    ...
    try {
        ...
        results.characterMotion = {
            passed: Boolean(firstPlan?.characterMotion.gestures &&
        ...
        results.productMotion = {
            passed: Boolean(firstPlan?.productMotion.rotation &&
        ...
        results.objectMotion = {
            passed: Boolean(firstPlan?.objectMotion.entry &&
        ...
        results.motionTiming = {
            passed: Boolean(firstPlan?.motionTiming.motionStart &&
        ...
        results.motionScores = {
            passed: (firstPlan?.scores.motionQualityScore ?? 0) >= 55 &&
        ...
        fs.writeFileSync(path.join(projectStateDir, "AI-Motion-Generation-Report.md"), buildMainReport(status, results, storageRoot, allPassed, techMotion.plans, fashionMotion.plans, beautyMotion.plans), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Character-Motion-Report.md"), buildCharacterReport(techMotion.plans), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Product-Motion-Report.md"), buildProductReport(techMotion.plans), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Motion-Synchronization-Report.md"), buildSyncReport(techMotion.plans), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Motion-Readiness-Report.md"), buildReadinessReport(status, techMotion.plans, fashionMotion.plans, beautyMotion.plans), "utf8");
        fs.writeFileSync(path.join(process.cwd(), "STEP-8E-VALIDATION-REPORT.md"), buildMainReport(status, results, storageRoot, allPassed, techMotion.plans, fashionMotion.plans, beautyMotion.plans), "utf8");
...
function buildSyncReport(plans) {
    ...
    for (const plan of plans?.slice(0, 6) ?? []) {
        ...
        lines.push(`## ${plan.profile.sceneId}`, "", `- Camera: ${s.cameraMovement}`, `- Character: ${s.characterMovement}`, `- Product: ${s.productMovement}`, `- Timing: ${s.sceneTiming}`, `- Sync points: ${s.syncPoints.join("; ")}`, "");
    }
    return lines.join("\n");
}
...
void main();
//# sourceMappingURL=validate-motion-generation-engine.js.map