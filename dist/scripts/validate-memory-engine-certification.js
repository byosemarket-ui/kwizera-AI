...
import { AiCore, LearningCategory, LearningOutcome, LearningSource, MemoryAccessOperation, MemoryCategory, MemoryLifecycleState, MemoryStorageType, ProjectStatus, ProjectType, SearchMode, createAiCore, } from "../ai/index.js";
...
async function seedBulkRecords(storage, count, projectIds) {
...
async function main() {
    ...
    try {
        ...
        stressResults.searchPerformance = {
            passed: performance.searchMs < 30000 &&
        ...
        const passRate = (group) => Object.values(group).filter((r) => r.passed).length / Math.max(Object.keys(group).length, 1);
        const baseScores = {
        ...
        const reports = {
            certification: buildCertificationReport(moduleCertification, integrationResults, liveResults, stressResults, integrityResults, knowledgeResults, healthResults, performance, scores, storageRoot, stress, phase3Approved, healthReport),
        ...
        fs.writeFileSync(path.join(certRecordDir, "phase-3-certification.json"), JSON.stringify({
            phase: 3,
            step: "3O",
...
function buildCertificationReport(moduleCertification, integrationResults, liveResults, stressResults, integrityResults, knowledgeResults, healthResults, performance, scores, storageRoot, stress, approved, healthStatus) {
    const section = (title, results) => Object.entries(results)
...
void main();
//# sourceMappingURL=validate-memory-engine-certification.js.map