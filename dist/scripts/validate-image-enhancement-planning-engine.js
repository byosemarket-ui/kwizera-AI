...
import { createAiCore, EnhancementPlatform, EnhancementPlanType, ImageAnalysisType, ImageColorSpace, ImageCompressionType, ImageFileFormat, ImageUnderstandingMarketingGoal, ImageUnderstandingPlatform, } from "../ai/index.js";
...
async function runIntelligencePipeline(foundation, sample, opts) {
...
async function main() {
    ...
    try {
        ...
        results.qualityAnalysis = {
            passed: (product.record?.qualityAnalysis.sharpness ?? 0) >= 50 &&
        ...
        results.qualityScores = {
            passed: (product.record?.scores.enhancementReadinessScore ?? 0) >= 55 &&
        ...
        fs.writeFileSync(path.join(projectStateDir, "Image-Enhancement-Report.md"), buildEnhancementReport(status, results, storageRoot, allPassed, product.record, lifestyle.record, banner.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Image-Restoration-Report.md"), buildRestorationReport(product.record, lifestyle.record, banner.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Image-Quality-Improvement-Report.md"), buildQualityReport(product.record, lifestyle.record, banner.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Enhancement-Readiness-Report.md"), buildReadinessReport(status, allPassed), "utf8");
        fs.writeFileSync(path.join(process.cwd(), "STEP-6I-VALIDATION-REPORT.md"), buildEnhancementReport(status, results, storageRoot, allPassed, product.record, lifestyle.record, banner.record), "utf8");
...
function buildEnhancementReport(status, results, storageRoot, allPassed, product, lifestyle, banner) {
    return [
...
function buildRestorationReport(product, lifestyle, banner) {
    ...
    return [
        ...
        "|-------|---------|------|-----------|------|------------------|-------------------|",
        ...rows.map((r) => `| ${r.imageId} | ${r.restorationPlan.scratchRemoval.slice(0, 25)}... | ${r.restorationPlan.dustRemoval.slice(0, 20)}... | ${r.restorationPlan.artifactReduction.slice(0, 25)}... | ${r.restorationPlan.blurReduction.slice(0, 20)}... | ${r.restorationPlan.qualityRecovery.slice(0, 30)}... | ${r.scores.restorationScore} |`),
        "",
    ].join("\n");
}
function buildQualityReport(product, lifestyle, banner) {
    ...
    return [
        ...
        "|-------|------------|-----------|-------|---------|----------|---------------|",
        ...rows.map((r) => `| ${r.imageId} | ${r.qualityAnalysis.resolutionQuality} | ${r.qualityAnalysis.sharpness} | ${r.qualityAnalysis.noise} | ${r.qualityAnalysis.visualClarity} | ${r.qualityAnalysis.exposure} | ${r.scores.imageQualityScore} |`),
        "",
        "## Planned Improvements",
        "",
        ...rows.map((r) => `- ${r.imageId}: ${r.enhancementPlan.sharpening.slice(0, 60)}...`),
        "",
    ].join("\n");
}
...
void main();
//# sourceMappingURL=validate-image-enhancement-planning-engine.js.map