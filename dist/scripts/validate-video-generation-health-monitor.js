...
import { createAiCore, CreativePlatform, MarketingObjective, ProductAnalysisCategory, ProductAvailabilityStatus, ProductBusinessType, ProductUnderstandingMarketingGoal, StoryboardGenerationPlatform, } from "../ai/index.js";
...
async function prepareFullPipeline(piFoundation, genFoundation) {
...
async function main() {
    ...
    try {
        ...
        const logFile = path.join(DEFAULT_STORAGE_ROOT, "logs", `video-generation-health-monitor-engine-${logDate}.jsonl`);
        results.logging = {
        ...
        results.projectStateReports = {
            passed: fs.existsSync(path.join(projectStateDir, "AI-Video-Generation-Health-Report.md")) &&
        ...
        fs.writeFileSync(path.join(process.cwd(), "STEP-8N-VALIDATION-REPORT.md"), buildReport(status, results, storageRoot, allPassed, check, modules, checkMs), "utf8");
...
void main();
//# sourceMappingURL=validate-video-generation-health-monitor.js.map