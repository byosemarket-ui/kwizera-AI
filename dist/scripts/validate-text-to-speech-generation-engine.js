...
import { ALL_TTS_PLATFORMS, createAiCore, CreativePlatform, EmotionType, MarketingObjective, ProductAnalysisCategory, ProductAvailabilityStatus, ProductBusinessType, ProductUnderstandingMarketingGoal, SUPPORTED_TTS_LANGUAGES, TtsLanguage, TtsOutputUseCase, TtsPlatform, VoiceType, } from "../ai/index.js";
...
async function main() {
    ...
    try {
        ...
        results.textAnalysis = {
            passed: Boolean(tech.record?.textAnalysis.language &&
        ...
        results.pronunciationPlanning = {
            passed: Boolean(tech.record?.pronunciationPlan.numberReadingRules.length >= 2 &&
        ...
        results.emotionPlanning = {
            passed: Boolean(tech.record?.emotionPlan.primaryEmotion &&
        ...
        results.speechNaturalness = {
            passed: Boolean(tech.record?.naturalnessPlan.pauses.length >= 3 &&
        ...
        results.multiLanguageSupport = {
            passed: tech.record?.textAnalysis.language === TtsLanguage.English &&
        ...
        results.speechPlanScores = {
            passed: (tech.record?.scores.pronunciationScore ?? 0) >= 55 &&
                (tech.record?.scores.naturalnessScore ?? 0) >= 55 &&
                (tech.record?.scores.emotionScore ?? 0) >= 50 &&
        ...
        fs.writeFileSync(path.join(projectStateDir, "AI-Text-to-Speech-Report.md"), buildMainReport(status, results, storageRoot, allPassed, tech.record, health.record, finance.record, kinyarwanda.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Pronunciation-Planning-Report.md"), buildPronunciationReport(tech.record, health.record, finance.record, kinyarwanda.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Emotion-Planning-Report.md"), buildEmotionReport(tech.record, health.record, finance.record, kinyarwanda.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Speech-Naturalness-Report.md"), buildNaturalnessReport(tech.record, health.record, finance.record, kinyarwanda.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Text-to-Speech-Readiness-Report.md"), buildReadinessReport(status, tech.record, health.record, finance.record, kinyarwanda.record), "utf8");
        fs.writeFileSync(path.join(process.cwd(), "STEP-10B-VALIDATION-REPORT.md"), buildMainReport(status, results, storageRoot, allPassed, tech.record, health.record, finance.record, kinyarwanda.record), "utf8");
...
function buildPronunciationReport(...records) {
    ...
    for (const r of rows) {
        lines.push(`| ${r.speechPlanId} | ${r.textAnalysis.language} | ${Object.keys(r.pronunciationPlan.pronunciationDictionary).length} | ${Object.keys(r.pronunciationPlan.acronymExpansions).length} | ${r.scores.pronunciationScore}/100 |`);
    }
    if (rows[0]) {
...
function buildEmotionReport(...records) {
    ...
    for (const r of rows) {
        lines.push(`| ${r.profile.productId ?? r.speechPlanId} | ${r.emotionPlan.primaryEmotion} | ${r.emotionPlan.secondaryEmotion ?? "n/a"} | ${r.emotionPlan.emotionIntensity} | ${r.scores.emotionScore}/100 |`);
    }
    return lines.join("\n");
}
function buildNaturalnessReport(...records) {
    const rows = records.filter(Boolean);
    const lines = [
...
function buildReadinessReport(status, ...records) {
    ...
    return [
        ...
        "|------|---------------|-------------|---------|-------|------------|------------|-------|",
        ...rows.map((r) => `| ${r.profile.language} | ${r.scores.pronunciationScore} | ${r.scores.naturalnessScore} | ${r.scores.emotionScore} | ${r.scores.brandConsistencyScore} | ${r.scores.productionReadinessScore} | ${r.scores.aiConfidenceScore} | ${r.productionReady ? "✅" : "❌"} |`),
        "",
        "## Performance",
        "",
...
void main();
//# sourceMappingURL=validate-text-to-speech-generation-engine.js.map