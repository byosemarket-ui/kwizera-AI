...
import { ALL_IMAGE_EDIT_GEN_PLATFORMS, ALL_IMAGE_EDIT_IDENTITY_TARGETS, ALL_IMAGE_EDIT_INPAINTING_TYPES, ALL_IMAGE_EDIT_MASK_TYPES, ALL_IMAGE_EDIT_OPERATIONS, ALL_IMAGE_EDIT_OUTPAINTING_TYPES, BackgroundGenPlatform, BackgroundGenType, BackgroundMarketingPreset, createAiCore, CreativePlatform, ImageEditGenPlatform, ImageEditInpaintingType, ImageEditOperationType, ImageEditOutpaintingType, MarketingObjective, ProductAnalysisCategory, ProductAvailabilityStatus, ProductBusinessType, ProductImageGenPlatform, ProductUnderstandingMarketingGoal, } from "../ai/index.js";
...
async function main() {
    ...
    try {
        ...
        results.imageAnalysis = {
            passed: Boolean(tech.record?.imageAnalysis.subject &&
        ...
        results.inpainting = {
            passed: Boolean(tech.record?.inpaintingPlan.inpaintingType &&
        ...
        results.outpainting = {
            passed: Boolean(tech.record?.outpaintingPlan.outpaintingType &&
        ...
        results.maskManagement = {
            passed: (tech.record?.maskManagement.masks.length ?? 0) === ALL_IMAGE_EDIT_MASK_TYPES.length &&
        ...
        results.identityPreservation = {
            passed: (tech.record?.identityPreservation.targets.length ?? 0) === ALL_IMAGE_EDIT_IDENTITY_TARGETS.length &&
        ...
        results.nonDestructiveEditing = {
            passed: tech.record?.nonDestructiveEditing.originalPreserved === true &&
        ...
        results.editingScores = {
            passed: (tech.record?.scores.editingQualityScore ?? 0) >= 55 &&
        ...
        fs.writeFileSync(path.join(projectStateDir, "AI-Image-Editing-Report.md"), buildMainReport(status, results, storageRoot, allPassed, tech.record, fashion.record, food.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Inpainting-Report.md"), buildInpaintingReport(tech.record, fashion.record, food.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Outpainting-Report.md"), buildOutpaintingReport(tech.record, fashion.record, food.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Mask-Management-Report.md"), buildMaskReport(tech.record, fashion.record, food.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Image-Editing-Readiness-Report.md"), buildReadinessReport(status, tech.record, fashion.record, food.record), "utf8");
        fs.writeFileSync(path.join(process.cwd(), "STEP-9F-VALIDATION-REPORT.md"), buildMainReport(status, results, storageRoot, allPassed, tech.record, fashion.record, food.record), "utf8");
...
function buildInpaintingReport(tech, fashion, food) {
    ...
    for (const record of rows) {
        lines.push(`| ${record.profile.productId} | ${record.inpaintingPlan.inpaintingType} | ${record.inpaintingPlan.reconstructionStrategy.slice(0, 30)}... | ${record.inpaintingPlan.textureNotes.length} notes | ${record.scores.reconstructionScore}/100 |`);
    }
    return lines.join("\n");
}
function buildOutpaintingReport(tech, fashion, food) {
    const rows = [tech, fashion, food].filter(Boolean);
...
function buildMaskReport(tech, fashion, food) {
    const rows = [tech, fashion, food].filter(Boolean);
    const lines = [
    ...
    for (const record of rows) {
        ...
        lines.push(`| ${record.profile.productId} | ${record.maskManagement.masks.length} | ${record.maskManagement.protectedRegions.length} | ${editable} | ${locked} |`);
    }
    return lines.join("\n");
}
function buildReadinessReport(status, tech, fashion, food) {
    ...
    return [
        ...
        "|---------|--------------|--------------|----------------|-------|------------|------------|-------|",
        ...rows.map((r) => `| ${r.profile.productId} | ${r.scores.editingQualityScore} | ${r.scores.identityPreservationScore} | ${r.scores.reconstructionScore} | ${r.scores.brandConsistencyScore} | ${r.scores.productionReadinessScore} | ${r.scores.aiConfidenceScore} | ${r.productionReady ? "✅" : "❌"} |`),
        "",
        "## Performance",
        "",
...
void main();
//# sourceMappingURL=validate-image-editing-engine.js.map