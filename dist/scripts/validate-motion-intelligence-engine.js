...
import { createAiCore, VideoAnalysisType, VideoCodec, AudioCodec, VideoContainer, VideoFileFormat, FrameRateMode, VideoColorSpace, MotionEventType, ObjectMotionType, } from "../ai/index.js";
...
async function main() {
    ...
    try {
        ...
        results.objectTracking = {
            passed: (commercial.record?.subjectTracks.length ?? 0) >= 1 &&
        ...
        results.motionClassification = {
            passed: (commercial.record?.classifications.length ?? 0) >= 2 &&
        ...
        results.motionPlanning = {
            passed: Boolean(commercial.record?.motionPlan.motionPath) &&
        ...
        const logFile = path.join(storageRoot, "logs", `motion-intelligence-engine-${new Date().toISOString().slice(0, 10)}.jsonl`);
        ...
        fs.writeFileSync(path.join(projectStateDir, "Motion-Analysis-Report.md"), buildAnalysisReport(status, results, storageRoot, allPassed), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Motion-Tracking-Report.md"), buildTrackingReport(commercial.record, social.record, tutorial.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Motion-Planning-Report.md"), buildPlanningReport(commercial.record, social.record, tutorial.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Motion-Quality-Report.md"), buildQualityReport(commercial.record, social.record, tutorial.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Motion-Readiness-Report.md"), buildReadinessReport(status, commercial.record, social.record, tutorial.record, allPassed), "utf8");
        fs.writeFileSync(path.join(process.cwd(), "STEP-7G-VALIDATION-REPORT.md"), buildAnalysisReport(status, results, storageRoot, allPassed), "utf8");
...
function buildTrackingReport(a, b, c) {
    ...
    return [
        ...
        "| Video | Subject Types |",
        "|-------|---------------|",
        ...rows.map((r) => `| ${r.videoId} | ${[...new Set(r.subjectTracks.map((t) => t.subjectType))].join(", ")} |`),
        "",
    ].join("\n");
}
function buildPlanningReport(a, b, c) {
    const rows = [a, b, c].filter(Boolean);
    return [
        ...
        "|-------|----------------|-------------------|------------|-----------|",
        ...rows.map((r) => `| ${r.videoId} | ${r.dominantClassification} | ${r.motionPlan.motionTimeline.length} | ${r.motionPlan.motionContinuity} | ${r.motionPlan.aiMotionBlueprint.slice(0, 50)}... |`),
        "",
    ].join("\n");
}
function buildQualityReport(a, b, c) {
    const rows = [a, b, c].filter(Boolean);
    return [
        ...
        "|-------|---------|-----------|----------|-----------|------------|------------|",
        ...rows.map((r) => `| ${r.videoId} | ${r.scores.motionQualityScore} | ${r.scores.motionStabilityScore} | ${r.scores.trackingAccuracyScore} | ${r.scores.cinematicMotionScore} | ${r.scores.productionReadinessScore} | ${r.scores.aiConfidenceScore} |`),
        "",
    ].join("\n");
}
...
void main();
//# sourceMappingURL=validate-motion-intelligence-engine.js.map