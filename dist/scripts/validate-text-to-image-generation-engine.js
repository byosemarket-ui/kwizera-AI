...
import { ALL_TEXT_TO_IMAGE_PLATFORMS, createAiCore, CreativePlatform, ImageArtisticStyle, MarketingObjective, ProductAnalysisCategory, ProductAvailabilityStatus, ProductBusinessType, ProductImageType, ProductUnderstandingMarketingGoal, TextToImagePlatform, } from "../ai/index.js";
...
async function main() {
    ...
    try {
        ...
        results.promptAnalysis = {
            passed: Boolean(tech.record?.promptAnalysis.subject &&
        ...
        results.compositionPlanning = {
            passed: Boolean(tech.record?.compositionPlan.composition &&
        ...
        results.stylePlanning = {
            passed: Boolean(tech.record?.stylePlan.style &&
        ...
        results.colorPlanning = {
            passed: Boolean((tech.record?.colorPlan.primaryColors.length ?? 0) >= 2 &&
        ...
        results.imagePlanScores = {
            passed: (tech.record?.scores.promptQualityScore ?? 0) >= 55 &&
        ...
        fs.writeFileSync(path.join(projectStateDir, "AI-Text-to-Image-Report.md"), buildMainReport(status, results, storageRoot, allPassed, tech.record, fashion.record, beauty.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Prompt-Analysis-Report.md"), buildPromptAnalysisReport(tech.record, fashion.record, beauty.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Composition-Planning-Report.md"), buildCompositionReport(tech.record, fashion.record, beauty.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Style-Planning-Report.md"), buildStyleReport(tech.record, fashion.record, beauty.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Text-to-Image-Readiness-Report.md"), buildReadinessReport(status, tech.record, fashion.record, beauty.record), "utf8");
        fs.writeFileSync(path.join(process.cwd(), "STEP-9B-VALIDATION-REPORT.md"), buildMainReport(status, results, storageRoot, allPassed, tech.record, fashion.record, beauty.record), "utf8");
...
function buildPromptAnalysisReport(tech, fashion, beauty) {
    ...
    for (const record of rows) {
        lines.push(`| ${record.profile.productId} | ${record.promptAnalysis.subject.slice(0, 30)}... | ${record.promptAnalysis.environment.slice(0, 25)}... | ${record.promptAnalysis.mood} | ${record.promptAnalysis.artisticStyle} | ${record.scores.promptQualityScore}/100 |`);
    }
    lines.push("", "## Detailed Analysis (Technology)", "");
    if (tech) {
        lines.push(`- **Subject:** ${tech.promptAnalysis.subject}`, `- **Objects:** ${tech.promptAnalysis.objects.join(", ")}`, `- **Emotion:** ${tech.promptAnalysis.emotion}`, `- **Camera:** ${tech.promptAnalysis.cameraPerspective}`, `- **Color Palette:** ${tech.promptAnalysis.colorPalette.join(", ")}`);
    }
    return lines.join("\n");
}
function buildCompositionReport(tech, fashion, beauty) {
    const rows = [tech, fashion, beauty].filter(Boolean);
    const lines = [
...
function buildStyleReport(tech, fashion, beauty) {
    const rows = [tech, fashion, beauty].filter(Boolean);
    const lines = [
    ...
    for (const record of rows) {
        lines.push(`| ${record.profile.productId} | ${record.stylePlan.style} | ${record.stylePlan.brandAlignment.slice(0, 35)}... | ${record.scores.styleScore}/100 | ${record.scores.brandConsistencyScore}/100 |`);
    }
    lines.push("", "## Lighting Plans", "");
    if (tech) {
        lines.push(`- Studio: ${tech.lightingPlan.studioLighting.slice(0, 70)}...`, `- Natural: ${tech.lightingPlan.naturalLighting.slice(0, 70)}...`, `- HDR: ${tech.lightingPlan.hdrPreparation}`);
    }
    return lines.join("\n");
}
function buildReadinessReport(status, tech, fashion, beauty) {
    ...
    return [
        ...
        "|---------|--------|-------------|-------|-------|------------|------------|-------|",
        ...rows.map((r) => `| ${r.profile.productId} | ${r.scores.promptQualityScore} | ${r.scores.compositionScore} | ${r.scores.styleScore} | ${r.scores.brandConsistencyScore} | ${r.scores.productionReadinessScore} | ${r.scores.aiConfidenceScore} | ${r.productionReady ? "✅" : "❌"} |`),
        "",
        "## Performance",
        "",
...
void main();
//# sourceMappingURL=validate-text-to-image-generation-engine.js.map