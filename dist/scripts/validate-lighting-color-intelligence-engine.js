...
import { createAiCore, ImageAnalysisType, ImageColorSpace, ImageCompressionType, ImageFileFormat, ImageUnderstandingMarketingGoal, ImageUnderstandingPlatform, LightingColorMarketingGoal, LightingType, } from "../ai/index.js";
...
async function runBasePipeline(foundation, sample, opts) {
...
async function main() {
    ...
    try {
        ...
        results.colorAnalysis = {
            passed: (product.record?.color.dominantColors.length ?? 0) >= 2 &&
        ...
        results.improvementPlanning = {
            passed: Boolean(product.record?.lightingPlan.exposureStrategy) &&
        ...
        results.qualityScores = {
            passed: (product.record?.scores.lightingQualityScore ?? 0) >= 55 &&
        ...
        fs.writeFileSync(path.join(projectStateDir, "Lighting-Analysis-Report.md"), buildLightingReport(status, results, storageRoot, allPassed, product.record, lifestyle.record, banner.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Color-Analysis-Report.md"), buildColorReport(product.record, lifestyle.record, banner.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Lighting-Quality-Report.md"), buildLightingQualityReport(product.record, lifestyle.record, banner.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Color-Readiness-Report.md"), buildReadinessReport(status, allPassed), "utf8");
        fs.writeFileSync(path.join(process.cwd(), "STEP-6G-VALIDATION-REPORT.md"), buildLightingReport(status, results, storageRoot, allPassed, product.record, lifestyle.record, banner.record), "utf8");
...
function buildLightingReport(status, results, storageRoot, allPassed, product, lifestyle, banner) {
    return [
...
function buildColorReport(product, lifestyle, banner) {
    ...
    return [
        ...
        "|-------|---------|-------------|---------|----------|------------|-------------|",
        ...rows.map((r) => `| ${r.imageId} | ${r.color.dominantColors.join(", ")} | ${r.color.colorTemperature} | ${r.color.colorHarmony} | ${r.color.colorContrast} | ${r.color.saturation} | ${r.color.brandColorMatching} |`),
        "",
        "## Color Scores",
        "",
...
function buildLightingQualityReport(product, lifestyle, banner) {
    const rows = [product, lifestyle, banner].filter(Boolean);
    return [
        ...
        "|-------|------|-----------|-----------|------------|----------|---------|------------|",
        ...rows.map((r) => `| ${r.imageId} | ${r.lighting.lightingType} | ${r.lighting.lightingDirection} | ${r.lighting.lightingIntensity} | ${r.lighting.lightingUniformity} | ${r.lighting.exposure} | ${r.lighting.shadows} | ${r.lighting.highlights} |`),
        "",
        "## Lighting Suitability",
        "",
        "| Image | Product Photo | Advertisement | Social | Video |",
        "|-------|---------------|---------------|--------|-------|",
        ...rows.map((r) => `| ${r.imageId} | ${r.lightingSuitability.productPhotography} | ${r.lightingSuitability.advertisement} | ${r.lightingSuitability.socialMedia} | ${r.lightingSuitability.videoProduction} |`),
        "",
    ].join("\n");
}
...
void main();
//# sourceMappingURL=validate-lighting-color-intelligence-engine.js.map