...
import { createAiCore, VideoAnalysisType, VideoCodec, AudioCodec, VideoContainer, VideoFileFormat, CreativeVideoType, CreativeVideoTemplateType, } from "../ai/index.js";
...
async function main() {
    ...
    try {
        ...
        results.visualAudioPlanning = {
            passed: Boolean(commercial.record?.visualPlan.cameraStyle) &&
        ...
        results.relationshipDetection = {
            passed: (commercial.record?.relationships.relatedEnhancementPlans.length ?? 0) >= 1 &&
        ...
        const logFile = path.join(storageRoot, "logs", `creative-video-intelligence-engine-${new Date().toISOString().slice(0, 10)}.jsonl`);
        ...
        fs.writeFileSync(path.join(projectStateDir, "Creative-Video-Report.md"), buildMainReport(status, results, storageRoot, allPassed), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Storyboard-Planning-Report.md"), buildStoryboardReport(commercial.record, social.record, tutorial.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Marketing-Planning-Report.md"), buildMarketingReport(commercial.record, social.record, tutorial.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Creative-Readiness-Report.md"), buildReadinessReport(status, commercial.record, social.record, tutorial.record, allPassed), "utf8");
        fs.writeFileSync(path.join(process.cwd(), "STEP-7J-VALIDATION-REPORT.md"), buildMainReport(status, results, storageRoot, allPassed), "utf8");
...
function buildStoryboardReport(a, b, c) {
    ...
    return [
        ...
        "|-------|-----------|------|--------|----------------|-----|--------|",
        ...rows.map((r) => `| ${r.videoId} | ${r.storyboard.storyStructure.slice(0, 30)} | ${r.storyboard.openingHook.slice(0, 25)} | ${r.storyboard.sceneOrder.length} | ${r.storyboard.productReveal.timingMs}ms | ${r.storyboard.ctaPlacement.timingMs}ms | ${r.storyboard.endingStrategy.slice(0, 25)} |`),
        "",
    ].join("\n");
}
function buildMarketingReport(a, b, c) {
    const rows = [a, b, c].filter(Boolean);
    return [
        ...
        "|-------|------------------|-----------------|--------------|-------------------|",
        ...rows.map((r) => `| ${r.videoId} | ${r.marketingPlan.productShowcase.slice(0, 35)} | ${r.marketingPlan.brandAwareness.slice(0, 30)} | ${r.marketingPlan.ctaStrategy.slice(0, 30)} | ${r.marketingPlan.socialEngagement.slice(0, 25)} |`),
        "",
    ].join("\n");
}
function buildReadinessReport(status, a, b, c, allPassed) {
    ...
    return [
        ...
        "|-------|----------|--------------|-----------|---------------|-------|------------|------------|",
        ...rows.map((r) => `| ${r.videoId} | ${r.scores.creativeScore} | ${r.scores.storytellingScore} | ${r.scores.marketingScore} | ${r.scores.visualImpactScore} | ${r.scores.brandConsistencyScore} | ${r.scores.productionReadinessScore} | ${r.scores.aiConfidenceScore} |`),
        "",
    ].join("\n");
}
void main();
//# sourceMappingURL=validate-creative-video-intelligence-engine.js.map