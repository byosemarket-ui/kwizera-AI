...
import { ALL_IMAGE_ENHANCE_GEN_PLATFORMS, ALL_IMAGE_ENHANCE_OPERATIONS, ALL_IMAGE_ENHANCE_PRESERVATION_TARGETS, ALL_IMAGE_ENHANCE_RESTORATION_TYPES, BackgroundGenPlatform, BackgroundGenType, createAiCore, CreativePlatform, ImageEditGenPlatform, ImageEditOperationType, ImageEnhanceCategory, ImageEnhanceGenPlatform, ImageEnhanceOperationType, ImageEnhanceRestorationType, MarketingObjective, ProductAnalysisCategory, ProductAvailabilityStatus, ProductBusinessType, ProductImageGenPlatform, ProductUnderstandingMarketingGoal, } from "../ai/index.js";
...
async function prepareUpstreamPlans(imgFoundation, productId, productImagePlatform) {
...
async function main() {
    ...
    try {
        ...
        results.upstreamPreparation = {
            passed: productUpstream.productPlan.success &&
        ...
        results.imageAnalysis = {
            passed: Boolean(product.record?.imageAnalysis.resolution &&
        ...
        results.restorationPlanning = {
            passed: Boolean(product.record?.restorationOperations.restorationStrategy &&
        ...
        results.preservationRules = {
            passed: (product.record?.preservation.targets.length ?? 0) === ALL_IMAGE_ENHANCE_PRESERVATION_TARGETS.length &&
        ...
        results.superResolution = {
            passed: Boolean(product.record?.superResolutionPlan.targetResolution &&
        ...
        results.printPreparation = {
            passed: Boolean(product.record?.printPreparation.printResolution &&
        ...
        results.enhancementScores = {
            passed: (product.record?.scores.enhancementScore ?? 0) >= 55 &&
        ...
        results.relationships = {
            passed: (product.record?.relationships.sourceImages.length ?? 0) >= 1 &&
        ...
        const repaired = await engine.repairEnhancementPlan(productUpstream.productPlan.record.productImagePlanId, ImageEnhanceGenPlatform.Mobile);
        ...
        results.operationsSupported = {
            passed: ALL_IMAGE_ENHANCE_OPERATIONS.length >= 11 &&
        ...
        fs.writeFileSync(path.join(projectStateDir, "AI-Image-Enhancement-Report.md"), buildMainReport(status, results, storageRoot, allPassed, product.record, fashion.record, historical.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Image-Restoration-Report.md"), buildRestorationReport(product.record, fashion.record, historical.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Super-Resolution-Report.md"), buildSuperResolutionReport(product.record, fashion.record, historical.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Print-Preparation-Report.md"), buildPrintReport(product.record, fashion.record, historical.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Image-Enhancement-Readiness-Report.md"), buildReadinessReport(status, product.record, fashion.record, historical.record), "utf8");
        fs.writeFileSync(path.join(process.cwd(), "STEP-9G-VALIDATION-REPORT.md"), buildMainReport(status, results, storageRoot, allPassed, product.record, fashion.record, historical.record), "utf8");
...
function buildMainReport(status, results, storageRoot, allPassed, product, fashion, historical) {
    return [
...
function buildRestorationReport(product, fashion, historical) {
    const rows = [product, fashion, historical].filter(Boolean);
    const lines = [
    ...
    for (const record of rows) {
        lines.push(`| ${record.profile.productId} | ${record.restorationOperations.restorationType} | ${record.restorationOperations.restorationStrategy.slice(0, 30)}... | ${record.restorationOperations.targetDamage.length} | ${record.scores.restorationScore}/100 |`);
    }
    return lines.join("\n");
}
function buildSuperResolutionReport(product, fashion, historical) {
    const rows = [product, fashion, historical].filter(Boolean);
...
function buildPrintReport(product, fashion, historical) {
    const rows = [product, fashion, historical].filter(Boolean);
    const lines = [
    ...
    for (const record of rows) {
        lines.push(`| ${record.profile.productId} | ${record.printPreparation.printResolution.slice(0, 25)}... | ${record.printPreparation.colorProfile.slice(0, 20)}... | ${record.printPreparation.dpiPlanning.slice(0, 20)}... | ${record.printPreparation.cmykPreparation.slice(0, 20)}... | ${record.printPreparation.largeFormatPreparation.slice(0, 20)}... |`);
    }
    return lines.join("\n");
}
function buildReadinessReport(status, product, fashion, historical) {
    const rows = [product, fashion, historical].filter(Boolean);
    return [
        ...
        "|---------|-------------|-------------|-----------|-------|-------|------------|------------|-------|",
        ...rows.map((r) => `| ${r.profile.productId} | ${r.scores.enhancementScore} | ${r.scores.restorationScore} | ${r.scores.sharpnessScore} | ${r.scores.colorAccuracyScore} | ${r.scores.brandConsistencyScore} | ${r.scores.productionReadinessScore} | ${r.scores.aiConfidenceScore} | ${r.productionReady ? "✅" : "❌"} |`),
        "",
        "## Performance",
        "",
...
void main();
//# sourceMappingURL=validate-image-enhancement-engine.js.map