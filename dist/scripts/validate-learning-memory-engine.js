...
import { createAiCore, LearningCategory, LearningOutcome, LearningSource, MemoryStorageType, } from "../ai/index.js";
...
async function main() {
    ...
    try {
        ...
        const learnResult = await learning.learnFromEvent({
            ...
            description: "Successfully delivered promotional video with strong user feedback and marketing performance.",
            relatedProject: "step3e-project",
        ...
        const correction = await learning.learnFromUserCorrection("Prefer shorter intro sequences in future videos", { category: LearningCategory.Video, relatedProject: "step3e-project" });
        ...
        results.selfImprovement = {
            passed: insights.recommendations.length > 0 &&
...
function buildReport(status, results, storageRoot, allPassed, learnMs) {
    return [
...
void main();
//# sourceMappingURL=validate-learning-memory-engine.js.map