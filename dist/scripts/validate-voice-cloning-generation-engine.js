...
import { AuthorizationStatus, createAiCore, CreativePlatform, EmotionType, MarketingObjective, ProductAnalysisCategory, ProductAvailabilityStatus, ProductBusinessType, ProductUnderstandingMarketingGoal, VcLanguage, VcOutputUseCase, VcPlatform, VOICE_LIBRARY_TYPES, VoiceLibraryType, VoiceType, } from "../ai/index.js";
...
async function main() {
    ...
    try {
        ...
        results.voiceAnalysis = {
            passed: Boolean(tech.record?.voiceAnalysis.pitch &&
        ...
        results.voiceProfile = {
            passed: Boolean(tech.record?.profile.voiceProfileId &&
        ...
        results.authorizationValidation = {
            passed: Boolean(tech.record?.authorizationValidation.overallAuthorized &&
        ...
        results.voiceConsistency = {
            passed: Boolean(tech.record?.consistencyPlan.voiceIdentity &&
        ...
        results.pronunciationQuality = {
            passed: Boolean(Object.keys(tech.record?.cloningPlan.pronunciationMapping ?? {}).length >= 1 &&
        ...
        results.emotionPreservation = {
            passed: Boolean(tech.record?.cloningPlan.emotionMapping.source &&
        ...
        results.cloningScores = {
            passed: (tech.record?.scores.voiceSimilarityScore ?? 0) >= 55 &&
                (tech.record?.scores.voiceStabilityScore ?? 0) >= 55 &&
        ...
        results.relationships = {
            passed: (tech.record?.relationships.voiceSamples.length ?? 0) >= 1 &&
        ...
        fs.writeFileSync(path.join(projectStateDir, "AI-Voice-Cloning-Report.md"), buildMainReport(status, results, storageRoot, allPassed, tech.record, health.record, finance.record, kinyarwanda.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Voice-Analysis-Report.md"), buildVoiceAnalysisReport(tech.record, health.record, finance.record, kinyarwanda.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Voice-Profile-Report.md"), buildVoiceProfileReport(tech.record, health.record, finance.record, kinyarwanda.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Authorization-Validation-Report.md"), buildAuthorizationReport(tech.record, health.record, finance.record, kinyarwanda.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Voice-Cloning-Readiness-Report.md"), buildReadinessReport(status, tech.record, health.record, finance.record, kinyarwanda.record), "utf8");
        fs.writeFileSync(path.join(process.cwd(), "STEP-10D-VALIDATION-REPORT.md"), buildMainReport(status, results, storageRoot, allPassed, tech.record, health.record, finance.record, kinyarwanda.record), "utf8");
...
function buildVoiceAnalysisReport(...records) {
    ...
    for (const r of rows) {
        lines.push(`| ${r.cloningPlanId.slice(0, 30)}... | ${r.voiceAnalysis.language} | ${r.voiceAnalysis.accent} | ${r.voiceAnalysis.pitch.slice(0, 20)}... | ${r.voiceAnalysis.voiceQualityScore}/100 | ${r.voiceAnalysis.detectedEmotion} |`);
    }
    return lines.join("\n");
}
function buildVoiceProfileReport(...records) {
    const rows = records.filter(Boolean);
    const lines = [
    ...
    for (const r of rows) {
        lines.push(`| ${r.cloningPlanId.slice(0, 20)} | ${r.profile.voiceProfileId.slice(0, 25)}... | ${r.profile.speakerId} | ${r.profile.voiceLibraryType} | ${r.profile.authorizationStatus} | v${r.profile.voiceVersion} |`);
    }
    return lines.join("\n");
}
function buildAuthorizationReport(...records) {
    const rows = records.filter(Boolean);
    const lines = [
    ...
    for (const r of rows) {
        lines.push(`| ${r.profile.consentId} | ${r.authorizationValidation.voiceConsentValid ? "✅" : "❌"} | ${r.authorizationValidation.overallAuthorized ? "✅" : "❌"} | ${r.scores.authorizationComplianceScore}/100 | ${r.authorizationValidation.validationNotes[0] ?? "n/a"} |`);
    }
    return lines.join("\n");
}
function buildReadinessReport(status, ...records) {
    ...
    return [
        ...
        "|------|------------|-----------|---------------|---------|------|------------|------------|-------|",
        ...rows.map((r) => `| ${r.profile.language} | ${r.scores.voiceSimilarityScore} | ${r.scores.voiceStabilityScore} | ${r.scores.pronunciationScore} | ${r.scores.emotionPreservationScore} | ${r.scores.authorizationComplianceScore} | ${r.scores.productionReadinessScore} | ${r.scores.aiConfidenceScore} | ${r.productionReady ? "✅" : "❌"} |`),
        "",
        "## Performance",
        "",
...
void main();
//# sourceMappingURL=validate-voice-cloning-generation-engine.js.map