...
import { createAiCore, ImageAnalysisType, ImageColorSpace, ImageCompressionType, ImageFileFormat, ImageSceneType, ImageUnderstandingMarketingGoal, ImageUnderstandingPlatform, } from "../ai/index.js";
...
async function main() {
    ...
    try {
        ...
        results.sceneUnderstanding = {
            passed: product.record?.scene.sceneType === ImageSceneType.ProductShowcase &&
        ...
        results.productUnderstanding = {
            passed: (product.record?.product.productVisibility ?? 0) >= 65 &&
        ...
        results.brandUnderstanding = {
            passed: product.record?.brand.logoPresence === true &&
        ...
        results.marketingUnderstanding = {
            passed: Boolean(product.record?.marketing.promotionalPurpose) &&
        ...
        results.understandingScores = {
            passed: (product.record?.scores.imageUnderstandingScore ?? 0) >= 55 &&
        ...
        fs.writeFileSync(path.join(projectStateDir, "Image-Understanding-Report.md"), buildUnderstandingReport(status, results, storageRoot, allPassed, product.record, lifestyle.record, banner.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Scene-Understanding-Report.md"), buildSceneReport(product.record, lifestyle.record, banner.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Marketing-Readiness-Report.md"), buildMarketingReport(product.record, lifestyle.record, banner.record), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Image-Readiness-Report.md"), buildReadinessReport(status, allPassed), "utf8");
        fs.writeFileSync(path.join(process.cwd(), "STEP-6C-VALIDATION-REPORT.md"), buildUnderstandingReport(status, results, storageRoot, allPassed, product.record, lifestyle.record, banner.record), "utf8");
...
function buildUnderstandingReport(status, results, storageRoot, allPassed, product, lifestyle, banner) {
    return [
...
function buildSceneReport(product, lifestyle, banner) {
    ...
    return [
        ...
        "|-------|------------|-------------|---------|------|-----------------|",
        ...rows.map((r) => `| ${r.identity.imageName} | ${r.scene.sceneType} | ${r.scene.environment} | ${r.scene.setting} | ${r.scene.mood} | ${r.scene.preparedScenes.join(", ")} |`),
        "",
    ].join("\n");
}
function buildMarketingReport(product, lifestyle, banner) {
    const rows = [product, lifestyle, banner].filter(Boolean);
    return [
        ...
        "|-------|---------------------|-------------------|--------------------|------|-----------------|",
        ...rows.map((r) => `| ${r.identity.imageName} | ${r.scores.marketingReadinessScore}/100 | ${r.scores.brandConsistencyScore}/100 | ${r.scores.creativeReadinessScore}/100 | ${r.marketingGoal} | ${r.marketing.ctaOpportunity.slice(0, 40)}... |`),
        "",
    ].join("\n");
}
...
void main();
//# sourceMappingURL=validate-image-understanding-engine.js.map