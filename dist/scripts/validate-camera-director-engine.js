...
import { createAiCore, CreativePlatform, MarketingObjective, ProductAnalysisCategory, ProductAvailabilityStatus, ProductBusinessType, ProductUnderstandingMarketingGoal, StoryboardGenerationPlatform, CAMERA_PLATFORM_TARGETS, DirectorCameraAngle, DirectorShotType, } from "../ai/index.js";
...
async function main() {
    ...
    try {
        ...
        results.focusPlanning = {
            passed: Boolean(firstPlan?.focusPlanning.focusSubject &&
        ...
        results.compositionPlanning = {
            passed: Boolean(firstPlan?.compositionPlanning.primaryStrategy &&
        ...
        results.cameraScores = {
            passed: (firstPlan?.scores.cameraDirectionScore ?? 0) >= 55 &&
        ...
        fs.writeFileSync(path.join(projectStateDir, "AI-Camera-Director-Report.md"), buildMainReport(status, results, storageRoot, allPassed, techPlans.plans, fashionPlans.plans, beautyPlans.plans), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Camera-Planning-Report.md"), buildPlanningReport(techPlans.plans), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Composition-Planning-Report.md"), buildCompositionReport(techPlans.plans), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Camera-Continuity-Report.md"), buildContinuityReport(techPlans.plans), "utf8");
        fs.writeFileSync(path.join(projectStateDir, "Camera-Readiness-Report.md"), buildReadinessReport(status, techPlans.plans, fashionPlans.plans, beautyPlans.plans), "utf8");
        fs.writeFileSync(path.join(process.cwd(), "STEP-8D-VALIDATION-REPORT.md"), buildMainReport(status, results, storageRoot, allPassed, techPlans.plans, fashionPlans.plans, beautyPlans.plans), "utf8");
...
function buildPlanningReport(plans) {
...
function buildCompositionReport(plans) {
    const lines = [
...
function buildContinuityReport(plans) {
    const lines = ["# Camera Continuity Report — Step 8D", "", `**Date:** ${new Date().toISOString()}`, ""];
    for (const plan of plans?.slice(0, 6) ?? []) {
        const c = plan.continuity;
        lines.push(`## ${plan.profile.sceneId}`, "", `- Camera consistency: ${c.cameraConsistency ? "✅" : "❌"}`, `- Motion continuity: ${c.motionContinuity ? "✅" : "❌"}`, `- Lighting continuity: ${c.lightingContinuity ? "✅" : "❌"}`, `- Story continuity: ${c.storyContinuity ? "✅" : "❌"}`, `- Notes: ${c.notes.join("; ")}`, "");
    }
    return lines.join("\n");
}
...
void main();
//# sourceMappingURL=validate-camera-director-engine.js.map