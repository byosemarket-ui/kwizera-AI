...
import {
  ColorAnalysis,
  ColorImprovementPlan,
  ColorSuitability,
  ColorTemperature,
  LightingAnalysis,
  LightingColorIntelligenceRecommendation,
  LightingDirection,
  LightingImprovementPlan,
  LightingSuitability,
  LightingType,
} from "./types.js";
...
export class LightingColorAnalyzer {
  ...
  private inferLightingType(
    brightness: number,
    background: string,
    bgRecord?: BackgroundIntelligenceRecord | null
  ): LightingType {
    ...
    if (lower.includes("sunset") || lower.includes("gradient")) return LightingType.Mixed;
    if (brightness < 45) return LightingType.LowKey;
    if (brightness >= 75) return LightingType.HighKey;
    if (lower.includes("urban") || lower.includes("outdoor")) return LightingType.Natural;
    return LightingType.Artificial;
  }

  private inferLightingDirection(type: LightingType, contrast: number): LightingDirection {
    if (type === LightingType.Backlit) return LightingDirection.Back;
    if (type === LightingType.SideLit) return LightingDirection.Side;
    if (type === LightingType.TopLit) return LightingDirection.Top;
    if (type === LightingType.Studio || type === LightingType.HighKey) return LightingDirection.Diffused;
    if (contrast >= 75) return LightingDirection.Side;
    return LightingDirection.Front;
  }

  private inferReflections(background: string, sharpness: number): number {
    let reflections = 20;
    if (background.toLowerCase().includes("studio") || background.toLowerCase().includes("white")) {
      reflections += 25;
    }
    if (sharpness >= 80) reflections += 15;
    return Math.min(100, reflections);
  }

  private computeColorHarmony(colors: string[]): number {
    if (colors.length === 0) return 60;
    if (colors.length <= 3) return 88;
    if (colors.length <= 5) return 74;
    return 58;
  }

  private inferHueDistribution(colors: string[]): string {
    if (colors.length === 0) return "neutral-balanced";
    if (colors.some((c) => c.includes("ff") || c.includes("e9"))) return "warm-accent-dominant";
    if (colors.some((c) => c.includes("1a") || c.includes("2d"))) return "cool-dark-dominant";
    return "mixed-balanced";
  }

  private inferColorTemperature(brightness: number, whiteBalance: number): ColorTemperature {
    if (whiteBalance >= 70 || brightness >= 72) return ColorTemperature.Warm;
    if (whiteBalance <= 45) return ColorTemperature.Cool;
  ...
  private buildLightingPlan(lighting: LightingAnalysis, color: ColorAnalysis): LightingImprovementPlan {
    return {
      exposureStrategy:
        lighting.overexposure > 10
  ...
  private buildColorPlan(color: ColorAnalysis, understanding: ImageUnderstandingRecord): ColorImprovementPlan {
    return {
      colorHarmonyStrategy:
        color.colorHarmony < 70
  ...
  private buildRecommendations(
    lighting: LightingAnalysis,
    color: ColorAnalysis,
    lightingSuit: LightingSuitability,
    colorSuit: ColorSuitability
  ): LightingColorIntelligenceRecommendation[] {
    const recs: LightingColorIntelligenceRecommendation[] = [];
