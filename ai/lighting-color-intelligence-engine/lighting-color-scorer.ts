import {
  ...
  ColorSuitability,
  LightingAnalysis,
  LightingColorIntelligenceScores,
  LightingSuitability,
} from "./types.js";
...
export class LightingColorScorer {
  computeScores(
    ...
    const lightingQualityScore = Math.round(
    ...
    const colorQualityScore = Math.round(
      (color.colorHarmony + color.colorContrast + color.saturation + color.vibrance) / 4
    );
    ...
    const marketingReadinessScore = Math.round(
      (lightingValues.reduce((a, b) => a + b, 0) / lightingValues.length +
        colorValues.reduce((a, b) => a + b, 0) / colorValues.length) /
        2
    );

    let creativeReadinessScore = 55;
    if (lighting.lightingUniformity >= 70) creativeReadinessScore += 12;
    if (color.colorHarmony >= 70) creativeReadinessScore += 12;
    if (color.brandColorMatching >= 65) creativeReadinessScore += 10;
    if (lighting.overexposure < 10 && lighting.underexposure < 10) creativeReadinessScore += 8;
