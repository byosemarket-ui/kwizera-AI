...
export class LightingColorProcessor {
  constructor(
    ...
    private readonly analyzer: LightingColorAnalyzer,
    private readonly scorer: LightingColorScorer,
    private readonly linker: LightingColorLinker,
    private readonly records: LightingColorIntelligenceRecordStore,
    private readonly logger: LightingColorLogger
  ) {}

  async analyze(input: LightingColorIntelligenceInput): Promise<LightingColorIntelligenceResult> {
    ...
    const scores = this.scorer.computeScores(
      built.lighting,
      built.color,
      built.lightingSuitability,
      built.colorSuitability
    );
    ...
    const draft: LightingColorIntelligenceRecord = {
      imageId: input.imageId,
      lightingColorId,
  ...
  search(query: LightingColorIntelligenceSearchQuery): LightingColorIntelligenceRecord[] {
    ...
    if (query.colorPalette) {
      ...
      results = results.filter(
        (r) =>
          r.color.colorPalette.some((c) => c.toLowerCase().includes(q)) ||
