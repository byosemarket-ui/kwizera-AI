...
import {
  ...
  AudioGenerationBlueprintStage,
  AudioGenerationBlueprintStageEntry,
} from "./types.js";
...
export class AudioGenerationBlueprintManager {
  private blueprints = new Map<string, AudioGenerationBlueprint>();
  ...
  createBlueprint(input: {
    ...
    const blueprint: AudioGenerationBlueprint = {
      blueprintId,
  ...
  private buildDefaultStages(): AudioGenerationBlueprintStageEntry[] {
    const deps: Partial<Record<AudioGenerationBlueprintStage, AudioGenerationBlueprintStage[]>> = {
  ...
  private loadFromDisk(): void {
    const data = JSON.parse(fs.readFileSync(this.catalogPath, "utf8")) as {
      blueprints: AudioGenerationBlueprint[];
    };
