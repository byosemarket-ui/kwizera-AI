...
const ACCENT_LANGUAGE_MAP: Partial<Record<S2sLanguage, AccentType>> = {
...
export class SpeechToSpeechGenerationAnalyzer {
  ...
  buildEmotionPreservation(
    ...
    return {
      ...
      emotionMapping: { [source]: target } as Record<EmotionType, EmotionType>,
      intensityPreservation: 75,
      emotionalArc: speechAnalysis.speakerSegments.map(
        (s, i) => `Segment ${i + 1}: preserve ${s.emotion} through transformation`
      ),
      sceneEmotionNotes: [
  ...
  buildPronunciationAdaptation(speechAnalysis: SpeechAnalysis, context: TransformationContext): PronunciationAdaptationPlan {
    ...
    return {
      phonemeMapping: Object.fromEntries(
        speechAnalysis.properNames.map((n) => [n, `${n} phoneme chain preserved`])
      ),
      pronunciationDictionary: dict,
      namePreservation: dict,
      technicalVocabulary: Object.fromEntries(
        speechAnalysis.technicalTerms.map((t) => [t, `${t} [adapted for ${speechAnalysis.language}]`])
      ),
      acronymHandling: {},
      numberReadingRules: [
  ...
  buildRecommendations(
    speechAnalysis: SpeechAnalysis,
    emotionPlan: EmotionPreservationPlan,
    context: TransformationContext
  ): string[] {
    const recs: string[] = [];
  ...
  resolvePlatform(input: SpeechToSpeechGenerationInput, context: TransformationContext): S2sPlatform {
    ...
    if (context.creative?.profile.platform === "youtube") return S2sPlatform.YouTube;
    if (context.creative?.profile.platform === "tiktok") return S2sPlatform.TikTok;
    if (context.creative?.profile.platform === "instagram-reels") return S2sPlatform.Instagram;
  ...
  private detectLanguage(input: SpeechToSpeechGenerationInput, context: TransformationContext): S2sLanguage {
    if (input.language) return input.language;
    const text = input.transcriptHint ?? context.transcriptHint ?? "";
    if (/\b(murakoze|amakuru)\b/i.test(text)) return S2sLanguage.Kinyarwanda;
    if (/\b(bonjour|merci)\b/i.test(text)) return S2sLanguage.French;
    if (/\b(asante|habari|karibu)\b/i.test(text)) return S2sLanguage.Swahili;
    return S2sLanguage.English;
  }

  private estimateSpeakingRate(durationMs: number, transcript: string): string {
  ...
  private extractKeywords(transcript: string, context: TransformationContext): string[] {
  ...
  private extractProperNames(transcript: string, context: TransformationContext): string[] {
    const names: string[] = [];
  ...
  private extractTechnicalTerms(transcript: string, context: TransformationContext): string[] {
    const terms: string[] = [];
    if (context.industry === "technology") terms.push("AI", "API");
