...
const INDUSTRY_CONTEXT: Record<string, Partial<ProductContext>> = {
  technology: {
...
export class ProductUnderstandingAnalyzer {
  buildFromAnalysis(
    ...
    marketingGoal: ProductUnderstandingMarketingGoal = ProductUnderstandingMarketingGoal.Conversion
  ): {
    ...
    const customer: CustomerUnderstanding = {
  ...
  private inferPrimaryPurpose(description: string, useCase: string): string {
    if (description.length >= 30) return description.slice(0, 120);
    return `Enable ${useCase.replace(/-/g, " ")} for target customers`;
  }

  private inferCustomerNeeds(
    profile: ProductAnalysisIntelligenceRecord["profile"],
    classification: ProductAnalysisIntelligenceRecord["classification"]
  ): string[] {
    return [
  ...
  private inferPainPoints(industry: string, profile: ProductAnalysisIntelligenceRecord["profile"]): string[] {
  ...
  private inferBenefits(profile: ProductAnalysisIntelligenceRecord["profile"]): string[] {
    return profile.features.map((f) => `Benefit: ${f.replace(/-/g, " ")}`);
  }

  private inferExpectations(businessType: string): string[] {
    return [
  ...
  private inferMotivations(
    classification: ProductAnalysisIntelligenceRecord["classification"],
    goal: ProductUnderstandingMarketingGoal
  ): string[] {
    const base = [`achieve ${classification.useCase.replace(/-/g, " ")} goals`];
    if (goal === ProductUnderstandingMarketingGoal.Conversion) base.push("make confident purchase decision");
    if (goal === ProductUnderstandingMarketingGoal.Awareness) base.push("discover trusted brands");
    return base;
  }
  ...
  private inferPurchasingSituations(
    ...
    if (businessType === "b2b") situations.push("team evaluation and procurement");
    if (goal === ProductUnderstandingMarketingGoal.Launch) situations.push("product launch campaigns");
    return situations;
  }

  private buildMarketingPreparation(
    analysisMarketing: ProductAnalysisIntelligenceRecord["marketingPreparation"],
    customer: CustomerUnderstanding,
    value: ValueAnalysis
  ): UnderstandingMarketingPreparation {
    ...
    if (customer.customerNeeds.length >= 2) prepared.push("audience-needs");
    else gaps.push("audience-needs");

    if (value.commercialValue >= 60) prepared.push("commercial-value");
    else gaps.push("commercial-value");
