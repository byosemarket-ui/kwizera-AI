...
export class AudienceProcessor {
  constructor(
    ...
    private readonly analyzer: AudienceAnalyzer,
    private readonly scorer: AudienceScorer,
    private readonly linker: AudienceLinker,
    private readonly records: AudienceRecordStore,
    private readonly logger: AudienceLogger
  ) {}

  async analyze(input: AudienceIntelligenceInput): Promise<AudienceIntelligenceResult> {
    ...
    const psychological = this.analyzer.buildPsychologicalUnderstanding(
      understanding,
      profile.marketingGoal
    );
    const segmentation = this.analyzer.buildSegmentation(
      understanding,
      analysis,
      psychological,
      profile
    );
    const marketingPreparation = this.analyzer.buildMarketingPreparation(
      profile,
      psychological,
      segmentation
    );
    ...
    const draftRelationships: AudienceIntelligenceRecord["relationships"] = {
    ...
    const relationshipCount =
      draftRelationships.products.length +
    ...
    const draft: AudienceIntelligenceRecord = {
    ...
    const intelligenceValidation = this.foundation.validateProductIntelligence({
      ...
      verificationStatus:
        scores.audienceConfidenceScore >= 75
  ...
  search(query: AudienceSearchQuery): AudienceIntelligenceRecord[] {
    ...
    if (query.industry) {
      ...
      results = results.filter(
        (r) =>
          r.profile.industry.toLowerCase().includes(industryLower) ||
    ...
    if (query.language) {
      ...
      results = results.filter(
        (r) =>
          r.profile.preferredLanguage?.toLowerCase().includes(langLower) ||
    ...
    if (query.businessGoal) {
      ...
      results = results.filter((r) =>
        r.segmentation.businessGoals.some((g) => g.toLowerCase().includes(goalLower))
      );
    }
    if (query.customerNeed) {
      ...
      results = results.filter(
        (r) =>
          r.psychological.customerNeeds.some((n) => n.toLowerCase().includes(needLower)) ||
    ...
    if (query.text) {
      ...
      results = results.filter(
        (r) =>
          r.profile.audienceName.toLowerCase().includes(textLower) ||
