...
export class VideoAnalysisAnalyzer {
  analyze(input: VideoAnalysisEngineInput): {
    ...
    const technical: VideoTechnicalProfile = {
      videoName,
    ...
    const frame: VideoFrameAnalysis = {
      totalFrames,
      keyFrames,
    ...
    const timeline: VideoTimelineAnalysis = {
    ...
    const audioTracks: VideoAudioTrackAnalysis[] =
      input.audio?.tracks ??
      [
        {
    ...
    const audio: VideoAudioAnalysis = {
      ...
      overallAudioQualityScore:
        input.audio?.overallAudioQualityScore ??
        Math.round(
          audioTracks.reduce((s, t) => s + t.audioQualityScore, 0) / Math.max(audioTracks.length, 1)
        ),
    };

    const visual: VideoVisualAnalysis = {
  ...
  private inferFormat(filePath: string): VideoFileFormat {
    const ext = filePath.split(".").pop()?.toLowerCase();
    const map: Record<string, VideoFileFormat> = {
  ...
  private inferContainer(format: VideoFileFormat): VideoContainer {
    const map: Partial<Record<VideoFileFormat, VideoContainer>> = {
  ...
  private estimateBitrate(fileSizeBytes: number, durationMs: number): number {
    if (fileSizeBytes <= 0 || durationMs <= 0) return 5000;
  ...
  private computeOrientation(width: number, height: number): VideoOrientation {
    if (width === height) return VideoOrientation.Square;
    return width > height ? VideoOrientation.Landscape : VideoOrientation.Portrait;
  }

  private classifyVideoType(input: VideoAnalysisEngineInput): VideoAnalysisType {
    if (input.videoType) return input.videoType;
    const name = (input.videoName ?? "").toLowerCase();
    const path = (input.filePath ?? "").toLowerCase();

    if (name.includes("tutorial") || path.includes("tutorial")) return VideoAnalysisType.Tutorial;
    if (name.includes("social") || path.includes("reels") || path.includes("tiktok"))
      return VideoAnalysisType.SocialMedia;
    if (name.includes("commercial") || path.includes("commercial")) return VideoAnalysisType.Commercial;
    if (name.includes("product") || input.product) return VideoAnalysisType.ProductShowcase;
    if (name.includes("interview")) return VideoAnalysisType.Interview;
    if (name.includes("presentation")) return VideoAnalysisType.Presentation;
    if (name.includes("documentary")) return VideoAnalysisType.Documentary;
    if (name.includes("animation") || name.includes("animated")) return VideoAnalysisType.Animation;
    if (name.includes("corporate") || name.includes("brand")) return VideoAnalysisType.Corporate;
  ...
  private inferDominantColors(input: VideoAnalysisEngineInput): string[] {
  ...
  private buildTimelineSegments(
    ...
    if (input.timeline?.segments?.length) return input.timeline.segments;
  ...
  private distributeScenes(count: number): Record<string, number> {
    const dist: Record<string, number> = {};
    for (let i = 1; i <= count; i++) dist[`scene-${i}`] = Math.round(100 / count);
    return dist;
  }

  private distributeShots(count: number): Record<string, number> {
    const dist: Record<string, number> = {};
    for (let i = 1; i <= count; i++) dist[`shot-${i}`] = Math.round(100 / count);
    return dist;
  }
}
