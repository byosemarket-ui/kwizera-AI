...
const TYPE_TO_KNOWLEDGE: Record<VideoAnalysisType, VideoType> = {
...
export class VideoAnalysisProcessor {
  private readonly indexer: VideoAnalysisIndexer;

  constructor(
    private readonly foundation: AiVideoIntelligenceFoundation,
    private readonly analyzer: VideoAnalysisAnalyzer,
    private readonly completeness: VideoAnalysisCompletenessDetector,
    private readonly scorer: VideoAnalysisScorer,
    private readonly linker: VideoAnalysisLinker,
    private readonly records: VideoAnalysisRecordStore,
    private readonly logger: VideoAnalysisLogger
  ) {
    this.indexer = new VideoAnalysisIndexer(foundation);
  }

  async analyze(input: VideoAnalysisEngineInput): Promise<VideoAnalysisEngineResult> {
    ...
    const { scores, productionReadiness } = this.scorer.computeScores(
      analysis.technical,
      analysis.frame,
      analysis.timeline,
      analysis.audio,
    ...
    const draft: VideoAnalysisIntelligenceRecord = {
      videoId,
      analysisId,
      knowledgeId,
  ...
  search(query: VideoAnalysisSearchQuery): VideoAnalysisIntelligenceRecord[] {
    ...
    if (query.text) {
      ...
      results = results.filter(
        (r) =>
          r.technical.videoName.toLowerCase().includes(q) ||
