...
export class VideoAnalysisLinker {
  detectRelationships(
    ...
    allRecords: VideoAnalysisIntelligenceRecord[],
    knowledgeIds: string[] = [],
    memoryRefs: string[] = []
  ): VideoAnalysisRelationships {
    ...
    return {
      ...
      relatedVideos: [...new Set([...(record.relationships?.relatedVideos ?? []), ...relatedVideos])].slice(
        0,
        10
      ),
  ...
  classifySimilarity(a: VideoClassification, b: VideoClassification): number {
    let score = 0;
    if (a.videoType === b.videoType) score += 35;
