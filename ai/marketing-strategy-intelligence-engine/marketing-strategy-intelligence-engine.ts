...
import {
  MarketingObjective,
  MarketingStrategyEngineError,
  MarketingStrategyEngineStatusReport,
...
export class AiMarketingStrategyIntelligenceEngine {
  ...
  readonly logger = new MarketingStrategyLogger();
  readonly records = new MarketingStrategyRecordStore();

  private readonly analyzer = new MarketingStrategyAnalyzer();
  private readonly scorer = new MarketingStrategyScorer();
  private readonly linker = new MarketingStrategyLinker();
  private processor: MarketingStrategyProcessor | null = null;

  private strategyTimes: number[] = [];
  ...
  initialize(foundation: AiProductIntelligenceFoundation, storageRoot: string): void {
    ...
    this.processor = new MarketingStrategyProcessor(
      foundation,
      this.analyzer,
      this.scorer,
      this.linker,
  ...
  async prepareMarketingStrategy(input: MarketingStrategyInput): Promise<MarketingStrategyResult> {
    this.ensureReady();
    const result = await this.processor!.strategize(input);
    if (result.success) this.strategyTimes.push(result.durationMs);
    return result;
  }
  ...
  async repairStrategy(productId: string, objective?: MarketingObjective): Promise<MarketingStrategyResult | null> {
    this.ensureReady();
  ...
  private ensureReady(): void {
    if (!this.initialized || !this.foundation || !this.processor) {
      throw new MarketingStrategyEngineError(
        "Marketing Strategy Intelligence Engine not initialized",
        "NOT_INITIALIZED"
      );
    }
  }
