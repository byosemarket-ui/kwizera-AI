...
export class AudioMixingMasteringProcessor {
  constructor(
    ...
    private readonly analyzer: AudioMixingMasteringAnalyzer,
    private readonly scorer: AudioMixingMasteringScorer,
    private readonly linker: AudioMixingMasteringLinker,
    private readonly records: AudioMixingMasteringRecordStore,
    private readonly logger: AudioMixingMasteringLogger
  ) {}
  ...
  async generateMixMasterPlan(input: AudioMixMasterGenerationInput): Promise<AudioMixMasterGenerationResult> {
    ...
    try {
      ...
      const draft: AudioMixMasterGenerationRecord = {
        mixingPlanId: profile.mixingPlanId,
        masteringPlanId: profile.masteringPlanId,
        profile,
        multiTrackAnalysis,
  ...
  search(query: AudioMixMasterSearchQuery): AudioMixMasterGenerationRecord[] {
    ...
    if (query.mixingPlanId) results = results.filter((r) => r.mixingPlanId === query.mixingPlanId);
    if (query.masteringPlanId) results = results.filter((r) => r.masteringPlanId === query.masteringPlanId);
    if (query.sessionId) results = results.filter((r) => r.profile.sessionId === query.sessionId);
    ...
    if (query.keywords) {
      ...
      results = results.filter(
        (r) =>
          r.multiTrackAnalysis.keywords.some((k) => k.includes(kw)) ||
    ...
    if (query.text) {
      ...
      results = results.filter(
        (r) =>
          r.mixingPlanId.toLowerCase().includes(textLower) ||
  ...
  private async resolveContext(input: AudioMixMasterGenerationInput) {
  ...
  private registerGenerationAssets(record: AudioMixMasterGenerationRecord): void {
  ...
  private applySafeRepairs(
    mixing: { trackBalancing: Record<string, string>; processingChain: string[]; busRouting: string[] },
    mastering: { techniques: string[] },
    loudness: { platformTarget: string },
    spatial: { monoCompatibility: string },
    diagnostics: string[]
    ...
    if (diagnostics.some((d) => d.includes("Loudness"))) {
      if (!loudness.platformTarget) {
        (loudness as { platformTarget: string }).platformTarget = "-16 LUFS streaming";
        repairs.push("Default loudness target applied");
      }
    }
    if (diagnostics.some((d) => d.includes("Spatial"))) {
      if (!spatial.monoCompatibility) {
        (spatial as { monoCompatibility: string }).monoCompatibility = "Mono fold-down verified";
  ...
  private reject(start: number, message: string, diagnostics: string[]): AudioMixMasterGenerationResult {
