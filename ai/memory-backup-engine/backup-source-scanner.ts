...
const SOURCE_PATHS: { source: BackupSource; segment: string; subpath?: string }[] = [
...
export class BackupSourceScanner {
  ...
  scan(sources?: BackupSource[]): ScannedFile[] {
    ...
    for (const mapping of SOURCE_PATHS) {
      if (!targetSources.includes(mapping.source)) continue;

      const basePath = mapping.subpath
        ? path.join(resolveStoragePath(this.storageRoot, mapping.segment as never), mapping.subpath)
        : resolveStoragePath(this.storageRoot, mapping.segment as never);
