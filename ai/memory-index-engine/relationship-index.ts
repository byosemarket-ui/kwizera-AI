...
export class RelationshipIndex {
  ...
  initialize(indexesDir: string): void {
    ...
    if (fs.existsSync(this.graphPath)) {
      this.graph = JSON.parse(fs.readFileSync(this.graphPath, "utf8")) as RelationshipGraph;
    } else {
      this.persist();
    }
  }
  ...
  buildFromRecord(record: MemoryRecord, allIds: string[]): MemoryRelationshipNode {
    ...
    for (const id of allIds) {
      if (id === record.memoryId) continue;
      if (this.isRelated(record, id)) {
        relatedIds.push(id);
      }
    }

    const node: MemoryRelationshipNode = {
    ...
    this.graph.edgeCount = Object.values(this.graph.nodes).reduce(
      (sum, n) => sum + n.relatedIds.length,
      0
    );
  ...
  private isRelated(record: MemoryRecord, otherId: string): boolean {
    if (record.relatedProject && otherId.includes(record.relatedProject)) return true;
    if (record.relatedWorkflow && otherId.includes(record.relatedWorkflow)) return true;
    for (const tag of record.tags) {
      if (otherId.toLowerCase().includes(tag.toLowerCase())) return true;
    }
    return false;
  }
  ...
  verifyChecksum(): boolean {
    ...
    if (!fs.existsSync(this.graphPath) || !fs.existsSync(checksumFile)) return false;
