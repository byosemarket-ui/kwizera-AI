...
export class VideoIntelligenceOptimizationScorer {
  ...
  computeScores(
    ...
    const analysisImprovement =
      ...
      performance.analysisSpeedBeforeMs > 0
        ...
        ? Math.min(
            ...
            Math.round(
              ...
              ((performance.analysisSpeedBeforeMs - performance.analysisSpeedMs) /
    ...
    const storytellingImprovement = Math.min(

      100,

      Math.max(5, Math.round((qualityPrediction.scores.storytellingScore / 10) + avgImprovement))

    );

    const videoQualityImprovement = Math.min(

      100,

      Math.max(5, Math.round(qualityPrediction.scores.overallVideoQualityScore / 10 + avgImprovement))

    );

    const renderingReadinessScore = Math.min(

      100,

      Math.max(5, qualityPrediction.scores.productionReadinessScore + Math.round(avgImprovement / 2))
    ...
    const confidenceImprovement = Math.min(
      ...
      Math.max(5, avgImprovement + Math.round((planningImprovement + searchImprovement + analysisImprovement) / 4))
