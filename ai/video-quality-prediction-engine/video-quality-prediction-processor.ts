...
export class VideoQualityPredictionProcessor {
  ...
  constructor(
    ...
    private readonly analyzer: VideoQualityPredictionAnalyzer,

    private readonly scorer: VideoQualityPredictionScorer,

    private readonly linker: VideoQualityPredictionLinker,

    private readonly records: VideoQualityPredictionRecordStore,

    private readonly logger: VideoQualityPredictionLogger

  ) {}



  async predict(input: VideoQualityPredictionInput): Promise<VideoQualityPredictionResult> {
    ...
    const draft: VideoQualityPredictionRecord = {
    ...
    if (query.text) {
      ...
      results = results.filter(
        ...
        (r) =>
          ...
          r.profile.predictionId.toLowerCase().includes(q) ||
