...
import {
  VisualEffectsGenerationAnalyzer,
  VisualEffectsGenerationRecordDraft,
} from "./visual-effects-generation-analyzer.js";
...
export class VisualEffectsGenerationProcessor {
  constructor(
    ...
    private readonly analyzer: VisualEffectsGenerationAnalyzer,
    private readonly scorer: VisualEffectsGenerationScorer,
    private readonly linker: VisualEffectsGenerationLinker,
    private readonly records: VisualEffectsGenerationRecordStore,
    private readonly logger: VisualEffectsGenerationLogger
  ) {}
  ...
  async generateVisualEffectPlans(input: VisualEffectsGenerationInput): Promise<VisualEffectsGenerationResult> {
    ...
    try {
      ...
      const generated: VisualEffectsGenerationRecord[] = [];
      ...
      for (const { scene, cameraPlan, motionPlan, animationPlan } of bundles) {
        ...
        const draftBase = this.analyzer.buildVisualEffectPlan(
          scene,
          cameraPlan,
          motionPlan,
          animationPlan,
        ...
        const draft: VisualEffectsGenerationRecord = {
          ...draftBase,
          scores,
          relationships: this.linker.detectRelationships(
            { ...draftBase, scores } as VisualEffectsGenerationRecord,
            scene,
            motionPlan,
            cameraPlan,
            animationPlan,
  ...
  search(query: VisualEffectsGenerationSearchQuery): VisualEffectsGenerationRecord[] {
    ...
    if (query.visualEffectPlanId) results = results.filter((r) => r.visualEffectPlanId === query.visualEffectPlanId);
    ...
    if (query.keywords) {
      ...
      results = results.filter(
        (r) =>
          r.productEffects.productGlow.toLowerCase().includes(kw) ||
    ...
    if (query.text) {
      ...
      results = results.filter(
        (r) =>
          r.visualEffectPlanId.toLowerCase().includes(textLower) ||
  ...
  private resolveBundles(input: VisualEffectsGenerationInput): SceneUpstreamBundle[] {
  ...
  private registerGenerationAsset(record: VisualEffectsGenerationRecord, scene: SceneGenerationRecord): void {
  ...
  private reject(start: number, message: string, diagnostics: string[]): VisualEffectsGenerationResult {
