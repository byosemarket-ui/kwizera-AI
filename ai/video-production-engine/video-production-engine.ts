...
import {
  VideoProductionEngineError,
  VideoProductionEngineStatusReport,
...
export class AiVideoProductionEngine {
  ...
  readonly logger = new VideoProductionLogger();
  readonly records = new VideoProductionRecordStore();

  private readonly analyzer = new VideoProductionAnalyzer();
  private readonly scorer = new VideoProductionScorer();
  private readonly linker = new VideoProductionLinker();
  private processor: VideoProductionProcessor | null = null;
  ...
  initialize(foundation: AiVideoGenerationFoundation, storageRoot: string): void {
    ...
    this.processor = new VideoProductionProcessor(
      foundation,
      this.analyzer,
      this.scorer,
      this.linker,
  ...
  async repairProductionPlans(
    storyboardId: string,
    platform?: StoryboardGenerationPlatform
  ): Promise<VideoProductionResult | null> {
    this.ensureReady();
  ...
  buildStatusReport(): VideoProductionEngineStatusReport {
    ...
    if (!this.foundation?.getMarketingVideoEngine().isStartupComplete()) readinessScore -= 10;
