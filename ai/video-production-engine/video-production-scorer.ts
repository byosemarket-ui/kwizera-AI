...
export class VideoProductionScorer {
  computeScores(
    ...
    const aiConfidenceScore = Math.round(
      (productionReadinessScore + assetReadinessScore + workflowScore + timelineScore + dependencyScore + performanceScore) / 6
    );
  ...
  isPlanValid(scores: VideoProductionScores, draft: VideoProductionRecordDraft): {
    ...
    if (draft.productionTimeline.renderingTimeline.length < 3) diagnostics.push("Rendering timeline incomplete");
  ...
  private computeProductionReadiness(draft: VideoProductionRecordDraft, storyboard: StoryboardGenerationRecord): number {
    let score = 45;
    if (draft.workflowValidation.productionWorkflowValidated) score += 25;
    if (storyboard.productionReady) score += 15;
    if (draft.renderPreparation.resolution.length > 3) score += 15;
    return Math.min(100, score);
  }

  private computeAssetReadiness(draft: VideoProductionRecordDraft): number {
    let score = 45;
  ...
  private computeWorkflow(draft: VideoProductionRecordDraft): number {
    let score = 40;
    if (draft.workflowValidation.storyboardValidated) score += 10;
    if (draft.workflowValidation.marketingPlansValidated) score += 10;
    if (draft.workflowValidation.audioSyncPlansValidated) score += 10;
    if (draft.productionWorkflow.validationGates.length >= 4) score += 20;
    if (draft.workflowValidation.productionWorkflowValidated) score += 10;
    return Math.min(100, score);
  }

  private computeTimeline(draft: VideoProductionRecordDraft): number {
    let score = 45;
    if (draft.productionTimeline.sceneTimeline.length >= 1) score += 15;
    if (draft.productionTimeline.audioTimeline.length >= 1) score += 15;
    if (draft.productionTimeline.renderingTimeline.length >= 4) score += 15;
    if (draft.productionTimeline.effectsTimeline.length >= 1) score += 10;
    return Math.min(100, score);
  }

  private computeDependency(draft: VideoProductionRecordDraft): number {
    if (draft.dependencyValidation.allDependenciesReady) return 100;
    const total = 14;
    const ready = total - draft.dependencyValidation.missingDependencies.length;
    return Math.max(0, Math.round((ready / total) * 100));
  }

  private computePerformance(draft: VideoProductionRecordDraft): number {
    let score = 70;
    if (draft.exportPreparation.formats.length >= 5) score += 15;
    if (draft.recoveryPlan.rollbackPoints.length >= 2) score += 15;
    return Math.min(100, score);
  }
}
