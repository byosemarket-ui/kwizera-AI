...
import type { AiMemoryIndexEngine } from "../memory-index-engine/memory-index-engine.js";
...
import {
  MemoryRetrievalEngineError,
  MemoryRetrievalResponse,
  MemoryRetrievalStatusReport,
  MemorySearchQuery,
...
export class AiMemoryRetrievalEngine {
  ...
  readonly logger = new MemoryRetrievalLogger();
  private readonly queryBuilder = new SearchQueryBuilder();
  private readonly usageTracker = new UsageTracker(this.logger);
  private readonly cache = new RetrievalCache(this.logger);
  private ranker: ResultRanker | null = null;
  private relatedFinder: RelatedMemoryFinder | null = null;
  private validator: RetrievalValidator | null = null;
  private indexEngine: AiMemoryIndexEngine | null = null;
  ...
  async search(query: MemorySearchQuery): Promise<MemorySearchResponse> {
    ...
    const results: RankedMemoryResult[] = [];
    let fromCache = false;
    ...
    for (const rankedResult of ranked) {
      ...
      if (hydrated) {
        ...
        if (hydrated.record) fromCache = fromCache || this.wasFromCache(rankedResult.memoryId);
      } else {
    ...
    const relatedMemories: RankedMemoryResult[] = [];
    const recommendations: RankedMemoryResult[] = [];
    ...
    if (results.length > 0 && results[0].record) {
      ...
      recommendations.push(
        ...this.relatedFinder!.recommend(
          { text: query.text, project: query.project, workflow: query.workflow },
          allEntries,
  ...
  async retrieve(memoryId: string, requesterId = "memory-retrieval-engine"): Promise<MemoryRetrievalResponse> {
    this.ensureReady();
    ...
    const recommendations = this.relatedFinder!.recommend(
      { text: record.title, project: record.relatedProject, workflow: record.relatedWorkflow },
      allEntries,
      [memoryId, ...relatedMemories.map((r) => r.memoryId)],
      5
    );
  ...
  private async hydrateResult(
    ...
    requesterId: string
  ): Promise<RankedMemoryResult | null> {
  ...
  private wasFromCache(memoryId: string): boolean {
  ...
  private ensureReady(): void {
    if (!this.initialized || !this.storageEngine || !this.foundation) {
      throw new MemoryRetrievalEngineError(
        "Memory Retrieval Engine not initialized",
        "NOT_INITIALIZED"
      );
    }
  }
