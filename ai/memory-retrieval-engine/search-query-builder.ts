...
import { MemorySearchQuery, SearchMode } from "./types.js";
...
export class SearchQueryBuilder {
  filterCandidates(
    ...
    if (query.project) {
      candidates = candidates.filter(
        (e) =>
          e.searchableText.includes(query.project!.toLowerCase()) ||
          e.category === "project"
      );
    }
    ...
    if (query.text) {
      ...
      if (mode === SearchMode.Exact) {
        candidates = candidates.filter(
          (e) => e.title.toLowerCase() === text || e.memoryId === text
        );
      } else {
  ...
  private applyTypeFilter(candidates: MemoryStorageIndexEntry[], query: MemorySearchQuery): void {
    const typeMap: Partial<Record<keyof MemorySearchQuery, MemoryStorageType>> = {
    ...
    for (const [field, type] of Object.entries(typeMap)) {
      const value = query[field as keyof MemorySearchQuery];
      if (typeof value === "string" && value.length > 0) {
        ...
        if (idx >= 0) {
          candidates.splice(
            ...
            candidates.length,
            ...candidates.filter(
              (e) => e.memoryType === type && e.searchableText.includes(value.toLowerCase())
            )
          );
        }
      }
