...
import { RankedMemoryResult, SearchMode } from "./types.js";
...
export class RelatedMemoryFinder {
  ...
  findRelated(
    source: MemoryRecord,
    allEntries: MemoryStorageIndexEntry[],
    limit = 10
  ): RankedMemoryResult[] {
  ...
  recommend(
    context: { text?: string; project?: string; workflow?: string },
    allEntries: MemoryStorageIndexEntry[],
    excludeIds: string[] = [],
    limit = 5
  ): RankedMemoryResult[] {
    ...
    return this.ranker.rank(
      ...
      { text: context.text, limit }
    );
  }

  private computeStrength(source: MemoryRecord, entry: MemoryStorageIndexEntry): number {
    let strength = 0;
  ...
  private computeRecommendationScore(
    ...
    if (context.project && entry.searchableText.includes(context.project.toLowerCase())) score += 30;
    if (context.workflow && entry.searchableText.includes(context.workflow.toLowerCase())) score += 25;
    if (entry.memoryType === MemoryStorageType.Learning) score += 10;
    return score;
  }
  ...
  private getTypeRelationStrength(
    ...
    const relations: Partial<Record<MemoryStorageType, MemoryStorageType[]>> = {
...
export function isRecordRetrievable(record: MemoryRecord): boolean {
  return (
    record.integrityStatus === MemoryIntegrityStatus.Verified ||
