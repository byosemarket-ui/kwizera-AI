...
export class GenerationOptimizationManager {
  ...
  async initialize(storageRoot: string, dependencies: { core: AiCoreManager; models: AiModelManager; images: ImageGenerationManager; videoAudio: VideoAudioGenerationManager }): Promise<void> {
    ...
    await fs.mkdir(this.root, { recursive: true }); this.store = await this.readStore();
  ...
  async getDashboard(projectId?: string): Promise<{ tasks: OptimizationTask[]; history: OptimizationTask[]; logs: OptimizationStore["logs"]; queue: Record<string, number | string>; performance: Record<string, number | string>; analytics: Record<string, number>; integrations: Record<string, boolean> }> {
...
export class GpuOptimizationManager { constructor(private readonly manager: GenerationOptimizationManager) {} usage(): number { return Math.min(95, this.manager.getModels().list().filter((model) => model.status === "loaded").length * 12); } }
