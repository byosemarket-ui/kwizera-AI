...
export class TimelineIntelligenceScorer {
  computeScores(
    ...
    if (timelineLengthMs > 0) timelineQualityScore += 10;
    if (sceneCount >= 3) timelineQualityScore += 15;
    if (shotCount >= sceneCount) timelineQualityScore += 10;
    if (variantCount >= 3) timelineQualityScore += 5;
    ...
    if (optimization.resourceUsageScore >= 80) performanceScore += 15;
    if (optimization.renderingEfficiencyScore >= 75) performanceScore += 15;
    if (trackCount <= 12) performanceScore += 5;
