...
import {
  TimelineIntelligenceEngineError,
  TimelineIntelligenceEngineStatusReport,
  TimelineIntelligenceInput,
  TimelineIntelligenceRecord,
  TimelineIntelligenceResult,
  TimelineIntelligenceSearchQuery,
  TimelineVariant,
  TrackType,
} from "./types.js";
...
export class AiTimelineIntelligenceEngine {
  ...
  readonly logger = new TimelineIntelligenceLogger();
  readonly records = new TimelineIntelligenceRecordStore();

  private readonly analyzer = new TimelineIntelligenceAnalyzer();
  private readonly scorer = new TimelineIntelligenceScorer();
  private readonly linker = new TimelineIntelligenceLinker();
  private processor: TimelineIntelligenceProcessor | null = null;
  ...
  initialize(foundation: AiVideoIntelligenceFoundation, storageRoot: string): void {
    ...
    this.processor = new TimelineIntelligenceProcessor(
      foundation,
      this.analyzer,
      this.scorer,
      this.linker,
  ...
  async analyzeTimeline(input: TimelineIntelligenceInput): Promise<TimelineIntelligenceResult> {
    this.ensureReady();
    ...
    if (result.success) {
      ...
      if (result.record) {
        this.indexingTimes.push(
          result.record.indexes.timelineIndexIds.length + result.record.indexes.sceneIndexIds.length
        );
      }
    }
    return result;
  ...
  detectRelationships(videoId: string): TimelineIntelligenceRecord["relationships"] | null {
    ...
    if (!analysis || !sceneDetection) return record.relationships;
    ...
    return this.linker.detectRelationships(
      ...
      record.relationships.relatedAudioPlans,
      record.relationships.relatedProductionPlans
    );
  }

  async repairTimeline(videoId: string): Promise<TimelineIntelligenceResult | null> {
    this.ensureReady();
    ...
    if (!sceneDetection?.validated) {
      ...
      if (!repairedScene?.success || !repairedScene.record) return null;
  ...
  buildStatusReport(): TimelineIntelligenceEngineStatusReport {
    ...
    if (!this.foundation?.getVideoAnalysisEngine().isStartupComplete()) readinessScore -= 10;
  ...
  private ensureReady(): void {
    if (!this.initialized || !this.foundation || !this.processor) {
      throw new TimelineIntelligenceEngineError(
        "Timeline Intelligence Engine not initialized",
        "NOT_INITIALIZED"
      );
    }
  }
