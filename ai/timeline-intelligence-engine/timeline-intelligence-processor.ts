...
export class TimelineIntelligenceProcessor {
  private readonly indexer: TimelineIntelligenceIndexer;

  constructor(
    private readonly foundation: AiVideoIntelligenceFoundation,
    private readonly analyzer: TimelineIntelligenceAnalyzer,
    private readonly scorer: TimelineIntelligenceScorer,
    private readonly linker: TimelineIntelligenceLinker,
    private readonly records: TimelineIntelligenceRecordStore,
    private readonly logger: TimelineIntelligenceLogger
  ) {
    this.indexer = new TimelineIntelligenceIndexer(foundation);
  }

  async analyze(input: TimelineIntelligenceInput): Promise<TimelineIntelligenceResult> {
    ...
    const scores = this.scorer.computeScores(
      ...
      sceneDetection.sceneCount,
      sceneDetection.shotCount,
      built.tracks.length,
      built.variants.length,
      built.synchronization,
      built.optimization,
      built.editingReadiness,
      built.renderingReadiness,
      analysis.frame.frameConsistencyScore
    );

    const validation = this.scorer.isTimelineValid(
      scores,
      sceneDetection.sceneCount,
      built.tracks.length
    );
    ...
    const draft: TimelineIntelligenceRecord = {
      videoId: input.videoId,
      intelligenceId,
    ...
    draft.relationships = this.linker.detectRelationships(
      ...
      input.relatedAudioPlans,
      input.relatedProductionPlans
    );
  ...
  search(query: TimelineIntelligenceSearchQuery): TimelineIntelligenceRecord[] {
    ...
    if (query.text) {
      ...
      results = results.filter(
        (r) =>
          r.timelineId.toLowerCase().includes(q) ||
