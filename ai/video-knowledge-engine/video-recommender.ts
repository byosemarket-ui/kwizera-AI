...
export class VideoRelationshipLinker {
  ...
  private similarityScore(a: VideoAnalysisRecord, b: VideoAnalysisRecord): number {
    let score = 0;
    if (a.brandName === b.brandName && a.brandName !== "unknown") score += 35;
    if (a.productName === b.productName && a.productName) score += 25;
    if (a.editing.editingStyle === b.editing.editingStyle) score += 20;
    if (a.structure.storyFlow === b.structure.storyFlow) score += 15;
    if (a.audio.backgroundMusic === b.audio.backgroundMusic) score += 15;
