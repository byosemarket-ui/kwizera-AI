...
export class VideoScorer {
  computeScores(
    ...
    const avgProductVisibility =
      structure.sceneSequence.reduce((s, sc) => s + sc.productVisibility, 0) / Math.max(sceneCount, 1);
    ...
    const editingScore = Math.round(
      editing.motionConsistency * 0.3 +
    ...
    const marketingScore = Math.round(
      marketing.customerAttention * 0.3 +
    ...
    const visualScore = Math.round(
      visual.brandingConsistency * 0.35 +
    ...
    const brandConsistencyScore = Math.round(
      visual.brandingConsistency * 0.6 + (visual.logoAnimation ? 20 : 0) + (structure.intro ? 10 : 0)
    );

    const aiConfidenceScore = Math.round(
      (storytellingScore + editingScore + marketingScore + visualScore + audioScore + brandConsistencyScore) / 6
    );
  ...
  isAnalysisValid(scores: VideoQualityScores): { valid: boolean; diagnostics: string[] } {
