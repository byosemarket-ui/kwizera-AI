...
export class VideoUnderstandingProcessor {
  private readonly graphBuilder = new VideoUnderstandingGraphBuilder();

  constructor(
    private readonly foundation: AiVideoIntelligenceFoundation,
    private readonly analyzer: VideoUnderstandingAnalyzer,
    private readonly scorer: VideoUnderstandingScorer,
    private readonly linker: VideoUnderstandingLinker,
    private readonly records: VideoUnderstandingRecordStore,
    private readonly logger: VideoUnderstandingLogger
  ) {}

  async understand(input: VideoUnderstandingInput): Promise<VideoUnderstandingResult> {
    ...
    const scores = this.scorer.computeScores(
      ...
      built.audience,
      built.marketing,
      analysis.scores.productionReadinessScore
    );
    ...
    const draft: VideoUnderstandingRecord = {
      videoId: input.videoId,
      understandingId,
  ...
  search(query: VideoUnderstandingSearchQuery): VideoUnderstandingRecord[] {
    ...
    if (query.product) {
      ...
      results = results.filter(
        (r) =>
          r.product.mainProduct.toLowerCase().includes(q) ||
    ...
    if (query.text) {
      ...
      results = results.filter(
        (r) =>
          r.identity.videoName.toLowerCase().includes(q) ||
