...
import {
  VideoStoryType,
  VideoUnderstandingEngineError,
  VideoUnderstandingEngineStatusReport,
  VideoUnderstandingInput,
  VideoUnderstandingMarketingGoal,
  VideoUnderstandingRecord,
  VideoUnderstandingResult,
  VideoUnderstandingSearchQuery,
} from "./types.js";
...
export class AiVideoUnderstandingEngine {
  ...
  readonly logger = new VideoUnderstandingLogger();
  readonly records = new VideoUnderstandingRecordStore();

  private readonly analyzer = new VideoUnderstandingAnalyzer();
  private readonly scorer = new VideoUnderstandingScorer();
  private readonly linker = new VideoUnderstandingLinker();
  private processor: VideoUnderstandingProcessor | null = null;

  private understandingTimes: number[] = [];
  private searchTimes: number[] = [];
  private relationshipTimes: number[] = [];
  private graphBuildTimes: number[] = [];
  ...
  initialize(foundation: AiVideoIntelligenceFoundation, storageRoot: string): void {
    ...
    this.processor = new VideoUnderstandingProcessor(
      foundation,
      this.analyzer,
      this.scorer,
      this.linker,
  ...
  async understandVideo(input: VideoUnderstandingInput): Promise<VideoUnderstandingResult> {
    this.ensureReady();
  ...
  detectRelationships(videoId: string): VideoUnderstandingRecord["relationships"] | null {
    ...
    const updated = this.linker.detectRelationships(
      ...
      record.relationships.relatedCreativePlans
    );
    this.relationshipTimes.push(Date.now() - start);
    return updated;
  }

  async repairUnderstanding(videoId: string): Promise<VideoUnderstandingResult | null> {
    this.ensureReady();
  ...
  buildStatusReport(): VideoUnderstandingEngineStatusReport {
    ...
    return {
      ...
      sceneUnderstandingStatus:
        "opening, hook, main content, product demo, promotional, CTA and ending scenes prepared",
  ...
  private ensureReady(): void {
    if (!this.initialized || !this.foundation || !this.processor) {
      throw new VideoUnderstandingEngineError(
        "Video Understanding Engine not initialized",
        "NOT_INITIALIZED"
      );
    }
  }
