...
import {
  AudioProductionEngineError,
  AudioProductionEngineStatusReport,
...
export class AiAudioProductionEngine {
  ...
  readonly logger = new AudioProductionLogger();
  readonly records = new AudioProductionRecordStore();

  private readonly analyzer = new AudioProductionAnalyzer();
  private readonly scorer = new AudioProductionScorer();
  private readonly linker = new AudioProductionLinker();
  private processor: AudioProductionProcessor | null = null;
  ...
  initialize(foundation: AiAudioGenerationFoundation, storageRoot: string): void {
    ...
    this.processor = new AudioProductionProcessor(
      foundation,
      this.analyzer,
      this.scorer,
      this.linker,
  ...
  async repairProductionPlan(productId: string, platform?: AudioProductionPlatform): Promise<AudioProductionResult | null> {
    this.ensureReady();
