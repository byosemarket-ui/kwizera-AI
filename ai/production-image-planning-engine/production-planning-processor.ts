...
export class ProductionPlanningProcessor {
  constructor(
    ...
    private readonly records: ProductionImagePlanningRecordStore,
    private readonly logger: ProductionPlanningLogger
  ) {}

  async plan(input: ProductionImagePlanningInput): Promise<ProductionImagePlanningResult> {
    ...
    const scores = this.scorer.computeScores(
      built.dependencies,
      built.assets,
      enhancementPlan.scores.enhancementReadinessScore,
      creativePlan.scores.creativeLayoutScore
    );
    ...
    const draft: ProductionImagePlanningRecord = {
      imageId: input.imageId,
      profile: {
        ...built.profile,
        productionImagePlanId:
          existing?.profile.productionImagePlanId ?? built.profile.productionImagePlanId,
      },
  ...
  private fail(start: number, message: string): ProductionImagePlanningResult {
    return {
      success: false,
      durationMs: Date.now() - start,
      diagnostics: [message],
