...
import {
  ProductionImagePlanningEngineError,
  ProductionImagePlanningEngineStatusReport,
  ProductionImagePlanningInput,
  ProductionImagePlanningRecord,
  ProductionImagePlanningResult,
  ProductionImagePlanningSearchQuery,
} from "./types.js";
...
export class AiProductionImagePlanningEngine {
  ...
  readonly records = new ProductionImagePlanningRecordStore();
  ...
  async planProduction(input: ProductionImagePlanningInput): Promise<ProductionImagePlanningResult> {
    this.ensureReady();
  ...
  detectRelationships(imageId: string): ProductionImagePlanningRecord["relationships"] | null {
    ...
    if (!analysis || !understanding || !enhancementPlan || !creativePlan) return record.relationships;
  ...
  async repairProductionPlan(imageId: string): Promise<ProductionImagePlanningResult | null> {
    this.ensureReady();
  ...
  buildStatusReport(): ProductionImagePlanningEngineStatusReport {
    ...
    if (!this.foundation?.getCreativeImageIntelligenceEngine().isStartupComplete()) readinessScore -= 10;
    if (!this.foundation?.getImageEnhancementPlanningEngine().isStartupComplete()) readinessScore -= 10;
  ...
  private ensureReady(): void {
    if (!this.initialized || !this.foundation || !this.processor) {
      throw new ProductionImagePlanningEngineError(
        "Production Image Planning Engine not initialized",
        "NOT_INITIALIZED"
      );
    }
  }
