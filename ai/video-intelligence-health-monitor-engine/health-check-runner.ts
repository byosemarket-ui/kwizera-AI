...
export class VideoIntelligenceHealthCheckRunner {
  constructor(
    ...
    private readonly moduleChecker: VideoIntelligenceModuleHealthChecker,
    private readonly resourceMonitor: VideoIntelligenceResourceMonitor,
    private readonly earlyWarning: VideoIntelligenceEarlyWarningSystem,
    private readonly autoRepair: VideoIntelligenceAutoRepairHandler,
    private readonly history: VideoIntelligenceHealthHistoryStore,
    private readonly logger: VideoIntelligenceHealthMonitorLogger
  ) {}

  async runCheck(): Promise<VideoIntelligenceHealthCheckResult> {
    ...
    if (
      overallLevel === VideoIntelligenceHealthScoreLevel.Critical ||
    ...
    const qualityPredictionStatus =
      this.foundation.getVideoQualityPredictionEngine().buildStatusReport().readinessScore >= 75;
    ...
    const sceneModule = moduleScores.find(
      (m) => m.module === MonitoredVideoIntelligenceModule.SceneDetection
    );
    ...
    const result: VideoIntelligenceHealthCheckResult = {
      checkId,
    ...
    const historyEntry: VideoIntelligenceHealthHistoryEntry = {
      checkId,
