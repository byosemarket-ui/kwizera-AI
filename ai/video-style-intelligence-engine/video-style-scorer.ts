...
export class VideoStyleScorer {
  computeScores(
    ...
    cinematicScoreBase: number,
    marketingBase: number,
    templateMatchScore: number
  ): StyleQualityScores {
    let styleConsistencyScore = 55;
    if (brand.visualConsistency >= 70) styleConsistencyScore += 15;
    if (brand.visualConsistency >= 85) styleConsistencyScore += 10;
    if (visual.visualIdentity.length > 10) styleConsistencyScore += 5;
    ...
    if (editing.pacing) editingQualityScore += 10;
    if (editing.audioSyncStyle.includes("sync")) editingQualityScore += 10;
    if (editing.editingRhythm) editingQualityScore += 10;
    editingQualityScore = Math.min(100, editingQualityScore);

    const marketingReadinessScore = Math.round(
      (marketingBase + brandStyleScore + styleConsistencyScore) / 3
    );
  ...
  isAnalysisValid(
    ...
    cinematicStyleCount: number,
    templateCount: number
  ): { valid: boolean; diagnostics: string[] } {
    const diagnostics: string[] = [];
    if (cinematicStyleCount < 1) diagnostics.push("At least 1 cinematic style classification required");
    if (templateCount < 1) diagnostics.push("At least 1 style template match required");
