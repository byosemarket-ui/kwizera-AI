...
export function deriveImageIntelligencePerformanceIssues(
  metrics: ImageIntelligenceResourceMetrics
): string[] {
  const issues: string[] = [];
  if (metrics.planningPerformanceMs > 120000) issues.push("Slow image planning detected");
  if (metrics.searchPerformanceMs > 200) issues.push("Slow image search detected");
  if (metrics.diskUsageMb > 5000) issues.push("High image intelligence disk usage");
