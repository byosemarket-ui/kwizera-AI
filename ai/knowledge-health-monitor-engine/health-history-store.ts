...
import { KnowledgeHealthHistoryEntry, KnowledgeHealthScoreLevel } from "./types.js";

export class KnowledgeHealthHistoryStore {
  private historyPath = "";
  private entries: KnowledgeHealthHistoryEntry[] = [];
  ...
  initialize(healthDir: string): void {
    ...
    if (fs.existsSync(this.historyPath)) {
      ...
      ) as KnowledgeHealthHistoryEntry[];
    }
  }
  ...
  getRecent(count = 10): KnowledgeHealthHistoryEntry[] {
...
export class KnowledgeTrendAnalyzer {
  analyze(history: KnowledgeHealthHistoryEntry[]) {
    if (history.length < 2) {
      return {
        ...
        scoreChange: 0,
        warningTrend: 0,
        prediction: "Insufficient data — knowledge monitoring continues",
      };
    }
    ...
    let prediction = "Knowledge system health is stable";
    if (direction === "declining") {
      prediction = "Knowledge health declining — run optimization within 24 hours";
    } else if (warningTrend > 3) {
      prediction = "Warning frequency increasing — knowledge audit recommended";
    } else if (direction === "improving") {
      prediction = "Knowledge health improving — continue current maintenance";
    }
