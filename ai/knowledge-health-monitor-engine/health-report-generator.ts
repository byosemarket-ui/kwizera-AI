...
export class KnowledgeHealthReportGenerator {
  ...
  generateAll(
    ...
    check: KnowledgeHealthCheckResult | null,
    history: KnowledgeHealthHistoryEntry[],
    ...
    fs.writeFileSync(
      ...
      this.buildPerformanceReport(check, status),
      "utf8"
    );
    fs.writeFileSync(
  ...
  private buildHistoryReport(history: KnowledgeHealthHistoryEntry[]): string {
    return [
