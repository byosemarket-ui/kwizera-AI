...
export class CreativePipelineManager {
  ...
  private async readStore(): Promise<PipelineStore> {
    const { readJsonSafe } = await import("../../storage/safe-json.js");
    const result = await readJsonSafe<PipelineStore>(path.join(this.root, "pipeline.json"), { jobs: [], history: [] });
    if (result.recovered) {
      console.warn("[creative-pipeline] recovered from corrupt pipeline.json", result.error);
    }
    return result.value;
  }
