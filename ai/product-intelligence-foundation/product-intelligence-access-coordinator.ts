...
export class ProductIntelligenceAccessCoordinator {
  ...
  constructor(
    private readonly logger: ProductIntelligenceFoundationLogger,
    private readonly history: ProductIntelligenceHistoryStore,
    private readonly registry: ProductIntelligenceRegistry,
    private readonly storage: ProductIntelligenceStorageManager
  ) {}
  ...
  async requestAccess(request: ProductIntelligenceAccessRequest): Promise<ProductIntelligenceAccessResult> {
    ...
    if (!prepared) {
      throw new ProductIntelligenceFoundationError(
        `Unknown product intelligence category: ${request.category}`,
        "UNKNOWN_CATEGORY"
      );
    }
    ...
    if (!registration) {
      throw new ProductIntelligenceFoundationError(
        `Product Intelligence module not in registry: ${prepared.moduleId}`,
        "NOT_REGISTERED"
      );
    }
    ...
    if (!granted) {
      const result: ProductIntelligenceAccessResult = {
        granted: false,
    ...
    if (
      request.operation === ProductIntelligenceAccessOperation.Read ||
      request.operation === ProductIntelligenceAccessOperation.Query
    ) {
      this.readTimes.push(durationMs);
    } else if (
      request.operation === ProductIntelligenceAccessOperation.Write ||
  ...
  private recordAccess(
    request: ProductIntelligenceAccessRequest,
    result: ProductIntelligenceAccessResult,
    success: boolean
  ): void {
    this.history.append({
  ...
  private hasPermission(
    permissions: ProductIntelligenceAccessPermission[],
    required: ProductIntelligenceAccessPermission
  ): boolean {
    if (permissions.includes(required)) return true;
    if (
      required === ProductIntelligenceAccessPermission.Update &&
      permissions.includes(ProductIntelligenceAccessPermission.Write)
    ) {
      return true;
    }
    if (
      required === ProductIntelligenceAccessPermission.Read &&
