...
import { RelationshipDiscoveryResult, RelationshipType } from "./types.js";
...
export class RelationshipDiscovery {
  ...
  async discover(memoryId?: string): Promise<RelationshipDiscoveryResult> {
    ...
    for (const entry of entries) {
      ...
      if (record.relatedProject) {
        ...
        for (const relatedId of byProject.memoryIds) {
          if (relatedId === record.memoryId) continue;
          ...
          const edge = this.graph.createEdge(
            ...
            relatedId,
            record.memoryType,
            relatedEntry.memoryType,
            RelationshipType.Related,
            `Shared project: ${record.relatedProject}`,
            70,
            85
          );
          if (edge) discovered++;
      ...
      for (const tag of record.tags) {
        ...
        for (const relatedId of byTag.memoryIds) {
          ...
          const edge = this.graph.createEdge(
            ...
            relatedId,
            record.memoryType,
            storage.findIndexEntry(relatedId)?.memoryType ?? MemoryStorageType.Project,
            RelationshipType.FrequentlyUsedTogether,
            `Shared tag: ${tag}`,
            55,
            70
          );
      ...
      if (record.category) {
        ...
        for (const relatedId of byCategory.memoryIds) {
          if (relatedId === record.memoryId) continue;
          const relatedEntry = storage.findIndexEntry(relatedId);
          if (!relatedEntry || relatedEntry.memoryType !== record.memoryType) continue;
