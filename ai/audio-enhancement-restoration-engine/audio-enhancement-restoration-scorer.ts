...
export class AudioEnhancementRestorationScorer {
  computeScores(
    ...
    voicePlan: VoiceImprovementPlan,
    musicPlan: MusicImprovementPlan,
  ...
  isEnhancementPlanValid(
    scores: AudioEnhancementScores,
    record: Pick<
      AudioEnhancementGenerationRecord,
      "audioQualityAnalysis" | "enhancementPlan" | "restorationPlan" | "syncPlan" | "voiceImprovementPlan"
    >
  ): { valid: boolean; diagnostics: string[] } {
    const diagnostics: string[] = [];

    if (scores.audioClarityScore < 55) diagnostics.push(`Audio clarity ${scores.audioClarityScore} below threshold (55)`);
    if (scores.restorationScore < 55) diagnostics.push(`Restoration score ${scores.restorationScore} below threshold (55)`);
    if (scores.noiseReductionScore < 55) diagnostics.push(`Noise reduction ${scores.noiseReductionScore} below threshold (55)`);
    ...
    if (
      record.audioQualityAnalysis.audioCategory === AudioInputCategory.VoiceAudio &&
  ...
  isBrandConsistent(context: EnhancementContext, instructions: ProductionEnhancementInstructions): boolean {
    if (!context.brandName) return instructions.clarityGuidance.length >= 1;
  ...
  private computeClarityScore(
    analysis: AudioQualityAnalysis,
    enhancement: EnhancementPlan,
    voice: VoiceImprovementPlan
  ): number {
    let score = 45;
    if (analysis.sampleRate >= 44100) score += 10;
    if (enhancement.techniques.length >= 2) score += 15;
    if (voice.speechClarity.length >= 10 || analysis.audioCategory !== AudioInputCategory.VoiceAudio) score += 15;
    if (analysis.signalToNoiseRatioDb >= 30) score += 15;
    return Math.min(100, score);
  }

  private computeRestorationScore(analysis: AudioQualityAnalysis, restoration: RestorationPlan): number {
    let score = 45;
    if (restoration.techniques.length >= 1) score += 20;
    if (restoration.defectTargets && Object.keys(restoration.defectTargets).length >= 1) score += 15;
    if (restoration.recoveryNotes.length >= 2) score += 10;
    if (analysis.defects.length <= 3 || restoration.severityLevel !== "high") score += 10;
    return Math.min(100, score);
  }

  private computeNoiseReductionScore(analysis: AudioQualityAnalysis, enhancement: EnhancementPlan): number {
    let score = 45;
    if (enhancement.techniques.includes(EnhancementTechnique.NoiseReduction)) score += 25;
    if (analysis.backgroundNoiseLevel === "low" || analysis.defects.includes("background-noise")) score += 15;
    if (enhancement.targetLoudnessLufs !== 0) score += 15;
    return Math.min(100, score);
  }

  private computeSynchronizationScore(sync: AudioSyncPlan): number {
    let score = 45;
    if (sync.timelineAlignment.length >= 10) score += 20;
    if (sync.multiTrackAlignment.length >= 2) score += 15;
    if (sync.syncNotes.length >= 2) score += 10;
    if (sync.lipSyncMetadata.length >= 3) score += 10;
    return Math.min(100, score);
  }

  private computeBrandConsistency(
    context: EnhancementContext,
    instructions: ProductionEnhancementInstructions
  ): number {
    let score = 45;
  ...
  private computeProductionReadiness(
    ...
    if (enhancement.processingChain.length >= 2) score += 15;
    if (restoration.techniques.length >= 1) score += 15;
