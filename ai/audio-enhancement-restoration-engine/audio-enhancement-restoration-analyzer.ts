...
import {
  ...
  OutputPreparationPlan,
  PLATFORM_ENHANCEMENT_CONFIG,
  ProductionEnhancementInstructions,
  RestorationPlan,
...
const INDUSTRY_TYPE_MAP: Record<string, AudioEnhancementType> = {
...
export class AudioEnhancementRestorationAnalyzer {
  analyzeAudioQuality(input: AudioEnhancementGenerationInput, context: EnhancementContext): AudioQualityAnalysis {
    ...
    if (humDetected) defects.push("hum");
    if (clicksDetected) defects.push("clicks");
    if (popsDetected) defects.push("pops");
    if (distortionDetected) defects.push("distortion");
    if (clippingDetected) defects.push("clipping");
    if (prompt.includes("noise") || prompt.includes("hiss")) defects.push("background-noise");
    if (prompt.includes("echo") || prompt.includes("reverb")) defects.push("echo-reverb");
    if (prompt.includes("old") || prompt.includes("vintage")) defects.push("age-degradation");
    if (defects.length === 0 && category === AudioInputCategory.VoiceAudio) defects.push("minor-noise");
  ...
  buildEnhancementPlan(analysis: AudioQualityAnalysis, platform: AudioEnhancementPlatform): EnhancementPlan {
    ...
    const techniques: EnhancementTechnique[] = [EnhancementTechnique.NoiseReduction];
    ...
    if (analysis.audioCategory === AudioInputCategory.MusicAudio) {
      techniques.push(
        EnhancementTechnique.MusicEnhancement,
        EnhancementTechnique.DynamicRangeOptimization,
        EnhancementTechnique.StereoEnhancement
      );
    }
  ...
  buildRestorationPlan(analysis: AudioQualityAnalysis): RestorationPlan {
    const techniques: RestorationTechnique[] = [];

    if (analysis.clicksDetected) techniques.push(RestorationTechnique.ClickRemoval);
    if (analysis.popsDetected) techniques.push(RestorationTechnique.PopRemoval);
    if (analysis.humDetected) techniques.push(RestorationTechnique.HumRemoval);
    if (analysis.defects.includes("background-noise")) techniques.push(RestorationTechnique.HissRemoval);
    if (analysis.echoLevel !== "minimal") techniques.push(RestorationTechnique.EchoReduction);
    if (analysis.distortionDetected) techniques.push(RestorationTechnique.DistortionReduction);
    if (analysis.clippingDetected) techniques.push(RestorationTechnique.ClippingRecovery);
    if (analysis.defects.includes("age-degradation")) {
      techniques.push(RestorationTechnique.OldRecordingRestoration);
    }
    if (analysis.silenceGaps > 0) techniques.push(RestorationTechnique.MissingAudioReconstruction);

    if (techniques.length === 0) techniques.push(RestorationTechnique.HissRemoval);

    const defectTargets: Record<string, string> = {};
  ...
  buildVoiceImprovementPlan(analysis: AudioQualityAnalysis): VoiceImprovementPlan {
    const isVoice =
      analysis.audioCategory === AudioInputCategory.VoiceAudio ||
  ...
  buildMusicImprovementPlan(analysis: AudioQualityAnalysis): MusicImprovementPlan {
    const isMusic =
      analysis.audioCategory === AudioInputCategory.MusicAudio ||
  ...
  resolvePlatform(input: AudioEnhancementGenerationInput, context: EnhancementContext): AudioEnhancementPlatform {
    ...
    if (context.creative?.profile.platform === "youtube") return AudioEnhancementPlatform.YouTube;
    if (context.creative?.profile.platform === "tiktok") return AudioEnhancementPlatform.TikTok;
    if (context.industry === "health") return AudioEnhancementPlatform.Podcast;
  ...
  private detectAudioCategory(prompt: string, input: AudioEnhancementGenerationInput): AudioInputCategory {
  ...
  private detectAudioEnhancementType(context: EnhancementContext, analysis: AudioQualityAnalysis): AudioEnhancementType {
    if (analysis.audioCategory === AudioInputCategory.MusicAudio) return AudioEnhancementType.Music;
    if (analysis.audioCategory === AudioInputCategory.VoiceAudio) return AudioEnhancementType.Voice;
    if (analysis.audioCategory === AudioInputCategory.AmbientAudio) return AudioEnhancementType.Ambient;
    if (analysis.audioCategory === AudioInputCategory.SoundEffects) return AudioEnhancementType.SoundEffects;
    if (analysis.audioCategory === AudioInputCategory.VideoAudio) return AudioEnhancementType.VideoAudio;
    const industry = context.industry ?? "default";
    return INDUSTRY_TYPE_MAP[industry] ?? INDUSTRY_TYPE_MAP.default;
  }

  private extractKeywords(prompt: string, context: EnhancementContext): string[] {
