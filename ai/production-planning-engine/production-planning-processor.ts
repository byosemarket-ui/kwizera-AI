...
export class ProductionPlanningProcessor {
  constructor(
    ...
    private readonly analyzer: ProductionPlanningAnalyzer,
    private readonly scorer: ProductionPlanningScorer,
    private readonly linker: ProductionPlanningLinker,
    private readonly records: ProductionPlanningRecordStore,
    private readonly logger: ProductionPlanningLogger
  ) {}
  ...
  async createProductionPlan(input: ProductionPlanningInput): Promise<ProductionPlanningResult> {
    ...
    if (!storyboard?.validated || !storyboard.productionReady) {
      ...
      if (!storyboardResult.success || !storyboardResult.record) {
        return this.reject(
          start,
          storyboardResult.message ?? "Production-ready storyboard required before production planning",
    ...
    if (!scriptPlan?.validated || !scriptPlan.productionReady) {
      ...
      if (!scriptResult.success || !scriptResult.record) {
        return this.reject(
          start,
          scriptResult.message ?? "Production-ready script plan required before production planning",
    ...
    if (!visualPlan?.validated || !visualPlan.productionReady) {
      ...
      if (!visualResult.success || !visualResult.record) {
        return this.reject(
          start,
          visualResult.message ?? "Production-ready visual plan required before production planning",
    ...
    let audioPlan = input.audioPlanId
      ...
      : audioEngine
          ...
          .find(
            (r) =>
              r.storyboardId === storyboard!.storyboardId &&
    ...
    if (!audioPlan?.validated || !audioPlan.productionReady) {
      ...
      if (!audioResult.success || !audioResult.record) {
        return this.reject(
          start,
          audioResult.message ?? "Production-ready audio plan required before production planning",
          audioResult.diagnostics.length > 0
    ...
    let dependencies = this.analyzer.buildDependencyValidation(
      this.foundation,
      storyboard,
      scriptPlan,
      visualPlan,
    ...
    let sceneProductionPlans = this.analyzer.buildSceneProductionPlans(
      storyboard,
      scriptPlan,
      visualPlan,
      audioPlan
    ...
    let alignment = this.analyzer.validateAlignment(
    ...
    if (!alignment.aligned) {
      sceneProductionPlans = this.analyzer.buildSceneProductionPlans(
        storyboard,
        scriptPlan,
        visualPlan,
        audioPlan
      );
      alignment = this.analyzer.validateAlignment(
        sceneProductionPlans,
        storyboard,
        scriptPlan,
        visualPlan,
    ...
    let validation = this.scorer.isProductionPlanValid(
      scores,
      dependencies,
      assets,
      sceneProductionPlans,
    ...
    if (!validation.valid) {
      ...
      if (repaired.assets || repaired.scenes) {
        if (repaired.assets) assets = repaired.assets;
        if (repaired.scenes) sceneProductionPlans = repaired.scenes;
        dependencies = this.analyzer.buildDependencyValidation(
          this.foundation,
          storyboard,
          scriptPlan,
          visualPlan,
        ...
        validation = this.scorer.isProductionPlanValid(
          repairedScores,
          dependencies,
          assets,
          sceneProductionPlans,
          storyboard,
          this.analyzer.validateAlignment(
            sceneProductionPlans,
            storyboard,
            scriptPlan,
            visualPlan,
    ...
    if (!validation.valid) {
      ...
      return {
        ...
        diagnostics: validation.diagnostics,
        message:
          "Production planning validation failed — every dependency must pass validation before production approval",
      };
    }
    ...
    const draft: ProductionPlanningRecord = {
  ...
  search(query: ProductionPlanningSearchQuery): ProductionPlanningRecord[] {
    ...
    if (query.text) {
      ...
      results = results.filter(
        (r) =>
          r.productionPlanId.toLowerCase().includes(textLower) ||
  ...
  private applyRepairs(
    ...
    sceneProductionPlans: ProductionPlanningRecord["sceneProductionPlans"]
  ): { assets?: AssetManagement; scenes?: ProductionPlanningRecord["sceneProductionPlans"] } {
    ...
    if (!hasMissing && !scenesNeedRepair) return {};

    const repairedAssets = hasMissing
      ? {
          ...assets,
          images: assets.images.map((a) => ({ ...a, status: "planned" as const })),
          logos: assets.logos.map((a) => ({ ...a, status: "planned" as const })),
          videos: assets.videos.map((a) => ({ ...a, status: "planned" as const })),
        }
      : undefined;
  ...
  private reject(start: number, message: string, diagnostics: string[]): ProductionPlanningResult {
