...
export class ImageGenerationHealthCheckRunner {
  constructor(
    ...
    private readonly moduleChecker: ImageGenerationModuleHealthChecker,
    private readonly resourceMonitor: ImageGenerationResourceMonitor,
    private readonly earlyWarning: ImageGenerationEarlyWarningSystem,
    private readonly autoRepair: ImageGenerationAutoRepairHandler,
    private readonly history: ImageGenerationHealthHistoryStore,
    private readonly logger: ImageGenerationHealthMonitorLogger
  ) {}

  async runCheck(): Promise<ImageGenerationHealthCheckResult> {
    ...
    if (
      overallLevel === ImageGenerationHealthScoreLevel.Critical ||
    ...
    const optimizationStatus =
      this.foundation.getImageGenerationOptimizationEngine().buildStatusReport().readinessScore >= 75;
    ...
    const validationModule = moduleScores.find(
      (m) => m.module === MonitoredImageGenerationModule.ImageQualityValidation
    );
    ...
    const result: ImageGenerationHealthCheckResult = {
      checkId,
    ...
    const historyEntry: ImageGenerationHealthHistoryEntry = {
      checkId,
