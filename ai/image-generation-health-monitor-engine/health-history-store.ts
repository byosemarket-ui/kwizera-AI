...
export class ImageGenerationHealthHistoryStore {
  private historyPath = "";
  private entries: ImageGenerationHealthHistoryEntry[] = [];
  ...
  initialize(healthDir: string): void {
    ...
    if (fs.existsSync(this.historyPath)) {
      ...
      ) as ImageGenerationHealthHistoryEntry[];
    }
  }
  ...
  getRecent(count = 10): ImageGenerationHealthHistoryEntry[] {
...
export class ImageGenerationTrendAnalyzer {
  analyze(history: ImageGenerationHealthHistoryEntry[]) {
    if (history.length < 2) {
      return {
        ...
        scoreChange: 0,
        warningTrend: 0,
        prediction: "Insufficient data — image generation monitoring continues",
      };
    }
    ...
    let prediction = "Image generation system health is stable";
    if (direction === "declining") {
      prediction = "Image generation health declining — run optimization within 24 hours";
    } else if (warningTrend > 3) {
      prediction = "Warning frequency increasing — image generation audit recommended";
    } else if (direction === "improving") {
      prediction = "Image generation health improving — continue current maintenance";
    }
