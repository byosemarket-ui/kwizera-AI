...
import {
  ImageGenerationAutoRepairResult,
...
export class ImageGenerationAutoRepairHandler {
  ...
  async attemptRepairs(warnings: ImageGenerationHealthWarning[]): Promise<ImageGenerationAutoRepairResult> {
    const repairs: string[] = [];
    let success = true;

    const critical = warnings.some(
      (w) =>
        w.severity === ImageGenerationHealthScoreLevel.Critical ||
        w.severity === ImageGenerationHealthScoreLevel.Failed
    );

    const hasCriticalWarning = warnings.some(
      (w) =>
        w.type === ImageGenerationWarningType.DatabaseProblems ||
    ...
    if (critical || hasCriticalWarning || warnings.length > 3) {
      ...
      this.foundation.integration.reportCriticalIssue(
        `Critical image generation health: ${warnings.map((w) => w.message).join("; ")}`
      );
