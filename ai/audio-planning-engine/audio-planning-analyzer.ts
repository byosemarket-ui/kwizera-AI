...
import {
  AudioPlanningInput,
  AudioPlanningProfile,
  AudioSynchronization,
  EmotionalFlowPlanning,
  MusicPlanning,
  PlatformAudioRules,
  SceneAudioPlan,
  SoundEffectPlanning,
...
const PLATFORM_AUDIO_CONFIG: Record<
  CreativePlatform,
  { maxVoiceSec: number; musicVolume: string; sfxDensity: string; voiceRequired: boolean; pacing: string; wpm: number }
> = {
...
const PURPOSE_EMOTION: Record<string, string> = {
  hook: "curiosity",
...
const PURPOSE_SFX: Record<string, string[]> = {
  hook: ["whoosh", "impact"],
...
export class AudioPlanningAnalyzer {
  ...
  buildVoicePlanning(
    ...
    const pauseTiming: Record<number, string> = {};
  ...
  buildSoundEffectPlanning(storyboard: StoryboardIntelligenceRecord): SoundEffectPlanning {
    const sceneEffects: Record<number, string[]> = {};

    for (const scene of storyboard.scenes) {
      sceneEffects[scene.sceneNumber] = (PURPOSE_SFX[scene.scenePurpose] ?? ["transition"]).map(
        (s) => `Plan ${s} for scene ${scene.sceneNumber}`
      );
    }
  ...
  buildSynchronization(
    ...
    const voiceTiming: Record<number, string> = {};
    const musicTiming: Record<number, string> = {};
    const subtitleTiming: Record<number, string> = {};
    const sceneTiming: Record<number, string> = {};
    const transitionTiming: Record<number, string> = {};
