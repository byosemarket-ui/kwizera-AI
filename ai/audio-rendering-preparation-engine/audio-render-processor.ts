...
import {
  ...
  AudioRenderRecord,
  AudioRenderResult,
  AudioRenderSearchQuery,
} from "./types.js";

export class AudioRenderProcessor {
  constructor(
    private readonly foundation: AiAudioGenerationFoundation,
    private readonly analyzer: AudioRenderAnalyzer,
    private readonly scorer: AudioRenderScorer,
    private readonly linker: AudioRenderLinker,
    private readonly records: AudioRenderRecordStore,
    private readonly logger: AudioRenderLogger
  ) {}

  async generateRenderPlan(input: AudioRenderInput): Promise<AudioRenderResult> {
    ...
    try {
      ...
      if (!context) {
        return this.reject(
          start,
          "Unable to resolve render context — provide productionId, productId with production plan, or render prompt",
          ["Production plan or render prompt required"]
        );
      }
      ...
      const draftPartial: AudioRenderRecord = {
  ...
  search(query: AudioRenderSearchQuery): AudioRenderRecord[] {
    ...
    if (query.audioRenderPlanId) results = results.filter((r) => r.audioRenderPlanId === query.audioRenderPlanId);
    ...
    if (query.codec) results = results.filter((r) => r.renderSettings.codec === query.codec);
    if (query.keywords) {
      const kw = query.keywords.toLowerCase();
      results = results.filter(
        (r) =>
          r.audioRenderPlanId.toLowerCase().includes(kw) ||
    ...
    if (query.text) {
      ...
      results = results.filter(
        (r) =>
          r.audioRenderPlanId.toLowerCase().includes(textLower) ||
  ...
  private async resolveContext(input: AudioRenderInput) {
  ...
  private registerRenderAssets(record: AudioRenderRecord, input: AudioRenderInput): void {
  ...
  private applySafeRepairs(
    renderValidation: { validated: boolean; stage: string; notes: string[] }[],
    trackValidation: { validated: boolean; check: string; notes: string[] }[],
    timelineValidation: { validated: boolean; check: string; notes: string[] }[],
    assetValidation: { assetType: AudioRenderAssetType; assetId: string; validated: boolean; notes: string[] }[],
    diagnostics: string[]
  ...
  private reject(start: number, message: string, diagnostics: string[]): AudioRenderResult {
