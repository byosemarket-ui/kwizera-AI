...
import {
  AudioRenderEngineError,
  AudioRenderEngineStatusReport,
  AudioRenderInput,
  AudioRenderPlatform,
  AudioRenderRecord,
  AudioRenderResult,
...
export class AiAudioRenderingPreparationEngine {
  ...
  readonly logger = new AudioRenderLogger();
  readonly records = new AudioRenderRecordStore();

  private readonly analyzer = new AudioRenderAnalyzer();
  private readonly scorer = new AudioRenderScorer();
  private readonly linker = new AudioRenderLinker();
  private processor: AudioRenderProcessor | null = null;
  ...
  initialize(foundation: AiAudioGenerationFoundation, storageRoot: string): void {
    ...
    this.processor = new AudioRenderProcessor(
      foundation,
      this.analyzer,
      this.scorer,
      this.linker,
  ...
  async repairRenderPlan(productId: string, platform?: AudioRenderPlatform): Promise<AudioRenderResult | null> {
    this.ensureReady();
  ...
  buildStatusReport(): AudioRenderEngineStatusReport {
    ...
    const avgRender =
      all.length > 0 ? Math.round(all.reduce((s, r) => s + r.scores.renderReadinessScore, 0) / all.length) : 0;
    const avgTrack =
      all.length > 0 ? Math.round(all.reduce((s, r) => s + r.scores.trackIntegrityScore, 0) / all.length) : 0;
