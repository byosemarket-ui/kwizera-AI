...
import {
  ...
  ObjectDetectionEngineStatusReport,
...
export class AiObjectDetectionIntelligenceEngine {
  ...
  readonly logger = new ObjectDetectionLogger();
  readonly records = new ObjectDetectionRecordStore();

  private readonly analyzer = new ObjectDetectionAnalyzer();
  private readonly scorer = new ObjectDetectionScorer();
  private readonly linker = new ObjectDetectionLinker();
  private processor: ObjectDetectionProcessor | null = null;
  ...
  initialize(foundation: AiImageIntelligenceFoundation, storageRoot: string): void {
    ...
    this.processor = new ObjectDetectionProcessor(
      foundation,
      this.analyzer,
      this.scorer,
      this.linker,
  ...
  async detectObjects(input: ObjectDetectionInput): Promise<ObjectDetectionResult> {
    this.ensureReady();
    const result = await this.processor!.detect(input);
    if (result.success) this.detectionTimes.push(result.durationMs);
    return result;
  }
  ...
  async repairDetection(imageId: string): Promise<ObjectDetectionResult | null> {
    this.ensureReady();
  ...
  private ensureReady(): void {
    if (!this.initialized || !this.foundation || !this.processor) {
      throw new ObjectDetectionEngineError(
        "Object Detection Intelligence Engine not initialized",
        "NOT_INITIALIZED"
      );
    }
  }
