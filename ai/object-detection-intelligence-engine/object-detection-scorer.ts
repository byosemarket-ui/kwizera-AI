...
export class ObjectDetectionScorer {
  computeScores(
    ...
    const hasStructure =
      objects.some((o) => o.objectType === DetectedObjectType.Product) ||
    ...
    if (product.mainProduct) creativeReadinessScore += 15;
    if (logo.logoPresent) creativeReadinessScore += 10;
    if (text.textPresent) creativeReadinessScore += 10;
    if (objects.length >= 3) creativeReadinessScore += 10;
    creativeReadinessScore = Math.min(100, creativeReadinessScore);

    const aiConfidenceScore = Math.round(
      (objectDetectionScore + productVisibilityScore + sceneOrganizationScore + brandVisibilityScore + creativeReadinessScore) / 5
    );
