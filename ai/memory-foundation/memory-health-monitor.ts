...
export class MemoryHealthMonitor {
  private lastReport: MemoryHealthReport | null = null;
  ...
  async runHealthCheck(
    ...
    access: MemoryAccessCoordinator
    ...
    const failedModules = modules.filter(
      (m) => m.healthStatus === MemoryHealthLevel.Critical || m.healthStatus === MemoryHealthLevel.Failed
    );
  ...
  private scoreToLevel(score: number): MemoryHealthLevel {
    if (score >= 95) return MemoryHealthLevel.Excellent;
    if (score >= 80) return MemoryHealthLevel.Good;
    if (score >= 60) return MemoryHealthLevel.Warning;
    if (score >= 40) return MemoryHealthLevel.Critical;
