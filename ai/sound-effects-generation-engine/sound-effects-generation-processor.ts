...
export class SoundEffectsGenerationProcessor {
  constructor(
    ...
    private readonly analyzer: SoundEffectsGenerationAnalyzer,
    private readonly scorer: SoundEffectsGenerationScorer,
    private readonly linker: SoundEffectsGenerationLinker,
    private readonly records: SoundEffectsGenerationRecordStore,
    private readonly logger: SoundEffectsGenerationLogger
  ) {}
  ...
  async generateSoundEffectPlan(input: SoundEffectsGenerationInput): Promise<SoundEffectsGenerationResult> {
    ...
    try {
      ...
      const draft: SoundEffectsGenerationRecord = {
        soundPlanId: profile.soundPlanId,
        profile,
        soundAnalysis,
        soundEffectPlan,
  ...
  search(query: SoundEffectsSearchQuery): SoundEffectsGenerationRecord[] {
    ...
    if (query.soundPlanId) results = results.filter((r) => r.soundPlanId === query.soundPlanId);
    ...
    if (query.soundCategory) results = results.filter((r) => r.profile.soundCategory === query.soundCategory);
    ...
    if (query.keywords) {
      ...
      results = results.filter(
        (r) =>
          r.soundAnalysis.keywords.some((k) => k.includes(kw)) ||
    ...
    if (query.text) {
      ...
      results = results.filter(
        (r) =>
          r.soundPlanId.toLowerCase().includes(textLower) ||
  ...
  private async resolveContext(input: SoundEffectsGenerationInput) {
  ...
  private registerGenerationAssets(record: SoundEffectsGenerationRecord): void {
  ...
  private applySafeRepairs(
    soundPlan: { foleySounds: string[]; transitionSounds: string[]; objectSounds: string[] },
    foley: { foleyTypes: FoleyType[] },
    environmental: { ambientLayers: string[] },
    timeline: { cuePoints: { timeSec: number; label: string; soundType: string }[] },
    ...
    if (diagnostics.some((d) => d.includes("sound layers"))) {
      if (soundPlan.foleySounds.length < 1) soundPlan.foleySounds.push("default foley");
      if (soundPlan.transitionSounds.length < 1) soundPlan.transitionSounds.push("whoosh");
      repairs.push("Default sound layers applied");
    }
    if (diagnostics.some((d) => d.includes("Foley"))) {
      if (foley.foleyTypes.length < 1) foley.foleyTypes.push(FoleyType.Footsteps);
      repairs.push("Default foley type applied");
    }
    if (diagnostics.some((d) => d.includes("Environmental"))) {
      if (environmental.ambientLayers.length < 1) environmental.ambientLayers.push("room tone bed");
    ...
    if (diagnostics.some((d) => d.includes("Timeline"))) {
      if (timeline.cuePoints.length < 3) {
        timeline.cuePoints.push(
          ...
          { timeSec: 5, label: "Action", soundType: "foley" },
          { timeSec: 10, label: "End", soundType: "fade-out" }
        );
        repairs.push("Default timeline cue points applied");
      }
    }
    if (diagnostics.some((d) => d.includes("Sync preparation"))) {
      if (sync.hitPoints.length < 1) sync.hitPoints.push("Primary hit at midpoint");
  ...
  private reject(start: number, message: string, diagnostics: string[]): SoundEffectsGenerationResult {
