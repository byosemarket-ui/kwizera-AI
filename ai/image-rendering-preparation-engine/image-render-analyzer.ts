...
import {
  ...
  IMAGE_RENDER_PLATFORM_CONFIG,
  ImageRenderAssetType,
  ImageRenderColorSpace,
  ImageRenderInput,
  ImageRenderLayerCheck,
  ImageRenderMaskType,
  ImageRenderPlanProfile,
  ImageRenderPlatform,
  ImageRenderValidationStage,
...
export class ImageRenderAnalyzer {
  buildProfile(
    ...
    const productionId =
      input.productionId ?? context.productionPlan?.imageProductionId ?? `production-${context.productId ?? "unknown"}`;
    const imageId =
      input.imageId ??
  ...
  buildRenderSettings(profile: ImageRenderPlanProfile): RenderSettingsPlan {
    ...
    const isPrint =
      profile.platform === ImageRenderPlatform.Print ||
  ...
  resolvePlatform(input: ImageRenderInput, context: ImageRenderContext): ImageRenderPlatform {
    return (
      input.platform ??
      (context.productionPlan?.profile.platform as ImageRenderPlatform | undefined) ??
  ...
  private buildOutputProfile(platform: ImageRenderPlatform): OutputProfileEntry {
  ...
  private resolveMaskId(maskType: ImageRenderMaskType, context: ImageRenderContext): string {
    switch (maskType) {
  ...
  private resolveAssetSource(assetType: ImageRenderAssetType, context: ImageRenderContext): string {
