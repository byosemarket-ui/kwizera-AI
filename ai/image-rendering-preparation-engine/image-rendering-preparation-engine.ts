...
import {
  ImageRenderEngineError,
  ImageRenderEngineStatusReport,
  ImageRenderInput,
  ImageRenderPlatform,
  ImageRenderRecord,
  ImageRenderResult,
...
export class AiImageRenderingPreparationEngine {
  ...
  readonly logger = new ImageRenderLogger();
  readonly records = new ImageRenderRecordStore();

  private readonly analyzer = new ImageRenderAnalyzer();
  private readonly scorer = new ImageRenderScorer();
  private readonly linker = new ImageRenderLinker();
  private processor: ImageRenderProcessor | null = null;
  ...
  initialize(foundation: AiImageGenerationFoundation, storageRoot: string): void {
    ...
    this.processor = new ImageRenderProcessor(
      foundation,
      this.analyzer,
      this.scorer,
      this.linker,
  ...
  async repairRenderPlan(productId: string, platform?: ImageRenderPlatform): Promise<ImageRenderResult | null> {
    this.ensureReady();
  ...
  buildStatusReport(): ImageRenderEngineStatusReport {
    ...
    const avgLayer =
      all.length > 0 ? Math.round(all.reduce((s, r) => s + r.scores.layerIntegrityScore, 0) / all.length) : 0;
