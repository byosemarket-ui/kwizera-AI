...
import {
  ...
  ImageEnhancePreservationTarget,
...
const CATEGORY_ENHANCEMENT_MAP: Record<ImageEnhanceCategory, ImageEnhanceOperationType> = {
...
const CATEGORY_RESTORATION_MAP: Record<ImageEnhanceCategory, ImageEnhanceRestorationType> = {
...
const INDUSTRY_CATEGORY_MAP: Record<string, ImageEnhanceCategory> = {
...
export class ImageEnhancementAnalyzer {
  analyzeImage(context: ImageEnhancementContext, input: ImageEnhancementInput): EnhancementImageAnalysis {
    const resolution =
      context.editingPlan?.imageAnalysis.resolution ??
  ...
  buildProfile(
    ...
    const category =
      input.imageCategory ?? INDUSTRY_CATEGORY_MAP[context.industry ?? "default"] ?? ImageEnhanceCategory.Product;
    const primaryEnhancement =
      input.primaryEnhancement ??
  ...
  resolveSourceImageId(input: ImageEnhancementInput, context: ImageEnhancementContext): string | null {
    ...
    if (input.editedImageId) return input.editedImageId;
    if (input.imageEditingPlanId) return input.imageEditingPlanId;
    if (input.productImagePlanId) return input.productImagePlanId;
    if (context.editingPlan) return context.editingPlan.profile.editedImageId;
  ...
  extractContextFromProduct(
    ...
    backgroundPlan?: BackgroundGenerationRecord | null,
    editingPlan?: ImageEditingRecord | null
  ): ImageEnhancementContext | null {
    if (!analysis && !understanding && !productImagePlan && !editingPlan) return null;
  ...
  private preservationNote(target: ImageEnhancePreservationTarget, subject: string, brand?: string): string {
    const map: Record<ImageEnhancePreservationTarget, string> = {
