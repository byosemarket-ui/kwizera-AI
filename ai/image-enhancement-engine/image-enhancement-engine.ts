...
import {
  ...
  ImageEnhancementEngineError,
  ImageEnhancementEngineStatusReport,
...
export class AiImageEnhancementEngine {
  ...
  readonly logger = new ImageEnhancementLogger();
  readonly records = new ImageEnhancementRecordStore();

  private readonly analyzer = new ImageEnhancementAnalyzer();
  private readonly scorer = new ImageEnhancementScorer();
  private readonly linker = new ImageEnhancementLinker();
  private processor: ImageEnhancementProcessor | null = null;
  ...
  initialize(foundation: AiImageGenerationFoundation, storageRoot: string): void {
    ...
    this.processor = new ImageEnhancementProcessor(
      foundation,
      this.analyzer,
      this.scorer,
      this.linker,
  ...
  ): Promise<ImageEnhancementResult | null> {
    this.ensureReady();
  ...
  buildStatusReport(): ImageEnhancementEngineStatusReport {
    ...
    const avgEnhancement =
      all.length > 0 ? Math.round(all.reduce((s, r) => s + r.scores.enhancementScore, 0) / all.length) : 0;
    const avgProduction =
      all.length > 0
  ...
  private ensureReady(): void {
    if (!this.initialized || !this.foundation || !this.processor) {
      throw new ImageEnhancementEngineError(
        "Image Enhancement & Restoration Engine not initialized",
        "NOT_INITIALIZED"
      );
    }
  }
