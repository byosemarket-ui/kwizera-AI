...
import {
  ...
  ImageEnhancementSearchQuery,
} from "./types.js";

export class ImageEnhancementProcessor {
  constructor(
    private readonly foundation: AiImageGenerationFoundation,
    private readonly analyzer: ImageEnhancementAnalyzer,
    private readonly scorer: ImageEnhancementScorer,
    private readonly linker: ImageEnhancementLinker,
    private readonly records: ImageEnhancementRecordStore,
    private readonly logger: ImageEnhancementLogger
  ) {}

  async generateEnhancementPlan(input: ImageEnhancementInput): Promise<ImageEnhancementResult> {
    ...
    try {
      ...
      if (!context) {
        return this.reject(
          start,
          "Unable to resolve enhancement context — provide sourceImageId, editedImageId, productId with pipeline, imageEditingPlanId, or productImagePlanId",
          ["Source image or upstream pipeline required"]
        );
      }
      ...
      const draftPartial: ImageEnhancementRecord = {
      ...
      const draft: ImageEnhancementRecord = {
        ...draftPartial,
  ...
  search(query: ImageEnhancementSearchQuery): ImageEnhancementRecord[] {
    ...
    if (query.enhancementPlanId) results = results.filter((r) => r.enhancementPlanId === query.enhancementPlanId);
    ...
    if (query.primaryEnhancement) results = results.filter((r) => r.profile.primaryEnhancement === query.primaryEnhancement);
    if (query.restorationType) results = results.filter((r) => r.profile.primaryRestoration === query.restorationType);
    if (query.imageCategory) results = results.filter((r) => r.profile.imageCategory === query.imageCategory);
    if (query.keywords) {
      const kw = query.keywords.toLowerCase();
      results = results.filter(
        (r) =>
          Object.values(r.enhancementOperations.operationPrompts).some((p) => p.toLowerCase().includes(kw)) ||
    ...
    if (query.text) {
      ...
      results = results.filter(
        (r) =>
          r.enhancementPlanId.toLowerCase().includes(textLower) ||
  ...
  private async resolveContext(input: ImageEnhancementInput) {
  ...
  private registerGenerationAssets(record: ImageEnhancementRecord, input: ImageEnhancementInput): void {
  ...
  private applySafeRepairs(
    restoration: { restorationStrategy: string },
    operations: { operationPrompts: Record<string, string> },
    preservation: { identityLock: boolean; productLock: boolean; compositionLock: boolean },
    print: { dpiPlanning: string },
    superResolution: { upscalingMethod: string },
    diagnostics: string[]
  ...
  private reject(start: number, message: string, diagnostics: string[]): ImageEnhancementResult {
