...
const CATEGORY_PHOTOGRAPHY_MAP: Record<string, ProductPhotographyMode> = {
...
const CATEGORY_BACKGROUND_MAP: Record<string, ProductImageBackgroundType> = {
...
export class ProductImageGenerationAnalyzer {
  ...
  buildPresentationPlan(context: ProductGenerationContext): ProductPresentationPlan {
    ...
    const views: ProductPresentationViewDefinition[] = ALL_PRODUCT_PRESENTATION_VIEWS.map((view, index) =>
      this.buildViewDefinition(view, product, context, index + 1)
    );
  ...
  buildPhotographyPlan(
    input: ProductImageGenerationInput,
    context: ProductGenerationContext
  ): ProductPhotographyPlan {
    const primary =
      input.photographyMode ??
    ...
    return {
      ...
      luxuryTreatment:
        context.industry === "beauty" || context.industry === "fashion"
  ...
  buildBackgroundPlan(
    ...
    const descriptions: Record<ProductImageBackgroundType, ProductBackgroundPlan> = {
  ...
  buildMarketingVariations(
    ...
    const platformMap: Record<ProductMarketingVariation, ProductImageGenPlatform> = {
  ...
  resolvePlatform(input: ProductImageGenerationInput, context: ProductGenerationContext): ProductImageGenPlatform {
    ...
    if (context.creative?.profile.platform === "instagram-reels") return ProductImageGenPlatform.Instagram;
    if (context.creative?.profile.platform === "tiktok") return ProductImageGenPlatform.TikTok;
  ...
  extractContextFromProduct(
    ...
    strategy: MarketingStrategyRecord | null,
    input: ProductImageGenerationInput
  ): ProductGenerationContext | null {
    if (!analysis && !understanding) return null;
  ...
  private buildViewDefinition(
    ...
    const viewConfig: Record<ProductPresentationView, { angle: string; framing: string; desc: string }> = {
  ...
  private getMarketplaceNotes(platform: ProductImageGenPlatform): string[] {
    const map: Record<ProductImageGenPlatform, string[]> = {
