...
export class StoryGenerationProcessor {
  constructor(
    ...
    private readonly analyzer: StoryGenerationAnalyzer,
    private readonly scorer: StoryGenerationScorer,
    private readonly linker: StoryGenerationLinker,
    private readonly records: StoryGenerationRecordStore,
  ...
  async generateStoryboard(input: StoryboardGenerationInput): Promise<StoryboardGenerationResult> {
    ...
    try {
      ...
      const draft: StoryboardGenerationRecord = {
  ...
  search(query: StoryboardGenerationSearchQuery): StoryboardGenerationRecord[] {
    ...
    if (query.storyType) results = results.filter((r) => r.profile.storyType === query.storyType);
    if (query.keywords) {
      const kw = query.keywords.toLowerCase();
      results = results.filter(
        (r) =>
          r.storyStructure.openingHook.toLowerCase().includes(kw) ||
    ...
    if (query.text) {
      ...
      results = results.filter(
        (r) =>
          r.storyboardId.toLowerCase().includes(textLower) ||
  ...
  private async resolveContext(input: StoryboardGenerationInput) {
    ...
    if (input.productId && productFoundation) {
      ...
      if (!intelligence?.validated) {
        ...
        if (created.success && created.record) intelligence = created.record;
      }
  ...
  private registerGenerationAsset(record: StoryboardGenerationRecord): void {
  ...
  private reject(start: number, message: string, diagnostics: string[]): StoryboardGenerationResult {
