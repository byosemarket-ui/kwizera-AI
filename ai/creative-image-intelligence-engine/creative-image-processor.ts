...
export class CreativeImageProcessor {
  ...
  constructor(
    ...
    private readonly analyzer: CreativeImageAnalyzer,

    private readonly scorer: CreativeImageScorer,

    private readonly linker: CreativeImageLinker,

    private readonly records: CreativeImageIntelligenceRecordStore,

    private readonly logger: CreativeImageLogger

  ) {}



  async plan(input: CreativeImageIntelligenceInput): Promise<CreativeImageIntelligenceResult> {
    ...
    const enhancementPlan =
      ...
      this.foundation.getImageEnhancementPlanningEngine().getEnhancementPlan(input.imageId) ?? null;
    ...
    const draft: CreativeImageIntelligenceRecord = {
    ...
    if (query.creativeStyle) {
      ...
      results = results.filter(
        ...
        (r) =>
          ...
          r.creativeStyle.primaryStyle === query.creativeStyle ||
