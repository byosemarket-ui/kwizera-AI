...
export class ImageProductionProcessor {
  constructor(
    ...
    private readonly analyzer: ImageProductionAnalyzer,
    private readonly scorer: ImageProductionScorer,
    private readonly linker: ImageProductionLinker,
    private readonly records: ImageProductionRecordStore,
    private readonly logger: ImageProductionLogger
  ) {}
  ...
  async generateProductionPlan(input: ImageProductionInput): Promise<ImageProductionResult> {
    ...
    try {
      ...
      if (!context) {
        return this.reject(
          start,
          "Unable to resolve production context — provide imagePlanId, stylePlanId, productId with pipeline, or productImagePlanId",
          ["Image plan or product intelligence pipeline required"]
        );
      }
      ...
      const draftPartial: ImageProductionRecord = {
      ...
      const draft: ImageProductionRecord = {
        ...draftPartial,
  ...
  search(query: ImageProductionSearchQuery): ImageProductionRecord[] {
    ...
    if (query.imageProductionId) results = results.filter((r) => r.imageProductionId === query.imageProductionId);
    ...
    if (query.assetId) {
      results = results.filter((r) =>
        r.assetValidation.some((a) => a.assetId === query.assetId || a.assetId.includes(query.assetId!))
      );
    }
    if (query.keywords) {
      ...
      results = results.filter(
        (r) =>
          r.imageProductionId.toLowerCase().includes(kw) ||
    ...
    if (query.text) {
      ...
      results = results.filter(
        (r) =>
          r.imageProductionId.toLowerCase().includes(textLower) ||
  ...
  private async resolveContext(input: ImageProductionInput) {
  ...
  private registerProductionAssets(record: ImageProductionRecord, input: ImageProductionInput): void {
  ...
  private applySafeRepairs(
    workflowValidation: { validated: boolean; stage: string }[],
    assetValidation: { validated: boolean; assetType: string; assetId: string; notes: string[] }[],
    dependencyValidation: { available: boolean; dependency: string; notes: string[] }[],
    productionStructure: { layerStructure: unknown[]; maskStructure: unknown[] },
    diagnostics: string[]
  ...
  private reject(start: number, message: string, diagnostics: string[]): ImageProductionResult {
