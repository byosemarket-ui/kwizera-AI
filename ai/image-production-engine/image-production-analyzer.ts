...
import {
  ...
  IMAGE_PRODUCTION_PLATFORM_CONFIG,
  ImageProductionAssetType,
  ImageProductionColorSpace,
  ImageProductionDependency,
  ImageProductionExportFormat,
  ImageProductionInput,
  ImageProductionPlatform,
  ImageProductionProfile,
  ImageProductionWorkflowStage,
...
export class ImageProductionAnalyzer {
  ...
  buildRenderPreparation(profile: ImageProductionProfile): RenderPreparationPlan {
    ...
    const isPrint =
      profile.platform === ImageProductionPlatform.Print ||
  ...
  buildExportPreparation(input: ImageProductionInput): ExportPreparationPlan {
    const exports: ExportPreparationEntry[] = ALL_IMAGE_PRODUCTION_EXPORT_FORMATS.map((format) => ({
      format,
      ...
      colorSpace:
        format === ImageProductionExportFormat.Pdf && input.platform === ImageProductionPlatform.Print
  ...
  buildPlatformRules(input: ImageProductionInput): PlatformProductionRules[] {
    if (input.preparePlatformRules === false) {
      return ALL_IMAGE_PRODUCTION_PLATFORMS.slice(0, 1).map((platform) =>
        this.buildPlatformRule(platform)
      );
    }
  ...
  resolvePlatform(input: ImageProductionInput, context: ImageProductionContext): ImageProductionPlatform {
    return (
      input.platform ??
      (context.stylePlan?.profile.platform as ImageProductionPlatform | undefined) ??
      ImageProductionPlatform.Website
    );
  }

  resolveImagePlanId(input: ImageProductionInput, context: ImageProductionContext): string | null {
    if (input.imagePlanId) return input.imagePlanId;
    if (input.stylePlanId) return input.stylePlanId;
    if (context.stylePlan) return context.stylePlan.stylePlanId;
  ...
  extractContextFromProduct(
    analysis: ProductAnalysisIntelligenceRecord | null | undefined,
    understanding: ProductUnderstandingRecord | null | undefined,
    creative: CreativeDirectionRecord | null | undefined,
    strategy: MarketingStrategyRecord | null | undefined,
    input: ImageProductionInput,
    productImagePlan?: ProductImageGenerationRecord | null,
    brandingPlan?: BrandingDesignRecord | null,
    stylePlan?: MultiStyleImageRecord | null
  ...
  private buildPlatformRule(platform: ImageProductionPlatform): PlatformProductionRules {
    const config = IMAGE_PRODUCTION_PLATFORM_CONFIG[platform];
    const exportFormats =
      platform === ImageProductionPlatform.Print || platform === ImageProductionPlatform.Packaging
  ...
  private resolveAssetSource(assetType: ImageProductionAssetType, context: ImageProductionContext): string {
    ...
    if (
      (assetType === ImageProductionAssetType.Logo ||
