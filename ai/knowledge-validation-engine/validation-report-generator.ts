...
export class ValidationReportGenerator {
  constructor(
    ...
    private readonly runner: KnowledgeValidationRunner,
  ...
  async generateAll(status: KnowledgeValidationStatusReport): Promise<{
  ...
  private buildValidationReport(
    ...
    results: ReturnType<KnowledgeValidationRunner["getAllResults"]>,
    relationships: Awaited<ReturnType<KnowledgeRelationshipValidator["validateAll"]>>
  ): string {
    return [
      ...
      ...results.map(
        (r) =>
          `| ${r.knowledgeId} | ${r.validationLevel} | ${r.trusted ? "yes" : "no"} | ${r.scores.qualityScore} | ${r.issues.length} |`
      ),
      "",
  ...
  private buildQualityReport(results: ReturnType<KnowledgeValidationRunner["getAllResults"]>): string {
    const avg = (field: keyof (typeof results)[0]["scores"]) =>
      results.length > 0
    ...
    return [
      ...
      ...results.map(
        (r) =>
          `| ${r.knowledgeId} | ${r.scores.qualityScore} | ${r.scores.reliabilityScore} | ${r.scores.completenessScore} | ${r.scores.consistencyScore} | ${r.scores.confidenceScore} | ${r.validationLevel} |`
      ),
      "",
    ].join("\n");
  }
  ...
  private buildIntegrityReport(
    integrity: Awaited<ReturnType<KnowledgeIntegrityValidator["validateAll"]>>,
    relationships: Awaited<ReturnType<KnowledgeRelationshipValidator["validateAll"]>>
  ): string {
    return [
