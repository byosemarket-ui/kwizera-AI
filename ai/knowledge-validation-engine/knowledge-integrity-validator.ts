...
export class KnowledgeIntegrityValidator {
  ...
  async validateAll(): Promise<KnowledgeIntegrityValidationResult> {
    ...
    for (const entry of storage.getIndexEntries()) {
      ...
      try {
        ...
        checksumFailures++;
        diagnostics.push(
          `Checksum verification error for ${entry.knowledgeId}: ${error instanceof Error ? error.message : String(error)}`
        );
      }
    ...
    const result: KnowledgeIntegrityValidationResult = {
  ...
  async quarantineCorruptRecords(): Promise<number> {
    ...
    for (const entry of storage.getIndexEntries()) {
      ...
      if (unreadable) {
        const repaired = await storage.quarantineUnreadableRecord(
          entry.knowledgeId,
