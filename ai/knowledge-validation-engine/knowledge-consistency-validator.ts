...
export class KnowledgeConsistencyValidator {
  ...
  async validateAll(repair = false): Promise<KnowledgeConsistencyValidationResult> {
    ...
    for (const [, ids] of titleMap) {
      if (ids.length > 1) {
        const hashes = new Set(
          ids.map((id) => entries.find((e) => e.knowledgeId === id)?.contentHash)
        );
        if (hashes.size > 1) conflictingRecords++;
      }
    }
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        orphanRecords++;
        diagnostics.push(
          `Unreadable record: ${entry.knowledgeId} (${error instanceof Error ? error.message : String(error)})`
        );
        continue;
      }
    ...
    const result: KnowledgeConsistencyValidationResult = {
  ...
  async rejectInvalidRecords(): Promise<number> {
    const storage = this.foundation.getStorageEngine();
    let rejected = 0;
    ...
    for (const entry of storage.getIndexEntries()) {
      ...
      const record = read.record;
      if (
        record.integrityStatus === KnowledgeIntegrityStatus.Corrupted ||
