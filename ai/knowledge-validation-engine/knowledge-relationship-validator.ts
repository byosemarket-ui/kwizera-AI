...
export class KnowledgeRelationshipValidator {
  ...
  validateRecord(record: KnowledgeRecord): { valid: boolean; issues: string[] } {
    ...
    if (
      !graph.nodes[record.knowledgeId] &&
  ...
  async validateAll(repair = false): Promise<KnowledgeRelationshipValidationResult> {
    ...
    const result: KnowledgeRelationshipValidationResult = {
