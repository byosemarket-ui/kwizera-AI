...
export class MultiStyleImageProcessor {
  constructor(
    ...
    private readonly analyzer: MultiStyleImageAnalyzer,
    private readonly scorer: MultiStyleImageScorer,
    private readonly linker: MultiStyleImageLinker,
    private readonly records: MultiStyleImageRecordStore,
    private readonly logger: MultiStyleImageLogger
  ) {}
  ...
  async generateStylePlan(input: MultiStyleImageInput): Promise<MultiStyleImageResult> {
    ...
    try {
      ...
      if (!context) {
        return this.reject(
          start,
          "Unable to resolve multi-style context — provide sourceImageId, productId with pipeline, productImagePlanId, or brandGuidelines",
      ...
      const draftPartial: MultiStyleImageRecord = {
      ...
      const draft: MultiStyleImageRecord = {
        ...draftPartial,
  ...
  search(query: MultiStyleImageSearchQuery): MultiStyleImageRecord[] {
    ...
    if (query.stylePlanId) results = results.filter((r) => r.stylePlanId === query.stylePlanId);
    ...
    if (query.styleCategory) results = results.filter((r) => r.profile.styleCategory === query.styleCategory);
    if (query.templateId) results = results.filter((r) => r.relationships.templates.includes(query.templateId!));
    if (query.keywords) {
      const kw = query.keywords.toLowerCase();
      results = results.filter(
        (r) =>
          r.styleTransformation.styleMapping.toLowerCase().includes(kw) ||
    ...
    if (query.text) {
      ...
      results = results.filter(
        (r) =>
          r.stylePlanId.toLowerCase().includes(textLower) ||
  ...
  private async resolveContext(input: MultiStyleImageInput) {
  ...
  private registerGenerationAssets(record: MultiStyleImageRecord, input: MultiStyleImageInput): void {
  ...
  private applySafeRepairs(
    transformation: { styleMapping: string; colorAdaptation: string },
    variations: { variations: unknown[] },
    preservation: { identityLock: boolean; productLock: boolean; logoLock: boolean },
    diagnostics: string[]
  ...
  private reject(start: number, message: string, diagnostics: string[]): MultiStyleImageResult {
