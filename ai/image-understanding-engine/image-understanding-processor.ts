...
export class ImageUnderstandingProcessor {
  constructor(
    ...
    private readonly analyzer: ImageUnderstandingAnalyzer,
    private readonly scorer: ImageUnderstandingScorer,
    private readonly linker: ImageUnderstandingLinker,
    private readonly records: ImageUnderstandingRecordStore,
    private readonly logger: ImageUnderstandingLogger
  ) {}

  async understand(input: ImageUnderstandingInput): Promise<ImageUnderstandingResult> {
    ...
    const draft: ImageUnderstandingRecord = {
      imageId: input.imageId,
      understandingId,
  ...
  search(query: ImageUnderstandingSearchQuery): ImageUnderstandingRecord[] {
    ...
    if (query.product) {
      ...
      results = results.filter(
        (r) =>
          r.product.productContext.toLowerCase().includes(q) ||
    ...
    if (query.text) {
      ...
      results = results.filter(
        (r) =>
          r.identity.imageName.toLowerCase().includes(q) ||
