...
const SCENE_BY_TYPE: Partial<Record<ImageAnalysisType, ImageSceneType>> = {
...
export class ImageUnderstandingAnalyzer {
  buildFromAnalysis(
    ...
    marketingGoal: ImageUnderstandingMarketingGoal = ImageUnderstandingMarketingGoal.Conversion,
    platform: ImageUnderstandingPlatform = ImageUnderstandingPlatform.MultiPlatform,
    industry = "general"
  ): {
    ...
    const context: ImageContextUnderstanding = {
  ...
  private inferPrimaryPurpose(
    classification: ImageAnalysisIntelligenceRecord["classification"],
    product: string,
    brand: string
  ): string {
  ...
  private buildSceneUnderstanding(analysis: ImageAnalysisIntelligenceRecord): SceneUnderstanding {
    ...
    if (bg.includes("outdoor") || bg.includes("urban") || bg.includes("street")) environment = "outdoor";
    else if (bg.includes("studio") || bg.includes("white")) environment = "studio";
    else if (bg.includes("indoor")) environment = "indoor";

    const preparedScenes: ImageSceneType[] = [sceneType];
    if (environment === "studio") preparedScenes.push(ImageSceneType.Studio);
    if (classification.imageType === ImageAnalysisType.LifestyleImage) preparedScenes.push(ImageSceneType.Lifestyle);
    if (classification.imageType === ImageAnalysisType.ProductImage) preparedScenes.push(ImageSceneType.ProductShowcase);
  ...
  private buildVisualUnderstanding(analysis: ImageAnalysisIntelligenceRecord): VisualUnderstanding {
  ...
  private buildMarketingUnderstanding(
    analysis: ImageAnalysisIntelligenceRecord,
    goal: ImageUnderstandingMarketingGoal,
    industry: string,
    productName: string
  ): MarketingUnderstanding {
    const audience =
    ...
    return {
      ...
      ctaOpportunity:
        analysis.classification.imageType === ImageAnalysisType.Banner ||
  ...
  private buildRecommendations(
    ...
    const recs: UnderstandingRecommendation[] = [];
  ...
  private technicalFallback(analysis: ImageAnalysisIntelligenceRecord): string {
