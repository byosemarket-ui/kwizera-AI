...
export class ImageUnderstandingRecordStore {
  private storePath = "";
  private records = new Map<string, ImageUnderstandingRecord>();

  initialize(engineDir: string): void {
    fs.mkdirSync(engineDir, { recursive: true });
    this.storePath = path.join(engineDir, "image-understanding-records.json");
    if (fs.existsSync(this.storePath)) {
      const raw = fs.readFileSync(this.storePath, "utf8");
      const list = JSON.parse(raw) as ImageUnderstandingRecord[];
