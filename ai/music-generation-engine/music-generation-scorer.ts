...
export class MusicGenerationScorer {
  ...
  isMusicPlanValid(
    ...
    record: Pick<
      MusicGenerationRecord,
      "musicAnalysis" | "compositionPlan" | "arrangementPlan" | "moodPlan" | "syncPreparation"
    >
  ...
  isBrandConsistent(context: MusicContext, moodPlan: MoodPlan): boolean {
    if (!context.brandName) return moodPlan.brandMoodAlignment.length >= 10;
    return moodPlan.brandMoodAlignment.toLowerCase().includes(context.brandName.toLowerCase());
  }

  private computeCompositionScore(analysis: MusicAnalysis, plan: CompositionPlan): number {
    let score = 45;
    if (plan.melodyStructure.length >= 2) score += 15;
    if (plan.intro && plan.verse && plan.chorus && plan.outro) score += 20;
    if (plan.chordProgression.length >= 2) score += 10;
    if (analysis.durationSec > 0) score += 10;
    return Math.min(100, score);
  }

  private computeHarmonyScore(plan: CompositionPlan): number {
    let score = 45;
    if (plan.harmonyStructure.length >= 2) score += 20;
    if (plan.chordProgression.length >= 3) score += 20;
    if (plan.bridge.length >= 10) score += 15;
    return Math.min(100, score);
  }

  private computeRhythmScore(plan: CompositionPlan, analysis: MusicAnalysis): number {
    let score = 45;
    if (plan.rhythmStructure.length >= 2) score += 20;
    if (analysis.tempo.length >= 5) score += 15;
    if (analysis.timeSignature) score += 10;
    if (plan.intro && plan.chorus) score += 10;
    return Math.min(100, score);
  }

  private computeEmotionalScore(moodPlan: MoodPlan, analysis: MusicAnalysis): number {
    let score = 45;
    if (moodPlan.emotionalArc.length >= 3) score += 20;
    if (moodPlan.moodTransitions.length >= 2) score += 15;
    if (moodPlan.primaryMood === analysis.mood) score += 15;
    if (moodPlan.intensityCurve.length >= 10) score += 5;
    return Math.min(100, score);
  }

  private computeBrandConsistency(context: MusicContext, moodPlan: MoodPlan): number {
    let score = 45;
    if (moodPlan.brandMoodAlignment.length >= 15) score += 20;
  ...
  private computeProductionReadiness(
    ...
    arrangement: ArrangementPlan,
    syncPlan: SyncPreparationPlan,
    instructions: ProductionMusicInstructions
  ): number {
    let score = 45;
    if (analysis.durationSec > 0) score += 10;
    if (arrangement.activeInstruments.length >= 2) score += 15;
    if (syncPlan.hitPoints.length >= 3) score += 15;
    if (instructions.renderNotes.length >= 2) score += 10;
