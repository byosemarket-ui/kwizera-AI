...
export class ImageQualityPredictionProcessor {
  constructor(
    ...
    private readonly analyzer: ImageQualityPredictionAnalyzer,
    private readonly scorer: ImageQualityPredictionScorer,
    private readonly linker: ImageQualityPredictionLinker,
    private readonly records: ImageQualityPredictionRecordStore,
    private readonly logger: ImageQualityPredictionLogger
  ) {}

  async predict(input: ImageQualityPredictionInput): Promise<ImageQualityPredictionResult> {
    ...
    const draft: ImageQualityPredictionRecord = {
      imageId: input.imageId,
      profile: {
        ...built.profile,
  ...
  search(query: ImageQualityPredictionSearchQuery): ImageQualityPredictionRecord[] {
    ...
    if (query.riskLevel) {
      ...
      results = results.filter(
        (r) => this.scorer.severityRank(r.highestRiskLevel) >= this.scorer.severityRank(target)
      );
    }
  ...
  private fail(start: number, message: string): ImageQualityPredictionResult {
    return {
      success: false,
      durationMs: Date.now() - start,
      diagnostics: [message],
