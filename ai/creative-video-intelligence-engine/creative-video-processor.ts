...
export class CreativeVideoProcessor {
  constructor(
    ...
    private readonly analyzer: CreativeVideoAnalyzer,
    private readonly scorer: CreativeVideoScorer,
    private readonly linker: CreativeVideoLinker,
    private readonly records: CreativeVideoRecordStore,
    private readonly logger: CreativeVideoLogger
  ) {}

  async planCreative(input: CreativeVideoIntelligenceInput): Promise<CreativeVideoIntelligenceResult> {
    ...
    const productionBase =
      enhancement?.scores.productionReadinessScore ?? analysis.scores.productionReadinessScore;
    ...
    const validation = this.scorer.isPlanValid(
      scores,
      built.storyboard.sceneOrder.length,
      built.recommendations.length,
    ...
    const draft: CreativeVideoIntelligenceRecord = {
      videoId: input.videoId,
      intelligenceId,
  ...
  search(query: CreativeVideoSearchQuery): CreativeVideoIntelligenceRecord[] {
    ...
    if (query.creativeType) results = results.filter((r) => r.creativeType === query.creativeType);
    ...
    if (query.text) {
      ...
      results = results.filter(
        (r) =>
          r.profile.creativeVideoId.toLowerCase().includes(q) ||
  ...
  private reject(start: number, videoId: string, message: string): CreativeVideoIntelligenceResult {
