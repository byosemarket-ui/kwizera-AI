...
import {
  CreativeVideoEngineStatusReport,
  CreativeVideoIntelligenceEngineError,
  CreativeVideoIntelligenceInput,
  CreativeVideoIntelligenceRecord,
  CreativeVideoIntelligenceResult,
  CreativeVideoSearchQuery,
...
export class AiCreativeVideoIntelligenceEngine {
  ...
  readonly logger = new CreativeVideoLogger();
  readonly records = new CreativeVideoRecordStore();
  readonly templateLibrary = new CreativeVideoTemplateLibrary();

  private readonly analyzer = new CreativeVideoAnalyzer();
  private readonly scorer = new CreativeVideoScorer();
  private readonly linker = new CreativeVideoLinker();
  private processor: CreativeVideoProcessor | null = null;
  ...
  initialize(foundation: AiVideoIntelligenceFoundation, storageRoot: string): void {
    ...
    this.processor = new CreativeVideoProcessor(
      foundation,
      this.analyzer,
      this.scorer,
      this.linker,
  ...
  async planCreativeVideo(input: CreativeVideoIntelligenceInput): Promise<CreativeVideoIntelligenceResult> {
    this.ensureReady();
  ...
  async repairCreativePlan(videoId: string): Promise<CreativeVideoIntelligenceResult | null> {
    this.ensureReady();
  ...
  buildStatusReport(): CreativeVideoEngineStatusReport {
    ...
    return {
      ...
      storyboardPlanningStatus:
        "story structure, hook, scene order, timing, product/brand reveal, CTA, ending",
      creativePlanningStatus: "story flow, emotional flow, marketing flow, viewer/conversion journey",
      marketingPlanningStatus:
        "product showcase, offer, brand awareness, social engagement, lead gen, CTA",
      visualPlanningStatus:
        "camera, motion, composition, lighting, color, typography, graphics, transitions, effects",
  ...
  private ensureReady(): void {
    if (!this.initialized || !this.foundation || !this.processor) {
      throw new CreativeVideoIntelligenceEngineError(
        "Creative Video Intelligence Engine not initialized",
        "NOT_INITIALIZED"
      );
    }
  }
