import { CreativeVideoTemplate, CreativeVideoTemplateType, CreativeVideoType } from "./types.js";
import { VideoAnalysisType } from "../video-analysis-engine/types.js";

const BASE_TEMPLATES: Omit<CreativeVideoTemplate, "matchScore">[] = [
  {
...
export class CreativeVideoTemplateLibrary {
  ...
  private scoreTemplate(
    tpl: Omit<CreativeVideoTemplate, "matchScore">,
    videoType: VideoAnalysisType,
    creativeType: CreativeVideoType,
    industry?: string
  ): number {
    ...
    if (ind.includes("tech") && tpl.type === CreativeVideoTemplateType.Electronics) score += 15;
    if (ind.includes("fashion") && tpl.type === CreativeVideoTemplateType.Fashion) score += 20;
    if (ind.includes("health") && tpl.type === CreativeVideoTemplateType.Healthcare) score += 20;
    if (ind.includes("education") && tpl.type === CreativeVideoTemplateType.Education) score += 15;
