...
export class StoryboardProcessor {
  constructor(
    ...
    private readonly analyzer: StoryboardAnalyzer,
    private readonly scorer: StoryboardScorer,
    private readonly linker: StoryboardLinker,
    private readonly records: StoryboardRecordStore,
  ...
  async createStoryboard(input: StoryboardIntelligenceInput): Promise<StoryboardIntelligenceResult> {
    ...
    if (!creative?.validated) {
      ...
      if (!creativeResult.success || !creativeResult.record) {
        return this.reject(
          start,
          creativeResult.message ?? "Complete creative direction required before storyboard",
          creativeResult.diagnostics.length > 0
    ...
    const draft: StoryboardIntelligenceRecord = {
  ...
  search(query: StoryboardSearchQuery): StoryboardIntelligenceRecord[] {
    ...
    if (query.scenePurpose) {
      ...
      results = results.filter((r) =>
        r.scenes.some((s) => s.scenePurpose.toLowerCase().includes(purposeLower))
      );
    }
    if (query.text) {
  ...
  private reject(start: number, message: string, diagnostics: string[]): StoryboardIntelligenceResult {
