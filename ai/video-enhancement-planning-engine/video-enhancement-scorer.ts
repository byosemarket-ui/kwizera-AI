...
export class VideoEnhancementScorer {
  computeScores(
  ...
  ): EnhancementQualityScores {
    let enhancementReadinessScore = 55;
    if (recommendationCount >= 3) enhancementReadinessScore += 10;
    if (platformRuleCount >= 4) enhancementReadinessScore += 10;
    if (quality.visualClarity >= 70) enhancementReadinessScore += 5;
    if (quality.audioQuality >= 70) enhancementReadinessScore += 5;
    enhancementReadinessScore = Math.min(100, enhancementReadinessScore);

    const visualQualityScore = Math.round(
      (quality.visualClarity + quality.frameQuality + quality.colorAccuracy + quality.lighting) / 4
    );
    ...
    const productionReadinessScore = Math.round(
      (productionBase + enhancementReadinessScore + visualQualityScore) / 3
    );
  ...
  isPlanValid(
    ...
    nonDestructiveValid: boolean
  ): { valid: boolean; diagnostics: string[] } {
    const diagnostics: string[] = [];
    if (recommendationCount < 2) diagnostics.push("At least 2 enhancement recommendations required");
    if (platformCount < 4) diagnostics.push("At least 4 platform optimization rules required");
    if (!nonDestructiveValid) diagnostics.push("Non-destructive policy must preserve original and support undo/redo");
