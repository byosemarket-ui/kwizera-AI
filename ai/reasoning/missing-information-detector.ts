import { MissingInformationItem, ReasoningRequest, ReasoningType } from "./types.js";

const CRITICAL_FIELDS: Partial<Record<ReasoningType, string[]>> = {
...
export class MissingInformationDetector {
  detect(request: ReasoningRequest): MissingInformationItem[] {
    const missing: MissingInformationItem[] = [];
    const required = CRITICAL_FIELDS[request.type] ?? [];

    for (const field of required) {
      if (
        request.inputs[field] === undefined ||
