...
export class AnimationGenerationScorer {
  computeScores(
    ...
    const aiConfidenceScore = Math.round(
      (animationQualityScore + smoothnessScore + visualAppealScore + synchronizationScore + productionReadinessScore) / 5
    );
  ...
  isPlanValid(scores: AnimationGenerationScores, draft: AnimationGenerationRecordDraft): {
    ...
    if (!draft.timeline.animationDuration) diagnostics.push("Animation duration required");
  ...
  private computeQuality(draft: AnimationGenerationRecordDraft, scene: SceneGenerationRecord): number {
    let score = 45;
    if (draft.characterAnimation.gesture.length > 3) score += 10;
    if (draft.productAnimation.reveal.length > 5) score += 10;
    if (draft.textAnimation.reveal.length > 5) score += 10;
    if (draft.transitionAnimation.dissolve.length > 3) score += 10;
    if (scene.structure.sceneObjectives.length >= 1) score += 15;
    return Math.min(100, score);
  }

  private computeSmoothness(timeline: AnimationGenerationRecordDraft["timeline"]): number {
    let score = 45;
    if (timeline.easing) score += 25;
    if (timeline.animationDuration) score += 15;
    if (timeline.layerPriority.length >= 4) score += 15;
    return Math.min(100, score);
  }

  private computeVisualAppeal(draft: AnimationGenerationRecordDraft): number {
    let score = 45;
    if (draft.logoAnimation.logoReveal.length > 5) score += 15;
    if (draft.environmentAnimation.particles.length > 3) score += 15;
    if (draft.textAnimation.kineticTypography.length > 5) score += 15;
    if (draft.productAnimation.showcase.length > 5) score += 10;
    return Math.min(100, score);
  }

  private computeSync(sync: AnimationGenerationRecordDraft["synchronization"]): number {
    let score = 40;
    if (sync.motionSync.length >= 1) score += 20;
    if (sync.cameraSync.length >= 1) score += 20;
    if (sync.audioSync.length >= 1) score += 20;
    return Math.min(100, score);
  }
  ...
  private computeProductionReadiness(
    ...
    let score = 45;
    if (scene.productionReady && motionPlan.productionReady && cameraPlan.productionReady) score += 25;
    if (draft.platformOptimizations.length >= 7) score += 15;
    if (draft.timeline.layerPriority.length >= 5) score += 15;
    return Math.min(100, score);
  }
}
