import { ProductQualityScores, ProductRecord } from "./types.js";

export class ProductScorer {
  computeScores(product: Partial<ProductRecord>, presentationRating?: number): ProductQualityScores {
    ...
    const learningScore = Math.min(
      100,
      (product.lessonsLearned?.length ?? 0) * 15 + (product.patterns?.length ?? 0) * 10
    );

    const aiConfidenceScore = Math.round(
      (profileScore + visualScore + marketingScore + learningScore) / 4
    );
