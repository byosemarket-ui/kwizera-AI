...
import {
  ...
  BackgroundIntelligenceRecommendation,
  BackgroundQuality,
...
export class BackgroundAnalyzer {
  buildFromIntelligence(
    ...
    const bgAnalysis: BackgroundAnalysis = {
      backgroundType,
    ...
    const classification: BackgroundClassification = {
      backgroundType,
  ...
  private classifyBackgroundType(
    ...
    if (lower.includes("gradient") || lower.includes("sunset")) return BackgroundType.Gradient;
    if (lower.includes("abstract") || lower.includes("pattern")) return BackgroundType.Abstract;
    if (lower.includes("office") || lower.includes("desk")) return BackgroundType.Office;
    ...
    if (sceneType === ImageSceneType.Lifestyle) return BackgroundType.Lifestyle;
    if (sceneType === ImageSceneType.Commercial) return BackgroundType.Commercial;
    if (sceneType === ImageSceneType.Indoor) return BackgroundType.Indoor;
    if (analysis.classification.imageType === "background") return BackgroundType.Custom;

    return BackgroundType.Custom;
  }

  private assessComplexity(detection: ObjectDetectionRecord, backgroundLabel: string): BackgroundComplexity {
    const bgObjects = detection.objects.filter(
      (o) => o.objectType === DetectedObjectType.BackgroundObject || o.objectType === DetectedObjectType.DecorativeElement
    );
    ...
    if (totalObjects <= 2) return BackgroundComplexity.Low;
    if (totalObjects <= 4) return BackgroundComplexity.Medium;
    if (bgObjects.length >= 2 || totalObjects > 5) return BackgroundComplexity.High;
    return BackgroundComplexity.Medium;
  }

  private inferTexture(label: string, complexity: BackgroundComplexity): string {
    const lower = label.toLowerCase();
    if (lower.includes("gradient")) return "smooth-gradient";
    if (lower.includes("urban") || lower.includes("street")) return "urban-textured";
    if (lower.includes("studio") || lower.includes("white")) return "smooth-matte";
    if (complexity === BackgroundComplexity.Minimal) return "flat";
    return complexity === BackgroundComplexity.High ? "detailed-textured" : "moderate-textured";
  }

  private inferPattern(label: string): string {
    const lower = label.toLowerCase();
    if (lower.includes("gradient")) return "gradient";
    if (lower.includes("abstract")) return "abstract";
    if (lower.includes("urban")) return "urban-elements";
    if (lower.includes("studio") || lower.includes("white")) return "solid";
    return "contextual";
  }

  private inferDepth(sceneType: ImageSceneType, complexity: BackgroundComplexity): string {
    if (sceneType === ImageSceneType.Studio || complexity === BackgroundComplexity.Minimal) return "shallow";
    if (sceneType === ImageSceneType.Outdoor || sceneType === ImageSceneType.Lifestyle) return "deep";
    return "moderate";
  }

  private computeCleanliness(detection: ObjectDetectionRecord, complexity: BackgroundComplexity): number {
    let score = 85;
    const distractions = detection.objects.filter(
      (o) =>
        o.objectType !== DetectedObjectType.Product &&
        o.objectType !== DetectedObjectType.Logo &&
        o.objectType !== DetectedObjectType.BackgroundObject
    );
    score -= distractions.length * 5;
    if (complexity === BackgroundComplexity.High || complexity === BackgroundComplexity.VeryHigh) score -= 10;
    if (complexity === BackgroundComplexity.Minimal) score += 10;
    return Math.max(20, Math.min(100, score));
  }

  private computeDistraction(detection: ObjectDetectionRecord, complexity: BackgroundComplexity): number {
    const nonEssential = detection.objects.filter(
      (o) =>
        o.objectType !== DetectedObjectType.Product &&
        o.objectType !== DetectedObjectType.BackgroundObject
    ).length;
    let distraction = nonEssential * 8;
    if (complexity === BackgroundComplexity.High) distraction += 15;
    if (complexity === BackgroundComplexity.Minimal) distraction = Math.max(0, distraction - 20);
    return Math.min(100, distraction);
  }

  private computeColorHarmony(colors: string[]): number {
    if (colors.length === 0) return 60;
    if (colors.length <= 3) return 85;
    if (colors.length <= 5) return 72;
    return 58;
  }
  ...
  private buildRecommendations(
    ...
    quality: BackgroundQuality,
    suitability: BackgroundSuitability,
    plan: BackgroundReplacementPlan
  ): BackgroundIntelligenceRecommendation[] {
    const recs: BackgroundIntelligenceRecommendation[] = [];
