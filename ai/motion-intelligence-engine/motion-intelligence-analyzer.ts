...
export class MotionIntelligenceAnalyzer {
  analyze(
    ...
    const motionEvents = this.detectMotionEvents(
      sceneDetection,
      objectMotions,
      subjectTracks,
      camera,
    ...
    const classifications = this.classifyMotion(
      analysis,
      motionDensity,
      camera,
      understanding,
    ...
    const motionPlan = this.buildMotionPlan(
      classifications,
      metrics,
      timeline,
      camera,
  ...
  private inferGlobalDirection(
    ...
    if (motionDensity < 20) return MotionDirection.Static;
    if (videoType === VideoAnalysisType.SocialMedia) return MotionDirection.Mixed;
    if (motionDensity > 65) return MotionDirection.Forward;
    if (motionDensity > 45) return MotionDirection.Right;
    return MotionDirection.Left;
  }

  private inferSpeed(motionDensity: number): MotionSpeed {
    if (motionDensity < 20) return MotionSpeed.Static;
    if (motionDensity < 40) return MotionSpeed.Slow;
    if (motionDensity < 65) return MotionSpeed.Normal;
    if (motionDensity < 80) return MotionSpeed.Fast;
  ...
  private analyzeObjectMotions(
    ...
    const motions: ObjectMotionAnalysis[] = [];
    ...
    if (motions.length === 0 && sceneDetection.shots.length > 0) {
      ...
      motions.push(
        this.objectMotion("default", ObjectMotionType.BackgroundMotion, TrackingSubjectType.Object, shot, motionDensity, MotionDirection.Static, MotionSpeed.Static, 65)
      );
    }
  ...
  private inferShotDirection(index: number, motionDensity: number): MotionDirection {
    if (motionDensity < 20) return MotionDirection.Static;
    const dirs = [
  ...
  private buildSubjectTracks(
    ...
    const bySubject = new Map<TrackingSubjectType, ObjectMotionAnalysis[]>();
  ...
  private detectTrackGaps(motions: ObjectMotionAnalysis[]): number {
    let gaps = 0;
    for (let i = 1; i < motions.length; i++) {
      if (motions[i]!.startMs - motions[i - 1]!.endMs > 1000) gaps++;
    }
    return gaps;
  }

  private summarizeDirections(motions: ObjectMotionAnalysis[]): string {
  ...
  private classifyMotion(
    ...
    const classes = new Set<MotionClassification>();

    if (motionDensity < 20) classes.add(MotionClassification.Static);
    if (motionDensity >= 20 && motionDensity < 40) classes.add(MotionClassification.SlowMotion);
    if (motionDensity >= 35 && motionDensity < 70) classes.add(MotionClassification.NormalMotion);
    if (motionDensity >= 65) classes.add(MotionClassification.FastMotion);
    if (motionDensity >= 75) classes.add(MotionClassification.Action);
    ...
    if (classes.size === 0) classes.add(MotionClassification.Other);
    return [...classes];
  }

  private getDominantClassification(classifications: MotionClassification[]): MotionClassification {
    const priority = [
  ...
  private buildMotionPlan(
    ...
    const segments: MotionTimelineSegment[] = sceneDetection.scenes.map((scene, i) => ({
  ...
  private buildRecommendations(
    ...
    const recs: MotionRecommendation[] = [];
...
function analysisStyleFallback(camera: CameraMovementRecord | null | undefined): string {
