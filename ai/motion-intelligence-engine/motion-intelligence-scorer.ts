...
export class MotionIntelligenceScorer {
  computeScores(
    metrics: { presence: boolean; stability: number; continuity: number; density: number },
    objectMotions: ObjectMotionAnalysis[],
    subjectTracks: SubjectTrack[],
    eventCount: number,
    productionBase: number,
    cinematicBase: number
  ): MotionQualityScores {
    let motionQualityScore = 55;
    if (metrics.presence) motionQualityScore += 5;
    if (objectMotions.length >= 2) motionQualityScore += 10;
    if (subjectTracks.length >= 1) motionQualityScore += 10;
    if (eventCount >= 3) motionQualityScore += 10;
    if (metrics.continuity >= 70) motionQualityScore += 5;
    ...
    const trackingAccuracyScore =
      subjectTracks.length > 0
        ? Math.round(
            subjectTracks.reduce((s, t) => s + t.trackingAccuracy, 0) / subjectTracks.length
          )
        : 60;
    ...
    const productionReadinessScore = Math.round(
      (productionBase + motionQualityScore + trackingAccuracyScore) / 3
    );
  ...
  isAnalysisValid(
    ...
    objectMotionCount: number,
    trackCount: number,
    eventCount: number
  ): { valid: boolean; diagnostics: string[] } {
    const diagnostics: string[] = [];
    if (objectMotionCount < 1) diagnostics.push("At least 1 object motion analysis required");
    if (trackCount < 1) diagnostics.push("At least 1 subject track required");
    if (eventCount < 2) diagnostics.push("At least 2 motion events required");
