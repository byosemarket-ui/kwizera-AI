...
export class MotionIntelligenceProcessor {
  constructor(
    ...
    private readonly analyzer: MotionIntelligenceAnalyzer,
    private readonly scorer: MotionIntelligenceScorer,
    private readonly linker: MotionIntelligenceLinker,
    private readonly records: MotionIntelligenceRecordStore,
    private readonly logger: MotionIntelligenceLogger
  ) {}

  async analyze(input: MotionIntelligenceInput): Promise<MotionIntelligenceResult> {
    ...
    const validation = this.scorer.isAnalysisValid(
      scores,
      built.objectMotions.length,
      built.subjectTracks.length,
      built.motionEvents.length
    );
    if (!validation.valid) {
    ...
    const draft: MotionIntelligenceRecord = {
      videoId: input.videoId,
      intelligenceId,
  ...
  search(query: MotionIntelligenceSearchQuery): MotionIntelligenceRecord[] {
    ...
    if (query.classification) {
      results = results.filter(
        (r) =>
          r.dominantClassification === query.classification ||
    ...
    if (query.subjectType) {
      results = results.filter((r) =>
        r.subjectTracks.some((t) => t.subjectType === query.subjectType)
      );
    }
    if (query.timelineId) {
    ...
    if (query.text) {
      ...
      results = results.filter(
        (r) =>
          r.motionPlan.motionPath.toLowerCase().includes(q) ||
  ...
  private reject(start: number, videoId: string, message: string): MotionIntelligenceResult {
