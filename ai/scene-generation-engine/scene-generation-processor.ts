...
export class SceneGenerationProcessor {
  constructor(
    ...
    private readonly analyzer: SceneGenerationAnalyzer,
    private readonly scorer: SceneGenerationScorer,
    private readonly linker: SceneGenerationLinker,
    private readonly records: SceneGenerationRecordStore,
    private readonly logger: SceneGenerationLogger
  ) {}
  ...
  async generateScenes(input: SceneGenerationInput): Promise<SceneGenerationResult> {
    ...
    try {
      ...
      const generated: SceneGenerationRecord[] = [];
      ...
      for (const sourceScene of sourceScenes) {
        ...
        const draft: SceneGenerationRecord = {
          ...draftBase,
          scores,
          relationships: this.linker.detectRelationships(
            { ...draftBase, scores } as SceneGenerationRecord,
            storyboard,
            input
          ),
  ...
  search(query: SceneGenerationSearchQuery): SceneGenerationRecord[] {
    ...
    if (query.sceneId) results = results.filter((r) => r.sceneId === query.sceneId);
    ...
    if (query.sceneType) results = results.filter((r) => r.structure.sceneType === query.sceneType);
    if (query.keywords) {
      const kw = query.keywords.toLowerCase();
      results = results.filter(
        (r) =>
          r.structure.scenePurpose.toLowerCase().includes(kw) ||
    ...
    if (query.text) {
      ...
      results = results.filter(
        (r) =>
          r.sceneId.toLowerCase().includes(textLower) ||
  ...
  private resolveStoryboard(input: SceneGenerationInput): StoryboardGenerationRecord | null {
    const storyEngine = this.foundation.getStoryGenerationEngine();
    return storyEngine.getStoryboard(input.storyboardId) ?? null;
  }

  private registerGenerationAsset(record: SceneGenerationRecord, storyboard: StoryboardGenerationRecord): void {
  ...
  private reject(start: number, message: string, diagnostics: string[]): SceneGenerationResult {
