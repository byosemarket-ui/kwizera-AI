...
import {
  ...
  BRAND_DESIGN_PLATFORM_CONFIG,
  BrandConsistencyPlan,
  BrandDesignConsistencyElement,
  BrandDesignGenPlatform,
  BrandDesignLogoVariant,
  BrandDesignMaterialType,
...
const INDUSTRY_DESIGN_MAP: Record<string, BrandDesignType> = {
...
export class BrandingDesignAnalyzer {
  buildProfile(
    ...
    const designType =
      input.designType ?? INDUSTRY_DESIGN_MAP[context.industry ?? "default"] ?? BrandDesignType.BrandingPlan;
  ...
  buildSocialMediaDesign(input: BrandingDesignInput, profile: BrandDesignProfile): SocialMediaDesignPlan {
    ...
    const formatSpecs: Record<string, { aspectRatio: string; resolution: string }> = {};
  ...
  buildPrintDesign(input: BrandingDesignInput, profile: BrandDesignProfile): PrintDesignPlan {
    ...
    const formatSpecs: Record<string, { dimensions: string; bleed: string }> = {};
    for (const format of formats) {
  ...
  buildPlatformOptimizations(
    profile: BrandDesignProfile,
    input: BrandingDesignInput
  ): Array<{ platform: BrandDesignGenPlatform; aspectRatio: string; resolution: string; notes: string[] }> {
  ...
  ): BrandingDesignRecord["productionInstructions"] {
    return {
