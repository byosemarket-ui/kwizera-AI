...
import { QualityIssueSeverity, QualityValidationScores } from "./types.js";
...
export class VideoQualityValidationScorer {
  computeScores(
    ...
    const aiConfidenceScore = Math.round(
      (overallQualityScore + renderReadinessScore + brandConsistencyScore) / 3
    );
  ...
  isValidationValid(scores: QualityValidationScores, draft: QualityValidationRecordDraft): {
    ...
    const criticalCount = draft.issues.filter(
      (i) => i.severity === QualityIssueSeverity.Critical && !i.repaired
    ).length;
  ...
  isApproved(scores: QualityValidationScores, draft: QualityValidationRecordDraft): boolean {
    const hasCritical = draft.issues.some(
      (i) => i.severity === QualityIssueSeverity.Critical && !i.repaired
    );
    return (
      !hasCritical &&
  ...
  private computeVisualQuality(draft: QualityValidationRecordDraft, storyboard: StoryboardGenerationRecord): number {
    let score = 45;
    if (draft.videoQuality.allVisualChecksPassed) score += 35;
    if (storyboard.productionReady) score += 10;
    if (draft.videoQuality.transitionConsistency.includes("consistent")) score += 10;
    return Math.min(100, score);
  }

  private computeAudioQuality(draft: QualityValidationRecordDraft): number {
    let score = 45;
    if (draft.audioQuality.allAudioChecksPassed) score += 35;
    if (draft.audioQuality.lipSync.length > 3) score += 10;
    if (draft.audioQuality.loudness.length > 3) score += 10;
    return Math.min(100, score);
  }

  private computeMotionScore(upstream: QualityValidationUpstreamAssets): number {
  ...
  private computeAnimationScore(upstream: QualityValidationUpstreamAssets): number {
  ...
  private computeCameraScore(upstream: QualityValidationUpstreamAssets): number {
  ...
  private computeBrandConsistency(
    ...
    let score = 45;
    if (draft.brandQuality.allBrandChecksPassed) score += 35;
    if (storyboard.brandConsistent) score += 10;
    if (upstream.marketingPlan.brandConsistent) score += 10;
    return Math.min(100, score);
  }

  private computePlatformCompatibility(draft: QualityValidationRecordDraft): number {
    const ready = draft.platformValidations.filter(
      (p) => p.resolutionReady && p.aspectRatioReady && p.durationReady
    ).length;
  ...
  private computeRenderReadiness(
    ...
    let score = 45;
    if (draft.productionReadiness.renderPlansReady) score += 25;
    if (upstream.renderPlan.renderReady) score += 15;
    if (draft.technicalQuality.allTechnicalChecksPassed) score += 15;
    return Math.min(100, score);
  }
}

function draftBrandCheck(upstream: QualityValidationUpstreamAssets): boolean {
