...
import {
  ...
  mapStorageTypeToFoundationCategory,
} from "./storage-type-config.js";
...
import {
  KnowledgeIntegrityCheckResult,
  KnowledgeIntegrityStatus,
  KnowledgeRecord,
  KnowledgeRecordInput,
  KnowledgeRecordStatus,
  KnowledgeRecordUpdate,
  KnowledgeStorageEngineError,
  KnowledgeStorageReadResult,
  KnowledgeStorageStatusReport,
  KnowledgeStorageType,
  KnowledgeStorageValidationCode,
  KnowledgeStorageWriteResult,
  KnowledgeStorageVersionEntry,
} from "./types.js";
...
export class AiKnowledgeStorageEngine {
  ...
  readonly logger = new KnowledgeStorageLogger();
  private readonly validator = new KnowledgeRecordValidator();
  private readonly store = new KnowledgeRecordStore(this.logger);
  private readonly index = new KnowledgeRecordIndex(this.logger);
  private readonly duplicateDetector = new KnowledgeDuplicateDetector(this.logger, this.validator);
  private readonly versionManager = new KnowledgeVersionManager(this.logger);
  private readonly classifier = new KnowledgeClassifier();
  private readonly validationHistory = new KnowledgeValidationHistoryStore(this.logger);
  private integrityChecker: KnowledgeIntegrityChecker | null = null;

  private writeTimes: number[] = [];
  private readTimes: number[] = [];
  private lastIntegrity: KnowledgeIntegrityCheckResult | null = null;
  private onRecordChanged: ((knowledgeId: string, operation: "create" | "update") => void) | null = null;
  ...
  initialize(foundation: AiKnowledgeFoundation, storageRoot: string, knowledgeRoot: string): void {
    ...
    this.integrityChecker = new KnowledgeIntegrityChecker(
      this.logger,
      this.validator,
      this.store,
      this.index,
  ...
  async storeRecord(
  ...
  ): Promise<KnowledgeStorageWriteResult> {
    this.ensureReady();
    ...
    const verificationStatus = this.validator.resolveVerificationStatus(
      qualityScore,
      confidenceScore,
  ...
  async updateRecord(
    ...
    const access = await this.requestFoundationAccess(
      ...
      KnowledgeAccessOperation.Update,
      requesterId
    );
    if (!access.granted) {
      return {
    ...
    const { data: existing } = this.store.readRecord<KnowledgeRecord>(indexEntry.storageLocation);
    if (!existing) {
      return {
        success: false,
        validation: {
    ...
    const verificationStatus = this.validator.resolveVerificationStatus(
      qualityScore,
      confidenceScore,
      update.verificationStatus ?? existing.verificationStatus
    );
  ...
  async rollbackToVersion(
    ...
    const { data: current } = this.store.readRecord<KnowledgeRecord>(indexEntry.storageLocation);
    if (current) {
      this.versionManager.archiveBeforeUpdate(current, current.storageLocation);
    }

    const restored: KnowledgeRecord = {
      ...historical,
  ...
  findIndexEntry(knowledgeId: string) {
  ...
  validateRecordIntegrity(record: KnowledgeRecord) {
    return this.validator.validateRecordIntegrity(record);
  }

  async quarantineUnreadableRecord(
    knowledgeId: string,
    requesterId = "knowledge-storage-engine"
  ): Promise<boolean> {
    this.ensureReady();
  ...
  setRecordChangeHandler(
    handler: ((knowledgeId: string, operation: "create" | "update") => void) | null
  ): void {
  ...
  private recordValidationHistory(
    ...
    this.validationHistory.append({
      ...
      verificationStatus:
        record?.verificationStatus ?? input?.verificationStatus ?? KnowledgeVerificationStatus.Pending,
  ...
  private generateKnowledgeId(knowledgeType: KnowledgeStorageType): string {
