...
import {
  MemoryRecoveryEngineError,
  MemoryRecoveryRequest,
  MemoryRecoveryResult,
  MemoryRecoverySource,
  MemoryRecoveryStatusReport,
  MemoryRecoveryType,
  PostRecoveryIntegrity,
...
export class AiMemoryRecoveryEngine {
  ...
  readonly logger = new MemoryRecoveryLogger();
  readonly history = new RecoveryHistoryStore();

  private preValidator: PreRecoveryValidator | null = null;
  private safetySnapshot: SafetySnapshotManager | null = null;
  private stateComparator: StateComparator | null = null;
  private postIntegrity: PostRecoveryIntegrityChecker | null = null;
  private autoRecovery: AutoRecoveryMonitor | null = null;
  ...
  async recover(request: MemoryRecoveryRequest): Promise<MemoryRecoveryResult> {
    this.ensureReady();
    const start = Date.now();
  ...
  async recoverProject(projectId: string, backupId?: string): Promise<MemoryRecoveryResult> {
    return this.recover({
  ...
  async recoverLearning(backupId?: string): Promise<MemoryRecoveryResult> {
    return this.recover({
  ...
  async recoverRelationships(backupId?: string): Promise<MemoryRecoveryResult> {
    return this.recover({
  ...
  async recoverConfiguration(backupId?: string): Promise<MemoryRecoveryResult> {
    return this.recover({
  ...
  async autoRecover(reason: string): Promise<MemoryRecoveryResult> {
  ...
  async validateBeforeRecovery(backupId: string): Promise<PreRecoveryValidation> {
    this.ensureReady();
    const start = Date.now();
  ...
  async verifyIntegrity(): Promise<PostRecoveryIntegrity> {
    this.ensureReady();
  ...
  private ensureReady(): void {
    if (!this.initialized || !this.foundation) {
      throw new MemoryRecoveryEngineError(
        "Memory Recovery Engine not initialized",
        "NOT_INITIALIZED"
      );
    }
  }
