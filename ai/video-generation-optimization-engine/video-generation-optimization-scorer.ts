...
export class VideoGenerationOptimizationScorer {
  computeScores(
    ...
    const aiConfidenceScore = Math.round(
      (optimizationScore + performanceScore + resourceEfficiencyScore + qualityImprovementScore + productionReadinessScore) / 5
    );
  ...
  private computeOptimizationScore(draft: OptimizationRecordDraft): number {
    let score = 45;
    if (draft.pipelineOptimization.allPipelineOptimized) score += 20;
    if (draft.componentOptimization.storyboardOptimized) score += 10;
    if (draft.componentOptimization.renderPreparationOptimized) score += 15;
    if (draft.searchOptimization.allSearchOptimized) score += 10;
    return Math.min(100, score);
  }

  private computePerformanceScore(draft: OptimizationRecordDraft): number {
    let score = 50;
    if (draft.performanceOptimization.allPerformanceOptimized) score += 30;
    if (draft.performanceOptimization.scalability.length > 5) score += 20;
    return Math.min(100, score);
  }

  private computeResourceEfficiency(draft: OptimizationRecordDraft): number {
    let score = 50;
    if (draft.resourceOptimization.allResourcesOptimized) score += 35;
    if (draft.resourceOptimization.parallelProcessing.includes("parallel")) score += 15;
    return Math.min(100, score);
  }
  ...
  private computeQualityImprovement(
    ...
    let score = 50;
    if (draft.qualityOptimization.qualityMaintainedOrImproved) score += 30;
    if (upstream.validationReport.approved) score += 20;
    return Math.min(100, score);
  }
  ...
  private computeProductionReadiness(
    ...
    let score = 45;
    if (upstream.validationReport.approved) score += 25;
    if (upstream.renderPlan.renderReady) score += 15;
    if (draft.recoveryOptimization.allRecoveryOptimized) score += 15;
    return Math.min(100, score);
  }
}
