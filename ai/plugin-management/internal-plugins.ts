...
import type { PluginFactory, PluginManifest } from "./types.js";

function statusPlugin(toolId: string, permission: string): PluginFactory { return () => ({ async initialize() {}, async execute(action, input, sandbox) { if (action !== "status") throw new Error("Unsupported plugin action"); return sandbox.executeTool(toolId, input, [permission]); }, async shutdown() {}, async healthCheck() { return { healthy: true, message: "Trusted internal adapter operational" }; } }); }

export function createInternalPlugins(_core: AiCoreManager): Array<{ manifest: PluginManifest; factory: PluginFactory }> {
  return [
