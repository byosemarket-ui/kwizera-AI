...
import {
  ...
  ALL_BACKGROUND_MARKETING_PRESETS,
  ...
  BackgroundPlanProfile,
  BackgroundPlatformOptimization,
  BackgroundReplacementPlan,
  BackgroundReplacementVariation,
  BackgroundReplacementVariationType,
  DepthPlanningPlan,
  LightingMatchingPlan,
  PLATFORM_CONFIG,
  ProductionBackgroundInstructions,
  QualityImprovementPlan,
  SubjectPreservationPlan,
  SubjectPreservationTarget,
} from "./types.js";
...
const PRESET_BACKGROUND_MAP: Record<BackgroundMarketingPreset, BackgroundGenType> = {
...
const INDUSTRY_PRESET_MAP: Record<string, BackgroundMarketingPreset> = {
...
export class BackgroundGenerationAnalyzer {
  ...
  buildReplacementPlan(profile: BackgroundPlanProfile, input: BackgroundGenerationInput): BackgroundReplacementPlan {
    ...
    const variations: BackgroundReplacementVariation[] = input.generateReplacements !== false
      ? [
          {
  ...
  extractContextFromProduct(
    ...
    input: BackgroundGenerationInput,
    productImagePlan?: ProductImageGenerationRecord | null
  ): BackgroundGenerationContext | null {
    if (!analysis && !understanding && !productImagePlan) return null;
  ...
  private describeEnvironment(type: BackgroundGenType): string {
    const map: Record<BackgroundGenType, string> = {
  ...
  private preservationNote(target: SubjectPreservationTarget, subject: string): string {
    const map: Record<SubjectPreservationTarget, string> = {
  ...
  private getPresetNotes(preset: BackgroundMarketingPreset, platform: BackgroundGenPlatform): string[] {
    return [`${preset} preset applied`, `${platform} format requirements enforced`];
  }
}

function profileColorTemp(industry?: string): string {
  if (industry === "beauty" || industry === "fashion") return "5200K warm daylight with subtle golden bias";
  if (industry === "technology") return "5500K neutral daylight";
