import {
  ...
  LanguageKnowledgeQualityScores,
...
export class LanguageScorer {
  computeScores(
    ...
    const readabilityScore = Math.round(
      (content.length >= 20 && content.length <= 500 ? 30 : 15) +
    ...
    const marketingScore = Math.round(
      (marketing.headlines.length >= 1 ? 20 : 5) +
    ...
    const translationReadinessScore = Math.round(
      localization.translationReadiness * 0.5 +
        localization.localizationReadiness * 0.3 +
        (localization.relatedLanguages.length >= 1 ? 15 : 0) +
        (grammar.issues.length === 0 ? 10 : 0)
    );

    const subtitleQualityScore = Math.round(
      subtitles.syncQuality * 0.4 +
