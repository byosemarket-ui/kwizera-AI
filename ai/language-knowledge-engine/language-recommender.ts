import {
  ...
  LanguageKnowledgeRelationships,
  LanguageScriptType,
...
export class LanguageRecommender {
  recommend(record: LanguageAnalysisRecord): LanguageKnowledgeRecommendation[] {
    const recs: LanguageKnowledgeRecommendation[] = [];
...
export class LanguageRelationshipLinker {
  detectSimilar(
    ...
    allRecords: LanguageAnalysisRecord[]
  ): LanguageKnowledgeRelationships {
    const relationships: LanguageKnowledgeRelationships = {
      relatedLanguages: [],
    ...
    for (const other of allRecords) {
      ...
      if (
        sharedTags.some((t) => t.includes("subtitle")) ||
        record.scriptType === LanguageScriptType.Subtitle
      ) {
  ...
  private similarityScore(a: LanguageAnalysisRecord, b: LanguageAnalysisRecord): number {
    let score = 0;
    if (a.language === b.language) score += 35;
    if (a.writingStyle === b.writingStyle) score += 20;
    if (a.brandName === b.brandName && a.brandName) score += 20;
    if (a.productName === b.productName && a.productName) score += 15;
    if (a.scriptType === b.scriptType) score += 10;
