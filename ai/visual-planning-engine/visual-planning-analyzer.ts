...
import type { SceneScriptPlan, ScriptPlanningRecord } from "../script-planning-engine/types.js";
import {
  BackgroundPlanning,
  ...
  SceneVisualPlan,
  VisualPlanningInput,
  VisualPlanningProfile,
  VisualStylePlanning,
} from "./types.js";

const PURPOSE_CAMERA: Record<string, { angle: string; distance: string; movement: string }> = {
...
export class VisualPlanningAnalyzer {
  ...
  private buildSceneVisualPlan(
    scene: ScenePlan,
    scriptScene: SceneScriptPlan | undefined,
    creative: CreativeDirectionRecord,
    understanding: ProductUnderstandingRecord
  ): SceneVisualPlan {
    ...
    const palette = creative.visualDirection.colorPalette.join(" / ");
    const logoPlacement =
      scene.ctaPlacement !== "none" || scene.scenePurpose === "ending"
