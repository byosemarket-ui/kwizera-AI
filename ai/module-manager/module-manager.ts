...
export class AiModuleManager {
  ...
  private readonly permanentRegistry = new Map<string, ModuleRegistryRecord>();
  private readonly plugins = new Map<string, AiModulePlugin>();
  private readonly startupTimes = new Map<string, number>();

  readonly logger = new ModuleManagerLogger();
  readonly history = new ModuleHistoryStore();
  private readonly dependencyValidator = new ModuleDependencyValidator();
  private readonly compatibilityChecker = new ModuleCompatibilityChecker();
  private readonly healthMonitor: ModuleHealthMonitor;
  private readonly router: ModuleCommunicationRouter;
  private readonly recovery: ModuleRecoveryManager;
  ...
  constructor() {
    ...
    this.router = new ModuleCommunicationRouter(
      (id) => this.resolveRecord(id),
      this.logger
    );
  ...
  async registerModule(options: RegisterModuleOptions): Promise<void> {
    ...
    const depResult = this.dependencyValidator.validate(
      plugin,
      this.core!,
      dependencies,
      this.storageRoot
    ...
    if (!depResult.compatible) {
      throw new ModuleManagerError(
        depResult.rejectionReason ?? "Dependency validation failed",
        "DEPENDENCY_REJECTED"
      );
    }
    ...
    if (record) {
      ...
      const newRecord: ModuleRegistryRecord = {
        moduleId,
  ...
  async initializeModule(moduleIdOrSlot: string): Promise<void> {
    this.ensureReady();
    ...
    try {
      ...
      const stats: ModulePerformanceStats = {
  ...
  async loadModule(moduleIdOrSlot: string): Promise<void> {
    const record = this.resolveRecord(moduleIdOrSlot);
    if (!record) {
  ...
  async unloadModule(moduleIdOrSlot: string): Promise<void> {
    const record = this.resolveRecord(moduleIdOrSlot);
    const plugin = this.resolvePlugin(moduleIdOrSlot);
    if (!record) {
      return;
  ...
  async restartModule(moduleIdOrSlot: string): Promise<boolean> {
    const record = this.resolveRecord(moduleIdOrSlot);
    const plugin = this.resolvePlugin(moduleIdOrSlot);
    if (!record || !plugin) {
      throw new ModuleManagerError(`Module not found: ${moduleIdOrSlot}`, "NOT_FOUND");
  ...
  async registerAndInitialize(plugin: AiModulePlugin, options?: Omit<RegisterModuleOptions, "plugin">): Promise<void> {
  ...
  async routeCommunication(
    ...
    if (this._communicationBus) {
      const result = await this._communicationBus.routeLegacyRequest(
        request.senderId,
        request.receiverId,
        request.action,
        request.payload,
  ...
  async monitorHealth(moduleIdOrSlot?: string): Promise<void> {
  ...
  getRegisteredPluginCount(): number {
    return Array.from(this.permanentRegistry.values()).filter(
      (r) => r.status === ManagedModuleState.Running && r.moduleId !== "ai-core"
    ).length;
  }
  ...
  buildStatusReport(): ModuleManagerStatusReport {
    ...
    const avgComm =
      commRecords.length > 0
        ? Math.round(
            commRecords.reduce((a, r) => a + r.executionTimeMs, 0) / commRecords.length
          )
        : 0;
    ...
    if (failed.length) readinessScore -= failed.length * 15;
    if (isolated.length) readinessScore -= isolated.length * 10;
    ...
    return {
      ...
      recoveryStatus:
        this.recovery.getDiagnostics().length > 0
  ...
  private resolvePlugin(id: string): AiModulePlugin | undefined {
