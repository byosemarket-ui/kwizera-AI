...
export class ProductionVideoProcessor {
  constructor(
    ...
    private readonly analyzer: ProductionVideoAnalyzer,
    private readonly scorer: ProductionVideoScorer,
    private readonly linker: ProductionVideoLinker,
    private readonly records: ProductionVideoPlanningRecordStore,
    private readonly logger: ProductionVideoLogger
  ) {}

  async planProduction(input: ProductionVideoPlanningInput): Promise<ProductionVideoPlanningResult> {
    ...
    const scores = this.scorer.computeScores(
      ...
      creativePlan.scores.creativeScore
    );
    ...
    const draft: ProductionVideoPlanningRecord = {
      videoId: input.videoId,
      intelligenceId,
  ...
  search(query: ProductionVideoPlanningSearchQuery): ProductionVideoPlanningRecord[] {
    ...
    if (query.text) {
      ...
      results = results.filter(
        (r) =>
          r.profile.productionPlanId.toLowerCase().includes(q) ||
          r.keywords.some((k) => k.toLowerCase().includes(q))
      );
    }
