...
import {
  ProductionVideoExportFormat,
  ProductionVideoPlanningEngineError,
  ProductionVideoPlanningEngineStatusReport,
  ProductionVideoPlanningInput,
  ProductionVideoPlanningRecord,
  ProductionVideoPlanningResult,
  ProductionVideoPlanningSearchQuery,
...
export class AiProductionVideoPlanningEngine {
  ...
  readonly logger = new ProductionVideoLogger();
  readonly records = new ProductionVideoPlanningRecordStore();

  private readonly analyzer = new ProductionVideoAnalyzer();
  private readonly scorer = new ProductionVideoScorer();
  private readonly linker = new ProductionVideoLinker();
  private processor: ProductionVideoProcessor | null = null;
  ...
  initialize(foundation: AiVideoIntelligenceFoundation, storageRoot: string): void {
    ...
    this.processor = new ProductionVideoProcessor(
      foundation,
      this.analyzer,
      this.scorer,
      this.linker,
  ...
  async planProductionVideo(input: ProductionVideoPlanningInput): Promise<ProductionVideoPlanningResult> {
    this.ensureReady();
  ...
  async repairProductionPlan(videoId: string): Promise<ProductionVideoPlanningResult | null> {
    this.ensureReady();
  ...
  buildStatusReport(): ProductionVideoPlanningEngineStatusReport {
    ...
    if (!this.foundation?.getCreativeVideoIntelligenceEngine().isStartupComplete()) readinessScore -= 10;
    if (!this.foundation?.getVideoEnhancementPlanningEngine().isStartupComplete()) readinessScore -= 10;
    if (!integration?.knowledgeEngine) readinessScore -= 10;

    return {
      engineStatus: this.startupComplete ? "operational" : "initializing",
      workflowPlanningStatus:
        "analysis, understanding, scene, timeline, camera, motion, style, enhancement, creative, render, export, delivery",
      assetValidationStatus:
        "source videos, images, audio, voice, music, SFX, logos, fonts, templates, motion graphics, effects, LUTs, captions, subtitles, brand assets",
  ...
  private ensureReady(): void {
    if (!this.initialized || !this.foundation || !this.processor) {
      throw new ProductionVideoPlanningEngineError(
        "Production Video Planning Engine not initialized",
        "NOT_INITIALIZED"
      );
    }
  }
