...
export class ProductionVideoScorer {
  computeScores(
    ...
    enhancementReadiness: number,
    creativeScore: number
  ): ProductionVideoPlanningScores {
    const dependencyScore = Math.round(
      (dependencies.passedCount / Math.max(dependencies.totalRequired, 1)) * 100
    );
    ...
    const workflowScore = Math.round(
      (dependencyScore + assetReadinessScore + enhancementReadiness) / 3
    );

    const productionReadinessScore = Math.round(
      (dependencyScore + assetReadinessScore + workflowScore + creativeScore) / 4
    );
  ...
  isPlanValid(
    ...
    if (!dependencies.allRequiredPassed) {
      ...
      diagnostics.push(
        `Required dependencies not validated: ${failed.map((f) => f.moduleName).join(", ")}`
      );
    }
