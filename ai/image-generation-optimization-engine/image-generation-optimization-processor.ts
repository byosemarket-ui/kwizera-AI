...
export class ImageGenerationOptimizationProcessor {
  constructor(
    ...
    private readonly analyzer: ImageGenerationOptimizationAnalyzer,
    private readonly scorer: ImageGenerationOptimizationScorer,
    private readonly linker: ImageGenerationOptimizationLinker,
    private readonly records: ImageGenerationOptimizationRecordStore,
    private readonly logger: ImageGenerationOptimizationLogger
  ) {}
  ...
  async optimizeImageGeneration(input: ImageGenerationOptimizationInput): Promise<ImageGenerationOptimizationResult> {
    ...
    try {
      ...
      if (!context || !context.validation?.approved) {
        return this.reject(
          start,
          "Unable to resolve optimization context — provide validationId or productId with approved quality validation and full pipeline",
          ["Approved quality validation and full upstream pipeline required before optimization"]
        );
      }
      ...
      const draftPartial: ImageGenerationOptimizationRecord = {
      ...
      const draft: ImageGenerationOptimizationRecord = {
        ...draftPartial,
  ...
  search(query: ImageGenerationOptimizationSearchQuery): ImageGenerationOptimizationRecord[] {
    ...
    if (query.keywords) {
      ...
      results = results.filter(
        (r) =>
          r.optimizationId.toLowerCase().includes(kw) ||
  ...
  private async resolveContext(input: ImageGenerationOptimizationInput) {
  ...
  private registerOptimizationAssets(record: ImageGenerationOptimizationRecord): void {
  ...
  private reject(start: number, message: string, diagnostics: string[]): ImageGenerationOptimizationResult {
