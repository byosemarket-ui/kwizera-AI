...
const CATEGORY_INDUSTRY: Record<ProductAnalysisCategory, ProductAnalysisIndustry> = {
...
const CATEGORY_DEFAULTS: Record<
  ProductAnalysisCategory,
  { subcategory: string; materials: string[]; packaging: string }
> = {
...
export class ProductAnalysisAnalyzer {
  analyze(input: ProductAnalysisEngineInput): {
    ...
    const profile: ProductAnalysisProfile = {
      productName,
      category,
    ...
    const visual: ProductVisualPreparation = {
    ...
    const marketingPreparation = this.buildMarketingPreparation(
      profile,
      visual,
      category,
  ...
  private inferUseCase(category: ProductAnalysisCategory, profile: ProductAnalysisProfile): string {
  ...
  private inferTargetCustomer(category: ProductAnalysisCategory): string {
    const map: Partial<Record<ProductAnalysisCategory, string>> = {
  ...
  private buildMarketingPreparation(
    ...
    targetCustomerInput?: string
  ): ProductMarketingPreparation {
    const gaps: string[] = [];
    const prepared: string[] = [];

    if (profile.description.length >= 20) prepared.push("description");
    else gaps.push("description");

    if (profile.features.length >= 2) prepared.push("features");
    else gaps.push("features");

    if (profile.price > 0) prepared.push("pricing");
    else gaps.push("pricing");

    if (visual.productVisibility >= 70) prepared.push("visual-visibility");
    else gaps.push("visual-visibility");
