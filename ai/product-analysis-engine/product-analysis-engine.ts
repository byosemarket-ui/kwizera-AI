...
import {
  ...
  ProductAnalysisEngineStatusReport,
  ProductAnalysisIntelligenceRecord,
  ProductAnalysisSearchQuery,
  ProductAnalysisEngineError,
} from "./types.js";
...
export class AiProductAnalysisEngine {
  ...
  readonly logger = new ProductAnalysisLogger();
  readonly records = new ProductAnalysisRecordStore();

  private readonly analyzer = new ProductAnalysisAnalyzer();
  private readonly completeness = new ProductAnalysisCompletenessDetector();
  private readonly scorer = new ProductAnalysisScorer();
  private readonly linker = new ProductAnalysisLinker();
  private processor: ProductAnalysisProcessor | null = null;
  ...
  initialize(foundation: AiProductIntelligenceFoundation, storageRoot: string): void {
    ...
    this.processor = new ProductAnalysisProcessor(
      foundation,
      this.analyzer,
      this.completeness,
      this.scorer,
  ...
  async analyzeProduct(input: ProductAnalysisEngineInput): Promise<ProductAnalysisEngineResult> {
    this.ensureReady();
  ...
  async repairProduct(productId: string): Promise<ProductAnalysisEngineResult | null> {
    this.ensureReady();
    const existing = this.records.get(productId);
    if (!existing) return null;

    const repairedInput: ProductAnalysisEngineInput = {
      productId,
