...
import {
  ...
  MarketingKnowledgeEngineError,
  MarketingKnowledgeStatusReport,
...
export class AiMarketingKnowledgeEngine {
  ...
  readonly logger = new MarketingKnowledgeLogger();
  readonly patterns = new MarketingPatternStore();
  readonly records = new MarketingRecordStore();

  private readonly analyzer = new MarketingAnalyzer();
  private readonly scorer = new MarketingScorer();
  private readonly recommender = new MarketingRecommender();
  private readonly linker = new MarketingRelationshipLinker();
  ...
  async runStartup(): Promise<void> {
    ...
    for (const entry of entries) {
      ...
      if (read.success && read.record?.payload) {
        const payload = read.record.payload as unknown as MarketingAnalysisRecord;
        if (payload.campaignId) this.records.upsert(payload);
      }
    }
  ...
  async analyzeCampaign(input: MarketingAnalysisInput): Promise<MarketingAnalysisResult> {
    this.ensureReady();
  ...
  async searchCampaigns(query: MarketingSearchQuery): Promise<MarketingAnalysisRecord[]> {
    this.ensureReady();
    const start = Date.now();
  ...
  detectRelationships(campaignId: string) {
    this.ensureReady();
    const record = this.records.get(campaignId);
  ...
  private ensureReady(): void {
    if (!this.initialized || !this.foundation) {
      throw new MarketingKnowledgeEngineError(
        "Marketing Knowledge Engine not initialized",
        "NOT_INITIALIZED"
      );
    }
  }
