...
export class MarketingProcessor {
  constructor(
    ...
    private readonly analyzer: MarketingAnalyzer,
    private readonly scorer: MarketingScorer,
    private readonly recommender: MarketingRecommender,
    private readonly linker: MarketingRelationshipLinker,
    private readonly learner: MarketingLearner,
    private readonly records: MarketingRecordStore,
    private readonly logger: MarketingKnowledgeLogger
  ) {}

  async analyze(input: MarketingAnalysisInput): Promise<MarketingAnalysisResult> {
    ...
    const draft: MarketingAnalysisRecord = {
      campaignId,
      knowledgeId,
  ...
  async search(query: MarketingSearchQuery): Promise<MarketingAnalysisRecord[]> {
    ...
    let results = search.results
      .map((r) =>
        r.record?.payload ? (r.record.payload as unknown as MarketingAnalysisRecord) : undefined
      )
      .filter((r): r is MarketingAnalysisRecord => Boolean(r));
  ...
  private filterLocal(
    records: MarketingAnalysisRecord[],
    query: MarketingSearchQuery
  ): MarketingAnalysisRecord[] {
    return records.filter((r) => {
      if (query.campaignType && r.campaignType !== query.campaignType) return false;
      ...
      if (query.storytelling && r.storytelling.narrativeArc !== query.storytelling) return false;
      if (query.cta && !r.structure.callToAction.toLowerCase().includes(query.cta.toLowerCase())) {
        return false;
      }
      if (
        query.minConversionReadiness &&
      ...
      if (query.keywords?.length) {
        ...
        if (!query.keywords.some((k) => haystack.includes(k.toLowerCase()))) return false;
      }
      return true;
    });
  }
  ...
  private buildKnowledgeDescription(record: MarketingAnalysisRecord): string {
    return [
