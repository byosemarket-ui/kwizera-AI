...
export class ImageScorer {
  computeScores(
    ...
    const compositionScore = Math.round(
      metrics.compositionQuality * 0.5 + design.visualBalance * 0.3 + metrics.sharpness * 0.2
    );

    const productVisibilityScore = Math.round(
      product.visibility * 0.4 + product.focus * 0.35 + (visual.products.length > 0 ? 15 : 0)
    );

    const brandQualityScore = Math.round(
  ...
  isAnalysisValid(scores: ImageQualityScores): { valid: boolean; diagnostics: string[] } {
