...
const CRITICAL_FIELDS: Partial<Record<DecisionType, string[]>> = {
...
export class QualityEvaluator {
  evaluate(
    ...
    const checks: QualityAssessment["checks"] = [];
  ...
  private hasRequiredFields(type: DecisionType, data: Record<string, unknown>): boolean {
  ...
  private requiresImages(type: DecisionType): boolean {
    return type === "image-analysis" || type === "video-generation";
  }

  private hasRequiredResources(request: DecisionRequest): boolean {
