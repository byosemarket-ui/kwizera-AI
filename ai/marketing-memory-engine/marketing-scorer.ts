import { MarketingQualityScores, MarketingRecord } from "./types.js";

export class MarketingScorer {
  computeScores(campaign: Partial<MarketingRecord>, effectivenessRating?: number): MarketingQualityScores {
    ...
    const engagementScore = Math.min(
      100,
      (content?.emotionalTriggers?.length ?? 0) * 12 + (content?.hashtags?.length ?? 0) * 3 + 30
    );
    const conversionScore = Math.min(
      100,
      hasCta ? 60 + (content?.callToActions?.length ?? 0) * 10 : 30
    );
    const learningScore = Math.min(
      100,
      (campaign.lessonsLearned?.length ?? 0) * 15 + (campaign.patterns?.length ?? 0) * 10
    );
    const aiConfidenceScore = Math.round(
      (qualityScore + effectivenessScore + engagementScore + conversionScore) / 4
    );
