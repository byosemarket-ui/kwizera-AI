import {
  AnimationKnowledge,
  CinematicKnowledge,
  CreativeKnowledgeQualityScores,
...
export class CreativeScorer {
  computeScores(
    ...
    const visualDesignScore = Math.round(
      visual.balance * 0.2 +
    ...
    const storytellingScore = Math.round(
      storytelling.attentionRetention * 0.35 +
    ...
    const animationScore = Math.round(
      animation.animationQuality * 0.4 +
    ...
    const creativeQualityScore = Math.round(
      (visualDesignScore + storytellingScore + animationScore + cinematic.visualContinuity) / 4
    );
  ...
  isAnalysisValid(scores: CreativeKnowledgeQualityScores): { valid: boolean; diagnostics: string[] } {
