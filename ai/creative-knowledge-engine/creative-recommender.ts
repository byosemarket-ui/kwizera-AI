...
export class CreativeRelationshipLinker {
  detectSimilar(
    ...
    allRecords: CreativeAnalysisRecord[]
  ): CreativeKnowledgeRelationships {
    const relationships: CreativeKnowledgeRelationships = {
      creativeStyles: [],
      relatedProducts: [],
      relatedBrands: [],
      relatedVideos: [],
  ...
  private similarityScore(a: CreativeAnalysisRecord, b: CreativeAnalysisRecord): number {
    let score = 0;
    if (a.brandName === b.brandName && a.brandName) score += 30;
    if (a.creativeStyle === b.creativeStyle) score += 25;
    if (a.domain === b.domain) score += 20;
