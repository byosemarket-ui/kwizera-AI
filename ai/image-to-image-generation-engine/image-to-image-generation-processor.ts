...
export class ImageToImageGenerationProcessor {
  constructor(
    ...
    private readonly analyzer: ImageToImageGenerationAnalyzer,
    private readonly scorer: ImageToImageGenerationScorer,
    private readonly linker: ImageToImageGenerationLinker,
    private readonly records: ImageToImageGenerationRecordStore,
    private readonly logger: ImageToImageGenerationLogger
  ) {}
  ...
  async generateTransformationPlan(input: ImageToImageGenerationInput): Promise<ImageToImageGenerationResult> {
    ...
    try {
      ...
      if (!context) {
        return this.reject(
          start,
          "Unable to resolve transformation context — provide sourceImageId, sourceImageMetadata, or textToImagePlanId",
          ["Source image or text-to-image plan required"]
        );
      }
      ...
      const draftPartial: ImageToImageGenerationRecord = {
      ...
      const draft: ImageToImageGenerationRecord = {
        ...draftPartial,
  ...
  search(query: ImageToImageSearchQuery): ImageToImageGenerationRecord[] {
    ...
    if (query.style) results = results.filter((r) => r.profile.targetStyle === query.style);
    if (query.keywords) {
      const kw = query.keywords.toLowerCase();
      results = results.filter(
        (r) =>
          r.sourceAnalysis.subject.toLowerCase().includes(kw) ||
    ...
    if (query.text) {
      ...
      results = results.filter(
        (r) =>
          r.transformationPlanId.toLowerCase().includes(textLower) ||
  ...
  private async resolveContext(input: ImageToImageGenerationInput) {
    ...
    if (input.sourceImageMetadata || input.sourceImageId || textToImagePlan) {
      ...
      if (imgFoundation) ctx.industry = "general";
      return ctx;
    }
  ...
  private registerGenerationAssets(record: ImageToImageGenerationRecord, source: SourceImageMetadata): void {
  ...
  private applySafeRepairs(
    sourceAnalysis: { subject: string },
    transformationPlan: { steps: unknown[]; transformationPrompt: string },
    maskPlan: { masks: unknown[] },
    preservationPlan: { rules: unknown[]; identityLock: boolean },
    diagnostics: string[]
  ...
  private reject(start: number, message: string, diagnostics: string[]): ImageToImageGenerationResult {
