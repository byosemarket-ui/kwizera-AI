...
export class EnhancementPlanningScorer {
  computeScores(
    ...
    hasRestorationNeed: boolean
  ): ImageEnhancementPlanningScores {
    ...
    const enhancementReadinessScore = Math.round(
      (imageQualityScore + restorationScore + platformReadiness) / 3
    );

    const creativeReadinessScore = Math.min(
      100,
      Math.round(imageQualityScore * 0.5 + enhancementReadinessScore * 0.3 + platformReadiness * 0.2)
    );
