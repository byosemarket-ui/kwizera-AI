...
import {
  VideoGenerationAutoRepairResult,
...
export class VideoGenerationAutoRepairHandler {
  ...
  async attemptRepairs(warnings: VideoGenerationHealthWarning[]): Promise<VideoGenerationAutoRepairResult> {
    const repairs: string[] = [];
    let success = true;

    const critical = warnings.some(
      (w) =>
        w.severity === VideoGenerationHealthScoreLevel.Critical ||
        w.severity === VideoGenerationHealthScoreLevel.Failed
    );

    const hasCriticalWarning = warnings.some(
      (w) =>
        w.type === VideoGenerationWarningType.DatabaseProblems ||
    ...
    if (critical || hasCriticalWarning || warnings.length > 3) {
      ...
      this.foundation.integration.reportCriticalIssue(
        `Critical video generation health: ${warnings.map((w) => w.message).join("; ")}`
      );
