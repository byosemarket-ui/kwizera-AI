...
export class VideoGenerationHealthCheckRunner {
  constructor(
    ...
    private readonly moduleChecker: VideoGenerationModuleHealthChecker,
    private readonly resourceMonitor: VideoGenerationResourceMonitor,
    private readonly earlyWarning: VideoGenerationEarlyWarningSystem,
    private readonly autoRepair: VideoGenerationAutoRepairHandler,
    private readonly history: VideoGenerationHealthHistoryStore,
    private readonly logger: VideoGenerationHealthMonitorLogger
  ) {}

  async runCheck(): Promise<VideoGenerationHealthCheckResult> {
    ...
    if (
      overallLevel === VideoGenerationHealthScoreLevel.Critical ||
    ...
    const optimizationStatus =
      this.foundation.getVideoGenerationOptimizationEngine().buildStatusReport().readinessScore >= 75;
    ...
    const validationModule = moduleScores.find(
      (m) => m.module === MonitoredVideoGenerationModule.VideoQualityValidation
    );
    ...
    const result: VideoGenerationHealthCheckResult = {
      checkId,
    ...
    const historyEntry: VideoGenerationHealthHistoryEntry = {
      checkId,
