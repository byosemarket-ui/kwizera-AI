...
export const VIDEO_PLATFORM_PROFILES: Record<VideoPlatformId, VideoPlatformProfile> = {
  ...
  instagram_portrait: { id: "instagram_portrait", label: "Instagram Portrait 4:5", aspectRatio: "4:5", width: 1080, height: 1350, maxDurationMs: 60_000 },
  ...
  facebook_reels: { id: "facebook_reels", label: "Facebook Reels", aspectRatio: "9:16", width: 1080, height: 1920, maxDurationMs: 90_000 },
};
...
export function resolvePlatformId(value?: string): VideoPlatformId {
  ...
  if (/facebook.*reel/.test(text)) return "facebook_reels";
  if (/facebook/.test(text)) return "facebook_feed";
  if (/instagram.*reel|reels/.test(text)) return "instagram_reels";
  if (/instagram.*portrait|4:5|portrait.?feed/.test(text)) return "instagram_portrait";
  ...
  if (/4:5/.test(text)) return "instagram_portrait";
