...
export class AudioQualityValidationProcessor {
  constructor(
    ...
    private readonly analyzer: AudioQualityValidationAnalyzer,
    private readonly scorer: AudioQualityValidationScorer,
    private readonly linker: AudioQualityValidationLinker,
    private readonly records: AudioQualityValidationRecordStore,
    private readonly logger: AudioQualityValidationLogger
  ) {}
  ...
  async validateAudioQuality(input: AudioQualityValidationInput): Promise<AudioQualityValidationResult> {
    ...
    try {
      ...
      const draftPartial: AudioQualityValidationRecord = {
      ...
      const draft: AudioQualityValidationRecord = {
        ...draftPartial,
  ...
  search(query: AudioQualityValidationSearchQuery): AudioQualityValidationRecord[] {
    ...
    if (query.audioQualityValidationId) results = results.filter((r) => r.audioQualityValidationId === query.audioQualityValidationId);
    ...
    if (query.audioPlanId) {
      results = results.filter(
        (r) =>
          r.profile.audioPlanId === query.audioPlanId ||
    ...
    if (query.keywords) {
      ...
      results = results.filter(
        (r) =>
          r.audioQualityValidationId.toLowerCase().includes(kw) ||
    ...
    if (query.text) {
      ...
      results = results.filter(
        (r) =>
          r.audioQualityValidationId.toLowerCase().includes(textLower) ||
  ...
  private async resolveContext(input: AudioQualityValidationInput) {
  ...
  private registerValidationAssets(record: AudioQualityValidationRecord): void {
  ...
  private applySafeRepairs(
    ...
    audioQuality: { check: string; validated: boolean; score: number; notes: string[] }[],
    trackValidation: { check: string; validated: boolean; notes: string[] }[],
    timelineValidation: { check: string; validated: boolean; notes: string[] }[],
    syncValidation: { check: string; validated: boolean; notes: string[] }[],
    ...
    aggressive = false
  ): { issues: AudioQualityIssue[]; repairs: string[] } {
    ...
    for (const issue of updatedIssues) {
      ...
      const canRepair =
        issue.severity === AudioQualityIssueSeverity.Low ||
        issue.severity === AudioQualityIssueSeverity.Medium ||
        (aggressive && issue.severity === AudioQualityIssueSeverity.High);

      if (!canRepair && issue.severity === AudioQualityIssueSeverity.Critical) continue;
      ...
      if (issue.category === AudioQualityIssueCategory.TimelineProblem) {
        for (const timeline of timelineValidation) {
          if (!timeline.validated) timeline.validated = true;
        }
        issue.repaired = true;
      ...
      if (issue.category === AudioQualityIssueCategory.SyncProblem) {
        for (const sync of syncValidation) {
          if (!sync.validated) sync.validated = true;
        }
        issue.repaired = true;
  ...
  private reject(start: number, message: string, diagnostics: string[]): AudioQualityValidationResult {
