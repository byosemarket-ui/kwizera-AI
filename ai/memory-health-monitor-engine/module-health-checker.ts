...
export class ModuleHealthChecker {
  ...
  checkAll(): MonitoredModuleHealthScore[] {
    const scores: MonitoredModuleHealthScore[] = [];
  ...
  scoreToLevel(score: number): MemoryHealthScoreLevel {
    if (score >= 95) return MemoryHealthScoreLevel.Excellent;
    if (score >= 80) return MemoryHealthScoreLevel.Good;
    if (score >= 60) return MemoryHealthScoreLevel.Warning;
    if (score >= 40) return MemoryHealthScoreLevel.Critical;
