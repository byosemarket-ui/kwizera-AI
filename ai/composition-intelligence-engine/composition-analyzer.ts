...
import {
  CompositionAnalysis,
  CompositionImprovementPlan,
  CompositionIntelligenceRecommendation,
...
export class CompositionAnalyzer {
  buildFromIntelligence(
    ...
    const compositionAnalysis: CompositionAnalysis = {
      compositionType,
  ...
  private inferCompositionType(
    ...
    if (analysis.classification.creativeStyle === "editorial") return CompositionType.Asymmetry;
    if (
      productObj &&
    ...
    if (detection.objects.length <= 2) return CompositionType.Minimal;
    if (understanding.visual.visualHierarchy === "brand-first") return CompositionType.Symmetry;
    if (productObj) return CompositionType.RuleOfThirds;
    if (detection.objects.length > 4) return CompositionType.Layered;
    return CompositionType.Dynamic;
  }

  private computeSymmetry(isCenter: boolean, position?: ObjectPosition): number {
    if (isCenter || position === ObjectPosition.Center) return 88;
    if (position === ObjectPosition.TopLeft || position === ObjectPosition.TopRight) return 62;
    return 72;
  }
  ...
  private computeNegativeSpace(
    ...
    let space = 50;
    if (detection.objects.length <= 3) space += 20;
    if (background?.analysis.backgroundComplexity === "minimal") space += 15;
    if (background?.quality.backgroundDistraction) space -= Math.min(20, background.quality.backgroundDistraction * 0.3);
    return Math.max(15, Math.min(95, Math.round(space)));
  }

  private computeBalance(symmetry: number, negativeSpace: number, productVisibility: number): number {
    return Math.round((symmetry + negativeSpace + productVisibility) / 3);
  }

  private inferLeadingLines(understanding: ImageUnderstandingRecord, type: CompositionType): string {
    if (type === CompositionType.RuleOfThirds) return "diagonal-product-focus";
    if (understanding.scene.environment.includes("urban")) return "street-perspective-lines";
  ...
  private inferFraming(
    ...
    if (analysis.technical.orientation === "portrait") return "vertical-tight-frame";
    if (analysis.classification.imageType === "banner") return "wide-horizontal-frame";
    return `${understanding.scene.sceneType}-frame`;
  }

  private inferCropping(analysis: ImageAnalysisIntelligenceRecord): string {
    const ratio = analysis.technical.aspectRatio;
    if (ratio.includes("16:9") || ratio.includes("1.78")) return "widescreen-safe-crop";
    if (ratio.includes("1:1") || ratio.includes("1.00")) return "square-crop-ready";
  ...
  private buildImprovementPlan(
    ...
    return {
      cropStrategy:
        comp.cropping === "widescreen-safe-crop"
      ...
      focusStrategy:
        hierarchy.mainSubjectVisibility < 75
      ...
      visualHierarchyStrategy:
        hierarchy.readingFlow === "product-to-brand"
  ...
  private buildRecommendations(
    ...
    const recs: CompositionIntelligenceRecommendation[] = [];
