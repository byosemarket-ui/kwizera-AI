...
import {
  CompositionIntelligenceEngineError,
  CompositionIntelligenceEngineStatusReport,
...
export class AiCompositionIntelligenceEngine {
  ...
  readonly logger = new CompositionLogger();
  readonly records = new CompositionIntelligenceRecordStore();

  private readonly analyzer = new CompositionAnalyzer();
  private readonly scorer = new CompositionScorer();
  private readonly linker = new CompositionLinker();
  private processor: CompositionProcessor | null = null;
  ...
  initialize(foundation: AiImageIntelligenceFoundation, storageRoot: string): void {
    ...
    this.processor = new CompositionProcessor(
      foundation,
      this.analyzer,
      this.scorer,
      this.linker,
  ...
  async analyzeComposition(input: CompositionIntelligenceInput): Promise<CompositionIntelligenceResult> {
    this.ensureReady();
  ...
  async repairComposition(imageId: string): Promise<CompositionIntelligenceResult | null> {
    this.ensureReady();
  ...
  buildStatusReport(): CompositionIntelligenceEngineStatusReport {
    ...
    return {
      ...
      compositionAnalysisStatus:
        "rule of thirds, center, symmetry, balance, negative space, leading lines, depth, perspective, framing and cropping analyzed",
  ...
  private ensureReady(): void {
    if (!this.initialized || !this.foundation || !this.processor) {
      throw new CompositionIntelligenceEngineError(
        "Composition Intelligence Engine not initialized",
        "NOT_INITIALIZED"
      );
    }
  }
