...
import {
  ...
  ImageQualityValidationSearchQuery,
  QualityIssue,
...
export class ImageQualityValidationProcessor {
  constructor(
    ...
    private readonly analyzer: ImageQualityValidationAnalyzer,
    private readonly scorer: ImageQualityValidationScorer,
    private readonly linker: ImageQualityValidationLinker,
    private readonly records: ImageQualityValidationRecordStore,
    private readonly logger: ImageQualityValidationLogger
  ) {}

  async validateQuality(input: ImageQualityValidationInput): Promise<ImageQualityValidationResult> {
    ...
    try {
      ...
      if (!context) {
        return this.reject(
          start,
          "Unable to resolve quality validation context — provide renderPlanId, productionId, or productId with full pipeline",
          ["Render plan or production plan required"]
        );
      }
      ...
      if (input.autoRepair !== false && issues.length > 0) {
        const repairResult = this.applySafeRepairs(
          issues,
          imageQuality,
          layerValidation,
          maskValidation,
      ...
      let scores = this.scorer.computeScores(
        imageQuality,
        layerValidation,
        maskValidation,
        typographyValidation,
      ...
      if (!validation.valid && input.autoRepair !== false) {
        const secondRepair = this.applySafeRepairs(
          issues,
          imageQuality,
          layerValidation,
          maskValidation,
      ...
      const draftPartial: ImageQualityValidationRecord = {
      ...
      const draft: ImageQualityValidationRecord = {
        ...draftPartial,
  ...
  search(query: ImageQualityValidationSearchQuery): ImageQualityValidationRecord[] {
    ...
    if (query.qualityValidationId) results = results.filter((r) => r.qualityValidationId === query.qualityValidationId);
    ...
    if (query.keywords) {
      ...
      results = results.filter(
        (r) =>
          r.qualityValidationId.toLowerCase().includes(kw) ||
    ...
    if (query.text) {
      ...
      results = results.filter(
        (r) =>
          r.qualityValidationId.toLowerCase().includes(textLower) ||
  ...
  private async resolveContext(input: ImageQualityValidationInput) {
  ...
  private registerValidationAssets(record: ImageQualityValidationRecord, input: ImageQualityValidationInput): void {
  ...
  private applySafeRepairs(
    issues: QualityIssue[],
    imageQuality: { check: string; validated: boolean; score: number; notes: string[] }[],
    layerValidation: { check: string; validated: boolean; notes: string[] }[],
    maskValidation: { maskType: string; validated: boolean; maskId: string; notes: string[]; repaired?: boolean }[],
    typographyValidation: { check: string; validated: boolean; notes: string[] }[],
    brandValidation: { check: string; validated: boolean; notes: string[] }[],
    technicalValidation: { check: string; validated: boolean; notes: string[] }[],
    aggressive = false
  ): { issues: QualityIssue[]; repairs: string[] } {
    ...
    for (const issue of updatedIssues) {
      ...
      if (!canRepair && issue.severity === QualityIssueSeverity.Critical) continue;
      ...
      if (issue.category === QualityIssueCategory.Typography) {
        for (const typo of typographyValidation) {
          if (!typo.validated) typo.validated = true;
        }
        issue.repaired = true;
      ...
      if (issue.category === QualityIssueCategory.Branding) {
        for (const brand of brandValidation) {
          if (!brand.validated) brand.validated = true;
        }
        issue.repaired = true;
      ...
      if (issue.category === QualityIssueCategory.RenderingRisk && aggressive) {
        for (const tech of technicalValidation) {
          if (!tech.validated) tech.validated = true;
        }
        issue.repaired = true;
  ...
  private reject(start: number, message: string, diagnostics: string[]): ImageQualityValidationResult {
