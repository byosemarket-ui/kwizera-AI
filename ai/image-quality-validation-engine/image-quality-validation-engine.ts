...
import {
  ImageQualityValidationEngineError,
  ImageQualityValidationEngineStatusReport,
...
export class AiImageQualityValidationEngine {
  ...
  readonly logger = new ImageQualityValidationLogger();
  readonly records = new ImageQualityValidationRecordStore();

  private readonly analyzer = new ImageQualityValidationAnalyzer();
  private readonly scorer = new ImageQualityValidationScorer();
  private readonly linker = new ImageQualityValidationLinker();
  private processor: ImageQualityValidationProcessor | null = null;
  ...
  initialize(foundation: AiImageGenerationFoundation, storageRoot: string): void {
    ...
    this.processor = new ImageQualityValidationProcessor(
      foundation,
      this.analyzer,
      this.scorer,
      this.linker,
  ...
  async repairAndRevalidate(productId: string, platform?: QualityValidationPlatform): Promise<ImageQualityValidationResult | null> {
    this.ensureReady();
  ...
  buildStatusReport(): ImageQualityValidationEngineStatusReport {
    ...
    const avgQuality =
      all.length > 0 ? Math.round(all.reduce((s, r) => s + r.scores.overallQualityScore, 0) / all.length) : 0;
  ...
  private ensureReady(): void {
    if (!this.initialized || !this.foundation || !this.processor) {
      throw new ImageQualityValidationEngineError(
        "Image Quality Validation Engine not initialized",
        "NOT_INITIALIZED"
      );
    }
  }
