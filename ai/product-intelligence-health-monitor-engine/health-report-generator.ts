...
export class ProductIntelligenceHealthReportGenerator {
  constructor(private readonly projectStateDir: string) {}

  generateAll(
    status: ProductIntelligenceHealthMonitorStatusReport,
    check: ProductIntelligenceHealthCheckResult | null,
    history: ProductIntelligenceHealthHistoryEntry[],
    modules: MonitoredProductIntelligenceModuleHealthScore[]
  ): {
    ...
    const healthReportPath = path.join(this.projectStateDir, "Product-Health-Report.md");
    const historyReportPath = path.join(this.projectStateDir, "Product-Health-History.md");
    const performanceReportPath = path.join(this.projectStateDir, "Product-Performance-Report.md");
    const recommendationsReportPath = path.join(this.projectStateDir, "Product-Recommendations.md");
  ...
  private buildHealthReport(
    ...
    return [
      "# Product Health Report — Step 5N",
      "",
      `**Generated:** ${new Date().toISOString()}`,
      `**Engine Status:** ${status.engineStatus}`,
      `**Overall Product Intelligence Health:** ${status.overallProductIntelligenceHealth}`,
  ...
  private buildHistoryReport(history: ProductIntelligenceHealthHistoryEntry[]): string {
    return [
      "# Product Health History — Step 5N",
      "",
      `**Generated:** ${new Date().toISOString()}`,
  ...
  private buildPerformanceReport(
    ...
    return [
      "# Product Performance Report — Step 5N",
      "",
      `**Generated:** ${new Date().toISOString()}`,
      "",
      "| Metric | Value |",
  ...
  private buildRecommendationsReport(
    ...
    return [
      "# Product Recommendations — Step 5N",
      "",
      `**Generated:** ${new Date().toISOString()}`,
      "",
      ...(unique.length > 0
        ? unique.map((r) => `- ${r}`)
        : ["- No recommendations — product intelligence system healthy"]),
      "",
    ].join("\n");
  }
}
