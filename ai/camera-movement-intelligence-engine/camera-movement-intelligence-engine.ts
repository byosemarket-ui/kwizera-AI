...
import {
  CameraAngle,
  CameraMovementEngineError,
  CameraMovementEngineStatusReport,
  CameraMovementInput,
...
export class AiCameraMovementIntelligenceEngine {
  ...
  readonly logger = new CameraMovementLogger();
  readonly records = new CameraMovementRecordStore();

  private readonly analyzer = new CameraMovementAnalyzer();
  private readonly scorer = new CameraMovementScorer();
  private readonly linker = new CameraMovementLinker();
  private processor: CameraMovementProcessor | null = null;
  ...
  initialize(foundation: AiVideoIntelligenceFoundation, storageRoot: string): void {
    ...
    this.processor = new CameraMovementProcessor(
      foundation,
      this.analyzer,
      this.scorer,
      this.linker,
  ...
  async analyzeCamera(input: CameraMovementInput): Promise<CameraMovementResult> {
    this.ensureReady();
  ...
  async repairCameraAnalysis(videoId: string): Promise<CameraMovementResult | null> {
    this.ensureReady();
  ...
  buildStatusReport(): CameraMovementEngineStatusReport {
    ...
    if (!this.foundation?.getTimelineIntelligenceEngine().isStartupComplete()) readinessScore -= 10;
    if (!integration?.knowledgeEngine) readinessScore -= 10;

    return {
      engineStatus: this.startupComplete ? "operational" : "initializing",
      movementAnalysisStatus:
        "static, pan, tilt, zoom, dolly, truck, pedestal, crane, orbit, handheld, gimbal, drone, tracking, follow, push-in, pull-out",
  ...
  private ensureReady(): void {
    if (!this.initialized || !this.foundation || !this.processor) {
      throw new CameraMovementEngineError(
        "Camera Movement Intelligence Engine not initialized",
        "NOT_INITIALIZED"
      );
    }
  }
