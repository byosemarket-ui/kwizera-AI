...
export class AmbientAudioGenerationScorer {
  computeScores(
    ...
    ambientPlan: AmbientSoundPlan,
    urbanPlan: UrbanAmbiencePlan,
    indoorPlan: IndoorAmbiencePlan,
    weatherPlan: WeatherAmbiencePlan,
    spatialPlan: SpatialAudioPlan,
  ...
  isAmbientPlanValid(
    ...
    record: Pick<
      AmbientAudioGenerationRecord,
      "environmentAnalysis" | "ambientSoundPlan" | "weatherAmbiencePlan" | "spatialAudioPlan" | "timelinePlan" | "syncPreparation"
    >
  ): { valid: boolean; diagnostics: string[] } {
    const diagnostics: string[] = [];

    if (scores.environmentalRealismScore < 55) diagnostics.push(`Environmental realism ${scores.environmentalRealismScore} below threshold (55)`);
    if (scores.immersionScore < 50) diagnostics.push(`Immersion score ${scores.immersionScore} below threshold (50)`);
    if (scores.spatialAudioScore < 55) diagnostics.push(`Spatial audio score ${scores.spatialAudioScore} below threshold (55)`);
  ...
  isBrandConsistent(context: AmbientContext, instructions: ProductionAmbientInstructions): boolean {
    if (!context.brandName) return instructions.immersionGuidance.length >= 1;
    return Boolean(context.brandGuidelines) || instructions.renderNotes.length >= 1;
  }

  private computeEnvironmentalRealism(
    analysis: EnvironmentAnalysis,
    ambient: AmbientSoundPlan,
    indoor: IndoorAmbiencePlan,
    weather: WeatherAmbiencePlan
  ): number {
    let score = 45;
    if (analysis.location.length >= 3) score += 10;
    if (ambient.natureAmbience.length >= 1) score += 15;
    if (weather.weatherTypes.length >= 1) score += 15;
    if (indoor.roomTone.length >= 5 || analysis.indoorOutdoor === "outdoor") score += 15;
    return Math.min(100, score);
  }

  private computeImmersionScore(analysis: EnvironmentAnalysis, spatial: SpatialAudioPlan, timeline: AmbientTimelinePlan): number {
    let score = 45;
    if (analysis.intendedMood.length >= 3) score += 15;
    if (spatial.depth.length >= 10) score += 15;
    if (timeline.dynamicIntensity.length >= 2) score += 15;
    if (timeline.layerOrder.length >= 2) score += 10;
    return Math.min(100, score);
  }

  private computeSpatialAudioScore(spatial: SpatialAudioPlan): number {
    let score = 45;
    if (spatial.leftRightPositioning.length >= 10) score += 15;
    if (spatial.surroundPreparation.length >= 10) score += 20;
    if (spatial.binauralPreparation.length >= 10) score += 20;
    return Math.min(100, score);
  }

  private computeSynchronizationScore(timeline: AmbientTimelinePlan, sync: AmbientSyncPlan): number {
    let score = 45;
    if (timeline.cuePoints.length >= 3) score += 20;
    if (sync.hitPoints.length >= 2) score += 15;
    if (timeline.fadeIn && timeline.fadeOut) score += 10;
    if (sync.syncNotes.length >= 2) score += 10;
    return Math.min(100, score);
  }

  private computeBrandConsistency(context: AmbientContext, instructions: ProductionAmbientInstructions): number {
    let score = 45;
  ...
  private computeProductionReadiness(
    analysis: EnvironmentAnalysis,
    timeline: AmbientTimelinePlan,
    sync: AmbientSyncPlan,
    instructions: ProductionAmbientInstructions
  ): number {
    let score = 45;
    if (analysis.durationSec > 0) score += 10;
    if (timeline.layerOrder.length >= 2) score += 15;
