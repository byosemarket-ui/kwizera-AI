...
import type { AiModel, AiModelCategory, HardwareSnapshot, ModelLog, ModelSettings, ModelStore } from "./types.js";
...
export class AiModelManager {
  ...
  async load(modelId: string): Promise<AiModel> { const model = this.getMutable(modelId); await this.validation.validate(model); const hardware = await this.detectHardware(); this.resources.assertAvailable(model, hardware); model.status = "loaded"; model.loadedAt = new Date().toISOString(); model.usageCount += 1; model.lastUsedAt = model.loadedAt; this.cacheManager.touch(model); this.log("info", "loading", `Loaded ${model.name}`, model.id); await this.persist(); return structuredClone(model); }
  async unload(modelId: string): Promise<AiModel> { const model = this.getMutable(modelId); if (model.status === "loaded") model.status = "installed"; this.cache.delete(modelId); this.log("info", "unloading", `Unloaded ${model.name}`, model.id); await this.persist(); return structuredClone(model); }
  async remove(modelId: string): Promise<void> { const model = this.getMutable(modelId); await this.unload(modelId); if (model.artifactPath) await fs.rm(model.artifactPath, { force: true }); model.status = "removed"; this.log("warning", "removal", `Removed ${model.name}`, model.id); await this.persist(); }
  ...
  async persist(): Promise<void> { await fs.writeFile(path.join(this.root, "models.json"), `${JSON.stringify(this.store, null, 2)}\n`, "utf8"); }
  private async readStore(): Promise<ModelStore> { try { const saved = JSON.parse(await fs.readFile(path.join(this.root, "models.json"), "utf8")) as Partial<ModelStore>; return { models: saved.models ?? [], settings: { ...DEFAULT_SETTINGS, ...saved.settings }, logs: saved.logs ?? [] }; } catch (error) { if ((error as NodeJS.ErrnoException).code === "ENOENT") return { models: [], settings: DEFAULT_SETTINGS, logs: [] }; throw error; } }
