...
export class AiModelManager {
  ...
  async persist(): Promise<void> { const target = path.join(this.root, "models.json"); const temporary = `${target}.${randomUUID()}.tmp`; await fs.writeFile(temporary, `${JSON.stringify(this.store, null, 2)}\n`, "utf8"); await fs.rename(temporary, target); }
