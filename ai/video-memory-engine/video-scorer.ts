import { VideoQualityScores, VideoRecord } from "./types.js";

export class VideoScorer {
  computeScores(video: Partial<VideoRecord>): VideoQualityScores {
    ...
    const learningScore = Math.min(
      100,
      (video.lessonsLearned?.length ?? 0) * 15 +
    ...
    const aiConfidenceScore = Math.round(
      (videoQualityScore + marketingScore + learningScore + exportQuality) / 4
    );
