...
import {
  LearningCategory,
  LearningEventInput,
  LearningMemoryEngineError,
  LearningMemoryStatusReport,
  LearningOutcome,
...
export class AiLearningMemoryEngine {
  ...
  readonly logger = new LearningMemoryLogger();
  readonly history = new LearningHistoryStore();
  readonly preferences = new PreferenceStore(this.logger);

  private readonly evaluator = new LearningEvaluator();
  private patterns: PatternDetector | null = null;
  private processor: LearningProcessor | null = null;
  private selfImprovement: SelfImprovementAnalyzer | null = null;

  private learningTimes: number[] = [];
  ...
  initialize(foundation: AiMemoryFoundation, storageRoot: string): void {
    ...
    this.processor = new LearningProcessor(
      foundation,
      this.evaluator,
      this.history,
      this.patterns,
    ...
    this.selfImprovement = new SelfImprovementAnalyzer(
      this.history,
      this.patterns,
      this.preferences
    );
  ...
  async learnFromEvent(input: LearningEventInput): Promise<LearningProcessResult> {
    this.ensureReady();
  ...
  async learnFromUserCorrection(
    correction: string,
    context: Partial<LearningEventInput>
  ...
  async getRecommendationsForProject(projectId: string): Promise<string[]> {
    ...
    if (prefs.preferredWorkflow) recs.push(`Workflow: ${prefs.preferredWorkflow}`);
  ...
  buildStatusReport(): LearningMemoryStatusReport {
    ...
    return {
      ...
      preferenceLearningStatus:
        this.preferences.getPreferenceCount() > 0 ? "active" : "awaiting preferences",
  ...
  private ensureReady(): void {
    if (!this.initialized || !this.foundation) {
      throw new LearningMemoryEngineError(
        "Learning Memory Engine not initialized",
        "NOT_INITIALIZED"
      );
    }
  }
