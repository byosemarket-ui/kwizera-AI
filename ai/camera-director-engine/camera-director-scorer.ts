...
export class CameraDirectorScorer {
  computeScores(
    draft: CameraDirectorRecordDraft,
    ...
    const aiConfidenceScore = Math.round(
      (cameraDirectionScore + cinematicScore + compositionScore + storytellingScore + productionReadinessScore) / 5
    );
  ...
  isPlanValid(scores: CameraDirectorScores, draft: Pick<CameraDirectorRecordDraft, "shotPlans" | "continuity" | "focusPlanning" | "compositionPlanning">): {
    ...
    if (draft.shotPlans.length < 1) diagnostics.push("Camera plan must include at least one shot");
    ...
    if (!draft.focusPlanning.focusSubject) diagnostics.push("Focus subject required");
    if (!draft.compositionPlanning.primaryStrategy) diagnostics.push("Composition strategy required");
  ...
  private computeDirectionScore(shots: DirectorShotPlan[], scene: SceneGenerationRecord): number {
    let score = 45;
    if (shots.every((s) => s.cameraAngle && s.cameraMovement && s.framing)) score += 20;
    if (shots.length >= scene.shots.length) score += 15;
    if (shots.some((s) => s.marketingPurpose.length >= 10)) score += 20;
    return Math.min(100, score);
  }

  private computeCinematicScore(shots: DirectorShotPlan[], continuity: ContinuityPlanning): number {
    let score = 40;
    if (shots.length >= 2) score += 15;
    if (continuity.cameraConsistency) score += 15;
    if (continuity.motionContinuity) score += 15;
    if (continuity.lightingContinuity) score += 15;
    return Math.min(100, score);
  }

  private computeCompositionScore(composition: CameraDirectorRecordDraft["compositionPlanning"]): number {
    let score = 45;
    if (composition.ruleOfThirds && composition.productHighlight) score += 20;
    if (composition.brandVisibility && composition.negativeSpace) score += 20;
    if (composition.primaryStrategy) score += 15;
    return Math.min(100, score);
  }

  private computeStorytellingScore(scene: SceneGenerationRecord, shots: DirectorShotPlan[]): number {
    let score = 45;
    if (scene.structure.sceneObjectives.length >= 1) score += 15;
    if (shots.some((s) => s.shotType.includes("establishing") || s.shotType.includes("hero"))) score += 20;
    if (scene.structure.scenePurpose) score += 20;
    return Math.min(100, score);
  }

  private computeProductionReadiness(draft: CameraDirectorRecordDraft, scene: SceneGenerationRecord): number {
    let score = 45;
    if (draft.focusPlanning.depthOfField && draft.focusPlanning.focusSubject) score += 15;
    if (draft.platformOptimizations.length >= 7) score += 15;
    if (scene.productionReady) score += 15;
    if (draft.shotPlans.every((s) => s.duration)) score += 10;
    return Math.min(100, score);
  }
}
