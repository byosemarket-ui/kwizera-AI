...
import type { CameraDirectorRecordDraft } from "./camera-director-analyzer.js";

export class CameraDirectorProcessor {
  constructor(
    private readonly foundation: AiVideoGenerationFoundation,
    private readonly analyzer: CameraDirectorAnalyzer,
    private readonly scorer: CameraDirectorScorer,
    private readonly linker: CameraDirectorLinker,
    private readonly records: CameraDirectorRecordStore,
    private readonly logger: CameraDirectorLogger
  ) {}
  ...
  async planCamera(input: CameraDirectorInput): Promise<CameraDirectorResult> {
    ...
    try {
      ...
      const generated: CameraDirectorRecord[] = [];
      ...
      for (const scene of scenes) {
        ...
        const draft: CameraDirectorRecord = {
          ...draftBase,
          scores,
          relationships: this.linker.detectRelationships(
            { ...draftBase, scores } as CameraDirectorRecord,
            scene,
            storyboard,
            input
          ),
  ...
  search(query: CameraDirectorSearchQuery): CameraDirectorRecord[] {
    ...
    if (query.cameraPlanId) results = results.filter((r) => r.cameraPlanId === query.cameraPlanId);
    ...
    if (query.keywords) {
      ...
      results = results.filter(
        (r) =>
          r.marketingImpact.heroMoment.toLowerCase().includes(kw) ||
    ...
    if (query.text) {
      ...
      results = results.filter(
        (r) =>
          r.cameraPlanId.toLowerCase().includes(textLower) ||
  ...
  private resolveScenes(input: CameraDirectorInput): SceneGenerationRecord[] {
  ...
  private registerGenerationAsset(record: CameraDirectorRecord, scene: SceneGenerationRecord): void {
  ...
  private applySafeRepairs(
    ...
    draft.continuity.issues = draft.continuity.issues.filter(
      (i) => !diagnostics.some((d) => d.includes(i))
    );
    if (repairs.length > 0) {
  ...
  private reject(start: number, message: string, diagnostics: string[]): CameraDirectorResult {
