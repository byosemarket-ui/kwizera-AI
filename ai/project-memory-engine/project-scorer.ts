import { ProjectRecord, ProjectScores, ProjectStatus } from "./types.js";

export class ProjectScorer {
  computeScores(project: Partial<ProjectRecord>): ProjectScores {
    ...
    const recoveryScore =
      project.latestCheckpointId || project.status === ProjectStatus.Recovered ? 90 : 50;
    const aiConfidenceScore = Math.min(
      100,
      Math.round((qualityScore + completionScore + learningScore) / 3)
    );
  ...
  private countAssets(assets: ProjectRecord["assets"]): number {
    return Object.values(assets).reduce((sum, arr) => sum + arr.length, 0);
  }

  private countWorkflowEntries(history: ProjectRecord["workflowHistory"]): number {
